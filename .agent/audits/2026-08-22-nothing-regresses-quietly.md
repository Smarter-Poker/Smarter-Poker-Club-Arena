...
Every item below was found by reading what a system *published*, not by
...
- **Lag vs rewind.** An older commit that is still an ancestor of `main` is lag
  and resolves itself. A served sha that is *not* an ancestor never will:
...
Autopilot merges what *can* merge. What it did with the rest was print one line
...
required checks, and *no check ever reporting* — that last is the quiet killer,
...
whether a pull request *ever existed* for the ref — a squash merge leaves every
...
**Commits.** `reference-transaction` is the only hook that fires *before* a ref
...
work is *still* recoverable after that override; an ordinary forward commit is
untouched.
...
alone**. That removes a second expiry cliff *and* gives the push a distinct
...
- a direct push from an ordinary identity was refused — *"6 of 6 required
...
`.husky` copies had drifted in *both* directions and ran nowhere: `.git/hooks`
...
way: `.husky/pre-push` exited 0 early on *"No JS/TS files changed"*, and a push
...
require is the *job* name, so a rule naming it would have been ambiguous and
...
the App is required for *merges* (a `GITHUB_TOKEN` merge does not trigger
...
existing issue and each saw none. The check read GitHub's *search index*, which
...
Every one of these is now *named somewhere that updates itself and closes when
the condition clears*, which is the difference between a backlog and a list
nobody reads.
