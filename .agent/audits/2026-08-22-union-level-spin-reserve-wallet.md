...
AND YOU NEED TO CREATE THE WALLET TO HOLD THE SEEDED AND RESERVE FUNDS."*
...
debited a union wallet to fund a pool, so the *funding* looked union-level. The
...
still takes the *playing* club's id — resolution happens inside the database.
...
name and its unique index. A union already *is* a `clubs` row in this schema
...
**Rake did not move.** `rake_records` is still written with the *playing*
...
against the *calling* role, not the view's owner. Revoking it from `anon` would
...
lesson applied before it could bite: *what a deployed client selects is that
client's contract.*
