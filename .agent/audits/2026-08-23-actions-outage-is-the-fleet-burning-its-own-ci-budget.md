...
runner_name="" plus steps=0 plus no log blob is the canonical signature of *the
job was never dispatched*. With Actions enabled and GitHub healthy, that leaves
...
Estimate for that 40-minute window, club-arena alone - *this is an estimate, the
inputs above are measured*:
