...
module itself is correct, restart-safe and well-tested; the _policy_ it
...
| events where peak BB > _every chip in the event_                      | 72 (12.4%)      |
...
4. **Spin draw row-write failure has no repair path** — a game whose multiplier
   was drawn but whose row read NULL runs on the _smallest_ placeholder tier and
