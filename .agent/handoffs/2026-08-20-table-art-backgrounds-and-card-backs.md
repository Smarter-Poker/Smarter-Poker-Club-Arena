...
- **Design:** match the existing gradient palettes so nothing changes character
  ...
  treatment. Must read as a *card back*, i.e. rotationally symmetric or close
...
3. Update `tests/unit/cardBackVocabulary.test.ts` — it currently asserts the four
   ...
   should assert they resolve to *themselves*.
...
- Open a table at **375px** and at **1440px**. The background must be clearly
  visible and clearly *designed* at both, and must not compete with the felt.
...
- Real artwork for the four *table* skins that are currently unused aliases.
