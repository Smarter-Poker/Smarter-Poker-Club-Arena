...
the feature shipped, and it was unreachable *by construction*: the only event
...
*that file* can either — the missing thing was in a different file. It is the
...
handler exists for an event, assert that the event is *sent*. `tests/config/
...
- `src/components/tournament/TournamentResultCard.tsx` and the router-state
  ...
  path has no such flag and does not need one (the split is on the *presence* of
...
5. **Retire the 500x columns — reader first** (§6.6). World Hub
   ...
   separately. Ship the cron change, wait, *then* drop the columns.
