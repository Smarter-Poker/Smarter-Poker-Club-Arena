...
order.** Six PRs landed around mine, and `main` moved twice *while I was
writing this file* — #188 landed after #202 despite the lower number. Always
...
second checkout at `~/Documents/club-arena` — a *different* clone, also not
...
`GITHUB_TOKEN` in `~/Documents/club-arena/.env` (note: `club-arena`, the *other*
...
`mergeStateStatus: BLOCKED` means *either* "checks pending" *or* "checks
...
- **`psql` is installed but the stored DB password is stale.** I tried
  ...
  MCP is the route. (The service-role key in §3.1 *is* current; only the DB
  password is dead.)
...
Dan's ask, verbatim: *"THE RESERVE POOL COMES FROM THE UNION NOT THE CLUBS. IT
...
- **The engine is unchanged.** Every RPC keeps its exact signature and still
  takes the *playing* club's id; resolution happens inside the database. That
...
- **Rake did not move.** `rake_records` still takes the *playing* club's id.
...
2. **A pool merge cannot be booked as `kind='adjustment'`.**
   `v_spin_reserve_health` counts adjustments as *shortfall events* and
...
- `src/components/tournament/TournamentResultCard.tsx` **already exists**, built
  2026-08-20 for Dan's *"at the end of the tournament when you lose, you need to
  be auto removed from the table, placed inside the lobby and your tournament
  result card shown … winners should be auto removed at the end as well."*
...
**So the likely gap is not the card, it is the trigger:** does the *automatic*
...
1. **The shared working tree serves STALE content.** Six files had silently lost
   *committed* work on 2026-08-21 — `TablePage.tsx` was missing the entire
