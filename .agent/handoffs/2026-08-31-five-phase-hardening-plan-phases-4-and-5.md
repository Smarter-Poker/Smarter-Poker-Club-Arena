...
- Verify against reality. Never claim success before you can *see* it in production.
...
3. **`getTableState()`** — answers `GET /state/:id`, which is what
   ...
   inference can land *below* the truth. That is the original bug's own starting
...
*(#2020 and #2049 share a squash SHA in my notes; treat #2049 as the canonical phase-1 commit.)*
...
because it holds a seat *number*, not a player. So the guard could never become
...
positions** — seat 10 existing in state and rendering nowhere. *That is the
exact bug this phase exists to kill, reintroduced by its own fix.* Not reachable
...
export const MAX_SUPPORTED_SEATS = Object.keys(SEAT_LAYOUTS).reduce(
  (max, k) => Math.max(max, Number(k)), 2);
export function seatLayoutFor(maxPlayers: number) {
  return SEAT_LAYOUTS[Math.min(MAX_SUPPORTED_SEATS, Math.max(2, maxPlayers || MAX_SUPPORTED_SEATS))];
}
```
...
network is up — **nothing** about whether the player can *see* anything. Dan's
...
- `recordPlayerActed()` sets `everActed = true` — **never unset while the seat
  is held**, because a player who plays and *then* goes AFK is an ordinary AFK
...
inside the `? (() => {` immediately after the full gate) — *not* re-derived
...
were left alone *deliberately* rather than by accident.
...
Trigger *firing* does not re-check EXECUTE. Tested, not remembered. Had that
...
I characterised this to Dan as *"an alarm that fires a thousand times a night
cannot detect a real theft."*
...
**STEP 3 — the durable improvement:** make the alarm *believable*. Either
...
45-minute staleness cap *"is not a rare backstop — it is the ONLY way a deploy
ever lands."* **So the engine runs code up to 45 minutes old by design**, and a
...
- **The GitHub token lacks `checks:read`.** `gh api .../check-runs` 403s, which
  is *why* nobody can see which check is blocking a PR. **Cheapest fix on the
...
- **Post-Deploy E2E (production) is red and blocks nothing.** club-lobby specs
  time out with *"Target page, context or browser has been closed"*. PR #2036
...
- **Assertions in a migration must prove BOTH halves:** the thing you meant to
  change changed, *and* the thing you promised not to break still works.
- Write the *why* into the code, not the chat. Every fix in this session carries
...
*Written by cowork-claude, 2026-08-31. Every number in this document was
measured, not estimated. Where I could not verify something, it says so.*
