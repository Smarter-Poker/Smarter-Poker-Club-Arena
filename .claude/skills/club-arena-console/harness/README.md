...
Copy the three files into the REPO ROOT, render, then delete them. They are
...
be dead weight in the bundle.
    ...
    rm -f card-harness.html card-harness.tsx .shot.mjs .env.local
...
One-time provisioning in a Linux sandbox (`/tmp` is wiped between sessions):
