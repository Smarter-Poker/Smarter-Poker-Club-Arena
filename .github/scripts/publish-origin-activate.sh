#!/usr/bin/env bash
# THE ORIGIN ACTIVATION TRANSACTION.
#
# Piped to `bash -s` on the origin host by the "Publish through the
# host-owned immutable transaction" step of
# .github/workflows/publish-club-arena.yml, from the protected-main
# checkout that step makes. It ran as a heredoc inside that step until
# 2026-09-22, when the step grew past the size GitHub Actions accepts for
# one `run` and the whole workflow stopped parsing - so no job started, no
# log said why, and nothing published. Measured that day: a 17,304
# character step was accepted and a 24,094 character step was refused.
# tests/the-publisher-says-what-it-refused.law.test.ts holds every step
# under a budget with headroom, and this file has no such ceiling.
#
# Arguments, in order: ROOT SHA EXPECTED_SHA STAGE_NAME KEEP_RELEASES REPOSITORY
set -euo pipefail
ROOT="$1"
SHA="$2"
EXPECTED_SHA="$3"
