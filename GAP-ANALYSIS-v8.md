...
1. **Formal Table State Machine** -- Extract `HeadlessTableEngine.dealOneHand()` into an explicit FSM with entry/exit/fail conditions per state. This is the #1 structural gap.
   - STATUS: OPEN (server-side architecture work)

2. **Formal Turn State Machine** -- Unify `PreciseActionTimer` + `TimeBankEngine` + `PreActionEngine` + `DisconnectEngine` into a single `TurnStateMachine` class.
   - STATUS: OPEN (server-side architecture work)

3. **Time Bank Cap Alignment** -- Bible mandates max 2 total TB per hand (1 auto + up to 2 manual). Current system allows 4 uses with a pool model.
   - STATUS: OPEN (decision needed: keep flexible model or align to Bible's strict cap)

4. **Settlement Pipeline** -- Formalize the 15-step settlement order as a sequential pipeline.
   - STATUS: OPEN (server-side architecture work)
...
8. **Hand History Layers** -- DONE (2026-04-16, types only)
   - Full 4-tier type system: `HandHistoryLayers` with `raw_events`, `audit_log`, `player_summary`, `dispute_review`
   - Implementation in `HandPersistenceService` to produce all 4 layers: OPEN (server-side)
...
## RECOMMENDED IMPLEMENTATION ORDER (Updated 2026-04-16)

1. **Phase 1 (1-2 weeks):** Formal Table + Turn State Machines (server-side) -- biggest structural improvement
2. **Phase 2 (1 week):** Settlement pipeline formalization (server-side)
3. **Phase 3 (2 weeks):** Comprehensive test suite (200+ scenarios)
4. **Phase 4 (1 week):** Hand history 4-tier implementation in HandPersistenceService (server-side)
5. **Phase 5 (ongoing):** Physics animations, 3D replay, Prometheus, accessibility, i18n, Rabbit Cam

---

## COMPLIANCE SCORE PER CHAPTER (Updated 2026-04-16)
...
| **OVERALL**                    | **~75%** | **Strong foundation, server-side FSM is main remaining gap**                            |

---

## CURRENT STATUS (Updated 2026-04-16)

**Overall compliance jumped from ~65% to ~75%.** All P1 and P2 items are now DONE or already-implemented. The remaining work is concentrated in:

1. **P0 server-side architecture** (formal state machines, settlement pipeline) -- requires Hetzner VPS engine work
2. **P0 time bank cap alignment** -- decision pending from Dan
3. **P1 test suite** (200+ scenarios) -- requires dedicated testing sprint
4. **P3 future enhancements** (physics, 3D replay, i18n, accessibility)

**Decisions still needed from Dan:**

1. Time bank model: keep flexible pool (current) or align to Bible's strict 2-cap?
2. Priority of formal FSM work vs. feature development?
