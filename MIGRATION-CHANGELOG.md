...
## Post-Migration Verification — Round 10: Deep Re-Audit Steps 6-7 + Variant Cleanup (2026-03-25)

**Focus:** Deep re-audit of all Step 6+7 engines (RIT, Insurance, MixedGame, Rakeback, TableBreak, OFC, Telemetry), N-player RIT rewrite, Insurance best-hand-only, table creation wiring, and game variant cleanup per Dan's directive.

### RIT Engine Rewrite (FIX 94-98)

| Fix #  | File(s)                                                | Description                                                                                                                                                                                                                                                             |
| ------ | ------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- | --------------------------------------- | ---------------------------------------------------------------- |
| FIX 94 | `server/src/engine/ServerTableEngine.ts`               | **handleAllInRunout N-player RIT**: Rewrote to support ANY number of all-in players. Uses `evaluateHand()`/`evaluateOmahaHand()` + `compareHands()` to determine chooser (best actual hand). Passes `chooserPlayerId` + `allPlayerIds[]` to `RunItTwiceEngine.offer()`. |
| FIX 95 | `server/src/engine/ServerTableEngine.ts`               | **respondToRIT 2-phase flow**: Phase 1 — chooser picks 1/2/3 runs via `chooserDecides()`. Phase 2 — all others accept/decline. Any decline → instant run once. Broadcasts `rit_chooser_decided` with `waitingFor` list.                                                 |
| FIX 96 | `server/src/index.ts`, `src/services/GameServerAPI.ts` | **POST /rit 2-phase API**: Accepts `{ runs: 1                                                                                                                                                                                                                           | 2   | 3 }`for chooser OR`{ response: 'accept' | 'decline' }`for others. Client`respondToRIT()` updated to match. |
| FIX 97 | `server/src/engine/ServerTableEngine.ts`               | **dealAndResolveRIT N-player per-pot per-board**: Each pot evaluated independently on each board. Uses `Deck.getRemainingCards()` for independent board dealing. Handles ties (split among tied). Rounding correction for cent precision.                               |
| FIX 98 | `server/src/engine/RunItTwiceEngine.ts`                | **Split chooser/responder timeouts**: `chooserTimeout: 5` (5s for best hand to pick), `responderTimeout: 10` (10s for others to accept/decline). `maxRuns` changed from 2 to 3. Safety timeout 18s (5+10+3 buffer).                                                     |

### Table Creation Wiring (FIX 99-100)

| Fix #   | File(s)                                    | Description                                                                                                                                                                                                                                                             |
| ------- | ------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| FIX 99  | `src/components/club/CreateTableModal.tsx` | **RIT/Insurance mutual exclusion in UI**: Toggling RIT on disables Insurance, and vice versa.                                                                                                                                                                           |
| FIX 100 | `src/services/TableService.ts`             | **CRITICAL: Table creation didn't write dedicated Boolean columns**: `createTable()` only wrote JSONB `settings` — server reads `run_it_twice_enabled` / `insurance_enabled` columns. Features were NEVER enabled on server. Fixed INSERT to include dedicated columns. |

### Engine Audit Fixes (FIX 101-104)

| Fix #   | File(s)                                       | Description                                                                                                                                                                                  |
| ------- | --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| FIX 101 | `server/src/engine/TableBreakEngine.ts`       | **Redistribution searched ALL tables**: When consecutive tables full, now searches ALL remaining tables instead of just one more. Added `dispose()`/`disposeAll()`.                          |
| FIX 102 | `server/src/engine/OFCDealingOrchestrator.ts` | **disposeAll() clears turnTimers directly** instead of relying on per-table dispose.                                                                                                         |
| FIX 103 | `server/src/engine/ServerTableEngine.ts`      | **Insurance only to best hand**: Evaluates all hands, sorts by rank, checks ties. Only the clear leader gets an insurance offer. Tied hands = no insurance offered. Per Dan's explicit rule. |
| FIX 104 | `server/src/engine/ServerTableEngine.ts`      | **MixedGameEngine + RakebackEngine never configured**: Added `configurePreset()` and `configure()` calls in table init. (Later disabled — see FIX 105.)                                      |

### Client UI Updates

| Fix #   | File(s)                               | Description                                                                                                                                                                                                             |
| ------- | ------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| FIX 95b | `src/components/table/RunItTwice.tsx` | **Chooser UI**: New props `isChooser`, `onChooserDecide`, `maxRuns`, `playerCount`. Chooser sees "Run Once / Run Twice / Run 3x" buttons. Others see accept/decline.                                                    |
| FIX 95c | `src/pages/TablePage.tsx`             | **RIT state + handlers**: Added `ritIsChooser`, `ritChosenRuns`, `ritMaxRuns`, `ritPlayerCount` state. Handles `rit_offer`, `rit_chooser_decided`, `rit_result` Realtime events. Chooser gets 5s timer, others get 10s. |

### Game Variant Cleanup (FIX 105)

**Dan's directive:** Remove ALL mixed/HORSE games and round-by-round rotation. Only games supported: **NLH, PLO4, PLO5, PLO6, PLO8, Pineapple, Short Deck 6+, OFC**.

| Fix #    | File(s)                                                                         | Description                                                                                                                                                                                                                                     |
| -------- | ------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| FIX 105a | `server/src/types.ts`, `src/types/club.types.ts`, `src/types/database.types.ts` | **GameVariant type cleaned**: Removed `flh`, `plo`, `plo_hilo`, `double_board`, `crazy_pineapple`, `mixed`. Added `pineapple`. Unified to 9 variants: `nlh`, `plo4`, `plo5`, `plo6`, `plo8`, `pineapple`, `short_deck`, `ofc`, `ofc_pineapple`. |
| FIX 105b | `server/src/engine/MixedGameEngine.ts`                                          | **All HORSE presets removed**: `MIXED_GAME_PRESETS` emptied. Engine kept for future use but no presets configured.                                                                                                                              |
| FIX 105c | `server/src/engine/ServerTableEngine.ts`                                        | **Mixed game configuration disabled**: Removed `configurePreset()` call (was FIX 104).                                                                                                                                                          |
| FIX 105d | `src/components/club/CreateTableModal.tsx`                                      | **Added Pineapple to VARIANTS dropdown, removed Double Board toggle**.                                                                                                                                                                          |
| FIX 105e | `src/pages/CreateTablePage.tsx`                                                 | **GAME_TYPES array cleaned**: Removed FLH, FLO, Mixed Game cards. Added PLO4, PLO5, PLO6, PLO8, Pineapple, Short Deck cards.                                                                                                                    |
| FIX 105f | `src/pages/club/ClubLobby.tsx`                                                  | **variantMatchesFilter + getVariantLabel cleaned**: Removed all references to flh, plo_hilo, double_board, crazy_pineapple, mixed.                                                                                                              |
| FIX 105g | `src/components/lobby/DynamicGameCard.tsx`                                      | **VARIANT_DISPLAY + TOURNEY_VARIANT_MAP cleaned**: Removed all dead variant entries.                                                                                                                                                            |
| FIX 105h | `src/services/HorseOrchestrator.ts`                                             | **Removed dead variant table configs**: FLH, Crazy Pineapple, Double Board, Mixed/HORSE tables removed. PLO Hi-Lo changed to PLO8.                                                                                                              |
| FIX 105i | `src/config/RakeConfig.ts`                                                      | **Removed BBJ qualifying entries for flh, plo, plo_hilo aliases**.                                                                                                                                                                              |
| FIX 105j | `src/pages/ClubHomePage.tsx`                                                    | **Removed 'mixed' and 'double' from game filter**.                                                                                                                                                                                              |
| FIX 105k | `server/src/config/RakeConfig.ts`                                               | **Removed `flh` from BBJ qualifying check**.                                                                                                                                                                                                    |
| FIX 105l | `server/src/engine/PokerEngine.ts`                                              | **Simplified hi-lo check**: Removed `plo_hilo` and `.includes('hilo')` — now just checks `plo8`.                                                                                                                                                |
| FIX 105m | `server/src/engine/HandController.ts`                                           | **Removed `plo` case from getCardsPerPlayer, added `pineapple` (3 cards)**.                                                                                                                                                                     |

### Engines Verified Clean (No Fixes Needed in Round 10)

| Engine          | Status | Notes                                       |
| --------------- | ------ | ------------------------------------------- |
| InsuranceEngine | PASS   | Best-hand-only enforced by caller (FIX 103) |
| MixedGameEngine | PASS   | Code clean, presets emptied (FIX 105b)      |
| RakebackEngine  | PASS   | Clean, no bugs                              |
| ChipRaceEngine  | PASS   | Clean, tournament-only                      |
| TableBalancer   | PASS   | Clean, tournament-only                      |
| EngineTelemetry | PASS   | Clean, observability                        |

### Files Modified in Round 10

| File                                          | Changes                                                               |
| --------------------------------------------- | --------------------------------------------------------------------- |
| `server/src/engine/ServerTableEngine.ts`      | FIX 94, 95, 97, 103, 104→disabled (105c)                              |
| `server/src/engine/RunItTwiceEngine.ts`       | FIX 98: split timeouts, maxRuns: 3                                    |
| `server/src/engine/MixedGameEngine.ts`        | FIX 105b: presets emptied                                             |
| `server/src/engine/HandController.ts`         | FIX 105m: pineapple 3 cards, removed plo alias                        |
| `server/src/engine/PokerEngine.ts`            | FIX 105l: simplified hi-lo check                                      |
| `server/src/engine/TableBreakEngine.ts`       | FIX 101: search all tables                                            |
| `server/src/engine/OFCDealingOrchestrator.ts` | FIX 102: disposeAll timers                                            |
| `server/src/types.ts`                         | FIX 105a: GameVariant cleaned                                         |
| `server/src/config/RakeConfig.ts`             | FIX 105k: removed flh                                                 |
| `server/src/index.ts`                         | FIX 96: 2-phase /rit endpoint                                         |
| `src/types/club.types.ts`                     | FIX 105a: GameVariant cleaned                                         |
| `src/types/database.types.ts`                 | FIX 105a: GameVariant cleaned                                         |
| `src/components/club/CreateTableModal.tsx`    | FIX 99, 105d: mutual exclusion, pineapple added, double board removed |
| `src/components/table/RunItTwice.tsx`         | FIX 95b: chooser UI                                                   |
| `src/components/lobby/DynamicGameCard.tsx`    | FIX 105g: cleaned variant maps                                        |
| `src/pages/TablePage.tsx`                     | FIX 95c: RIT state + handlers                                         |
| `src/pages/CreateTablePage.tsx`               | FIX 105e: game type cards cleaned                                     |
| `src/pages/club/ClubLobby.tsx`                | FIX 105f: filter + label cleaned                                      |
| `src/pages/ClubHomePage.tsx`                  | FIX 105j: game filter cleaned                                         |
| `src/services/TableService.ts`                | FIX 100: dedicated boolean columns                                    |
| `src/services/GameServerAPI.ts`               | FIX 96: 2-phase respondToRIT                                          |
| `src/services/HorseOrchestrator.ts`           | FIX 105h: dead variant tables removed                                 |
| `src/config/RakeConfig.ts`                    | FIX 105i: removed dead BBJ entries                                    |
