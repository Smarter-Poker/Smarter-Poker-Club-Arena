...
Templates exist at `.env.example`, `server/.env.example`, `infra/monitoring/.env.example`, and `services/sentry-autofix/.env.example`. The retired Windows solver worker no longer carries a database-credential template; certified solver hosts use distinct HMAC gateway secrets and never a Supabase key.
