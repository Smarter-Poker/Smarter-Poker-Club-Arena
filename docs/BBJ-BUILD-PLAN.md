...
| 1.3 | Every EVENT frame carries the engine's clock (`ts`); the client keeps the offset (`src/lib/serverClock.ts`) and the freshness gate compares engine-to-engine. A phone running two minutes fast can no longer silently refuse every celebration.                                                                                                                                                   | `TableStateHub.ts`, `EngineStateClient.ts`, `bbjHitOnce.ts` |
