...
## 0g. Status 2026-09-20 (the rules armed themselves, and what is still open)

Read from production on 2026-09-20, project `kuklfnapbkmacvwxktbh`, read-only.

### The rule flips of September 15 and 16

Ruling 17 said each log-only rule becomes a refusal after its clean window. Four flipped on September 15 and one on September 16, all by the daily sweep rather than by hand. From `ca_diamond_rule_modes`:

| Rule                                      | Mode   | Flipped at           | By              |
| ----------------------------------------- | ------ | -------------------- | --------------- |
| DR2:balance_born_outside_the_mint         | refuse | 2026-09-15 06:50 UTC | `auto:flip_due` |
| DR6:balance_changed_without_journal       | refuse | 2026-09-15 06:50 UTC | `auto:flip_due` |
| DR7:test_mode_session_settled             | refuse | 2026-09-15 06:50 UTC | `auto:flip_due` |
| DR8:purchase_price_disagrees_with_package | refuse | 2026-09-15 06:50 UTC | `auto:flip_due` |
| DR4:credit_without_reference              | refuse | 2026-09-16 06:50 UTC | `auto:flip_due` |

Four rules are still `log`. `fn_ca_diamond_rule_flip_due(true)` gives the reason for each, and none of them is an oversight: `DR7:user_over_daily_cap` reads `unknown`, because its configuration changed five days ago and the seven clean days have not elapsed; `DR15:cross_asset_seat` and `DR16:deposit_inside_settlement_window` are `blocked` until 2026-09-22 by their own arming dates; `DR13:concentration_or_velocity` is `blocked` because it would refuse eight movements affecting zero players, and ruling 13 keeps it a report in any case.

The one health item reading `rules overdue` is `DR7:user_over_daily_cap` answering `unknown` rather than a rule that failed to arm. That is the correct answer for an evidence window that is too short, not a defect (CLAUDE.md 10.86).

### The clean-day streak

The last `DR11:trial_balance_break` was **2026-09-11 15:20 UTC**; the last `DR0:health_critical` was 2026-09-11 15:35 UTC. Measured at 2026-09-20 06:23 UTC that is a streak of **eight days and fifteen hours** with no break and no critical.

The hourly `DR11:trial_balance_summary` rows have been continuous throughout: 216 rows across the last ten calendar days, latest 2026-09-20 06:20 UTC. The streak is therefore a measured streak rather than a gap in measurement, which is the distinction worth making, because a watch that stopped running also files nothing.

`ca_diamond_snapshots` agrees: `unexplained` is 0.00 on every snapshot back through the window, the latest at 2026-09-20 06:10 UTC with a total of 5,279,498 diamonds and `arena_diamonds` 0. `fn_ca_diamond_health()` reads ok on money identity ("players + house + custody = register, exactly"), on the deploy gate and on the trial balance.

### Open critical incidents, and what resolves them

**Forty-five open `DR0:health_critical` rows in `ca_diamond_incidents`**, every one written between 2026-09-09 07:35 and 2026-09-11 15:35, naming rewards lost to expiry, trial balance, money identity and the deploy gate. Every one of those areas has read ok since 2026-09-11 16:35. They are open because **nothing in the estate resolves a DR0 row**: `fn_ca_diamond_health_watch` files and never writes `resolved_at`, and the only automatic resolution that exists touches info rows. To every consumer, an incident whose cause cleared eight days ago is indistinguishable from one whose cause is still there. Eight `DR11:trial_balance_break` warnings from 2026-09-11 are open for the same reason.

_What resolves them:_ migration `20260919223032_the_health_watch_resolves_what_it_filed.sql`, written and awaiting owner installation on branch `agent/cw-diamond-gate/fix/the-health-watch-resolves-what-it-filed`. It puts the resolution where the filing is: the watch that reads the source on every tick resolves a row once every area that row names has stopped reading critical, with a note saying what it read. This matters beyond tidiness, because the programme's public-release condition is "no open critical `ca_diamond_incidents`", and these rows are the only thing keeping that gate literally unmet.

**Two `fn_ca_diamond_unreachable_money()` findings.** The first is `send_wallet_diamond_transfer`, client-reachable but refused by the profile guard: the route is granted to `authenticated` and journals both legs, but `fn_guard_profile_privileged_columns` does not name it, so every transfer answers 42501 at the first wallet UPDATE and rolls back. `diamond_wallet_transfers` holds zero rows, ever. The second is DR16 having no consumer, its original `fn_arena_deposit` now a stub that raises. _What resolves both:_ migration `20260919223115_the_transfer_door_is_named_and_dr16_has_a_consumer.sql`, on the same branch.

**Open critical `financial_alerts` rows are chip-estate tournament incidents**, not Diamond ones: `Tournament.atomic_finish_refused`, `Tournament.atomic_finish_outcome_unknown`, `postHandTasks.hand_history_failed` and `ServerTableEngine.authoritative_hand_semantic_refusal`, the most recent at 2026-09-20 06:02 UTC. They belong to the tournament and engine estate and are recorded here only so that a reader counting open criticals is not surprised by them.

Two health areas read attention and are not defects. Per-user caps shows four user-days over cap, which is the rule reporting on itself. Budget plans shows three lines as fiction, which ruling 21 made a forecast that refuses nobody; setting them is Dan's.

### Transfers, and the scorecard row below

The "Transfers and gifts" row in section 1 still reads DOES NOT MEET against a decision that was taken on 2026-09-08. Ruling 4 was amended that day to allow player-to-player transfers through one atomic, authenticated, journaled path, and 20260909200327 built exactly that door. What is actually outstanding is narrower and is named above: the guard does not yet name the route. The law test that asserted transfers were off has been retargeted to what governs the door now, and its `docs/laws.d/` entry with it, because a written law contradicting a written ruling is decided by whichever one the next agent reads first (CLAUDE.md 10.8).
...
| Diamond Arena accounting           | PARTIAL        | Built and reconciling, closed behind both switches (2026-09-20). Platform club `002c2d27` live; custody replaced deposit/withdraw (`fn_poker_diamond_reserve`/`_release`, manual pair retired); reporting reads `asset`; Phases 1 to 8 complete and Phase 9 bounties built; `fn_ca_diamond_health()` reads ok on money identity, deploy gate and trial balance; clean-day streak eight days and fifteen hours; `poker_diamond_custody` 0 rows, `arena_diamonds` 0 | Both arena switches stay off until the owner moves them; 45 stale `DR0:health_critical` rows keep the no-open-criticals gate unmet until 20260919223032 installs; satellites, spins, guarantees, fee destinations and cross-format conservation tests are Phase 9 work not started |
