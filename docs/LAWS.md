...
| tests/a-page-never-guesses-its-club.law.test.ts                   | A global-path page honours the club the URL names and never picks one with an unordered limit(1); role-filtered fallbacks stay filtered but become deterministic                                  |
| tests/the-menu-stays-in-the-club.law.test.ts                      | Every Club Arena nav link carries the club you are inside, the identifier stays the slug the URL is using, and a page resolves the club the URL names instead of falling back to clubs[0]         |
