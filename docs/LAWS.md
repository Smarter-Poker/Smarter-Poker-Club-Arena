...
| server/src/services/spinRepairsCanFinish.law.test.ts                | Every recurring repair the engine drives gets a window it can finish in, and none reach for the unbounded auditor                                                                                             |
...
| server/src/tournament/MultiDayIsRefusedUntilItIsBuilt.law.test.ts   | Multi-day is refused at the database until it is built, on the structure columns and not just the badge                                                                                                       |
| server/src/tournament/ASeatIsMoneyEvenWhenNoWalletMoved.law.test.ts | A satellite seat is money on both sides of conservation - income to the target, payout by the satellite - and satellites are scanned                                                                          |
