...
# (Amended by R9, 2026-09-04: a host may choose a Manual Individual

# Table; it is still a cash_games row, with must_move = false.)
...
### Rulings made by the owner at Gate 2 (2026-09-04, applied)

| #   | Case                                                                                                                                                                                                                                                   | Ruling                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| R9  | Dan, verbatim: "after you select which game type you want to create for the table, you should then have to check off, manually create individual tables, or use automated must move games." This amends the header line "There is no standalone mode." | **A game has a TABLE MODE, chosen right after the variant.** `Automated Must Move` is the cluster of this document: one per stakes per club, Main 1 always on (R3), feeders open and close themselves (section 18). `Manual Individual Table` is one table the host runs by hand: it closes when it empties like any host table, it never grows, and the host may create as many as they like at the same stakes. Both are `cash_games` rows (`must_move` true/false) created by the same `fn_cash_game_create`; the browser still writes nothing. The lobby card shows the MUST MOVE badge only for must-move games. "There is no standalone mode" now reads: there is no standalone WRITER - every cash table is a game row with a table mode. |
| R10 | Dan, verbatim: "0/6 SHOULD NEVER BE A THING ON MUST MOVE GAMES, IT SHOULD ACT LIKE A TOURNAMENT COUNTER, AND COUNT HOW MANY PLAYERS ARE INSIDE THIS GAME TYPE."                                                                                        | **A must-move game is ONE row on the board and its player figure is the count inside the whole game.** No per-table x/6, no seat bar, no FULL: a full Main opens a feeder, so a game is running or open. Beside the count: how many tables are open. `get_club_home` carries `cluster_players`, `cluster_tables`, `cluster_must_move`, `cluster_template`, `cluster_state` per row; the lobby keeps only Main 1 of a cluster and paints those on it. The Gate 4 card shows the same two figures (PLAYERS, TABLES). Also from the same instruction: the fleet seeds a cluster's tables BEFORE its own clones, so a new game is never left at zero while horses are available.                                                                     |
