...
The first step back is the record: the twenty applied Diamond migrations are on `main` again under their original names, and the one that never had a file is written from the database's own stored statements ([the record changelog](changelog/2026-09-19-the-diamond-migrations-are-on-record-again.md)). Re-landing the engine, client and tests follows in dependency order, each against the doors these files define. Phase 8 and Phase 9 lines below were shown unchecked while that code was off `main`. It is back, and they are ticked again; the September 20 update records exactly what put each one back and what was checked before re-ticking it.

## Execution Update, September 19 to 20, 2026, The Record Catches Up With The Database

Five deliveries merged and published between September 19 and 20, and the repository now says what the database has said since September 14.

**What merged.** #4938 (`ab9e626376`) re-landed the Phase 8 and Phase 9 tournament code that the September 16 restore archived: the shared Diamond boundary and arena context, the tournament manager's Diamond arms, the eliminations path, the mystery chest built at the unit it read, and the laws that pin a Diamond prize to whole units. #4941 restored the twenty applied Diamond migration files to `supabase/migrations/` under their original names, so every door the engine and client call has a file again. #4945 fixed what a Diamond player sees: a freeloading event says None Scheduled rather than nothing, the seat copy speaks Diamonds, the chip bridge is out of the Diamond lobby, and Top Up is hidden where a seat cannot add funds. #4946 landed the nine laws that pin the text of those restored migrations plus the health-comparisons CI step. #4947 (`f97c368d10`) re-landed the wallet programme, phases 1 through 5.

**Phase 8 and Phase 9 ticks, and what was checked before re-ticking.** Every Phase 8 line and the Phase 9 bounty line is ticked again. Checked on September 20 before confirming them: `ab9e626376` is an ancestor of `main`; `server/src/domain/DiamondCashBoundary.ts`, `server/src/domain/ArenaContext.ts`, `server/src/tournament/TournamentManagerBase.ts` and `TournamentManagerEliminations.ts` carry the Diamond arms; `server/src/engine/DiamondTournamentBoundary.test.ts`, `server/src/tournament/aDiamondMysteryChestHoldsWholeDiamonds.test.ts` and `aDiamondPrizeDoesNotDivide.law.test.ts` are present and run; and all six tournament and bounty migrations (20260914024241, 032315, 034708, 111558, 111709, 113514) have a file on `main` and a row in production `supabase_migrations.schema_migrations`. No line is ticked for satellites, spins, guarantees, fee destinations or the cross-format conservation tests, which are not built.

**The engine build carrying the Phase 8 and 9 boundary is staged and NOT active.** Production serves `8825af51817f379c4261658ca29ecc9d8d81932d`, instance `1-3846b8bb`, and has throughout. Two things hold the cutover, and neither is a Diamond defect.

The first is a migration. `20260918232558_mixed_f06_custody_transfer_retains_original_operations.sql` (PR #4907) is on `main` and is not applied; `server/src/tournament/mixedF06Custody.ts` calls five functions defined only there, and `scripts/ci/check-engine-doors-exist.mjs` reads production's `pg_proc` before the break gate and refuses the build because none of the five exists. The gate is doing exactly its job.

The second is a stuck permit. One in-process F06 permit keeps `maintenance.readyForRestart` false at every hourly break, with `unparkedTables: 0` and `unparkedReasons: {"f06_preparation_unresolved": 1}`, so the restart certificate is never issued and the host transaction never cuts over. Underneath it, `smarter_private.f06_hand_permits` held 697 permits in state `reserved` across 470 running tournaments when measured on September 20, and `fn_f06_finish_hand` has no outcome that can resolve them. That is a platform defect belonging to the F06 owner, not to this programme, and it wants the third outcome set out in [the permit nobody can resolve](changelog/2026-09-18-the-permit-nobody-can-resolve.md). No data was changed to work around it.

The compatibility record for all of this, including the rollback floors and the custody-outstanding rule, is [the Diamond release manifest](dr/diamond-release-manifest.md). The production actions this session could not perform are [the owner steps](runbooks/diamond-launch-owner-steps-2026-09-20.md).

**Two migrations are written and awaiting owner installation.** Branch `agent/cw-diamond-gate/fix/the-health-watch-resolves-what-it-filed` (commit `f3d2ce36db`) carries `20260919223032_the_health_watch_resolves_what_it_filed.sql`, which lets the watch that files a `DR0:health_critical` row resolve it once every area it names has stopped reading critical, and `20260919223115_the_transfer_door_is_named_and_dr16_has_a_consumer.sql`, which names `send_wallet_diamond_transfer` in the profile guard and repoints DR16 at the deposit door that actually exists. The branch cannot be pushed before they apply, because the pre-push hook refuses a migration that is not already live. Until the second one installs, every player transfer answers 42501 at the first wallet write and `diamond_wallet_transfers` holds zero rows.

**Both arena switches stay off.** `cash_games_enabled` and `tournaments_enabled` are both false, confirmed on September 20. No funded Diamond game or tournament has run in public, `poker_diamond_custody` holds zero rows, and no agent flips either switch.
