...
C01 local release gate update, September 8 21:56 UTC: the unpublished-commit guard refused rebase and saved the commit. A normal rebase abort followed by a normal merge of origin/main preserved both histories without any guard override. On merged head 6869dd0c, 68 server/handler/idempotency/deadline/amount tests and 52 client retry/mapper/API tests passed; both TypeScript projects passed. The complete local production build exited 0 with behind-main=0. Source-map upload was disabled for this local verification by removing SENTRY_AUTH_TOKEN from the child environment, using the existing vite.config.ts plugin condition; no source-map upload messages occurred. No deployment was performed by this local build. Normal branch push, automatic PR checks and publisher/engine adoption remain separate gates.

Fresh endpoints at 21:55:41UTC: frontend origin and routed stamp both 243456f0231d4a2c5f4036f177604f3be6e2ccad, publisher34282845196. Engine health returned HTTP502 during the scheduled :55 cutover. This is not a post-thaw health pass; recheck after :00 without forcing a restart.

The first normal push was stopped by two run-it-twice chooser tests (530 passed,15 skipped in that server batch). Their runout snapshots correctly have no acting seat and omit betting history. Context generation now explicitly returns null outside an active betting decision, including runout/showdown/waiting, instead of attempting to generate a betting token there. A new regression pins this boundary; the existing chooser assertions remain unchanged. No hook or assertion was bypassed.

Runout follow-up: 48 tests in the context, HTTP, idempotency and multiway run-it-twice suites passed; server TypeScript passed. The initial new no-decision test tried changing the copied getState() snapshot; it was corrected to set the isolated controller's internal fixture state. Production thresholds and existing chooser assertions were unchanged.

Post-thaw September8 22:01:21UTC: engine health HTTP200, statusok, version54ed5bc1, maintenanceidle,532/532tablesresumed,8/8waves,stalled0,blockedsettlements0. Normal Hetzner run34281949098 job102248549676 succeeded. Logs specify full target54ed5bc1eb25dd5f1da213bb2cd0c99cb164eb37, containerhealthy21:56:40, independent engine_leader witness21:56:42 moved from61df0dc0, deployment truth attempt248 shipped=true. Current decision-context change is not in that build and still requires its normal push/CI/merge and later engine rollout.

## September 9 Recovery Of C01 Publication

The frozen C01 PR #3879 was still open at 55374e3cdb6aa632c0873e018ff91ca0f32f0211. Its CI34286646323 had passed, but merge conflicts remained. Main still lacked PlayerActionContext; the work was not redundant. Normal merges of current main preserved newer changes. Both independent changelog entries and all completion, pot-limit and context mapping tests were retained. The resulting diff against main remains the original 17 C01 files, 440 additions and 22 deletions, with no unrelated source replacement.

At verified head 82b8283b, 97 server tests passed across context, real HTTP dispatch, the 22-case idempotency law, action amount, runout and short-blind suites. All 80 client tests passed across retries/session boundaries, API responses, snapshot mapping and pot presets. Server TypeScript passed; the full npm run build passed with behind-main=0 and local source-map upload disabled. Earlier standalone client TypeScript also passed. The two production submitAction callers are TablePage and MultiTablePage; both carry the displayed context. The only external handlePlayerAction entry is the authenticated HTTP handler, which explicitly passes null for an old client. Trusted internal actions retain their separate omitted-argument path.

The pot-limit short-blind correction meanwhile merged as #3914, 7d6a8f6991ae6141e2394925076b387c14d3938c, at 01:33:00 UTC. CI34299095979 passed server tests/types, all four client shards, build, source/stub checks, and the CSS/Table Studio/375px decision browser gates. Live-production E2E and post-deploy verification jobs were skipped, not passed. That correction is included in this C01 verification base. Both new corrections still require their respective production adoption evidence before closing their release gates.
