...
The availability observation is now resolved in the reviewed follow-up: preparation takes KEY SHARE, and migration preflight pins the exact claimant FOR UPDATE and heartbeat NO KEY UPDATE function bodies, owners, ACLs and search paths. Four observed native races show the old FOR UPDATE heartbeat-busy counterexample, successful heartbeat while corrected preparation waits, claimant blocking behind the actual preparer, and post-wait expiry/generation refusal with full money/seat rollback equality. Final re-execution and source/evidence hashes remain the satellite lane's integration gate.
