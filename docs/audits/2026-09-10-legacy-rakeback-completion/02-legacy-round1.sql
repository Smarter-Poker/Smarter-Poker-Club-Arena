-- Candidate only. Existing final settlement witnesses remain first and unchanged.
BEGIN;
SET LOCAL lock_timeout='1s';
SET LOCAL statement_timeout='10s';
DO $gate$ BEGIN
 IF md5((SELECT prosrc FROM pg_proc WHERE oid='public.fn_union_weekly_rakeback_close(uuid,timestamp with time zone,timestamp with time zone)'::regprocedure)) <> 'a1f3147f67ba42b09348d7f1fb2adbc9' THEN RAISE EXCEPTION 'Legacy R1 owner changed since captured review'; END IF;
 EXECUTE $body$CREATE OR REPLACE FUNCTION public.fn_union_weekly_rakeback_close(p_union_id uuid, p_period_start timestamp with time zone, p_period_end timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
  ...
  WITH raw AS (
    SELECT t.id, t.amount, t.notes, t.created_at,
     ...
     WHERE t.union_id = p_union_id
       ...
       AND NOT EXISTS (SELECT 1 FROM public.ca_cash_bank_receipts cb WHERE cb.union_wallet_transaction_id=t.id)
  )
...
END $gate$;
DO $gate$ BEGIN
 IF md5((SELECT prosrc FROM pg_proc WHERE oid='public.fn_union_club_rake_basis(uuid,timestamp with time zone,timestamp with time zone,boolean)'::regprocedure)) <> 'dde1523d01a5f59d3c1ba481c706e24a' THEN RAISE EXCEPTION 'Legacy R1 owner changed since captured review'; END IF;
 EXECUTE $body$CREATE OR REPLACE FUNCTION public.fn_union_club_rake_basis(p_union_id uuid, p_start timestamp with time zone, p_end timestamp with time zone, p_live boolean DEFAULT false)
...
BEGIN
  ...
  /* THE SNAPSHOT SECOND (Phase 6 verification, 20260908). An OPEN window is
     ...
     witness above. */
  IF NOT p_live AND NOT EXISTS (SELECT 1 FROM public.ca_cash_bank_receipts cb
    WHERE cb.funding_union_id=p_union_id AND cb.bank_credit_at>=p_start AND cb.bank_credit_at<p_end) THEN
  ...
  WITH raw AS (
    ...
    SELECT a.club_id, 'cash'::text AS game_type, sum(a.rake_share) AS rake
     ...
     WHERE a.union_id = p_union_id
       ...
       AND NOT EXISTS (SELECT 1 FROM public.hand_atomic_commits h WHERE h.hand_id=a.hand_id AND h.commission_capture_version=1)
       AND NOT EXISTS (SELECT 1 FROM public.ca_cash_bank_receipts cb WHERE cb.rake_record_id=a.rake_record_id)
     GROUP BY a.club_id
  ), tw AS (
