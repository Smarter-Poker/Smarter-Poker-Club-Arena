...
- `after-proof.json`: 22 corrected groups, including observed transaction/advisory waits, authenticated actor scope, the two historical funding paths, mixed and captured-only periods, pointer evidence, shortfall preservation, final-journal rollback, and both captured old-body refusals.
