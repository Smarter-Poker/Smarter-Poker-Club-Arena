...
results=[]
modes=sys.argv[1:] or ['before','guards','after']
assert all(m in ['before','guards','after'] for m in modes)
for mode in modes:
 ...
 try:
  ...
  files=['fixture.sql','basis-fixture.sql']
  if mode!='before':files+=['money-guards.sql']
  if mode=='after':files+=['01-shared-basis.sql','03-legacy-claim.sql','04-legacy-round3.sql']
  ...
  with (root/(mode+'-native.log')).open('w') as out:
   subprocess.run(['/opt/homebrew/bin/node',str(root/'probe.mjs'),mode],env=dict(env,PGHOST=str(work)),check=True,stdout=out,stderr=subprocess.STDOUT,timeout=90)
...
(root/'runner-evidence.json').write_text(json.dumps({'runs':results,'source_hashes':{f.name:hashlib.sha256(f.read_bytes()).hexdigest() for f in root.iterdir() if f.suffix in ['.sql','.mjs','.py']}},indent=2)+'\n')
