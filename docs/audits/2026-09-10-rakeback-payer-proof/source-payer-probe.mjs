...
import { runWrapperTests } from './source-wrapper-probe.mjs';
import { runReviewTests } from './source-wrapper-review-probe.mjs';
import { createRequire } from 'node:module';
import { writeFileSync, readFileSync } from 'node:fs';
import { createHash } from 'node:crypto';
const { Client } = createRequire(import.meta.url)('pg');
const socket = process.env.PGHOST;
assert.match(socket || '', /^\/tmp\/ca-rakeback-payer\.[A-Za-z0-9]+$/);
...
async function connect(name, role = 'service_role') {
  ...
  assert.equal(
    (
      await c.query(
        "SELECT inet_server_addr() IS NULL AND current_database()='payer_test' local_only"
      )
    ).rows[0].local_only,
    true
  );
...
const results = [];
await q(
  "INSERT INTO ca_cash_commission_authority VALUES(true,1,'2025-01-01','fixture-before','fixture-after')"
);
async function setup({
  ...
  await q(
    "INSERT INTO clubs(id,name,owner_id,chip_treasury) VALUES($1,'Synthetic source payer', $2,1000)",
    [x.club, id(n + 5)]
  );
  await q(
    "INSERT INTO club_members(club_id,user_id,chip_balance,role,agent_id) VALUES($1,$2,0,'player',$3),($1,$3,$5,'agent',NULL),($1,$4,$5,'agent',NULL)",
    [x.club, x.user, x.payer, x.second, balance]
  );
  await q(
    'INSERT INTO rakeback_periods(id,user_id,club_id,period_start,period_end,status,rakeback_earned,rakeback_amount) VALUES($1,$2,$3,$5,$6,$4,999,999)',
    [x.period, x.user, x.club, status, start, end]
  );
  await q(
    'INSERT INTO ca_rakeback_source_periods(id,user_id,club_id,period_start,period_end) VALUES($1,$2,$3,$4,$5)',
    [x.period, x.user, x.club, start, end]
  );
  return x;
}
async function source(
  ...
  const h = id(hand++);
  await q(
    'INSERT INTO hand_atomic_commits(hand_id,post_commit_payload_hash,commission_capture_version,committed_at) VALUES($1,coalesce($2,md5($1::uuid::text)),$3,$4)',
    [h, receiptHash, generation, accepted]
  );
  await q(
  ...
  await q(
    "INSERT INTO ca_cash_commission_facts(hand_id,player_id,booked_club_id,payer_user_id,assignment_state,rake_credit,direct_commission_rate,player_rebate_rate,player_rebate_entitlement,player_terms,hierarchy,errors) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$6::numeric*$8::numeric,jsonb_build_object('errors',$9::jsonb),'[]',$10::jsonb)",
    [
      h,
      x.user,
      x.club,
...
async function pay(x, c = db) {
  return (await c.query('SELECT fn_pay_captured_rakeback_period($1) result', [x.period])).rows[0]
    .result;
}
async function snapshot(x) {
  return (
    await q(
      'SELECT (SELECT chip_treasury::text FROM clubs WHERE id=$1) treasury,(SELECT chip_balance::text FROM club_members WHERE club_id=$1 AND user_id=$2) player,(SELECT chip_balance::text FROM club_members WHERE club_id=$1 AND user_id=$3) payer,(SELECT chip_balance::text FROM club_members WHERE club_id=$1 AND user_id=$4) other_payer,(SELECT count(*)::int FROM ca_rakeback_source_payments WHERE club_id=$1) payments,(SELECT count(*)::int FROM ca_rakeback_source_accruals WHERE club_id=$1) accruals,(SELECT status FROM rakeback_periods WHERE id=$5) period_status',
      [x.club, x.user, x.payer, x.second, x.period]
    )
  )[0];
}
...
try {
  await test('mixed captured payers survive current assignment change and preserve treasury', async () => {
    const x = await setup();
    await source(x);
    await source(x, { payer: x.second, rate: '.20', direct: '.35' });
    await q('UPDATE club_members SET agent_id=NULL WHERE club_id=$1 AND user_id=$2', [
      x.club,
      x.user,
    ]);
    const r = await pay(x),
      s = await snapshot(x);
    assert.equal(r.new_payout, 35);
    assert.equal(s.player, '35.00');
    assert.equal(s.payer, '85.00');
    assert.equal(s.other_payer, '80.00');
    assert.equal(s.treasury, '1000.00');
    assert.equal(s.payments, 2);
    const replay = await pay(x);
    assert.equal(replay.new_payout, 0);
    assert.deepEqual(replay.paid_receipts, r.paid_receipts);
    assert.equal(r.source_final, false);
    for (const p of r.paid_receipts) {
      const rows = await q(
        "SELECT 1 FROM wallet_transactions w JOIN chip_ledger l ON l.id=$2 WHERE w.id=$1 AND w.related_entity_id=$3 AND w.amount=$4 AND l.amount=w.amount AND l.metadata->>'wallet_transaction_id'=w.id::text",
        [p.wallet_transaction_id, p.ledger_id, p.id, p.amount]
      );
  ...
  await test('fractional source zero stays provisional and later fractions pay aggregate cents', async () => {
    ...
    r = await pay(x);
    assert.equal(r.new_payout, 0.15);
    ...
    assert.equal(
      (
        await q(
          'SELECT sum(exact_entitlement)::text e FROM ca_rakeback_source_accruals WHERE club_id=$1',
          [x.club]
        )
      )[0].e,
      '0.1560'
    );
    for (let i = 0; i < 3; i++) await source(x, { rake: '.01' });
    r = await pay(x);
    assert.equal(r.new_payout, 0.01);
    assert.equal((await snapshot(x)).player, '0.16');
  });
  ...
  await test('invalid assigned source and unbound legacy policy never become zero paid', async () => {
    ...
    await source(x, {
      ...
      rate: null,
      direct: null,
      terms: ['unassigned_rebate_policy_unbound'],
    });
    await source(x, { direct: '0' });
    await source(x, { direct: null });
    const r = await pay(x);
    assert.equal(r.new_payout, 0);
    assert.equal(r.deferred.length, 4);
    assert.equal((await snapshot(x)).accruals, 0);
  });
  await test('pre-contract legacy sources and pending estimates cannot initiate backpay', async () => {
    const x = await setup();
    await source(x, { accepted: '2024-12-31', generation: null });
  ...
  await test('overlapping noncanonical period cannot pay a source twice', async () => {
    ...
    await q(
      "INSERT INTO rakeback_periods(id,user_id,club_id,period_start,period_end) VALUES($1,$2,$3,'2026-09-01','2026-09-07')",
      [other.period, x.user, x.club]
    );
    await assert.rejects(
      q(
        "INSERT INTO ca_rakeback_source_periods(id,user_id,club_id,period_start,period_end) VALUES($1,$2,$3,'2026-09-01','2026-09-07')",
        [other.period, x.user, x.club]
      ),
      (e) => e.code === '23514'
    );
    assert.equal((await snapshot(x)).player, '15.00');
  });
  await test('paid legacy period fields are not rewritten by prospective source receipts', async () => {
    const x = await setup({ status: 'paid' });
    await source(x);
    await pay(x);
    assert.equal((await snapshot(x)).period_status, 'paid');
  });
  await test('final journal error rolls back wallets source accrual and receipt together', async () => {
    ...
    try {
      await assert.rejects(pay(x), /injected final journal failure/);
    } finally {
      await q("SELECT set_config('test.fail_journal','',false)");
    }
  ...
  await test('observed concurrent source payment shares advisory lock and pays once', async () => {
    const x = await setup();
    await source(x);
    const blocker = await connect('source_block', null),
      a = await connect('source_first'),
      b = await connect('source_second');
    try {
      await blocker.query('BEGIN');
      await blocker.query('SELECT 1 FROM club_members WHERE club_id=$1 AND user_id=$2 FOR UPDATE', [
        x.club,
        x.payer,
      ]);
      const first = pay(x, a);
      const waitA = await observedWait('source_first');
      const second = pay(x, b);
      const waitB = await observedWait('source_second');
      assert.equal(waitB.wait_event, 'advisory');
      await blocker.query('COMMIT');
      const [ra, rb] = await Promise.all([first, second]);
      assert.equal(ra.new_payout + rb.new_payout, 15);
      assert.equal((await snapshot(x)).payments, 1);
      results.push({ evidence: 'observed_source_overlap', waitA, waitB });
    } finally {
      await Promise.all([blocker.end(), a.end(), b.end()]);
    }
  });
  ...
  await test('paid receipt protects wallet identity and ledger metadata from later mutation', async () => {
    ...
    await assert.rejects(
      q('UPDATE wallet_transactions SET related_entity_id=NULL WHERE id=$1', [
        p.wallet_transaction_id,
      ]),
      (e) => e.code === '55000'
    );
    await assert.rejects(
      q('UPDATE wallet_transactions SET user_id=$2 WHERE id=$1', [
        p.wallet_transaction_id,
        id(999999),
      ]),
      (e) => e.code === '55000'
    ...
    await assert.rejects(
      q("UPDATE chip_ledger SET metadata='{}' WHERE id=$1", [p.ledger_id]),
      (e) => e.code === '55000'
    );
  });
  await test('UTC earning bounds include both week edges across Chicago session and DST', async () => {
    for (const [start, end] of [
      ...
      for (const zone of ['UTC', 'America/Chicago']) {
        ...
        const following = new Date(end + 'T00:00:00Z');
        ...
        try {
          const r = await pay(x);
          assert.equal(r.new_payout, 30);
          assert.equal((await snapshot(x)).accruals, 2);
        } finally {
  ...
  await test('API roles cannot mutate source evidence or directly invoke internal payer', async () => {
    for (const role of ['anon', 'authenticated']) {
      ...
      try {
        await assert.rejects(
          c.query('SELECT fn_pay_captured_rakeback_period($1)', [id(1004)]),
          (e) => e.code === '42501'
        );
      } finally {
        await c.end();
  ...
  const inputFiles = [
    ...
    'run-local.sh',
    'source-payer-probe.mjs',
    'source-wrapper-probe.mjs',
    'source-wrapper-review-probe.mjs',
  ];
  ...
  const runtime = { node: process.version, postgres: (await q('SELECT version() v'))[0].v };
  writeFileSync(
    new URL('source-payer-proof.json', import.meta.url),
    JSON.stringify(
      {
        ...
        inputs,
        scope:
          'Prospective source payer and direct wrappers; captured financial guard subset, no full owner/Union calendar/funding/cascade/UI or production completeness claim',
