...
(p/'local-proof.json').write_text(json.dumps({'status':'direct_native_bank_checks_passed','checks':checks},indent=2)+'\n')
print(json.dumps({'passed':len(checks),'remaining':'separate overlap suites run next; full funding cascade remains outside this proof'}),flush=True)
