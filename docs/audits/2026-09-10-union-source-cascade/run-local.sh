...
proof_tmp="$(mktemp -d /tmp/ca-union-cascade-native.XXXXXX)"
readonly proof_tmp
...
if [[ "${CASCADE_OUTER_MODE:-0}" == 1 ]]; then
 ...
 export CASCADE_SOURCE_INPUT="$proof_tmp/source-archive/docs/audits/2026-09-10-accounting-legacy-exclusion"
 export CASCADE_PAYER_INPUT="$proof_tmp/payer-archive/docs/audits/2026-09-10-legacy-rakeback-completion"
fi
...
"$PGBIN/pg_ctl" -D "$proof_data" -l "$proof_tmp/postgres.log" -o "-h '' -k '$proof_socket' -p 55487" -w start >/dev/null
...
#!/bin/sh
exec env -i LC_ALL=C LANG=C PATH=/opt/homebrew/bin:/usr/bin:/bin "$PGBIN/psql" "\$@" --host="$proof_socket" --port=55487 --username=postgres --dbname=postgres --no-psqlrc
WRAPPER
chmod 700 "$proof_tmp/psql-local"
export COMMISSION_PSQL="$proof_tmp/psql-local" ROUND1_HERE="$base" ROUND1_INPUT="$proof_tmp/input" CASCADE_HERE="$HERE"
export CASCADE_DATA="$proof_data" CASCADE_SOCKET="$proof_socket"
...
d=json.loads(r.stdout);assert Path(d['data_directory']).resolve()==Path(os.environ['CASCADE_DATA']).resolve()
assert d['unix_socket_directories']==os.environ['CASCADE_SOCKET'] and d['listen_addresses']=='' and d['tcp_address'] is None
assert d['database']=='postgres' and d['user']=='postgres' and d['public_tables']==0
assert Path(os.environ['CASCADE_DATA']).parent.name.startswith('ca-union-cascade-native.')
Path(os.environ['CASCADE_HERE'],'cluster-identity-outer.json' if os.environ.get('CASCADE_OUTER_MODE')=='1' else 'cluster-identity.json').write_text(json.dumps(d,indent=2)+'\n')
print('ISOLATION VERIFIED: own fresh data directory, own Unix socket, no TCP listener, zero public tables',flush=True)
PY
if [[ "${CASCADE_OUTER_MODE:-0}" == 1 ]]; then python3 "$HERE/prepare-outer-fixture.py"; fi
...
if [[ "${CASCADE_OUTER_MODE:-0}" == 1 ]]; then "$COMMISSION_PSQL" -X -q -v ON_ERROR_STOP=1 -c "SELECT test_owner(2500100);" >/dev/null; fi
...
if [[ "${CASCADE_OUTER_MODE:-0}" == 1 ]]; then
 python3 "$HERE/outer-native-probe.py"
else
