...
SELECT statement_timestamp() checked_at,
 (SELECT jsonb_agg(to_jsonb(f) ORDER BY signature) FROM function_checks f) functions,
 (SELECT jsonb_agg(to_jsonb(t) ORDER BY trigger_name) FROM trigger_checks t) triggers,
 (SELECT bool_and(pass) FROM function_checks) AND (SELECT bool_and(pass) FROM trigger_checks) all_runtime_checks_pass;
