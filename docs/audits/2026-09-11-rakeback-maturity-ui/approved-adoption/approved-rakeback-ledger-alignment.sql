...
BEGIN
 ...
 WHERE version='20260911074903'
 FOR UPDATE;
 IF NOT FOUND THEN RAISE EXCEPTION 'Verified generated migration row is absent'; END IF;
 IF v_receipt.name IS DISTINCT FROM 'legacy_rakeback_closed_period_single_payer'
 OR cardinality(v_receipt.statements) IS DISTINCT FROM 1
 OR encode(sha256(convert_to(v_receipt.statements[1],'UTF8')),'hex') IS DISTINCT FROM 'a55e792f12040799a20fcf6d54969059efecaea3869c3aa148191fe1b083c4c7'
 OR encode(sha256(convert_to(array_to_json(v_receipt.statements)::text,'UTF8')),'hex') IS DISTINCT FROM '3c715b21e06f89f2995fee730c3f29d9f810c444b68c793913958b81f34406c2'
 THEN RAISE EXCEPTION 'Verified migration source or statement representation changed'; END IF;
 IF EXISTS(SELECT 1 FROM supabase_migrations.schema_migrations WHERE version='20260911072837')
 OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE name='legacy_rakeback_closed_period_single_payer') <> 1
 ...
 SET version='20260911072837'
 WHERE version='20260911074903'
 AND name='legacy_rakeback_closed_period_single_payer'
 ...
 IF NOT EXISTS(SELECT 1 FROM supabase_migrations.schema_migrations WHERE version='20260911072837'
 AND name=v_receipt.name AND statements IS NOT DISTINCT FROM v_receipt.statements)
 OR EXISTS(SELECT 1 FROM supabase_migrations.schema_migrations WHERE version='20260911074903')
