...
let a fifth seat through. Every one of the 24 breaches was a *booking* — a
...
tournament starts, its seat insert meets the seat trigger and is refused with
23514. The field is short a player who is on the list, and the seat-first fill
...
The cost is the opposite of a leak: horses read *busier* than they are, so the
...
above the active-player filter so a chair freed this tick is free for *this*
hand.
...
Worth recording that `horse_seat_unactable` is the same *class* as defect 2 —
