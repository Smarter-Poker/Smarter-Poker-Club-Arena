...
This work was written on 2026-09-05 on the `agent/cw-wallet/fix/realism-polish`
branch, whose first pull request (#3137) squash-merged while the branch was
still being pushed to. Every later push landed on a closed pull request
(rule 10.82, merged is not landed); autopilot opened #3140 for the moved
branch, the Silent Revert Guard held it on the CSS ratchet, and the estate's
staleness rule closed it on 2026-09-07. So for eight days production carried
the first three commits and none of these:

- the plate readout in the bay the artwork was drawn around, with the label
  `Balance` (the Receive pane's totals were rendering below the plate);
- the ledger with no floor, the "Diamonds You Have Sent" record, and
  `tests/the-mint-is-internal.law.test.ts`;
- `supabase/migrations/20260905160815_the_felt_colour_column_holds_only_felt_colours.sql`,
  which had been APPLIED to production (constraint
  `user_table_settings_color_theme_is_a_felt_palette` is live) and was not
  written down in this repo - exactly the drift `Applied Migrations Are
Recorded` exists to name.

Relanded by cherry-picking the three content commits onto a fresh branch off
current `main`, with main's five later edits to `PlayerWalletPage.tsx`
(club context, native links, the chip statement, the daily bonus copy, the
diamond crest) verified intact. The CSS leak ratchet was measured at 89
against a ceiling of 161 and set to 89.
