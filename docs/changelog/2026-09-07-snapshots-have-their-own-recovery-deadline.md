...
grace also applies to this deadline. An idle table with a valid snapshot
does not need state changes to remain connected.

Validation: 36 client tests pass locally across the mux and recovery suites.
Six new cases cover the missing initial snapshot, repeated sequence gaps,
same-sequence resync replies, rejected stale snapshots, engine rebuilds, and
background/wake recovery. The first two failed before the implementation.
