...
CI integration: moved the two new behavior tests into tests/unit, the client runner’s test tree. They were counted as unimported production modules under src/. The orphan baseline is unchanged; runtime imports and the lazy report chunk remain intact.
