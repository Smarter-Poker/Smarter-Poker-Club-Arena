...
Integration follow-up: the shared PostgreSQL fixture now owns entry_purchase_idempotency_receipts. Removed the duplicate CREATE TABLE from the rebuy fixture instead of weakening table constraints or skipping the test. The complete isolated run passes with the current main fixture, including all 46 rebuy cases and the shared helper, horse, ticket, agent, cashout, BBJ and insurance cases. CI failure was SQLSTATE 42P07 during fixture setup, before rebuy execution.
