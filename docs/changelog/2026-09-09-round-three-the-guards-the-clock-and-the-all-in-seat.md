...
- **A tournament player was moved with no warning at all.** `TableBreakEngine` is
  constructed for every engine and no method on it is ever called, so the
  30-second countdown, `TABLE_BREAK_STARTED`, `PLAYER_MOVED` and
  `TABLE_BREAK_COMPLETED` were dead — the live path's only announcement fires
  _after_ the seats have moved. Rather than delete a warning players should
  have, the warning is now wired into the live path: the balancer announces
  `table_break_warning` with the ids being moved, waits the window, then
  executes; the bridge relays it onto the long-declared bus event and the HUD
  shows "Table Breaking In 0:27" only to the hero being moved.
