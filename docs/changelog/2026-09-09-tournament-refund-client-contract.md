...
Concurrent main work supplied the shared exact receipt parser, ticket-aware return value and one transport replay. The merge preserves those changes and wraps the canonical unregister operation with durable identity rather than maintaining a second parser. Equivalent malformed-receipt cases are covered by the shared ticket service suite; duplicate cases were removed. The integrated run passed 106 tests across three files, and TypeScript passed. The new storage helper returns the canonical result so callers retain ticket refund messaging.
