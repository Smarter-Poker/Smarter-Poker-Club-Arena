...
The normal main merge changed only unrelated existing documentation. The integrated production build exited zero at 24bfeda9 with behind-main=0. Pre-push found one additional source assertion that forbade all registration replay because the former RPC had no identity; it now requires the retained-request RPC, explicit request ID and exact registration-row lookup, while retaining the no-generic-retry assertion. The behavioral retry tests remain unchanged. Publication is not yet claimed.
