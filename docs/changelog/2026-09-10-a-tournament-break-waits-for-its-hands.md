...
The endpoint follow-up reproduces a late entrant pause finishing after its break owner ended. A generation token now prevents that old completion from releasing a newer break; a completed pause for the same ended owner is released immediately. The caller reads the current countdown and deadline after the pause returns. One additional regression case failed before the correction and passes afterward, including both ended-owner and replaced-owner branches; the existing eight behavior cases were skipped. Server TypeScript passes.
