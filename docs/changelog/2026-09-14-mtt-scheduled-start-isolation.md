...
## Full CI follow-through

Required CI34795506067 retained two failures outside the original selected suite: DirectEngineRecovery.guard still counted four bootstrap jobs, and the new discovery test counted an import-time Supabase.FATAL warning from the credential-free CI environment as an operation error. The shutdown guard now explicitly includes the fifth scheduled-MTT loop and verifies its lifecycle fence. Each test clears pre-test module diagnostics before exercising discovery, retaining all under-test error assertions. No runtime source, secret, provider, shutdown or safety check is changed. With the service key absent, all87affected cases/sevenfiles pass and server TypeScript passes. The original full5,409/359local result remains distinct from required fullCI; a new ordinary source push is needed for CI requalification.
