...
The additive nullable column preserves the table's existing engine-only ACL and rejects incompatible schema/permissions. Legacy/unreadable snapshots retain the existing allowance fallback and report read errors; this does not certify preservation after an unplanned process loss without a durable park. Existing best-effort VIP consumption behavior is unchanged.
