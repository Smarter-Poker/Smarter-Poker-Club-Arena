-- 2026-09-17-sweeps-and-satellites-take-their-own-lanes.rollback.sql
--
-- Rollback of 20260917191322_sweeps_and_satellites_take_their_own_lanes.sql:
-- restores every definition it replaced, byte for byte as captured from
-- production on 2026-09-17 19:13 UTC, and drops the two helpers it created.
...
DROP FUNCTION IF EXISTS public.fn_ca_lock_settlement_lane_for_sweep_member(uuid);
DROP FUNCTION IF EXISTS public.fn_ca_lock_settlement_lane_for_satellite_finish(uuid);
DROP FUNCTION IF EXISTS public.fn_ca_settlement_lane_doctrine();
