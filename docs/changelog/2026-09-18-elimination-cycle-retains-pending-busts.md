...
The current-main full weekly accounting manifest still pinned the pre-PR4820 observer-error ratchet. Reused the independently qualified exact successor binding from PR4835; all historical captures and the strengthened zero-discarded-error assertion remain unchanged. This is a prerequisite of the existing required accounting check, with no second financial implementation.
