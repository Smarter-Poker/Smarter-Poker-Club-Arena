...
## Re-landed on 2026-09-17

#4713 merged at 20:03 UTC on 2026-09-16 and was removed from `main` twenty-
seven minutes later by #4715 (848b99f155), the restoration's replay hold,
before any engine release carried it: the engine that went live at 00:57 UTC
(a0d001e2) has none of it. Measured on that engine at 05:00 to 05:15 UTC:
15,000 to 19,000 wait-loop reads a minute (no backoff), and at the 05:00
hour boundary every one of 1,314 tournament managers lost its lease proof
inside four seconds and 1,978 table engines killed themselves, because the
Sep 13 baseline's blind clock retries `fn_publish_tournament_blind_level`
every second per manager while the platform is frozen (48,170 refusals in
three minutes; the fix for that, `blindClockNeedsThawResync` from #4607,
was also removed by the restore and belongs to the MTT workstream, not to
this change). This change is the same diff cherry-picked onto the restored
...
- Recorded for the MTT workstream, with evidence, not acted on here: (a) the
  hour-boundary blind publication storm above; (b) PostgREST connections
  measured sitting in `ClientRead` for 10 to 16 s mid-transaction while
  holding the `tournaments` row lock (`UPDATE tournaments SET
level_started_at`, `fn_publish_tournament_blind_level`), which is where
  `fn_eliminate_tournament_player_atomic` spends its 6 to 8 s and hits the
  8 s statement timeout (its own reads and writes measured under 40 ms in a
  rolled-back probe); (c) 1,125 RUNNING Spins at 05:00 UTC whose last bust
  is captured but unrecorded for that reason, holding their horses booked.
