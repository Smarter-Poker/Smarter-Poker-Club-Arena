...
* They never had a `jackpot_draw`: `chip_ledger` for the 106 holds only `tournament_buyin` (318 rows, 7,872.00) and `spin_entry` (106 rows, 7,242.24) legs; no `spin_prize` leg ever existed. No function in `pg_proc` contains `DELETE FROM spin_reserve_ledger` (checked), and `spin_reserve_receipt_is_immutable` forbids it. Crons `spin_sweep_unbooked` (*/5), `spin_repair_missing_multiplier` (*/15), `ca-spin-return-unawarded-draws-15m`, `spin_chip_conservation_hourly`, `spin_fairness_check_hourly`, `spin_unpaid_check` only touch RUNNING/COMPLETED/CANCELLED spins — none writes a REGISTERING row.
