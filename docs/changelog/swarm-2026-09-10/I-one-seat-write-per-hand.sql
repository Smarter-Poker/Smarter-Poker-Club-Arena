-- ===========================================================================
-- Workstream I: the settlement writes each seat row ONCE per hand
-- 2026-09-10, PokerIQ-Production (kuklfnapbkmacvwxktbh), Postgres 17
--
-- Two CREATE OR REPLACE statements, no signature change, no ACL change, no
-- other DDL. Apply both in one batch; core first, door second (either order
-- is safe: each side degrades to today's two-write behaviour while the other
-- side is still the old body, see I-one-seat-write-per-hand.md).
--
--   1. fn_ca_settle_hand_stacks_absolute (owner-only core): when the
--      12-argument door has published the hand's time-bank items for this
--      exact table+hand in the transaction-local setting
--      app.ca_hand_time_banks, the stack UPDATE also sets
--      time_bank_uses_remaining / time_bank_remaining under the same guards.
--      No envelope (any other caller) -> the unchanged stack-only UPDATE.
--   2. fn_ca_commit_hand_settlement (service door): publishes that envelope
--      before calling down, clears it right after, and in its time-bank loop
--      proves the exact seat state with the predicate it already used for the
--      lawful-noop rule BEFORE writing; it writes only a seat the core did not
--      carry (a cash seat that left during the hand; a diamond-asset cash
--      hand, which the core delegates before the fold).
--
-- Bases: the production bodies as of 2026-09-10 05:15 UTC, which already
-- include the diamonds-asset delegation added earlier tonight. Verify before
-- applying that they are still the current bodies:
--   SELECT proname, md5(pg_get_functiondef(oid)) FROM pg_proc
--    WHERE proname IN ('fn_ca_settle_hand_stacks_absolute','fn_ca_commit_hand_settlement');
--   expected: fn_ca_commit_hand_settlement      44a02f56937b379e01ab2524c859c892
--             fn_ca_settle_hand_stacks_absolute 5e38ac8623ed5dc3eebe1d18f4269e8e
--
-- Do NOT wrap in BEGIN/COMMIT (the orchestrator batches). Originals are in the
-- commented ROLLBACK section at the end; the self-test DO block after the two
-- statements is a rolled-back probe (must end in ERROR P0001 PROBE_ROLLED_BACK
-- ... VERDICT: PASS).
-- ===========================================================================

-- ---------------------------------------------------------------------------
-- 1. fn_ca_settle_hand_stacks_absolute (owner-only core)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. fn_ca_commit_hand_settlement (12-argument service door)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- SELF-TEST (run AFTER apply; it is a rolled-back probe and must end in
-- ERROR P0001 "PROBE_ROLLED_BACK ..."; if it returns success, it COMMITTED -
-- report that). It drives the REAL core on one live cash seat, twice:
--   (1) with the door's envelope published: the stack write must carry the
--       time-bank columns (1 row update on table_seats, values as requested)
--   (2) without the envelope: stack-only write, time-bank untouched
-- The hand numbers/ref are synthetic; every row the core inserts
-- (settlement_idempotency_keys, ca_settlements) rolls back with the probe.
-- ---------------------------------------------------------------------------
DO $$
DECLARE
  v_id uuid; v_table uuid; v_user uuid; v_joined timestamptz;
  v_stack numeric; v_uses int; v_secs int;
  v_new_uses int; v_new_secs int;
  v_upd0 bigint; v_upd1 bigint; v_upd2 bigint;
  v_res1 jsonb; v_res2 jsonb;
  v_uses1 int; v_secs1 int; v_stack1 numeric;
  v_uses2 int; v_secs2 int; v_stack2 numeric;
  v_proof int;
  v_out text := '';
BEGIN
  SELECT ts.id, ts.table_id, ts.user_id, ts.joined_at, ts.stack,
         ts.time_bank_uses_remaining, ts.time_bank_remaining
    INTO v_id, v_table, v_user, v_joined, v_stack, v_uses, v_secs
    FROM public.table_seats ts
    JOIN public.tables tb ON tb.id = ts.table_id
   WHERE ts.left_at IS NULL AND tb.tournament_id IS NULL AND ts.stack > 0
     AND ts.terminal_closed_at IS NULL AND ts.club_id IS NOT NULL
     AND NOT EXISTS (SELECT 1 FROM public.clubs c WHERE c.id = ts.club_id AND c.lifecycle_status = 'retired')
   ORDER BY ts.joined_at
   LIMIT 1;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'PROBE_ROLLED_BACK no live cash seat to test on';
  END IF;
  v_new_uses := v_uses + 1;
  v_new_secs := v_secs + 7;

  -- (1) the door's envelope, then the real core (delta 0, so the stack does
  --     not move and conservation holds with rake/bbj/inflow NULL)
  PERFORM set_config('app.ca_hand_time_banks', jsonb_build_object(
    'table_id', v_table, 'hand_number', 999999901::bigint, 'exact', true,
    'items', jsonb_build_array(jsonb_build_object(
      'user_id', v_user, 'seat_id', v_id, 'seat_joined_at', v_joined::text,
      'uses_remaining', v_new_uses, 'seconds_remaining', v_new_secs)))::text, true);
  SELECT n_tup_upd INTO v_upd0 FROM pg_stat_xact_user_tables
   WHERE schemaname = 'public' AND relname = 'table_seats';
  v_res1 := public.fn_ca_settle_hand_stacks_absolute(
    v_table, 999999901::bigint,
    jsonb_build_array(jsonb_build_object(
      'user_id', v_user, 'stack', v_stack, 'stack_before', v_stack,
      'seat_id', v_id, 'seat_joined_at', v_joined::text)),
    NULL, NULL, 'probe-I-selftest', NULL);
  PERFORM set_config('app.ca_hand_time_banks', '', true);
  SELECT n_tup_upd INTO v_upd1 FROM pg_stat_xact_user_tables
   WHERE schemaname = 'public' AND relname = 'table_seats';
  SELECT ts.stack, ts.time_bank_uses_remaining, ts.time_bank_remaining
    INTO v_stack1, v_uses1, v_secs1 FROM public.table_seats ts WHERE ts.id = v_id;
  -- the door's proof predicate (exact generation)
  SELECT count(*) INTO v_proof FROM public.table_seats s
   WHERE s.table_id = v_table AND s.user_id = v_user
     AND s.time_bank_uses_remaining = v_new_uses AND s.time_bank_remaining = v_new_secs
     AND s.id = v_id AND s.joined_at = v_joined;
  v_out := v_out || format(E'\n(1) folded: success=%s seat_writes=%s (expect 1) stack %s->%s uses %s->%s (expect %s) secs %s->%s (expect %s) door_proof=%s (expect 1)',
    v_res1->>'success', v_upd1 - COALESCE(v_upd0, 0), v_stack, v_stack1,
    v_uses, v_uses1, v_new_uses, v_secs, v_secs1, v_new_secs, v_proof);
  IF COALESCE((v_res1->>'success')::boolean, false) IS NOT TRUE THEN
    v_out := v_out || format(E'\n    core error: %s', v_res1->>'error');
  END IF;

  -- (2) no envelope: unchanged stack-only behaviour
  v_res2 := public.fn_ca_settle_hand_stacks_absolute(
    v_table, 999999902::bigint,
    jsonb_build_array(jsonb_build_object(
      'user_id', v_user, 'stack', v_stack + 1, 'stack_before', v_stack,
      'seat_id', v_id, 'seat_joined_at', v_joined::text)),
    NULL, NULL, 'probe-I-selftest', 1);
  SELECT n_tup_upd INTO v_upd2 FROM pg_stat_xact_user_tables
   WHERE schemaname = 'public' AND relname = 'table_seats';
  SELECT ts.stack, ts.time_bank_uses_remaining, ts.time_bank_remaining
    INTO v_stack2, v_uses2, v_secs2 FROM public.table_seats ts WHERE ts.id = v_id;
  v_out := v_out || format(E'\n(2) no envelope: success=%s seat_writes=%s (expect 1) stack %s->%s (expect %s) uses %s (expect %s, untouched) secs %s (expect %s, untouched)',
    v_res2->>'success', v_upd2 - v_upd1, v_stack1, v_stack2, v_stack + 1,
    v_uses2, v_uses1, v_secs2, v_secs1);
  IF COALESCE((v_res2->>'success')::boolean, false) IS NOT TRUE THEN
    v_out := v_out || format(E'\n    core error: %s', v_res2->>'error');
  END IF;

  v_out := v_out || format(E'\nVERDICT: %s',
    CASE WHEN COALESCE((v_res1->>'success')::boolean, false)
          AND COALESCE((v_res2->>'success')::boolean, false)
          AND v_upd1 - COALESCE(v_upd0, 0) = 1 AND v_upd2 - v_upd1 = 1
          AND v_uses1 = v_new_uses AND v_secs1 = v_new_secs AND v_stack1 = v_stack
          AND v_uses2 = v_uses1 AND v_secs2 = v_secs1 AND v_stack2 = v_stack + 1
          AND v_proof = 1
         THEN 'PASS' ELSE 'FAIL - do not leave this applied' END);
  RAISE EXCEPTION 'PROBE_ROLLED_BACK seat=% %', v_id, v_out;
END $$;


-- ===========================================================================
-- ROLLBACK: the production bodies as read by pg_get_functiondef at
-- 2026-09-10 05:15 UTC (md5 above). Strip the leading "-- " from every line
-- below and run both to restore. Applying the old door with the new core, or
-- the old core with the new door, is also safe (each side falls back to
-- today's two-write behaviour).
-- ===========================================================================

-- ---- original fn_ca_settle_hand_stacks_absolute ----
-- CREATE OR REPLACE FUNCTION public.fn_ca_settle_hand_stacks_absolute(p_table_id uuid, p_hand_number bigint, p_stacks jsonb DEFAULT '[]'::jsonb, p_rake numeric DEFAULT NULL::numeric, p_bbj numeric DEFAULT NULL::numeric, p_ref text DEFAULT NULL::text, p_inflow numeric DEFAULT NULL::numeric)
--  RETURNS jsonb
--  LANGUAGE plpgsql
--  SECURITY DEFINER
...
-- ---- original fn_ca_commit_hand_settlement ----
-- CREATE OR REPLACE FUNCTION public.fn_ca_commit_hand_settlement(p_table_id uuid, p_hand_number bigint, p_stacks jsonb, p_rake numeric, p_bbj numeric, p_ref text, p_inflow numeric, p_hand_row jsonb, p_units jsonb, p_instance_id text, p_lease_generation uuid, p_post_commit_obligations jsonb)
--  RETURNS jsonb
--  LANGUAGE plpgsql
--  SECURITY DEFINER
