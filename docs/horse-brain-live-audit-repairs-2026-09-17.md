...
## Production follow-up: reads during an unrelated pending append

PR 4760 merged as `a8d4b5396dff1b69f5855a27658dd27bde2f827b`. The final required CI passed 20,712 client and 15,294 server tests (162 existing server skips), plus TypeScript and the PostgreSQL qualification. Engine release `35222833688` completed successfully at 14:57 UTC. At 16:27 UTC, production independently reported that exact release and the validated image `sha256:c7356a480551f121d7a738b763889b8d1c7dbd6368280e2e5269308cf3203a56`.

A finite 46-receipt observation contained 49,788 completed decision graphs and 3,305 volatile plan applications, with zero reported application failure or binding/effect mismatch. All eight new ledger entries were present. These aggregate counters are not a request-by-request application census: unconfirmed retirement and terminal reclamation remain distinct, and a difference between totals cannot identify lost effects.

The archive opened and validated its original-journal binding. It held 702,657 new records in 45,343 committed/reserved segments, using 1,015,549,893 compressed bytes. The first hand read succeeded, but the next seven refused with `Horse archive custody pending`: `readHand` blocked on any pending batch, even when that batch belonged to a different hand. A zero-pending storage snapshot did not prevent another append from starting before a later hand read.

The follow-up narrows this check inside the existing catalog read transaction. It validates the bounded pending batch and refuses a hand whose own custody is pending; unrelated committed hands remain readable. Corrupt or oversized pending custody still fails closed. Reading does not complete, acknowledge or mutate any append, and retains the existing private-file, selected-record and decode limits. The operational verifier must use this authoritative hand-specific result rather than impose a second global zero-pending precondition.

Both writer and separate read-only regression cases failed on the unmodified source. The final correction passed 111 checks across the existing journal and retry suites, including malformed pending segments, second-segment membership and cumulative decoding, and the complete server TypeScript check. Independent source review found no blocker. These are source checks; the follow-up still requires its own protected merge, publication and natural readback.

Capture coverage remains separate from read availability. The same receipt sample reported 29,611 journal queue-capacity refusals and 3,109 capture-unavailable events. They remain visible gaps; neither continued storage nor readable retained hands reconstructs records that were not captured. The deployed catalogue/segment allocations are finite, not a promise of indefinite retention. The automatic cross-artifact run `35237070380` failed its live-page job; the owning UI/MTT workstreams retain that separate certificate diagnosis. No broad or full-population completion follows from this follow-up.
