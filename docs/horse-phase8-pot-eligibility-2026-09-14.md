...
The first actual-controller fixture completed all six objectives with legal actions, conserved chips and individually accepted software-baseline receipts. Its overall source-identity check failed because the owned patch was moved from the pre-merge commit onto protected main while the fixture ran. A full server-source comparison confirms unchanged bytes, but that does not override the harness's failed identity receipt. The complete original run is retained. A second run on stable commit `8a434fc694` passed the complete software baseline for all six objectives, with unchanged source and p99 from 3.101 to 4.694 ms. The outer Phase 8 receipt limit is 5 ms and its work budget is 4 ms; passing the former does not establish the stricter remaining performance target.

## Benchmark worker startup parity

The audit also found a real setup mismatch: production's decision worker prepares the bounded, identity-free synthetic card facts after hydration and before readiness, while the separate league worker previously left that preparation on the first eligible decision. Preparing facts in a launcher only warms the launcher's process. It does not prepare its child. The earlier tournament fixture results above therefore retain the old league startup behavior; they are not evidence of prepared-child parity.

The league worker now calls the same preparation function in its own process after optional solver hydration and before `READY`. This also applies to offline fixtures that explicitly disable hydration. The existing startup rejection prevents readiness and queued computation if preparation fails. Three old-source failures reproduce the absent preparation, fixture mismatch and erroneous readiness on preparation failure. All three pass after the repair; the 18 focused checks include the real worker simulator and existing live-worker preparation tests. The combined change compiles and passes the full 11,998-test suite in 807 files, with 145 existing skipped tests and one skipped file. A subsequent actual-child tournament run must bind to this final source. No live policy or financial producer changes are involved in the benchmark startup repair.
