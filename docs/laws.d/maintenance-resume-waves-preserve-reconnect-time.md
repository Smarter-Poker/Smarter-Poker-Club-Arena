# server/src/maintenance/ReconnectResumeWaves.law.test.ts
...
MaintenanceBreak records each actual resume before releasing the engine. DisconnectEngine applies that table's interval before either presenting an FSM deadline or arming an auto-action. ReconnectResumeWaves.law.test.ts executes those real owners across 200 tables and all eight waves. reconnectFreeze.test.ts pins incremental, expired and mid-wave grants; reconnectProtection.test.ts pins membership eligibility.
