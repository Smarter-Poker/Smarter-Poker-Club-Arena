...
async function run() {
  const { data } = await supabase.from('profiles').select('email, id, username, first_name, last_name, full_name, display_name');
  ...
  realUsers.forEach(u => {
    const name = u.full_name || u.display_name || `${u.first_name || ''} ${u.last_name || ''}`.trim() || 'N/A';
    console.log(`- ${name} (@${u.username}) -> ${u.email}`);
  });
}
run();
