...
const need = (v) => {
  if (!v) throw new Error('RELEASE_ENGINE_STAGE_CONTRACT_REFUSED');
};
export function artifactDownload(token, repo, fetcher = fetch) {
  need(
    typeof token === 'string' && token.length > 0 && /^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(repo)
  );
  return async function* (r) {
    need(/^[1-9][0-9]*$/.test(r.github_artifact_id));
    const response = await fetcher(
      `https://api.github.com/repos/${repo}/actions/artifacts/${r.github_artifact_id}/zip`,
      {
        redirect: 'manual',
    ...
    for await (const chunk of download.body) {
      bytes += chunk.length;
      need(bytes <= r.github_archive_bytes);
      hash.update(chunk);
      yield chunk;
    }
    need(
      bytes === r.github_archive_bytes && `sha256:${hash.digest('hex')}` === r.github_archive_digest
    );
  };
}
...
export function stageTransport(hostAlias, download, spawnChild = spawn) {
  ...
  return async (action, envelope) => {
    need(['current', 'preflight', 'receive', 'observe', 'inspect'].includes(action));
    ...
    const completion = new Promise((resolve, reject) => {
      ...
      child.stdout.on('data', (chunk) => {
        ...
        if (bytes > 65536) {
          child.kill('SIGTERM');
          reject(new Error('RELEASE_STAGE_OUTPUT_LIMIT'));
      ...
      child.on('close', (code) => {
        try {
          ...
          reject(new Error('RELEASE_STAGE_READBACK_UNRESOLVED'));
        }
      });
    });
...
export class EngineStageAdapter {
  ...
  validate(r) {
    need(
      r?.target === 'club-arena-engine' &&
        ...
        ['archive_digest', 'github_archive_digest', 'artifact_image_id'].every((k) =>
          /^sha256:[0-9a-f]{64}$/.test(r[k])
        ) &&
        /^[0-9a-f]{64}$/.test(r.manifest_digest) &&
        /^[1-9][0-9]*-1$/.test(r.run_key) &&
        /^[0-9a-f-]{36}$/.test(r.build_operation_id) &&
        /^[1-9][0-9]*$/.test(r.github_artifact_id) &&
        ['archive_bytes', 'github_archive_bytes'].every(
          (k) => Number.isSafeInteger(r[k]) && r[k] > 0 && r[k] <= 3 * 1024 ** 3
        )
    );
  }
  payload(r, operation) {
    return { request: r, operation_id: operation.id, epoch: operation.epoch };
  }
  async current(r, operation) {
    this.validate(r);
    const result = await this.request('current', this.payload(r, operation));
  ...
  async preflight(r, operation) {
    this.validate(r);
    const artifact = await this.github(
      `/repos/${this.repo}/actions/artifacts/${r.github_artifact_id}`
    );
    need(
      String(artifact.id) === r.github_artifact_id &&
        !artifact.expired &&
        artifact.digest === r.github_archive_digest &&
        artifact.size_in_bytes === r.github_archive_bytes &&
        String(artifact.workflow_run?.id) === r.build_run_id
    );
    const result = await this.request('preflight', this.payload(r, operation));
    need(
      result.ready === true &&
        result.source_sha === r.expected_current.source_sha &&
        result.image_id === r.expected_current.image_id
    );
    return {
      ready: true,
      github_artifact_id: r.github_artifact_id,
  ...
  async reconcile(r, operation, { execute = false, beforeEffect } = {}) {
    this.validate(r);
    if (execute) await beforeEffect?.();
    const result = await this.request(execute ? 'observe' : 'inspect', this.payload(r, operation));
    if (result.terminal !== true) return { terminal: false, reason: 'NATIVE_STAGE_UNRESOLVED' };
    need(
      result.operation_id === operation.id &&
        result.source_sha === r.source_sha &&
        result.control_sha === r.control_sha &&
        result.run_key === r.run_key &&
        result.image_id === r.artifact_image_id &&
        result.archive_digest === r.archive_digest &&
