...
const need = (v) => { if (!v) throw new Error('RELEASE_ENGINE_READINESS_UNPROVEN'); };
export const catalogueDigest = (rows) => createHash('sha256').update(JSON.stringify(rows)).digest('hex');
export async function readDoorCatalogue(client, names) {
  need(Array.isArray(names) && names.length > 0 && names.length <= 2048 &&
    names.every((name) => /^[A-Za-z_][A-Za-z0-9_]*$/.test(name)));
  await client.query('BEGIN READ ONLY');
  try {
    const result = await client.query(`SELECT p.proname AS name,
      ...
      ORDER BY p.proname,pg_get_function_identity_arguments(p.oid)`, [names]);
    return result.rows;
...
export function liveCatalogue(config, principal) {
  ...
  return async (names) => {
    ...
    try {
      ...
      need(role?.rolname === principal && role.rolcanlogin && !role.rolsuper && !role.rolbypassrls &&
...
export async function publicReleaseJSON(url, fetcher = fetch) {
  need(['https://engine.smarter.poker/health', 'https://ca-static.smarter.poker/build-info.json',
    'https://smarter.poker/hub/club-arena/build-info.json'].includes(url));
  const response = await fetcher(`${url}?release_probe=${randomUUID()}`, { redirect: 'error',
...
export class EngineReadiness {
  constructor({ catalogue, intake, publicJSON = publicReleaseJSON }) { Object.assign(this, { catalogue, intake, publicJSON }); }
  async verify(snapshot, request, operation) {
    const { queue: q, receipts } = snapshot;
    const component = q.resolution_manifest.components.find((c) => c.target === 'club-arena-engine');
    const compatibility = component?.compatibility;
    need(compatibility?.mode === 'backward-compatible-engine' &&
      ...
      Array.isArray(compatibility.receipt_refs) && compatibility.receipt_refs.length > 0);
    const doors = receipts.BUILD.data.database_doors;
    need(doors && Array.isArray(doors.names) && doors.exceptions &&
    ...
    need(doors.names.every((name) => present.has(name) || (typeof doors.exceptions[name] === 'string' && doors.exceptions[name].length > 0)));
    const origin = sourceSHA(await this.publicJSON('https://ca-static.smarter.poker/build-info.json'));
    const publicWeb = sourceSHA(await this.publicJSON('https://smarter.poker/hub/club-arena/build-info.json'));
    ...
    need(health.maintenance?.policyVersion === 2 && health.maintenance.policyDigest === operationPolicyDigest &&
      typeof health.maintenance.activationReceipt === 'string' && health.maintenance.activationReceipt.length > 0);
    need(health.running === true && health.releaseSha === request.expected_current.source_sha &&
      typeof health.instanceId === 'string' && health.instanceId.length > 0);
    return { compatibility_verified: true, technical_gates_passed: true,
      database_contract_digest: compatibility.database_contract_digest, operation_policy_digest: operationPolicyDigest,
      expected_current: request.expected_current, retained_components: { 'club-arena-web': { source_sha: origin } },
      maintenance_activation_receipt: health.maintenance.activationReceipt,
      prior_engine_instance: health.instanceId, provider_requests: { 'hetzner-intake': request }, host };
  }
}
