...
export function lastCommand(value, expected) {
  assert.equal(
    (Array.isArray(value) ? value : [value]).at(-1)?.command,
    expected,
    'FIXTURE_ROLE_NATIVE_TERMINAL'
  );
}
export class RoleNativeOwner {
  ...
  async bounded(action, cleanup = false) {
    ...
    assert.ok(
      performance.now() < until && (cleanup || !this.signal?.aborted),
      'FIXTURE_ROLE_NATIVE_DEADLINE'
    );
    let timer, aborted;
    try {
  ...
  make(
    ...
    const client = new this.Client({
      host: tcp ? '127.0.0.1' : '/run/postgresql',
      port: 5432,
      database,
      user: role,
      ...(tcp ? { password } : {}),
  ...
  async connect(role, options, cleanup = false) {
    ...
    const row = (
      await this.query(
        r,
        `SELECT pg_backend_pid() AS pid, session_user AS session_role,
    ...
    assert.equal(row.session_role, r.role);
    assert.equal(row.current_role, r.role);
    assert.equal(row.database, options?.database ?? 'club_arena_qualification');
    assert.equal(row.superuser, r.role === 'supabase_admin');
    assert.equal(row.tcp, r.tcp);
  ...
  async closeAll() {
    ...
    assert.ok(
      results.every((r) => r.status === 'fulfilled') &&
        this.records.every((r) => r.ended && !r.error),
      'FIXTURE_ROLE_NATIVE_CLIENT_CLEANUP'
    );
  }
  async snapshot(expected, heldPid, cleanup = false) {
    const observer = await this.connect('supabase_admin', { readOnly: true }, cleanup);
    try {
      const captured = await this.bounded(
        () => captureFixtureServicePreimage(observer.client, heldPid),
        cleanup
      );
      ...
      await this.close(observer);
    }
  }
  async absent(pids) {
    const observer = await this.connect('supabase_admin', { readOnly: true });
    try {
      // Closing the client socket is not acknowledgment that PostgreSQL has
      // retired its backend. Observe the actual exit without extending the
      // existing work deadline or admitting an unknown result.
      const until = Math.min(this.workDeadline, performance.now() + 1500);
      for (;;) {
        assert.ok(performance.now() < until, 'FIXTURE_ROLE_NATIVE_BACKEND_EXIT_UNOBSERVED');
        await this.query(observer, 'SELECT pg_stat_clear_snapshot()');
        const rows = (
          await this.query(
            observer,
            'SELECT NOT EXISTS(SELECT FROM pg_stat_activity WHERE pid=ANY($1::integer[])) AS absent',
            [pids]
          )
        ).rows;
        assert.ok(performance.now() < until, 'FIXTURE_ROLE_NATIVE_BACKEND_EXIT_UNOBSERVED');
        if (rows.length === 1 && rows[0].absent === true && Object.keys(rows[0]).length === 1)
          break;
        assert.deepEqual(rows, [{ absent: false }]);
        assert.ok(performance.now() < until, 'FIXTURE_ROLE_NATIVE_BACKEND_EXIT_UNOBSERVED');
        await this.bounded(() => new Promise((resolve) => setTimeout(resolve, 25)));
      }
    } finally {
      await this.close(observer);
    }
