...
# The attached Auth phase creates three synthetic local users only after the
# positive alignment/provider result. No application archive or funded seed.
...
docker run --detach --name "$preimage" --network "$network" --read-only --user 1000:1000 \
  ...
  --tmpfs /run:rw,nosuid,mode=0755,uid=1000,gid=1000,size=2g \
  --tmpfs /var/lib/postgresql:rw,nosuid,mode=0700,uid=1000,gid=1000,size=4g \
  "$image_id" fixture-server qualify-role-providers-auth >/dev/null
