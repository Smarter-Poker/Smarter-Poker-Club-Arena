...
function addedRows(before, after) {
  ...
  for (const row of before)
    assert.deepEqual(after.find((next) => next[0] === row[0]), row, 'INSURANCE_HISTORY_CHANGED');
...
export function verifyInsuranceEconomics({ owner, offered, accepted, settled }) {
  ...
  const start = structuredClone(offered), response = structuredClone(accepted), end = structuredClone(settled);
  ...
  const actor = start.entry.actor_id, hand = start.entry.hand_number;
  ...
  for (const checkpoint of [start, response, end]) {
    ...
    assert.deepEqual(checkpoint.facts.scope, [[tableId, clubId, null, null, 'false', null]], 'INSURANCE_SCOPE');
  }
  ...
  const quotedPremium = quoteCents(quote.fullPremium), insured = quoteCents(quote.fullInsuredAmount);
  ...
  assert.equal(end.facts.offers.filter((row) => row[2] === 'offered').length, 1, 'INSURANCE_OFFER_COUNT');
  ...
  assert.ok(Array.isArray(winners) && winners.length > 0 && winners.length <= 2, 'INSURANCE_WINNERS');
  assert.equal(new Set(winners).size, winners.length, 'INSURANCE_WINNERS');
  assert.ok(winners.every((id) => actorIds.includes(id)), 'INSURANCE_WINNERS');
  const won = winners.includes(actor), push = won && winners.length > 1;
  const premium = won && !push ? quotedPremium : 0n, payout = won ? 0n : insured;
  ...
  const beforeBank = start.facts.banks[0], afterBank = end.facts.banks[0];
  ...
  const bankRows = added.filter((r) => r[1] === 'insurance_bank' || r[3] === 'insurance_bank' || r[6] === 'insurance');
  assert.equal(bankRows.length, premium || payout ? 1 : 0, 'INSURANCE_LEDGER_COUNT');
  if (bankRows.length) {
    const ledger = bankRows[0];
    const endpoints = premium ? ['table_stack', tableId, 'insurance_bank', afterBank[2]] : ['insurance_bank', afterBank[2], 'table_stack', tableId];
  ...
  return {
    ...
    table_id: tableId, club_id: clubId, actor_id: actor, hand_number: hand,
    branch: push ? 'push' : won ? 'premium' : 'payout',
    premium_cents: String(premium), payout_cents: String(payout),
    bank_delta_cents: String(closing - opening), bank_wallet_id: afterBank[2],
    receipt_id: row[0], ledger_ids: bankRows.map((r) => r[0]),
