...
BEGIN
  ...
  IF v_count=1 THEN
    PERFORM r46_mtt_isolation.assert_true((SELECT max_players IS NULL AND min_players=3 AND table_size=9
      AND format_contract='mtt-v2' AND tournament_type=CASE WHEN p_case LIKE 'restart_%' THEN 'MTT' ELSE 'SATELLITE' END AND variant='satellite' AND status='REGISTERING'
