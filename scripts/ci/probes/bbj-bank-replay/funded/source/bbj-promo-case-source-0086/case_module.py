...
The sole financial statement is the genuine main->promo25 function invocation.
"""
...
SHARED=Path('/Users/smarter.poker/Documents/Codex/2026-09-11/you-are-the-program-overseer-for-2/work/g8/bbj-backup-source-0002')
def read(n):
 local={'INTEGRITY.json','SOURCE-PINS.json','OPERATION.json'}
 return json.loads(((HERE if n in local else SHARED)/n).read_text())
...
def verify_authorization(a,driver,seed,opening):
 ...
 fixed=read('OPERATION.json')
 require(operation==fixed['move_operation'] and reason==fixed['move_reason'],'Exact source-bound promo operation/reason required')
...
def metadata(c,check):
 ...
 b=r.get('backup',{});require(b.get('source_model_sha256')==sha(SHARED/'DEPENDENCY-CONTRACT.json') and b.get('passed')is True and b.get('raw_catalog_observations_retained')is True and b.get('historical_support_scope_matched')is True,'Root must qualify exact supplemental/historical support and preserve raw catalogs')
...
def transfer_sql(pool,operation,reason):
 canon_uuid(pool)
 return 'SELECT public.fn_bbj_move_between_banks('+literal(pool)+"::uuid,'main'::text,'promo'::text,25.00::numeric,"+literal(reason)+'::text,'+literal(operation)+'::text)'
...
def validate_transfer(before,after,reply,operation,reason,application_name):
 ...
 require(set(old)==set(new) and all(new[k]==v for k,v in old.items() if k not in {'main_balance','promo_balance','updated_at','_xmin'}),'Unrelated pool field changed')
 ...
 require(dec(old['main_balance'])==100 and dec(old['promo_balance'])==0 and dec(new['main_balance'])==75 and dec(new['promo_balance'])==25 and dec(new['backup_balance'])==dec(new['pool_amount'])==0,'Exact100->75/25 balances required')
 ...
 want={'pool_id':new['id'],'from_bank':'main','to_bank':'promo','amount':25,'reason':reason.strip(' '),'op_id':operation,'performed_by':ACTOR,'db_role':'postgres'}
 ...
 require(type(reply)is dict and reply.get('ok')is True and reply.get('replayed')is False and reply=={'ok':True,'replayed':False,'move_id':receipt['id'],'amount':25,'from_bank':'main','to_bank':'promo'},'Actual move reply mismatch/replay/refusal')
 ...
 for col,start,end in [('main_balance',100,75),('promo_balance',0,25)]:
  ...
  else:wanted.update(from_label=None,to_label='bbj_pools.promo_balance',pre_to_balance=start,post_to_balance=end)
...
def ordinary_and_aliases(c,pool,legs):
 ...
 expected=[]
 for col,net in [('main_balance',-25),('promo_balance',25)]:expected.append(dict(account_key='bbj_pool:'+pool['id']+':bbj_pools.'+col,account_type='bbj_pool',entity_id=pool['id'],club_id=None,column_name='bbj_pools.'+col,net=net,legs=1,unkeyable=0))
 ...
 aliases=[]
 for col,want in [('main_balance',75),('backup_balance',0),('promo_balance',25)]:
...
def pair(c,driver,opening,combined,observations,phase,prior,bank,main,backup):
 ...
 if phase=='FIRST-PROMO-POSITIVE':
  require(replay['baselines']==1 and replay['checked']==2 and bykey[backup]['is_baseline']is True and bykey[backup]['note']=='baseline: first reading of this account, not judged','Promo must be its first real25 baseline')
 ...
 else:require(phase=='STABLE-PROMO' and replay['baselines']==0 and replay['checked']==3 and all(x['is_baseline']is False and opening.notes(x)==(Decimal(0),)*5 for x in snaps),'Stable three-account replay required')
 return after
...
def execute_prepared_case(case,connect,seed,base_driver,authorization,combined=None,metadata_check=None,opening_module=None,funding_result=None):
 ...
 try:
  observer=conn('bbj_promo_observer');writer=conn('bbj_promo_writer');observations.append({'preflight':metadata(observer,metadata_check)})
  before=capture(observer);observations.append({'funded_before':before});pool,bank,main=validate_funding(funding_result,before,opening_module,authorization);backup='bbj_pool:'+pool['id']+':bbj_pools.promo_balance'
  ...
  fresh=conn('bbj_promo_durable');durable=capture(fresh);observations.append({'durable':durable});raw_equal(produced,durable);validate_transfer(before,durable,reply,authorization['move_operation'],authorization['move_reason'],identities[1]['application_name'])
  observations.append({'ordinary_and_aliases':ordinary_and_aliases(fresh,pool,legs)})
  cut=pair(fresh,base_driver,opening_module,combined,observations,'FIRST-PROMO-POSITIVE',durable,bank,main,backup)
  cut=pair(fresh,base_driver,opening_module,combined,observations,'STABLE-PROMO',cut,bank,main,backup)
  final=capture(fresh);raw_equal(cut,final);observations.append({'final':final,'postflight':metadata(fresh,metadata_check)})
  result.update(status='PASS_IMPLEMENTED_SUBSET',selected_class='bbj_pools.promo_balance',residual='SEQ06-GAP-SIDE-FILTERS',money_loss_claim=False)
  ...
  if before is not None and result['commit_status']=='NOT_ATTEMPTED':
   try:
    proof=capture(conn('bbj_promo_failure_fresh'));observations.append({'rollback_fresh':proof});raw_equal(before,proof)
   ...
   try:observations.append({'uncertain_commit_fresh':capture(conn('bbj_promo_uncertain_fresh'))})
