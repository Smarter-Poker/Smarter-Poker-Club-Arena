"""Inert composition: original genuine opening100, then reviewed promo25 once."""
...
def promo_module():
    return module('reviewed_promo0086', Path(load(HERE / 'SOURCE-PINS.json')['promo_packet']) / 'case_module.py')
...
def execute_prepared_case(case, connect, seed, base_driver, authorization, combined=None, metadata_check=None, database=None, database_oid=None):
    ...
    report = {'case': case, 'status': 'RUNNING', 'events': [], 'observations': [], 'funding_result': None,
              'promo_result': None, 'failures': [], 'root_preseed_cleanup': [], 'whole_original_case_qualified': False,
              'native_financial_foundation_qualified': False, 'opening_dispatch_count': 0, 'promo_dispatch_count': 0, 'seed_dispatch_count': 0, 'phase_calls': []}
    seed_calls = 0
    probe = None
    def single_seed():
        nonlocal seed_calls
    ...
    def phase_callback(names):
        ...
        return run
    try:
        probe = connect('promo_preseed_guard', report['events'])
        report['observations'].append({'preseed_metadata': metadata_check(probe)})
        empty = promo.capture(probe)
        report['observations'].append({'preseed_raw': empty})
        names = promo.read('FINGERPRINT-CONTRACT.json')['new_supporting_relations']
        ...
        require(funding.get('status') == 'PASS_IMPLEMENTED_SUBSET' and funding.get('cleanup_errors') == [] and seed_calls == 1,
                'Original fresh opening failed; promo not invoked')
        ...
        promo_auth = {'authorized': True, 'case': CASE, 'case_review_accepted': True, 'root_adapter_review_accepted': True,
                       'supplemental_source_review_accepted': True, 'packet_sha256': pins['promo_integrity_sha256'],
        ...
        report['promo_dispatch_count'] += 1
        report['promo_result'] = promo.execute_prepared_case(CASE, connect, single_seed, base_driver, promo_auth,
                    phase_callback(['promo:FIRST-PROMO-POSITIVE', 'promo:STABLE-PROMO']), metadata_check, opening, funding)
        require(report['promo_result'].get('status') == 'PASS_IMPLEMENTED_SUBSET', 'Promo case failed; full result retained')
        require(seed_calls == 1 and len(report['phase_calls']) == 5 and all(x['status'] == 'RETURNED' for x in report['phase_calls']), 'Exact opening3 plus promo2 phases required')
