...
step "b_claim_commit" { SELECT pg_temp.claim(); }
step "b_claim_rollback" { SELECT pg_temp.claim(); }
session "observer"
step "observed_wait" {
 ...
 RAISE EXCEPTION 'MOVEMENT_RACE_WAIT_NOT_PROVEN'; END IF; RAISE NOTICE 'MOVEMENT_RACE_WAIT_PROVEN'; END $$;
}
step "final_state" {
 ...
 RAISE NOTICE 'MOVEMENT_RACE_EFFECTS_PROVEN'; END $$;
}
