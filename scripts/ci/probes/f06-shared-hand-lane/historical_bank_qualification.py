...
def qualify(root,out,cmd,command,run,probe,require,results):
    ...
    def adapted(name,sql,expected=None,error=None):
        ...
        if name=='retired-origin-local-capture-shape':
            ...
            # with the byte-exact 20260919033536 text carrying interval '260 seconds'
            # in place of '285 seconds' (the 300 s countdown less the 285 s entry
            # reserve less the 25 s checkpoint budget; commit 5cf1486c moved the
            # process side to the same figure). Its pre-image refuses unless the
            ...
            run('historical-legacy-reserve-identity',"SELECT p.oid::regprocedure::text||' '||md5(prosrc)||' '||md5(pg_get_functiondef(p.oid)) FROM pg_proc p WHERE p.oid=to_regprocedure('public.fn_f06_prepare_mixed_manager_custody(uuid,uuid,uuid,uuid,jsonb,jsonb)');",
                'fn_f06_prepare_mixed_manager_custody(uuid,uuid,uuid,uuid,jsonb,jsonb) 6ec26321609a5e101bf8fba63a6beaaf 945338c6b340cee0d1464a7603f72992')
        if name=='retired-origin-local-proof-store':
            ...
            # The installed reserve, at the RPC's own clock: 259 s remaining refuses,
            # 260.5 s is admitted (half a second covers the statements between the
            # UPDATE's clock_timestamp() and the RPC's), so the figure is 260 exactly.
            ...
            probe('historical-legacy-reserve-refuses-259',service+remaining(259)+prepare,error='F06_MIXED_FROZEN_CHECKPOINT_UNPROVEN')
            probe('historical-legacy-reserve-admits-260',service+remaining('260.5')+'SELECT ('+prepare.rstrip(';')+")->'receipt';",'null')
