...
BEGIN
  ...
  IF (SELECT count(*) FROM pg_trigger WHERE tgrelid='public.tournaments'::regclass
      AND tgname IN ('a0_tournaments_unlimited_entry_capacity',
...
SELECT '46462000-0000-4000-8000-000000000003','46462000-0000-4000-8000-000000000002',
  'R46 isolation target','MTT','freezeout','NLH',180,20,10000,3,2,9,0,
  ...
  '[{"place":1,"percentage":100}]',1,1 FROM generate_series(1,24) n;
INSERT INTO public.tournaments(
  id,club_id,name,tournament_type,variant,game_type,buy_in_amount,buy_in_fee,
  starting_chips,min_players,max_players,table_size,current_players,status,start_time,ended_at,
  is_bounty,is_pko,is_mystery_bounty,is_premium_spin,guaranteed_prize,prize_pool,
  prize_pool_finalized,blind_structure,payout_structure,payout_math_version,payout_unit_cents,
  restart_every_minutes,satellite_target,satellite_seats,synchronized_breaks,late_reg_levels,late_reg_mins
)
SELECT '46462000-0000-4000-8000-000000000004',club_id,'R46 completed legacy source',
  'SATELLITE','sng',game_type,90,10,starting_chips,3,2,9,0,'COMPLETED',
  clock_timestamp()-interval '2 hours',clock_timestamp()-interval '1 minute',
  false,false,false,false,0,0,false,blind_structure,payout_structure,1,1,5,id,1,true,0,0
...
BEGIN
  ...
  PERFORM r46_mtt_isolation.assert_true(r->'ok'='true'::jsonb AND r->>'outcome'=p_expected
    ...
    AND r->'union_id'='null'::jsonb AND (r->>'tournament_id')::uuid IS NOT NULL, 'exact ensure outcome and owner/target receipt');
...
BEGIN
  BEGIN
    UPDATE public.tournaments SET variant='pko' WHERE id='46462000-0000-4000-8000-000000000003';
  EXCEPTION WHEN invalid_parameter_value THEN
...
BEGIN
  BEGIN
    INSERT INTO public.tournaments(
      ...
      guaranteed_prize,starting_chips,max_players,min_players,table_size,current_players,status,
      blind_structure,payout_structure,start_time,late_reg_levels,late_reg_mins,
      synchronized_breaks,satellite_target_id,satellite_seats,restart_every_minutes,restart_source_id
    ) SELECT club_id,union_id,'R46 restart '||p_actor,game_type,'satellite','SATELLITE',
      buy_in_amount,buy_in_fee,0,starting_chips,NULL,3,table_size,0,'REGISTERING',
      blind_structure,payout_structure,clock_timestamp()+make_interval(mins=>p_minutes),0,0,
      true,COALESCE(satellite_target_id,satellite_target),1,5,id
    FROM public.tournaments WHERE id='46462000-0000-4000-8000-000000000004'
...
BEGIN
  ...
  IF v_count=1 THEN
    PERFORM r46_mtt_isolation.assert_true((SELECT max_players IS NULL AND min_players=3 AND table_size=9
      AND tournament_type='SATELLITE' AND variant='satellite' AND status='REGISTERING'
      AND satellite_target_id='46462000-0000-4000-8000-000000000003'
      ...
      FROM public.tournaments WHERE id=v_event),'committed creation keeps its unlimited and unfunded contract');
    ...
    END IF;
  END IF;
  PERFORM r46_mtt_isolation.assert_true((SELECT variant=CASE WHEN p_case='edit_before_creation' THEN 'pko' ELSE 'freezeout' END
  ...
  PERFORM r46_mtt_isolation.assert_true(NOT EXISTS(SELECT 1 FROM public.tournament_players)
    ...
    AND NOT EXISTS(SELECT 1 FROM public.chip_ledger WHERE tournament_id IN
      (SELECT id FROM public.tournaments)),'creation race invents no entrant, table or chip ledger movement');
