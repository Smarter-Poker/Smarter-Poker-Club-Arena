...
setup {
 DO $check$ BEGIN
  IF inet_server_addr() IS NOT NULL OR current_database() !~ '^r46_mtt_'
   ...
   RAISE EXCEPTION 'REGISTRATION_ADMISSION_LOCK_PRIVATE_FIXTURE_REQUIRED';
  END IF;
 END $check$;
 ...
 VALUES('46464300-0000-4000-8000-000000000001','Native acquisition barrier','MTT','freezeout',3,3,9,0,0,10000,0,'REGISTERING',now()+interval '1 hour','mtt-v1');
 SET session_replication_role=origin;
}
teardown {
 SET session_replication_role=replica;
 DELETE FROM public.tournaments WHERE id='46464300-0000-4000-8000-000000000001';
 SET session_replication_role=origin;
}
session "reader"
setup { SET application_name='r46-registration-reader'; SET statement_timeout='10s'; SET idle_in_transaction_session_timeout='15s'; }
step "reader_begin" { BEGIN; }
step "reader_lock" { SELECT public.fn_ca_lock_tournament_seat_acquisition('46464300-0000-4000-8000-000000000001',NULL,NULL); }
step "reader_commit" { COMMIT; }
step "reader_rollback" { ROLLBACK; }
teardown { ROLLBACK; }
session "writer"
setup { SET application_name='r46-registration-writer'; SET statement_timeout='10s'; SET idle_in_transaction_session_timeout='15s'; }
 ...
 DO $wait$ BEGIN
  IF NOT EXISTS(SELECT 1 FROM pg_stat_activity w JOIN pg_stat_activity r
    ...
    WHERE w.datname=current_database() AND r.datname=current_database()
      AND w.application_name='r46-registration-writer' AND r.application_name='r46-registration-reader'
      AND w.wait_event_type='Lock') THEN
   RAISE EXCEPTION 'REGISTRATION_ADMISSION_LOCK_WAIT_NOT_PROVEN';
  END IF;
  RAISE NOTICE 'REGISTRATION_ADMISSION_LOCK_WAIT_PROVEN';
 END $wait$;
}
