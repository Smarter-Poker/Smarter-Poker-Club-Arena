...
-- The alternate original fee writer uses the same cancellation contract.
UPDATE pg_temp.tournaments SET status='REGISTERING',total_rake=0.48;
DELETE FROM pg_temp.rake_records WHERE source='atomic_cancel_tournament';
UPDATE pg_temp.rake_records SET source='fn_spin_settle_game';
result:=pg_temp.atomic_cancel_tournament(event,NULL);
IF (SELECT sum(rake_amount) FROM pg_temp.rake_records)<>0 OR NOT EXISTS(SELECT 1 FROM pg_temp.rake_records WHERE metadata->>'original_source'='fn_spin_settle_game') THEN RAISE EXCEPTION 'FAIL alternate Spin fee writer was not reversed'; END IF;
