...
BEGIN
 ...
 CREATE TEMP TABLE probe_marks(n integer) ON COMMIT DROP;
 INSERT INTO pg_temp.probe_marks VALUES(0);
 ...
 CREATE TEMP TABLE probe_values(invoice jsonb,r1 jsonb,r2 jsonb,r3 jsonb DEFAULT '{"amount":25,"payees":2,"shortfalls":0}') ON COMMIT DROP;
 INSERT INTO pg_temp.probe_values(invoice,r1,r2) VALUES('{"success":false,"error":"invoice transport failed"}','{"success":true,"period_rake":100,"total_rakeback":90,"union_retained":10}','{"amount":50,"payees":1,"shortfalls":0}');
 ...
 EXECUTE $f$CREATE FUNCTION pg_temp.fn_settle_round3_agents_to_players(uuid,timestamptz,timestamptz) RETURNS jsonb LANGUAGE sql AS 'SELECT r3 FROM pg_temp.probe_values'$f$;
 EXECUTE $f$CREATE FUNCTION pg_temp.fn_union_mark_period_settled(uuid,timestamptz,timestamptz) RETURNS void LANGUAGE plpgsql AS 'BEGIN UPDATE pg_temp.probe_marks SET n=n+1; END'$f$;
...
SELECT pg_get_functiondef('public.fn_union_settlement_conservation_assert(uuid,timestamptz,timestamptz,jsonb,jsonb,jsonb)'::regprocedure) INTO src; src:=replace(replace(replace(src,'public.','pg_temp.'),'''public''','''pg_temp'''),'now()','''2026-09-14 12:15Z''::timestamptz'); EXECUTE src;
 ...
 UPDATE pg_temp.union_wallets SET rake_wallet=0;
 UPDATE pg_temp.probe_marks SET n=0;
 UPDATE pg_temp.probe_values SET r1='{"success":true,"period_rake":100,"total_rakeback":90,"union_retained":10}',invoice='{"success":true,"invoices":2}',r2='{"amount":50,"payees":1,"shortfalls":1}';
 j:=pg_temp.fn_union_settlement_cascade(u,f,t);
 IF j->>'success' IS DISTINCT FROM 'false' OR (SELECT n FROM pg_temp.probe_marks)<>0 THEN RAISE EXCEPTION 'FAIL unpaid recipient marked period settled'; END IF;
 UPDATE pg_temp.probe_values SET r2='{"amount":50,"payees":1}';
 j:=pg_temp.fn_union_settlement_cascade(u,f,t);
 IF j->>'success' IS DISTINCT FROM 'false' OR (SELECT n FROM pg_temp.probe_marks)<>0 THEN RAISE EXCEPTION 'FAIL missing shortfall evidence marked period settled'; END IF;
 j:=pg_temp.fn_union_settlement_cascade_due();
 IF j->>'ran' IS DISTINCT FROM 'true' THEN RAISE EXCEPTION 'FAIL missing shortfall result considered complete by due runner'; END IF;
 UPDATE pg_temp.probe_values SET r2='{"amount":50,"payees":1,"shortfalls":null}';
 j:=pg_temp.fn_union_settlement_cascade(u,f,t);
 IF j->>'success' IS DISTINCT FROM 'false' OR (SELECT n FROM pg_temp.probe_marks)<>0 THEN RAISE EXCEPTION 'FAIL null shortfall evidence marked period settled'; END IF;
 UPDATE pg_temp.probe_values SET r2='{"amount":50,"payees":1,"shortfalls":"0"}';
 j:=pg_temp.fn_union_settlement_cascade(u,f,t);
 IF j->>'success' IS DISTINCT FROM 'false' OR (SELECT n FROM pg_temp.probe_marks)<>0 THEN RAISE EXCEPTION 'FAIL string shortfall evidence accepted'; END IF;
 UPDATE pg_temp.probe_values SET r2='{"amount":50,"payees":1,"shortfalls":0}';
 j:=pg_temp.fn_union_settlement_cascade(u,f,t);
 IF j->>'success' IS DISTINCT FROM 'true' OR (SELECT n FROM pg_temp.probe_marks)<>1 THEN RAISE EXCEPTION 'FAIL verified paid period not settled'; END IF;
 j:=pg_temp.fn_union_settlement_cascade_due();
 IF j->>'reason' IS DISTINCT FROM 'already_settled' THEN RAISE EXCEPTION 'FAIL verified complete period not recognized'; END IF;


 UPDATE pg_temp.probe_marks SET n=0;
 UPDATE pg_temp.probe_values SET r3='{"amount":25,"payees":2,"shortfalls":1}';
 j:=pg_temp.fn_union_settlement_cascade(u,f,t);
 IF j->>'success' IS DISTINCT FROM 'false' OR (SELECT n FROM pg_temp.probe_marks)<>0 THEN RAISE EXCEPTION 'FAIL round-three shortfall marked period settled'; END IF;
 UPDATE pg_temp.probe_values SET r3='{"amount":25,"payees":2}';
 j:=pg_temp.fn_union_settlement_cascade(u,f,t);
 IF j->>'success' IS DISTINCT FROM 'false' OR (SELECT n FROM pg_temp.probe_marks)<>0 THEN RAISE EXCEPTION 'FAIL round-three missing shortfalls marked period settled'; END IF;
 j:=pg_temp.fn_union_settlement_cascade_due();
 IF j->>'ran' IS DISTINCT FROM 'true' THEN RAISE EXCEPTION 'FAIL round-three missing shortfalls considered done'; END IF;

 RAISE EXCEPTION 'AUDIT_TEST_PASS: unpaid/missing/null/string shortfall replies stay incomplete and only verified zero settles;  actual cascade/due/assert bodies, invoice failure retry, refreshed invoice result, shortfall retry, eligible-union completion, exact one-cent rejection, missing totals, finalized replay totals and null bank rejection. All pg_temp rolled back. Financial round helpers, permissions, invoices and clock stubbed.';
END;
$probe$;