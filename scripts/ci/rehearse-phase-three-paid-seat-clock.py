...
WHOLE_SHA = "6383961ebbfa7ea9036892ebf29a161b1c97df02db706452308fb5ebd15fe38f"
INPUTS = [
    "scripts/ci/rehearse-whole-phase-three-cutover.py",
    "scripts/ci/rehearse-phase-three-paid-seat-clock.py",
    "scripts/ci/probes/phase-three-paid-seat-clock-setup.sql",
    "scripts/ci/probes/phase-three-paid-seat-clock-assertions.sql",
...
def main():
    ...
    prepared = subprocess.run([sys.executable, str(root / "scripts/ci/prepare-phase-three-current-thaw-native.py"), "--root", str(root),
    ...
    sql = sql.replace(marker, (root / "scripts/ci/probes/phase-three-paid-seat-clock-setup.sql").read_text() + "\n" + marker)
    marker = "DO $current_reconnect_suffix$"
    assert sql.count(marker) == 1
    sql = sql.replace(marker, (root / "scripts/ci/probes/phase-three-paid-seat-clock-assertions.sql").read_text() + "\n" + marker)
    ...
    whole.SOCKET = "/tmp/codex-chip-drift-owned-native-qeol56o5/socket"
