#!/usr/bin/env python3
"""Existing issue -> redemption -> actual old ticket return -> redemption -> cash.
...
PROBE="scripts/ci/probes/returned-ticket-cancellation-chain-native.sql"
...
"""
    ...
    direct=(root/"scripts/ci/probes/existing-ticket-cancellation-native.sql").read_text()
    first=direct.index("DO $restore_initially_deferred$")
    last=direct.index("END $restore_initially_deferred$;",first)+len("END $restore_initially_deferred$;")
    if probe.count("-- @RESTORE_INITIALLY_DEFERRED@")!=1 or probe.count("-- @CASH_POLICY_CANDIDATE@")!=1:
        raise ValueError("returned-ticket chain stage markers differ")
    probe=probe.replace("-- @RESTORE_INITIALLY_DEFERRED@",direct[first:last],1)
    whole=module(root/"scripts/ci/rehearse-whole-phase-three-cutover.py","returned_ticket_policy")
    probe=probe.replace("-- @CASH_POLICY_CANDIDATE@",whole.without_transaction(
      (root/"scripts/deploy/phase-three-cancellation-origin-cash.sql").read_text()),1)
...
def main():
    ...
    inputs.update({Path(__file__),root/PROBE,root/ticket.PROBE,
      root/"scripts/ci/probes/existing-ticket-cancellation-native.sql",
 ...
 EXISTS(SELECT 1 FROM public.tournaments WHERE id='e4100000-0000-4000-8000-000000000003')::text""")
    ...
    before=whole.snapshot();output=Path(tempfile.mkdtemp(prefix="codex-returned-ticket-chain-"))
    ...
    evidence={"recorded_at":datetime.datetime.now(datetime.timezone.utc).isoformat(),"passed":passed,
      ...
      "limits":["The exact existing historical200 ticket issue fixture is reused; its original source funding/issuance execution is not certified.",
       "Both authenticated redemptions, historical16ea ticket-return cancellation and current cash cancellation, fee reversals, late-fault rollback and both receipt replays run real authorities.",
       "The actual returned ticket uses the same tournament_ticket entitlement kind and is funded solely by the previous real cancellation of the already-redeemed200 liability.",
