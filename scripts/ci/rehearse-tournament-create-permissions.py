...
paths=[Path(__file__),root/'scripts/ci/probes/tournament-create-permission-native.sql',root/'scripts/ci/fixtures/phase-three-creation-client-configs.json',root/'scripts/ci/rehearse-whole-phase-three-cutover.py']
...
before=m.snapshot();out=Path(tempfile.mkdtemp(prefix='codex-create-permission-'))
...
rows=[x.split('=',1)[1] for x in run.stdout.splitlines() if x.startswith('CREATION_NATIVE_EVIDENCE=')];e=json.loads(rows[0]) if len(rows)==1 else None
passed=run.returncode==0 and before==after and inputs==after_inputs and e is not None
r={'recorded_at':datetime.datetime.now(datetime.timezone.utc).isoformat(),'status':'passed' if passed else 'failed','exit_code':run.returncode,'exact_rollback':before==after,'source_inputs_stable':inputs==after_inputs,'assertions':len(re.findall('NOTICE:  CREATION_PASS:',log)),'evidence':e,'source_sha256':inputs,'function_md5':expected,'before':before,'after':after,'output_directory':str(out),'coverage_limits':['Edit authority, complete displayed-option matrix, and live browser are not certified by this create-only probe.'],'failure_tail':run.stderr.splitlines()[-20:] if not passed else []}
