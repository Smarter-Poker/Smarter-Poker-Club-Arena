...
parser.add_argument('--output', type=Path, default=ROOT / 'artifacts/f06-zero-seat-hand-lane')
...
try:
    require(shutil.disk_usage('/tmp').free>512*1024**2,'Insufficient native fixture disk reserve')
    ...
    run('zero-seat-fixture-columns', "ALTER TABLE public.table_seats ADD COLUMN status text DEFAULT 'active', ADD COLUMN joined_at timestamptz DEFAULT '2026-09-01T00:00:00Z', ADD COLUMN club_id uuid;")
    run('current-terminal-helper', (ROOT/'scripts/ci/probes/f06-terminal-lock-order/global.sql').read_text())
    run('current-helper-privileges','REVOKE ALL ON FUNCTION public.fn_ca_share_settlement_lane_for_table(uuid) FROM PUBLIC,anon,authenticated,service_role; GRANT EXECUTE ON FUNCTION public.fn_ca_share_settlement_lane_for_table(uuid) TO service_role;')
    run('current-terminal-lane', (ROOT/'supabase/migrations/20260914000500_hand_terminal_lane_lock_order.sql').read_text())
    zero="UPDATE table_seats SET stack=0 WHERE id=2;"
    close="UPDATE table_seats SET left_at=now(),status='left' WHERE id=2;"
    with holder():
        probe('baseline-zero-close-promotes',zero+close,error='40001: F06_RETRY_CANONICAL_LANE')
    candidate=(ROOT/'supabase/migrations/20260914003000_f06_zero_seat_hand_lane.sql').read_text()
    run('zero-seat-predecessor-drift-refused','BEGIN; ALTER FUNCTION smarter_private.f06_source_guard() SET search_path=pg_catalog;'+candidate,error='P0001')
    run('zero-seat-binding-drift-refused','BEGIN; ALTER TABLE table_seats DISABLE TRIGGER a00_f06_source_seat;'+candidate,error='P0001')
    run('zero-seat-acl-drift-refused','BEGIN; GRANT EXECUTE ON FUNCTION smarter_private.f06_source_guard() TO authenticated;'+candidate,error='P0001')
    run('zero-seat-install',candidate)
    run('zero-seat-replay',candidate)
    baseline=run('zero-seat-state-before',snapshot)
    with holder():
        probe('zero-close-with-other-hand',zero+close+"SELECT stack=0 AND left_at IS NOT NULL AND status='left' FROM table_seats WHERE id=2;",'t')
        for name,change in [
            ('positive-stack',close),
            ('clear-positive-and-close',"UPDATE table_seats SET stack=0,left_at=now(),status='left' WHERE id=2;"),
            ('identity',zero+"UPDATE table_seats SET user_id=gen_random_uuid(),left_at=now(),status='left' WHERE id=2;"),
            ('null-identity',zero+"UPDATE table_seats SET user_id=NULL,left_at=now(),status='left' WHERE id=2;"),
            ('generation',zero+"UPDATE table_seats SET joined_at=now(),left_at=now(),status='left' WHERE id=2;"),
            ('club',zero+"UPDATE table_seats SET club_id=gen_random_uuid(),left_at=now(),status='left' WHERE id=2;"),
            ('seat',zero+"UPDATE table_seats SET seat_number=9,left_at=now(),status='left' WHERE id=2;"),
            ('table',zero+"UPDATE table_seats SET table_id='00000000-0000-4000-8000-000000000001',left_at=now(),status='left' WHERE id=2;"),
            ('positive-new-stack',zero+"UPDATE table_seats SET stack=1,left_at=now(),status='left' WHERE id=2;"),
            ('wrong-status',zero+"UPDATE table_seats SET left_at=now() WHERE id=2;"),
            ('row-id',zero+"UPDATE table_seats SET id=3,left_at=now(),status='left' WHERE id=2;")]:
            probe('zero-'+name+'-still-exclusive',change,error='40001: F06_RETRY_CANONICAL_LANE')
        park=bound.replace("'begun'","'park_requested'")
        hand_receipt="INSERT INTO smarter_private.f06_hand_permits VALUES('00000000-0000-4000-8000-000000000092','00000000-0000-4000-8000-000000000002'); INSERT INTO smarter_private.f06_hand_dispatch VALUES('00000000-0000-4000-8000-000000000092',txid_current());"
        probe('zero-begun-unreceipted-denied',bound+zero+close,error='55000: F06_SOURCE_EXCLUDED')
        probe('zero-park-unreceipted-denied',park+zero+close,error='55000: F06_SOURCE_EXCLUDED')
        probe('zero-begun-hand-receipt-denied',bound+hand_receipt+zero+close,error='55000: F06_SOURCE_EXCLUDED')
        probe('zero-park-exact-hand-receipt',park+hand_receipt+zero+close)
        probe('zero-park-wrong-transaction-denied',park+hand_receipt+"UPDATE smarter_private.f06_hand_dispatch SET xid=xid+1 WHERE true;"+zero+close,error='55000: F06_SOURCE_EXCLUDED')
        probe('zero-park-wrong-table-denied',park+hand_receipt+"UPDATE smarter_private.f06_hand_permits SET table_id='00000000-0000-4000-8000-000000000001' WHERE true;"+zero+close,error='55000: F06_SOURCE_EXCLUDED')
        probe('zero-begun-canonical-move-receipt',bound+receipt+zero+close)
    for name,lane in [('global',global_lane),('tournament',tournament_lane)]:
        with holder(lane,exclusive=True):
            probe('zero-'+name+'-exclusive-still-excludes',zero+close,error='40001: F06_RETRY_CANONICAL_LANE')
    require(run('zero-seat-all-rollback',snapshot)==baseline,'Zero-seat probes leaked state')
    run('zero-seat-browser-execution-closed',"SELECT NOT has_function_privilege('anon','smarter_private.f06_source_guard()','EXECUTE') AND NOT has_function_privilege('authenticated','smarter_private.f06_source_guard()','EXECUTE');",'t')
    results['passed'] = True
finally:
