#!/usr/bin/env python3
"""
Clean up the SVG - remove any existing dynamic text elements and their styles,
then add fresh, properly positioned text elements.
"""
...
def create_clean_svg():
    """Create a clean SVG with proper dynamic text elements."""
    ...
    if not text_positions.get('left'):
        text_positions['left'] = {'x': 100, 'y': 890}
    if not text_positions.get('center'):
        text_positions['center'] = {'x': 298, 'y': 890}
    if not text_positions.get('right'):
        text_positions['right'] = {'x': 497, 'y': 890}
    ...
    # The text elements with proper IDs for dynamic binding
    # Using a readable font, white color, centered alignment
    text_elements = f'''
  <!-- Dynamic Stat Text Elements - IDs for JavaScript binding -->
  <style type="text/css">
    .stat-value {{
  ...
  <text id="total-members" class="stat-value" x="{text_positions['left']['x']:.0f}" y="{text_positions['left']['y']:.0f}" font-size="32">0</text>
  <text id="club-level" class="stat-value" x="{text_positions['center']['x']:.0f}" y="{text_positions['center']['y']:.0f}" font-size="32">0</text>
  <text id="active-players" class="stat-value" x="{text_positions['right']['x']:.0f}" y="{text_positions['right']['y']:.0f}" font-size="32">0</text>
'''
    ...
    print(f"\n✅ Clean SVG saved to: {OUTPUT_SVG}")
    print(f"   Removed {total_removed} number paths")
    print(f"   Added 3 <text> elements with IDs:")
    print(f"     - total-members at ({text_positions['left']['x']:.0f}, {text_positions['left']['y']:.0f})")
    print(f"     - club-level at ({text_positions['center']['x']:.0f}, {text_positions['center']['y']:.0f})")
    print(f"     - active-players at ({text_positions['right']['x']:.0f}, {text_positions['right']['y']:.0f})")
