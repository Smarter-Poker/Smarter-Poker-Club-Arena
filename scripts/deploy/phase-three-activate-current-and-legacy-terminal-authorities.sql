-- Prepared current and sealed legacy terminal composition. NOT APPLIED.
...
-- BEGIN current-legacy-base.sql SHA256 a6a1fe59f1fed55e880cb5eeadde008c7db63a6488045024393398f789a2847b
...
SET LOCAL statement_timeout='30s';
DO $current_legacy_trigger_preflight$
BEGIN
 IF EXISTS(SELECT 1 FROM (VALUES
  ('public.tournaments','aa_guard_tournament_completing_claim','public.fn_guard_tournament_completing_claim()',19,false,false,false,'CREATE TRIGGER aa_guard_tournament_completing_claim BEFORE UPDATE OF status ON public.tournaments FOR EACH ROW EXECUTE FUNCTION fn_guard_tournament_completing_claim()'),
  ('public.tournaments','aaa_guard_atomic_satellite_completion','public.trg_guard_atomic_satellite_completion()',19,false,false,false,'CREATE TRIGGER aaa_guard_atomic_satellite_completion BEFORE UPDATE OF status ON public.tournaments FOR EACH ROW EXECUTE FUNCTION trg_guard_atomic_satellite_completion()'),
  ('public.tournaments','zzzz_freeze_finalized_tournament_prize_pool','public.trg_freeze_finalized_tournament_prize_pool()',19,false,false,false,'CREATE TRIGGER zzzz_freeze_finalized_tournament_prize_pool BEFORE UPDATE OF prize_pool, guaranteed_prize, prize_pool_finalized, payout_structure, spin_multiplier ON public.tournaments FOR EACH ROW EXECUTE FUNCTION trg_freeze_finalized_tournament_prize_pool()'),
  ('public.tournaments','zzzz_tournament_pool_finalization_window_guard','public.trg_tournament_pool_finalization_window_guard()',19,false,false,false,'CREATE TRIGGER zzzz_tournament_pool_finalization_window_guard BEFORE UPDATE OF prize_pool_finalized ON public.tournaments FOR EACH ROW EXECUTE FUNCTION trg_tournament_pool_finalization_window_guard()'),
  ('public.tournaments','zzzz_tournaments_atomic_place_completion_guard','public.trg_tournament_atomic_place_completion_guard()',19,false,false,true,'CREATE TRIGGER zzzz_tournaments_atomic_place_completion_guard BEFORE UPDATE ON public.tournaments FOR EACH ROW WHEN (((new.status = ''COMPLETED''::text) AND (old.status IS DISTINCT FROM ''COMPLETED''::text))) EXECUTE FUNCTION trg_tournament_atomic_place_completion_guard()'),
  ('public.tournaments','zzzzz_tournaments_atomic_final_table_deal_completion_guard','public.trg_atomic_final_table_deal_completion_guard()',19,false,false,true,'CREATE TRIGGER zzzzz_tournaments_atomic_final_table_deal_completion_guard BEFORE UPDATE OF status ON public.tournaments FOR EACH ROW WHEN (((new.status = ''COMPLETED''::text) AND (old.status IS DISTINCT FROM ''COMPLETED''::text))) EXECUTE FUNCTION trg_atomic_final_table_deal_completion_guard()'),
  ('public.tournaments','zzzzzz_tournaments_financial_certificate','public.fn_guard_tournament_completed_certificate()',19,false,false,true,'CREATE TRIGGER zzzzzz_tournaments_financial_certificate BEFORE UPDATE OF status ON public.tournaments FOR EACH ROW WHEN (((new.status = ''COMPLETED''::text) AND (old.status IS DISTINCT FROM ''COMPLETED''::text))) EXECUTE FUNCTION fn_guard_tournament_completed_certificate()'),
  ('public.tournament_legacy_finish_evidence','legacy_finish_evidence_is_append_only','public.fn_ca_guard_legacy_finish_evidence()',31,false,false,false,'CREATE TRIGGER legacy_finish_evidence_is_append_only BEFORE INSERT OR DELETE OR UPDATE ON public.tournament_legacy_finish_evidence FOR EACH ROW EXECUTE FUNCTION fn_ca_guard_legacy_finish_evidence()'),
  ('public.tournament_legacy_finish_evidence','legacy_finish_evidence_cannot_truncate','public.fn_ca_guard_legacy_finish_evidence()',34,false,false,false,'CREATE TRIGGER legacy_finish_evidence_cannot_truncate BEFORE TRUNCATE ON public.tournament_legacy_finish_evidence FOR EACH STATEMENT EXECUTE FUNCTION fn_ca_guard_legacy_finish_evidence()'),
  ('public.tournament_legacy_finish_evidence','legacy_finish_requires_same_transaction_completion','public.fn_ca_legacy_finish_must_close()',5,true,true,false,'CREATE CONSTRAINT TRIGGER legacy_finish_requires_same_transaction_completion AFTER INSERT ON public.tournament_legacy_finish_evidence DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION fn_ca_legacy_finish_must_close()'),
  ('public.table_seats','zy_tournament_live_seat_exit_requires_authority','public.fn_tournament_live_seat_exit_requires_authority()',27,false,false,false,'CREATE TRIGGER zy_tournament_live_seat_exit_requires_authority BEFORE DELETE OR UPDATE OF table_id, user_id, seat_number, left_at, status ON public.table_seats FOR EACH ROW EXECUTE FUNCTION fn_tournament_live_seat_exit_requires_authority()')
 ...
 WHERE NOT EXISTS(SELECT 1 FROM pg_trigger t
  ...
  RAISE EXCEPTION 'current_legacy_trigger_preflight requires exact enabled tournament, legacy seal and seat triggers';
 END IF;
END $current_legacy_trigger_preflight$;
DO $current_legacy_source_preflight$
DECLARE r record; p pg_proc%ROWTYPE;
BEGIN
 FOR r IN SELECT * FROM (VALUES
  ('public.fn_ca_accepted_tournament_settlement_fact(jsonb)','0be7ce46c91572336ee97c80e827428d',ARRAY['search_path=pg_catalog, public, pg_temp','TimeZone=UTC']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','i'),
  ('public.fn_ca_legacy_tournament_finish_witness(uuid)','3c525ec1bccfe36fe4306b3a62e33e60',ARRAY['search_path=pg_catalog, public, pg_temp','TimeZone=UTC']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','s'),
  ('public.fn_ca_legacy_finish_source_rows(uuid)','5273bda4358072fc67f87b0804dd702e',ARRAY['search_path=pg_catalog, public, pg_temp','TimeZone=UTC']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'sql','s'),
  ('public.fn_ca_guard_legacy_finish_evidence()','85fa303592250c8120ea65bde023091f',ARRAY['search_path=pg_catalog, public, pg_temp','TimeZone=UTC']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_ca_legacy_finish_sealed_witness(uuid)','6411062d3e9efec285396673a8a84061',ARRAY['search_path=pg_catalog, public, pg_temp','TimeZone=UTC']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','s'),
  ('public.fn_ca_legacy_finish_standings_are_exact(uuid)','f83563471a3ef99ce21989516f6fcb31',ARRAY['search_path=pg_catalog, public, pg_temp','TimeZone=UTC']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','s'),
  ('public.fn_ca_legacy_finish_must_close()','a00e8598d1f1f8c395e476e8edbc3940',ARRAY['search_path=pg_catalog, public, pg_temp','TimeZone=UTC']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_ca_complete_legacy_tournament(uuid,text)','745a49a8c5374080204cc64c75f80690',ARRAY['search_path=pg_catalog, public, pg_temp','TimeZone=UTC','statement_timeout=45s']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_settle_tournament_places(uuid,uuid)','0ee879b64942c1a1b73643e34dc62e39',ARRAY['search_path=public','statement_timeout=30s']::text[],ARRAY['postgres=X/postgres','service_role=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_refuse_new_entries_while_frozen()','0ba018bb1781aff4ffc0af13596c716c',ARRAY['search_path=public, pg_temp']::text[],NULL::aclitem[],false,'plpgsql','v'),
  ('public.fn_ca_verify_terminal_place_batch(uuid,boolean)','c13263e1ba4d0c5e28380b894cb2e74d',ARRAY['search_path=public, pg_temp']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_ca_legacy_satellite_contract_proof(uuid)','bd3f9193668ed45c24b3a61edcda5bf4',ARRAY['search_path=pg_catalog, public, pg_temp','TimeZone=UTC']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','s'),
  ('public.fn_ca_legacy_satellite_receipt_check(uuid,uuid,boolean)','93739b8d2813d367d1cdac710a70ce97',ARRAY['search_path=public','statement_timeout=30s']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.trg_capture_satellite_economics_on_start()','601f58995000547ab9e11c58368bb230',ARRAY['search_path=public, pg_temp']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.trg_guard_atomic_satellite_completion()','26b02f77a5c10e6e91c817484c985921',ARRAY['search_path=public, pg_temp']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_guard_tournament_completed_certificate()','e8e742499f243ac4443359f33c08b7ad',ARRAY['search_path=public, pg_temp']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_settle_satellite_tournament_pre_money_path_gate(uuid,uuid)','27ba48e50a46fefa2dfcc305429163c9',ARRAY['search_path=public','statement_timeout=30s']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_tournament_live_seat_exit_requires_authority()','74c1a1a6b2c9ccbf8fe04f875bfba2e2',ARRAY['search_path=public, pg_temp']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_complete_tournament_terminal(uuid,uuid,text)','541b6e9d029b8eec7f55aa4ad6965a63',ARRAY['search_path=public, pg_temp','statement_timeout=45s']::text[],ARRAY['postgres=X/postgres','service_role=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_settle_satellite_tournament(uuid,uuid)','486d0e6729de8d518d7faf0c253b65d3',ARRAY['search_path=public, pg_temp','statement_timeout=30s']::text[],ARRAY['postgres=X/postgres','service_role=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_ca_satellite_settlement_receipt(uuid,uuid)','381b3e0691a2b9303693653f5110d568',ARRAY['search_path=public','statement_timeout=30s']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v')
  ...
  IF NOT FOUND OR md5(p.prosrc) IS DISTINCT FROM r.body_md5
   ...
   RAISE EXCEPTION 'current_legacy_source_preflight source or metadata differs: %',r.identity;
  END IF;
 END LOOP;
END $current_legacy_source_preflight$;
...
BEGIN
 ...
 IF to_regprocedure('public.fn_ca_open_satellite_terminal_scope(uuid)') IS NOT NULL AND NOT EXISTS(SELECT 1 FROM pg_proc WHERE oid=to_regprocedure('public.fn_ca_open_satellite_terminal_scope(uuid)') AND md5(prosrc)='dc0b38d8e38494889e9a1989857a3cf8' AND proowner='postgres'::regrole AND prosecdef AND proconfig=ARRAY['search_path=public, pg_temp']::text[] AND proacl=ARRAY['postgres=X/postgres']::aclitem[]) THEN RAISE EXCEPTION 'existing satellite helper differs: fn_ca_open_satellite_terminal_scope'; END IF;
 ...
 IF NOT EXISTS(SELECT 1 FROM pg_proc WHERE oid='public.fn_ca_satellite_settlement_receipt(uuid,uuid)'::regprocedure
  ...
  OR NOT EXISTS(SELECT 1 FROM pg_proc WHERE oid='public.fn_guard_tournament_completed_certificate()'::regprocedure
     AND md5(prosrc)='e8e742499f243ac4443359f33c08b7ad') THEN
...
BEGIN
 ...
 IF md5(b)<>'e8e742499f243ac4443359f33c08b7ad' THEN RAISE EXCEPTION 'D9 certificate preimage differs'; END IF;
 EXECUTE replace(d,b,$combined_certificate$
DECLARE
  v_winner uuid;
  v_kind text;
  v_ready jsonb;
...
BEGIN
 SELECT prosrc,pg_get_functiondef(oid),to_jsonb(p)-'prosrc' INTO b,d,before_meta FROM pg_proc p WHERE oid=f;
 IF md5(b)<>'a00e8598d1f1f8c395e476e8edbc3940' THEN RAISE EXCEPTION 'D9 deferred certificate preimage differs'; END IF;
 EXECUTE replace(d,b,$combined_deferred_certificate$
...
BEGIN
 ...
 IF mode='satellite' THEN
  ...
  IF NEW.created_txid IS DISTINCT FROM txid_current()
  ...
  UPDATE public.tournament_finish_receipts SET certified_at=COALESCE(certified_at,clock_timestamp()),completed_at=(r->>'settled_at')::timestamptz,
   evidence=r,updated_at=clock_timestamp() WHERE tournament_id=NEW.tournament_id
    ...
    AND ((certified_at IS NULL AND evidence IS NULL) OR (
...
BEGIN
 ...
 IF v_oid IS NULL OR md5(v_body) NOT IN ('27ba48e50a46fefa2dfcc305429163c9','c6ee6dc1a0bcd6c35d185406b5330b13')
 ...
 IF md5(v_body)='27ba48e50a46fefa2dfcc305429163c9' THEN
...
$patch_body_core$);
 ...
 IF md5(v_body)<>'c6ee6dc1a0bcd6c35d185406b5330b13' OR v_before IS DISTINCT FROM v_after THEN
...
BEGIN
 SELECT prosrc,pg_get_functiondef(oid),to_jsonb(p)-'prosrc'
 INTO v_body,v_definition,v_before FROM pg_proc p WHERE oid=v_oid;
 IF v_oid IS NULL OR md5(v_body) NOT IN ('26b02f77a5c10e6e91c817484c985921','f218a7d769971c064cb11043fa6b45ce')
    OR (v_before->>'proowner')::oid<>'postgres'::regrole::oid
    OR (v_before->>'prosecdef')::boolean IS DISTINCT FROM true THEN
  RAISE EXCEPTION 'current satellite satellite_guard source or owner differs'; END IF;
 IF md5(v_body)='26b02f77a5c10e6e91c817484c985921' THEN
...
-- END current-legacy-base.sql

-- BEGIN current-legacy-manager.sql SHA256 6fed7595c3cf64876d5a78157d93a6a7f56a8f33aea596bdb4dda658fd5cfd6b
...
BEGIN
 ...
 IF f IS NULL OR md5(body) NOT IN ('c6ee6dc1a0bcd6c35d185406b5330b13','6eb5860aad223fd1cfd14a48de2064bb')
 ...
 IF md5(body)='c6ee6dc1a0bcd6c35d185406b5330b13' THEN
 ...
 IF md5(body)<>'6eb5860aad223fd1cfd14a48de2064bb' OR before_meta IS DISTINCT FROM after_meta THEN
...
-- END current-legacy-manager.sql

-- BEGIN current-legacy-strict.sql SHA256 6ac9ba0ab521c11df450aad9f7ccf0d2aabcc07069f91421fb0d6ed66c746776
...
BEGIN
  ...
  END IF;
  IF EXISTS(SELECT 1 FROM (VALUES
    ('public.fn_complete_tournament_terminal(uuid,uuid,text)','541b6e9d029b8eec7f55aa4ad6965a63'),
...
BEGIN
 IF EXISTS(SELECT 1 FROM (VALUES
  ...
  ('public.fn_settle_satellite_tournament_pre_money_path_gate(uuid,uuid)','6eb5860aad223fd1cfd14a48de2064bb'),
  ...
  ('public.fn_ca_open_satellite_terminal_scope(uuid)','dc0b38d8e38494889e9a1989857a3cf8'),
  ('public.fn_ca_close_satellite_terminal_scope(uuid,jsonb)','1470b469991c59834c66bfb3d4a2f432'),
  ('public.fn_ca_verify_current_satellite_terminal(uuid,uuid,boolean)','75ebfc28a5ba8d08823f2a7e8691d378'),
...
BEGIN
 ...
 IF md5(v_source)<>'0ee879b64942c1a1b73643e34dc62e39' THEN
...
DO $current_legacy_source_postflight$
DECLARE r record; p pg_proc%ROWTYPE;
BEGIN
 FOR r IN SELECT * FROM (VALUES
  ('public.fn_ca_accepted_tournament_settlement_fact(jsonb)','0be7ce46c91572336ee97c80e827428d',ARRAY['search_path=pg_catalog, public, pg_temp','TimeZone=UTC']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','i'),
  ...
  ('public.fn_ca_legacy_finish_sealed_witness(uuid)','6411062d3e9efec285396673a8a84061',ARRAY['search_path=pg_catalog, public, pg_temp','TimeZone=UTC']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','s'),
  ('public.fn_ca_legacy_finish_standings_are_exact(uuid)','f83563471a3ef99ce21989516f6fcb31',ARRAY['search_path=pg_catalog, public, pg_temp','TimeZone=UTC']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','s'),
  ('public.fn_ca_legacy_finish_must_close()','6bbcb99b92f73e4784b2666ba3546500',ARRAY['search_path=pg_catalog, public, pg_temp','TimeZone=UTC']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_ca_complete_legacy_tournament(uuid,text)','745a49a8c5374080204cc64c75f80690',ARRAY['search_path=pg_catalog, public, pg_temp','TimeZone=UTC','statement_timeout=45s']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_settle_tournament_places(uuid,uuid)','0ee879b64942c1a1b73643e34dc62e39',ARRAY['search_path=public','statement_timeout=30s']::text[],ARRAY['postgres=X/postgres','service_role=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_refuse_new_entries_while_frozen()','0ba018bb1781aff4ffc0af13596c716c',ARRAY['search_path=public, pg_temp']::text[],NULL::aclitem[],false,'plpgsql','v'),
  ('public.fn_ca_verify_terminal_place_batch(uuid,boolean)','c13263e1ba4d0c5e28380b894cb2e74d',ARRAY['search_path=public, pg_temp']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ...
  ('public.fn_ca_legacy_satellite_receipt_check(uuid,uuid,boolean)','93739b8d2813d367d1cdac710a70ce97',ARRAY['search_path=public','statement_timeout=30s']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.trg_capture_satellite_economics_on_start()','601f58995000547ab9e11c58368bb230',ARRAY['search_path=public, pg_temp']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.trg_guard_atomic_satellite_completion()','f218a7d769971c064cb11043fa6b45ce',ARRAY['search_path=public, pg_temp']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_guard_tournament_completed_certificate()','d994347e1b76c936ce13361d73f94fd2',ARRAY['search_path=public, pg_temp']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_settle_satellite_tournament_pre_money_path_gate(uuid,uuid)','6eb5860aad223fd1cfd14a48de2064bb',ARRAY['search_path=public','statement_timeout=30s']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_tournament_live_seat_exit_requires_authority()','74c1a1a6b2c9ccbf8fe04f875bfba2e2',ARRAY['search_path=public, pg_temp']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_complete_tournament_terminal(uuid,uuid,text)','541b6e9d029b8eec7f55aa4ad6965a63',ARRAY['search_path=public, pg_temp','statement_timeout=45s']::text[],ARRAY['postgres=X/postgres','service_role=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_settle_satellite_tournament(uuid,uuid)','486d0e6729de8d518d7faf0c253b65d3',ARRAY['search_path=public, pg_temp','statement_timeout=30s']::text[],ARRAY['postgres=X/postgres','service_role=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_ca_satellite_settlement_receipt(uuid,uuid)','381b3e0691a2b9303693653f5110d568',ARRAY['search_path=public','statement_timeout=30s']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_ca_satellite_terminal_scope(uuid)','0bd1220dbfb2e23b27e9e102959829e2',ARRAY['search_path=public, pg_temp']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'sql','v'),
  ('public.fn_ca_open_satellite_terminal_scope(uuid)','dc0b38d8e38494889e9a1989857a3cf8',ARRAY['search_path=public, pg_temp']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_ca_close_satellite_terminal_scope(uuid,jsonb)','1470b469991c59834c66bfb3d4a2f432',ARRAY['search_path=public, pg_temp']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_ca_satellite_pending_transition(public.tournaments,public.tournaments)','34412a6273353af272200836335737a1',ARRAY['search_path=public, pg_temp']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_ca_satellite_terminal_commit_proof()','61b521747a6794f4e473a25a6313c6d2',ARRAY['search_path=public, pg_temp']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_ca_satellite_terminal_scope_consumed()','fce60b982977156c653d56fe212de6ae',ARRAY['search_path=public, pg_temp']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_ca_satellite_manager_target_immutable()','fba02ebdf76a196cd8997199b56f6da9',ARRAY['search_path=public, pg_temp']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_ca_publish_satellite_manager_target(uuid,uuid,jsonb)','92a5126174e4098aa57f4c42fbb9939f',ARRAY['search_path=public, pg_temp']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_ca_satellite_manager_target_write(text,text,jsonb,jsonb)','d1b68a808b9ee22eaee833a25bec5ca6',ARRAY['search_path=public, pg_temp']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_ca_verify_current_satellite_terminal(uuid,uuid,boolean)','75ebfc28a5ba8d08823f2a7e8691d378',ARRAY['search_path=public','statement_timeout=30s']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_tournament_finish_readiness(uuid,uuid)','0388659818612493c16b02048dae5b3f',ARRAY['search_path=public, pg_temp']::text[],ARRAY['postgres=X/postgres']::aclitem[],true,'plpgsql','v'),
  ('public.fn_settle_tournament_obligation(uuid,text,integer,uuid,numeric,text,text,uuid)','642b0a5a4b5cd5ab2194e265eadba75c',ARRAY['search_path=public, pg_temp']::text[],ARRAY['postgres=X/postgres','service_role=X/postgres']::aclitem[],true,'plpgsql','v')
 ) expected(identity,body_md5,config,acl,definer,language,volatility) LOOP
  SELECT * INTO p FROM pg_proc WHERE oid=to_regprocedure(r.identity);
  IF NOT FOUND OR md5(p.prosrc) IS DISTINCT FROM r.body_md5
   OR p.proowner IS DISTINCT FROM 'postgres'::regrole
   ...
   OR p.prolang IS DISTINCT FROM (SELECT oid FROM pg_language WHERE lanname=r.language)
   OR p.provolatile::text IS DISTINCT FROM r.volatility THEN
   RAISE EXCEPTION 'current_legacy_source_postflight source or metadata differs: %',r.identity;
  END IF;
 END LOOP;
END $current_legacy_source_postflight$;
DO $current_legacy_trigger_postflight$
BEGIN
 IF EXISTS(SELECT 1 FROM (VALUES
  ('public.tournaments','aa_guard_tournament_completing_claim','public.fn_guard_tournament_completing_claim()',19,false,false,false,'CREATE TRIGGER aa_guard_tournament_completing_claim BEFORE UPDATE OF status ON public.tournaments FOR EACH ROW EXECUTE FUNCTION fn_guard_tournament_completing_claim()'),
  ('public.tournaments','aaa_guard_atomic_satellite_completion','public.trg_guard_atomic_satellite_completion()',19,false,false,false,'CREATE TRIGGER aaa_guard_atomic_satellite_completion BEFORE UPDATE OF status ON public.tournaments FOR EACH ROW EXECUTE FUNCTION trg_guard_atomic_satellite_completion()'),
 ...
 WHERE NOT EXISTS(SELECT 1 FROM pg_trigger t
  WHERE t.tgrelid=to_regclass(expected.relation) AND t.tgname=expected.name
   ...
   AND (t.tgqual IS NOT NULL)=expected.has_when
   AND pg_get_triggerdef(t.oid)=expected.definition)) THEN
  RAISE EXCEPTION 'current_legacy_trigger_postflight requires exact enabled tournament, legacy seal and seat triggers';
 END IF;
END $current_legacy_trigger_postflight$;
DO $current_legacy_no_capabilities$
BEGIN
 IF EXISTS(SELECT 1 FROM public.tournament_satellite_terminal_authorizations)
  OR EXISTS(SELECT 1 FROM public.tournament_satellite_manager_targets)
