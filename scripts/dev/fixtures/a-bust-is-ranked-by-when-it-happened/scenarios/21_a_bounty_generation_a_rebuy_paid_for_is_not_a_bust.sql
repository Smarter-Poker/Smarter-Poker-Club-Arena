-- FIXED. The orphan shape in a bounty event: the player busted, the knocker was
-- paid that generation's head (a settled bounty obligation with its complete
-- marker), a posted 1.00 wallet-to-pool rebuy leg bought the player back in
-- seconds later, the generation was never marked 'rebought', and the bounty door
-- then refused the player's next, real bust for ever
-- (unresolved_knockout_generation_chain). The leg proves the older generation
-- was bought back and its obligation proves its head was collected, so the door
-- now closes it with the newer bust, as the non-bounty door does. See 22 for the
-- heads that keep it refused - including one no obligation names at all.
...
SELECT probe.check((:'q_result'::jsonb->>'claimed')::boolean,
                   'the real bust is accepted, not refused for a generation whose head was collected: ' || :'q_result');
SELECT probe.check(:'q_result'::jsonb->'rebought_generations' = jsonb_build_array(:'q_orphan'::text),
...
SELECT probe.check(probe.eliminated_at(:'t', :'q') = '2026-09-11 01:33:10.821+00',
                   'stamped with the real bust, not the orphan');
SELECT probe.check((SELECT count(*) = 1 AND bool_and(o.state = 'settled')
                      FROM public.tournament_bounty_obligations o
                     WHERE o.tournament_id = :'t' AND o.eliminated_user_id = :'q'
                       AND o.hand_number = 8260161),
                   'the collected head is neither owed nor claimed again');
