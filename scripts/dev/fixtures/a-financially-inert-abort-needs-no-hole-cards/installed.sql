-- THE TWO FUNCTIONS THIS REPAIR EDITS, AS THEY ARE LIVE.
--
-- Both bodies are re-derived from the repo migrations that installed them -
-- smarter_private.f06_retired_origin_snapshot from 20260919034506 (lines
-- 13-328) and smarter_private.f06_retained_mtt_abort_snapshot from
-- 20260919024642 (lines 508-808) - and this file then refuses to load unless
-- md5(prosrc) of each equals what kuklfnapbkmacvwxktbh reports on 2026-09-20.
-- The two support functions the retired-origin branch needs are checked the
-- same way. Nothing below is transcribed by hand.
...
DECLARE want text[][] := ARRAY[
  ['smarter_private.f06_retired_origin_snapshot(jsonb)',     'eef2b4beccc02dffd5efeffe57081a0e'],
  ['smarter_private.f06_retained_mtt_abort_snapshot(jsonb)', '45d5e92898dd9fb2fb72f33917cf96d8'],
  ['smarter_private.f06_retired_origin_begin(jsonb)',        '26eebaeddeb611495c1c5504a794d56b'],
  ['smarter_private.f06_retired_origin_cohort(uuid)',        '5a52aab48fb382a454b020415a798591']];
  i integer; got text;
BEGIN
 FOR i IN 1..array_length(want,1) LOOP
  SELECT md5(prosrc) INTO got FROM pg_proc WHERE oid=to_regprocedure(want[i][1]);
  IF got IS DISTINCT FROM want[i][2] THEN
   RAISE EXCEPTION 'PROBE CAPTURE IS NOT THE LIVE BODY: % is % not %', want[i][1], coalesce(got,'absent'), want[i][2];
  END IF;
 END LOOP;
 RAISE NOTICE 'capture verified: % live bodies re-derived from the repo migrations', array_length(want,1);
END $capture$;

-- The definer identity these two functions must keep across the repair.
ALTER FUNCTION smarter_private.f06_retired_origin_snapshot(jsonb) OWNER TO postgres;
ALTER FUNCTION smarter_private.f06_retained_mtt_abort_snapshot(jsonb) OWNER TO postgres;
