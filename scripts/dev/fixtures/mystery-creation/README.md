...
The tournament table is intentionally minimal. Authentication, club authorization and delegated creation are explicit stand-ins. The installed outer creator, registered/lifecycle guards, legacy setter and candidate migration execute unchanged. This does **not** exercise the entire production trigger graph, funded entry ledger, dealer, payout provider, HTTP retry path or production tournament. Twelve passing groups are scoped database evidence, never full MTT certification.

`column-contract.json` captures the installed selected-column types, nullability, defaults and related checks. The runner applies the relevant metadata to its private table and binds each source file to the result using SHA-256.
