-- The LIVE bodies, byte-exact as read from production on 2026-09-21.
--
-- Nothing here is edited or abridged: these are the exact prosrc values the
-- migration's pre-image guard pins by md5, so the probe proves the migration
-- against what production actually runs. Bodies are installed with
-- check_function_bodies off because only the two the scenarios execute have
-- their tables in bootstrap.sql; the rest are present so the guard can see the
-- tree it refuses to run against.
\set ON_ERROR_STOP on
SET check_function_bodies = off;
DO $roles$
DECLARE r text;
BEGIN
  -- The roles Supabase gives every project: the migration names the pre-login
  -- ones when it revokes, so they must exist here too.
...
AS $live_fn_ca_assert_satellite_cohort_standings$
...
BEGIN
 ...
 END IF;
END $live_fn_ca_assert_satellite_cohort_standings$;
REVOKE ALL ON FUNCTION public.fn_ca_assert_satellite_cohort_standings(uuid) FROM PUBLIC;
...
AS $live_fn_ca_settle_satellite_cohort$
DECLARE
...
BEGIN
  ...
  RETURN public.fn_ca_satellite_cohort_receipt(
    p_tournament_id, p_observed_qualifier_ids);
END;
$live_fn_ca_settle_satellite_cohort$;
REVOKE ALL ON FUNCTION public.fn_ca_settle_satellite_cohort(uuid,uuid[]) FROM PUBLIC;
...
AS $live_fn_ca_verify_terminal_final_deal_batch$
DECLARE
...
END;
$live_fn_ca_verify_terminal_final_deal_batch$;
REVOKE ALL ON FUNCTION public.fn_ca_verify_terminal_final_deal_batch(uuid,boolean) FROM PUBLIC;
...
CREATE OR REPLACE FUNCTION public.fn_settle_satellite_tournament_pre_money_path_gate(p_tournament_id uuid, p_observed_winner_id uuid)
  ...
  SET search_path TO public
    SET statement_timeout TO '30s'
AS $live_fn_settle_satellite_tournament_pre_money_path_gate$
DECLARE
  v_accounting jsonb; v_deferred boolean := false;
  v_source record;
  v_target record;
...
BEGIN
  ...
  RETURN public.fn_ca_satellite_settlement_receipt(
    p_tournament_id, p_observed_winner_id);
END;
$live_fn_settle_satellite_tournament_pre_money_path_gate$;
REVOKE ALL ON FUNCTION public.fn_settle_satellite_tournament_pre_money_path_gate(uuid,uuid) FROM PUBLIC;
...
CREATE OR REPLACE FUNCTION public.fn_settle_tournament_bubble_protection(p_tournament_id uuid, p_observed_bubble_user_id uuid)
  ...
  SET search_path TO public
    SET statement_timeout TO '30s'
AS $live_fn_settle_tournament_bubble_protection$
DECLARE
  v_t record;
  v_ladder jsonb;
  v_field_size integer;
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'settlement', v_result);
END;
$live_fn_settle_tournament_bubble_protection$;
REVOKE ALL ON FUNCTION public.fn_settle_tournament_bubble_protection(uuid,uuid) FROM PUBLIC;
...
CREATE OR REPLACE FUNCTION public.fn_settle_tournament_final_table_deal(p_tournament_id uuid)
  ...
  SET search_path TO public
    SET statement_timeout TO '30s'
AS $live_fn_settle_tournament_final_table_deal$
DECLARE
  v_t record;
  v_status text;
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'money_path', 'fn_settle_tournament_final_table_deal');
END;
$live_fn_settle_tournament_final_table_deal$;
REVOKE ALL ON FUNCTION public.fn_settle_tournament_final_table_deal(uuid) FROM PUBLIC;
...
CREATE OR REPLACE FUNCTION public.fn_settle_tournament_places(p_tournament_id uuid, p_observed_winner_id uuid)
  ...
  SET search_path TO public
    SET statement_timeout TO '30s'
AS $live_fn_settle_tournament_places$
DECLARE
  v_t record;
  v_ladder jsonb;
  v_payouts jsonb := '[]'::jsonb;
...
END;
$live_fn_settle_tournament_places$;
