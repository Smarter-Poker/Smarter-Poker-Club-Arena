...
busy_manager_migration="$(migration_by_suffix a_busy_manager_keeps_its_lease.sql)"
hand_heartbeat_migration="$(migration_by_suffix a_hand_commit_does_not_hold_the_lease_against_its_own_heartb.sql)"
claim_source="$(migration_by_suffix tournament_leases_have_fencing_generations.sql)"
hand_source="$(migration_by_suffix table_leases_and_hand_commits_have_generations.sql)"
close_source="$(migration_by_suffix tournament_table_break_close_is_atomic.sql)"
hook_source="$(migration_by_suffix tournament_manager_requests_carry_lease_authority.sql)"
addon_source="$(migration_by_suffix post_commit_obligations_are_atomic_and_resumable.sql)"
current_postimage_source="$(migration_by_suffix stage_b_current_postimage_contraction.sql)"
...
# The cutover correctly requires every pre-existing authority heartbeat to be
# stale. The concurrency cases begin only after it commits, modelling the
# restarted engine generation by making both exact leases freshly live again.
"${psql_cmd[@]}" -c \
  'UPDATE public.engine_table_leases SET heartbeat_at=clock_timestamp(); UPDATE public.engine_tournament_leases SET heartbeat_at=clock_timestamp();' \
  >/dev/null
