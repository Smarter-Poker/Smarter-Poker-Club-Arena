#!/usr/bin/env bash
# UNRESOLVED HAND PERMITS RETAIN THEIR ORIGINAL HOLE CARDS - PostgreSQL 17 gate.
#
# Runs every scenario in
# scripts/dev/fixtures/unresolved-hand-permits-retain-their-original-hole-cards
# against a byte-exact capture of the LIVE public.cleanup_old_hole_cards()
...
#
# Last it feeds the migration four kinds of drift and requires a refusal with
...
fixture="$repo/scripts/dev/fixtures/unresolved-hand-permits-retain-their-original-hole-cards"
migration="$repo/supabase/migrations/20260920232341_unresolved_hand_permits_retain_their_original_hole_cards.sql"
...
root="$(mktemp -d "${PROBE_ROOT:-${TMPDIR:-/tmp}}/ca-holecard-pg17.XXXXXX")"
sock="$(mktemp -d "/tmp/hcsk.XXXXXX")"
port="$((46100 + $$ % 900))"
cleanup() {
...
# Permits in every state the enum admits, so the migration own executable
...
# The migration own proof must have been exercised, not merely present: it
# reports how many permit states it checked both directions of.
proof_states="$(grep -o 'hole-card retention proof: both directions hold for [0-9]* permit state' "$root/apply.log" | head -1 | grep -o '[0-9]*')"
if [[ "${proof_states:-0}" -eq 4 ]]; then
  echo "ok   the migration in-transaction proof checked both directions for all 4 permit states"
else
  echo "FAIL the migration in-transaction proof was vacuous: ${proof_states:-none} state(s)"; failures=$((failures + 1))
fi
grep -o 'NOTICE:  cleanup_old_hole_cards already retains.*' "$root/apply.log" | head -1 | sed 's/^/     /'
...
identity="$(psql_db fx_fixed -Atc "SELECT proowner::regrole::text||'|'||prosecdef::text||'|'||provolatile::text||'|'||proconfig::text||'|'||proacl::text FROM pg_proc WHERE oid='public.cleanup_old_hole_cards()'::regprocedure")"
if [[ "$identity" == "postgres|false|v|{search_path=public}|{postgres=X/postgres,service_role=X/postgres}" ]]; then
...
# Four kinds of drift, each refused, each leaving the function catalog exactly
...
pin = 'ba3ce182b251f06e3e838ad03105fbf0'
...
echo "== refusals"
for fault in body acl secdef permit_enum composition; do
...
DO $$ DECLARE d text; b text; BEGIN
 SELECT pg_get_functiondef(oid), prosrc INTO d, b FROM pg_proc
  WHERE oid='public.cleanup_old_hole_cards()'::regprocedure;
 EXECUTE replace(d, b, E'\n-- isolated source drift\n' || b);
END $$;
DRIFT
      expected='F06_HOLE_CARD_RETENTION_PREIMAGE_CHANGED' ;;
    acl)
      psql_db fx_fault -c 'GRANT EXECUTE ON FUNCTION public.cleanup_old_hole_cards() TO authenticated' >/dev/null
      expected='F06_HOLE_CARD_RETENTION_PREIMAGE_CHANGED' ;;
    secdef)
      psql_db fx_fault -c 'ALTER FUNCTION public.cleanup_old_hole_cards() SECURITY DEFINER' >/dev/null
      expected='F06_HOLE_CARD_RETENTION_PREIMAGE_CHANGED' ;;
    permit_enum)
      psql_db fx_fault <<'DRIFT' >/dev/null
...
DRIFT
      expected='F06_HOLE_CARD_RETENTION_DEPENDENCY_CHANGED' ;;
    composition)
      fault_migration="$root/wrong-postimage.sql"
      expected='F06_HOLE_CARD_RETENTION_COMPOSITION_CHANGED' ;;
  esac
...
echo "== literal row counts (before -> after each cleanup call)"
sort -k2,2 -k1,1r "$census" | awk '{printf "     %-6s %-28s %s %s\n", $1, $2, $3, $4}'
