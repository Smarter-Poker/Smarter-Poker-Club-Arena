...
DO $$ BEGIN IF current_user<>'fixture_bootstrap' OR current_database() !~ '^qual_owner_notify_[0-9a-f]{32}$' OR to_regclass('public.engine_alerts') IS NOT NULL OR to_regclass('public.engine_alert_delivery_receipts') IS NOT NULL THEN RAISE EXCEPTION 'direct source fixture requires empty maintained core allocation'; END IF; END $$;
