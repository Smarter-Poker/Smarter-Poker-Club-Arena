...
-- Catalog closure only: no outbox, history, player or money rows are returned.
...
SELECT jsonb_build_object(
  'complete', (SELECT count(*)=3 FROM selected)
     AND (SELECT count(*) BETWEEN 1 AND 8 FROM functions)
  ...
  'packet',CASE WHEN (SELECT count(*)=3 FROM selected)
     AND (SELECT count(*) BETWEEN 1 AND 8 FROM functions)
     AND octet_length(value::text)<=262144 THEN value ELSE NULL END
) FROM packet;
