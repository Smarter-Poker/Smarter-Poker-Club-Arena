...
BEGIN;
DO $$ BEGIN IF current_user<>'postgres' OR session_user<>'postgres' OR inet_server_addr() IS NOT NULL OR current_database()<>'qual_spin_expiry_'||replace(current_setting('qualification.execution_uuid'),'-','') OR EXISTS(SELECT 1 FROM public.ca_chip_store_coverage) THEN RAISE EXCEPTION 'exact empty synthetic store boundary required'; END IF; END $$;
