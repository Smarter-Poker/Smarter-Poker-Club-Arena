...
IF to_regprocedure('public.fn_tournament_payout_terms_committed_v1(uuid)') IS NOT NULL THEN RAISE EXCEPTION 'modern payout terms preimage differs'; END IF;
...
$function$;
ALTER FUNCTION public.fn_tournament_payout_terms_committed_v1(uuid) OWNER TO postgres;
REVOKE ALL ON FUNCTION public.fn_tournament_payout_terms_committed_v1(uuid) FROM PUBLIC,anon,authenticated,service_role,postgres;
