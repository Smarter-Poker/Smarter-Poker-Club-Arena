...
def private_socket():
    path = ROOT.parent / 'work' / 'socket'
    require(path.is_absolute() and path.resolve() == path,
            'exact private fixture socket directory required')
    info = path.lstat()
    require(stat.S_ISDIR(info.st_mode) and info.st_uid == os.geteuid()
            and stat.S_IMODE(info.st_mode) == 0o700, 'unsafe private fixture socket directory')
    endpoint = path / '.s.PGSQL.5432'
    info = endpoint.lstat()
    require(stat.S_ISSOCK(info.st_mode) and info.st_uid == os.geteuid(),
            'owned PostgreSQL socket required')
    return path
...
class Session:
    ...
    def __init__(self, executable, database, name, deadline):
        ...
        self.process = subprocess.Popen(
            [str(executable), '-X', '-w', '-qAt', '-h', str(private_socket()), '-p', '5432',
...
def run(args, events, sessions, deadline):
    ...
    require_private_endpoint(environment, database)
    require(0 < environment['deadlock_ms'] < 4000, 'private provider deadlock timeout differs')
