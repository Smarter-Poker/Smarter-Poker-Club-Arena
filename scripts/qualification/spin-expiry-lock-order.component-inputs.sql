-- SOURCE ONLY / UNRUN. Exact embedded component/source custody; not a provider.
-- Regenerate whenever either component changes.
INSERT INTO qualification_component_sources(name,sql) VALUES
('forward',$spin_expiry_forward_source$-- FIFO inbox 5 only. SOURCE-REVIEW CANDIDATE / NATIVE UNRUN / UNINSTALLED.
...
$spin_expiry_forward_source$),
('rollback',$spin_expiry_rollback_source$-- FIFO inbox 5 only. SOURCE-REVIEW CANDIDATE / NATIVE UNRUN / UNINSTALLED.
-- One new runtime call: current settlement G -> B before the first parent lock.
-- Existing predicates, cancellation calls, counter expressions and handler text
-- are unchanged. The new global acquisition is outside that handler: timeout
-- propagates and rolls back the sweep. Qualification contract R5 covers this.
...
END;
$spin_expiry_lock_order$;
$spin_expiry_rollback_source$),
('preimage',$spin_expiry_preimage_source$CREATE OR REPLACE FUNCTION public.fn_spin_expire_unfilled(p_limit integer DEFAULT 50)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
END;
$function$
$spin_expiry_preimage_source$),
('postimage',$spin_expiry_postimage_source$CREATE OR REPLACE FUNCTION public.fn_spin_expire_unfilled(p_limit integer DEFAULT 50)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
