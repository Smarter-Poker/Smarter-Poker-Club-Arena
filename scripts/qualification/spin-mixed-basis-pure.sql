...
-- Embedded source is byte-checked against the maintained sources and frozen original by the
...
CREATE TEMP TABLE mixed_pure_source(component_sql text NOT NULL, shape_sql text NOT NULL, preimage_sql text NOT NULL);
...
END;
$test$;
$mixed_shape_source$,$mixed_preimage_source$-- SOURCE-ONLY, PARTIAL FIFO5 candidate. Outside auto-applied migrations.
-- Installs private read-only evidence functions only. Does not replace the
-- settler, seal an admission, promote/rank players, or authorize any payment.
-- SQL/native qualification and current catalog/authority closure are UNRUN.
SET LOCAL lock_timeout = '5s';
...
BEGIN
 ...
 -- Reproduce the exact defect against the frozen original component. Only
 -- this precise regression error proves that the old validator accepted now.
 -- Any prior failure, different error, or unexpectedly passing script fails.
 BEGIN
  EXECUTE source.preimage_sql;
  IF (SELECT md5(prosrc) FROM pg_proc WHERE oid=to_regprocedure('public.fn_ca_spin_mixed_history_shape_v1(jsonb)'))
       IS DISTINCT FROM '5c7fcb7ef050039c78c4df05d77100f9' THEN
   RAISE EXCEPTION 'mixed pure: original timestamp validator body differs'; END IF;
  EXECUTE source.shape_sql;
  RAISE EXCEPTION 'mixed pure: frozen original did not reproduce relative timestamp acceptance';
 EXCEPTION WHEN SQLSTATE 'PZ020' THEN
  GET STACKED DIAGNOSTICS msg=MESSAGE_TEXT;
  IF msg<>'mixed shape accepted relative modern commit timestamps' THEN RAISE; END IF;
 END;
 IF pg_temp.mixed_pure_catalog() IS DISTINCT FROM before.catalog
  ...
  RAISE EXCEPTION 'mixed pure: original regression did not restore exact state'; END IF;
 BEGIN
  EXECUTE source.component_sql;
  FOR f IN SELECT * FROM (VALUES
   ...
   ('fn_ca_spin_mixed_history_shape_v1','a3ad6ecd2de90c3a4ab2a2a361b9b9da',true,'plpgsql',ARRAY['search_path=pg_catalog, public, pg_temp','TimeZone=UTC']::text[]),
  ...
  EXCEPTION WHEN SQLSTATE '55000' THEN
   ...
   IF msg<>'mixed-history evidence candidate preimage/authority differs' THEN RAISE; END IF;
  END;
  EXECUTE source.shape_sql; -- original controls plus strict relative-timestamp regression
  FOR f IN SELECT * FROM (VALUES
...
SELECT jsonb_build_object('qualification','spin_mixed_basis_pure_evidence',
 'shape_positive',1,'shape_negative',24,'key_scalar_controls',17,'private_invocation_refusals',9,
