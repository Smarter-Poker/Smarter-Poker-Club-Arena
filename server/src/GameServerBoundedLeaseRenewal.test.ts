...
function harness({
  ...
  const context: any = {
    ...
    leaseRenewalOutstandingTotal: {
      inc: (_n: number, { half }: { half: string }) => outstandingHalves.push(half),
    },
...
test('outstanding-half diagnostics survive bounded return until each original transport settles', async () => {
  const cash = deferred(),
    tournament = deferred(),
    h = harness({ cash: () => cash.promise, tournament: () => tournament.promise });
  h.p.renewOwnedEngineLeaseProofs();
  await h.advance(20001);
  assert.deepEqual(h.outstandingHalves, ['cash', 'tournament']);
  const outstanding = h.p.ownershipLeaseRenewalOutstanding;
  assert.equal(outstanding.cash, true);
  assert.equal(outstanding.tournament, true);
  await h.p.renewOwnedEngineLeaseProofs();
  assert.strictEqual(h.p.ownershipLeaseRenewalOutstanding, outstanding);
  assert.equal(outstanding.cash, true);
  assert.equal(outstanding.tournament, true);
  assert.equal(h.stats().cashCalls, 1);
  assert.equal(h.stats().tournamentCalls, 1);
  cash.reject(Error('original cash transport returned'));
  await h.flush();
  assert.equal(outstanding.cash, false);
  assert.equal(outstanding.tournament, true);
  tournament.resolve(tourProof());
  await h.flush();
  assert.equal(outstanding.tournament, false);
  assert.deepEqual(h.outstandingHalves, ['cash', 'tournament']);
