import { omahaVariantSpot } from './OmahaVariantPolicyEvidence.js';
import { evaluateOmahaVariantPolicy } from '../engine/omaha/OmahaVariantLivePolicy.js';
import { omahaVariantSeatCap } from '../engine/omaha/OmahaVariantPolicyPack.js';
        ...
        for (const seats of [2, omahaVariantSeatCap(variant, mode)])
          for (const street of ['preflop', 'flop', 'turn', 'river'] as const) {
            ...
            for (let i = -5; i < repeats; i++) {
              seedFastRandom(111001 + Math.max(0, i));
