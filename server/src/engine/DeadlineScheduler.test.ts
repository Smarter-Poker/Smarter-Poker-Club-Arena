...
describe('DeadlineScheduler', () => {
  ...
  it('fires a deadline at or after its deadlineMs', () => {
    ...
    harness.advance(100); harness.tick(); expect(fired).toBe(false);
    // Tick at t+500: fires
    harness.advance(400); harness.tick(); expect(fired).toBe(true);
  });

  it('re-scheduling same (tableId, eventId) replaces prior deadline', () => {
    let count = 0;
    const cb = () => { count++; };
    sched.schedule({ tableId: 't', eventId: 'turn', deadlineMs: harness.nowFn() + 200, callback: cb });
    // Re-schedule with later deadline and different callback
    let count2 = 0;
    const cb2 = () => { count2++; };
    sched.schedule({ tableId: 't', eventId: 'turn', deadlineMs: harness.nowFn() + 600, callback: cb2 });
    harness.advance(300); harness.tick();
    expect(count).toBe(0); // original was replaced
    harness.advance(400); harness.tick();
  ...
  it('cancel removes a pending deadline', () => {
    let fired = false;
    sched.schedule({
      tableId: 't', eventId: 'turn', deadlineMs: harness.nowFn() + 200, callback: () => { fired = true; },
    });
    sched.cancel('t', 'turn');
    harness.advance(500); harness.tick();
    expect(fired).toBe(false);
  });
  ...
  it('cancelAll drops every deadline for a table', () => {
    ...
    for (let i = 0; i < 5; i++) {
      sched.schedule({
        tableId: 't1', eventId: `e${i}`, deadlineMs: harness.nowFn() + 100 * (i + 1),
        callback: () => { fired++; },
      });
    }
    sched.schedule({
      tableId: 't2', eventId: 'e', deadlineMs: harness.nowFn() + 100,
      callback: () => { fired++; },
    });
    const removed = sched.cancelAll('t1');
    expect(removed).toBe(5);
    harness.advance(1000); harness.tick();
  ...
  it('tick fires no more than maxFirePerTick per tick', () => {
    ...
    for (let i = 0; i < 10; i++) {
      s.schedule({
        tableId: 't', eventId: `e${i}`, deadlineMs: harness.nowFn() + 10,
        callback: () => { fired++; },
      });
    }
    harness.advance(20);
  ...
  it('throwing callback does not stop the scheduler', () => {
    ...
    sched.schedule({
      tableId: 't', eventId: 'bad', deadlineMs: harness.nowFn() + 100,
      callback: () => { throw new Error('boom'); },
    });
    sched.schedule({
      tableId: 't', eventId: 'good', deadlineMs: harness.nowFn() + 200,
      callback: () => { otherFired = true; },
    });
    // Silence console
  ...
  it('persistPending returns JSON-ready entries for a table only', () => {
    sched.schedule({
      tableId: 't1', eventId: 'turn', deadlineMs: 5000, callback: () => {},
    });
    sched.schedule({
      tableId: 't1', eventId: 'timebank:u1', deadlineMs: 8000, callback: () => {},
    });
    sched.schedule({
      tableId: 't2', eventId: 'turn', deadlineMs: 6000, callback: () => {},
    });
  ...
  it('rehydrate reinstalls with supplied callbacks; past-due fires on next tick', () => {
    ...
    sched.rehydrate('t', persisted, (eventId) => {
      if (eventId === 'past') return () => { firedPast = true; };
      if (eventId === 'future') return () => { firedFuture = true; };
      return undefined;
    });
    ...
    harness.advance(2000); harness.tick();
    expect(firedFuture).toBe(true);
  });

  it('stop clears the interval and drops pending deadlines', () => {
    sched.schedule({
      tableId: 't', eventId: 'e', deadlineMs: harness.nowFn() + 100, callback: () => {},
    });
  ...
  it('tick drift correction: deadline 1000ms past fires within the same tick', () => {
    let fired = false;
    sched.schedule({
      tableId: 't', eventId: 'late', deadlineMs: harness.nowFn() - 1000,
      callback: () => { fired = true; },
    });
    harness.tick();
  ...
  it('multiple deadlines at the same time both fire in one tick', () => {
    ...
    sched.schedule({ tableId: 't', eventId: 'a', deadlineMs: t, callback: () => { a = true; } });
    sched.schedule({ tableId: 't', eventId: 'b', deadlineMs: t, callback: () => { b = true; } });
    sched.schedule({ tableId: 't', eventId: 'c', deadlineMs: t, callback: () => { c = true; } });
    harness.advance(150); harness.tick();
