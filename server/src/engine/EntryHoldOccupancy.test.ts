...
function fixture() {
  const e = new ServerTableEngine(id(1)) as any;
  engines.push(e);
  ...
  const original = {
    ...
    occupancy_id: id(3),
    user_id: id(4),
    seat_joined_at: '2026-09-15T00:00:00.123456Z',
    seat_number: 1,
    stack: 100,
  };
    ...
    const builder: any = {
      ...
      get then() {
        ...
        return (resolve: (value: unknown) => void, reject: (reason: unknown) => void) => {
          if (mode === 'reject') {
            reject(Error('modeled transport failure'));
            return;
          }
          if (mode === 'error') {
            resolve({ error: { message: 'modeled database refusal' } });
            return;
          }
...
describe('entry hold targets the occupancy observed at enqueue', () => {
  it.each(['new_row', 'same_row_reactivated'] as const)(
    'delayed patch does not update later occupancy: %s',
    async (mode) => {
      const f = fixture(),
        gate = barrier();
      f.e.entryHoldWriteChains.set(id(4), gate.promise);
      expect(f.enqueue({ hold: null, agreed: false })).toBeUndefined();
      const pending = f.tail();
  ...
  it('retains original table scalar when the Engine table changes while queued', async () => {
    const f = fixture(),
      gate = barrier();
    f.e.entryHoldWriteChains.set(id(4), gate.promise);
    f.enqueue({ hold: null, agreed: false });
    const pending = f.tail();
    f.e.tableId = id(9);
    ...
    gate.release();
    await pending;
    expect(f.rows[0].entry_hold).toBeNull();
    expect(f.rows[1].entry_hold).toBe('waiting');
  });
  it('preserves unchanged-original FIFO and copies patch values before delay', async () => {
    const f = fixture(),
      gate = barrier();
    f.e.entryHoldWriteChains.set(id(4), gate.promise);
    const state: { hold: 'waiting' | 'posting' | null; agreed: boolean } = {
      hold: 'posting',
      agreed: false,
    };
    f.enqueue(state);
    ...
    await last;
    expect(f.applied).toEqual([
      { entry_hold: 'posting', entry_post_agreed: false },
      { entry_hold: null },
    ]);
    expect(f.rows[0].entry_post_agreed).toBe(false);
    expect(f.rows[0].entry_hold).toBeNull();
    expect(f.e.entryHoldWriteChains.has(id(4))).toBe(false);
  });
  it.each(['missing', 'ambiguous', 'missing_seat', 'missing_occupancy'] as const)(
    'explicitly skips %s target without blocking the caller',
    async (kind) => {
      const f = fixture();
      ...
      expect(f.warning).toHaveBeenCalledWith(
        expect.stringContaining('original seat occupancy unavailable or ambiguous')
      );
      expect(f.e.entryHoldWriteChains.has(id(4))).toBe(false);
  ...
  it.each(['reject', 'getter_throw', 'error'] as const)(
    'later writes progress after %s thenable outcome',
    async (mode) => {
      const f = fixture();
      f.failures.push(mode);
      f.enqueue({ hold: 'posting', agreed: false });
      const first = f.tail();
      f.enqueue({ hold: null, agreed: false });
      const last = f.tail();
      await first;
      await last;
      expect(f.warning).toHaveBeenCalled();
  ...
  it('excludes tournaments before resolving a target or changing caller sets', async () => {
    const f = fixture();
    vi.mocked(f.e.isTournamentTable).mockReturnValue(true);
    f.e.seatedPlayers = [];
    expect(f.enqueue({ hold: null })).toBeUndefined();
    f.e.registerWaitForBB(id(4));
    await Promise.resolve();
    expect(from).not.toHaveBeenCalled();
  ...
  it('actual register and post caller preserve state and queue original patch order', async () => {
    const f = fixture(),
      gate = barrier();
    f.e.entryHoldWriteChains.set(id(4), gate.promise);
    vi.spyOn(f.e, 'getSBSeatIndex').mockReturnValue(2);
    vi.spyOn(f.e, 'getButtonSeatIndex').mockReturnValue(3);
    f.e.registerWaitForBB(id(4));
    expect(f.e.waitingForBB.has(id(4))).toBe(true);
    const result = f.e.postBBToEnter(id(4));
    expect(result.success).toBe(true);
    expect(f.e.waitingForBB.has(id(4))).toBe(false);
    expect(f.e.postingBBToEnter.has(id(4))).toBe(true);
    expect(f.original.stack).toBe(100);
    expect(f.calls).toHaveLength(0);
    const pending = f.tail();
    gate.release();
    await pending;
