    ...
    if (strength > 0.85) {
      if (facingThreeBet) {
        if (strength > 0.93 || Math.random() < 0.5) {
          return {
            action: 'raise',
            amount: toCents(Math.min(stack, currentBet * 2.5)),
            thinkTime: 0,
          };
        }
    ...
    if (strength > params.pfrThreshold) {
      ...
      if (facingRaise) {
        if (strength > params.threeBetThreshold) {
          return {
            action: 'raise',
            amount: toCents(Math.min(stack, currentBet * 3)),
            thinkTime: 0,
          };
        }
  ...
  private static decidePostflop(
    ...
    if (strength > 0.8) {
      if (!facingBet) {
        ...
        return {
          action: 'bet',
          amount: toCents(Math.min(stack, Math.max(betSize, gs.minRaise))),
          thinkTime: 0,
        };
      }
      if (Math.random() < params.checkRaiseFreq * 1.5) {
        const raiseSize = toCents(
          Math.min(stack, toCall + (pot + toCall) * (0.8 + Math.random() * 0.4) * sizeMult)
        );
    ...
    if (strength > 0.55) {
      ...
      if (strength > 0.7 && Math.random() < 0.35) {
        return {
          action: 'raise',
          amount: toCents(Math.min(stack, currentBet * (2.2 + Math.random() * 0.6))),
          thinkTime: 0,
        };
      }
    ...
    if (strength > 0.3) {
      ...
      if (strength > 0.45 && Math.random() < params.bluffFreq * 0.5) {
        return {
          action: 'raise',
          amount: toCents(Math.min(stack, currentBet * 2.5 * sizeMult)),
          thinkTime: 0,
        };
      }
      return { action: 'fold', thinkTime: 0 };
