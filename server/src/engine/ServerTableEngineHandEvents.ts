...
export abstract class ServerTableEngineHandEvents extends ServerTableEngineSettlement {
   ...
   *
   * Same guard as the inline `try { ... } catch { /* non-fatal *\/ }` blocks
   * elsewhere in this file; used where the payload is large enough that
   * wrapping it in place would re-indent a hundred lines for no gain.
   */
    ...
    switch (event.type) {
      ...
      case 'WINNERS': {
        ...
        if (this.currentHandWinnerIds.length > 0) {
          ...
          /* PER-WINNER SHARE IS READ, NOT ESTIMATED (2026-09-09).

             It used to be `(w.amount / totalEligibleAmount) * p.amount`, and
             every part of that was wrong:

               - `w.amount` is the winner's TOTAL across every pot in the hand
                 and `p.amount` is ONE pot, so the ratio pro-rated a single pot
                 by a whole-hand denominator. On any hand with a side pot the
                 numbers simply did not describe this pot;
               - the result is an unrounded IEEE float in a money field on the
                 wire (0.30000000000000004 chips);
               - the `|| 1` turned "no eligible winner amounts" into a silent
                 division by one, i.e. a fabricated share, on exactly the case
                 where the honest answer is "I cannot tell" (10.86).

             `buildPotAwardGroups()` already carries the EXACT post-rake share
             of each (board, pot, hi/lo half) - it is what `pot_awards` above
             publishes - so the same numbers are summed per user for this pot
             index and rounded to cents. When there are no award groups at all
             the field is OMITTED rather than invented; `pot_awards` and
             `winners` are both still on the event, and a missing field is
             something a client can notice. */
          const potBreakdown = capturedPots.map((p, idx) => {
            ...
            for (const group of capturedPotAwards) {
              if (group.pot_index !== idx) continue;
              for (const gw of group.winners) {
                shareByUser.set(gw.user_id, (shareByUser.get(gw.user_id) ?? 0) + gw.amount);
              }
            }
            return {
              pot_index: idx,
