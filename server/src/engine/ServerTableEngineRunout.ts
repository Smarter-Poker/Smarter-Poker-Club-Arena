...
export abstract class ServerTableEngineRunout extends ServerTableEngineTurns {
  ...
  protected wireRunItTwiceEvents(): void {
    ...
    this.runItTwiceEngine.addEventListener((event) => {
      ...
      /* THE EVENT KNOWS WHICH HAND IT IS ABOUT; THIS FORWARDER DID NOT
         (2026-09-09). The DeadlineScheduler fires the auto-decline on its own
         clock, and this listener read `this.handCount` at FIRE time while the
         event carries its own `handId` - `${tableId}:${handCount}`, stamped
         when the offer was made. A timeout that surfaces after the hand has
         turned over therefore toasted "Running It Once" on the NEXT hand, and
         because emitRitSingleRun stamps `ritSingleRunNotifiedHand`, it also
         SUPPRESSED that hand's own legitimate single-run notice. One late
         timer, two wrong answers, on a hand that may not have had an all-in at
         all.

         The hand number is taken from the event and passed through, so a stale
         timeout is dropped and a live one is announced against the hand it
         belongs to. */
      ...
      this.emitRitSingleRun('no_agreement', undefined, declinedHand);
    });
  }
