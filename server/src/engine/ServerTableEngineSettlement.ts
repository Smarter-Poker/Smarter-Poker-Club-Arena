  ...
  protected async postHandTasks(players: SeatedPlayer[]): Promise<void> {
    ...
    await runStep('bbj_mini_payout', true, async () => {
      ...
      if (outcome.status === 'queued') {
        ...
        this.hub?.emitEvent(this.tableId, {
          type: 'bbj_payout_pending', kind: 'mini', table_id: this.tableId,
          hand_number: snap.handNumber, emitted_at: Date.now(), replay_until: Date.now() + 60_000,
          loser: { userId: mini.loserUserId }, winner: { userId: mini.winnerUserId },
