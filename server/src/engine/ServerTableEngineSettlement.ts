...
export abstract class ServerTableEngineSettlement extends ServerTableEngineDealing {
  ...
  protected async postHandTasks(
    ...
    if (this.isTournamentTable() && snap.dealtStacks.size > 0) {
      ...
      if (!verdict.ok) {
        ...
        try {
          ...
          reportError(alertErr, 'Tournament.chip_conservation_alert_failed', {
            tableId: this.tableId,
            handNumber: snap.handNumber,
          });
        }
    ...
    await runStep('table_unlock', true, async () => {
      // The active-seat count is authoritative. An unavailable count is
      // unknown, never zero; the terminal compare-and-set in updateTableStatus
      // prevents a late hand from reopening a closed tournament table.
