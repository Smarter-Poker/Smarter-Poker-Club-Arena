...
class GameServer {
  ...
  getTournamentManager(tournamentId: string): TournamentManager | undefined {
    return this.tournamentEngines.get(tournamentId);
  }
...
class TournamentManager {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // TOURNAMENT DIRECTOR CONTROLS — Called via HTTP endpoints
  // ═══════════════════════════════════════════════════════════════════════════

  /** TD: Pause the tournament (stops blind timer, broadcasts pause event) */
  async tdPause(): Promise<{ success: boolean; error?: string }> {
    if (!this.running) return { success: false, error: 'Tournament not running' };
    if (this.onBreak) return { success: false, error: 'Already on break' };

    // Save blind timer remaining
    if (this.blindTimer) {
      const elapsed = Date.now() - this.blindTimerStartedAt;
      const blindStructure = this.tournamentCache?.blind_structure || [];
    ...
    await supabase.from('tournaments').update({ is_paused: true }).eq('id', this.tournamentId);
    await this.broadcast('tournament_paused', { level: this.currentLevel, paused_at: new Date().toISOString() });
    console.log(`[Tournament:${this.tournamentId.slice(0, 8)}] TD PAUSED tournament`);
    return { success: true };
  }

  /** TD: Resume the tournament from a TD pause */
  async tdResume(): Promise<{ success: boolean; error?: string }> {
    if (!this.running) return { success: false, error: 'Tournament not running' };
    if (!this.onBreak) return { success: false, error: 'Not paused' };

    this.onBreak = false;
    await supabase.from('tournaments').update({ is_paused: false }).eq('id', this.tournamentId);
    await this.broadcast('tournament_resumed', { level: this.currentLevel, resumed_at: new Date().toISOString() });
    ...
    console.log(`[Tournament:${this.tournamentId.slice(0, 8)}] TD RESUMED tournament`);
    return { success: true };
  }

  /** TD: Add time to current blind level */
  async tdAddTime(minutes: number): Promise<{ success: boolean; error?: string }> {
    if (!this.running) return { success: false, error: 'Tournament not running' };
    if (minutes < 1 || minutes > 30) return { success: false, error: 'Minutes must be 1-30' };

    const addMs = minutes * 60 * 1000;

    if (this.onBreak) {
      // If paused, just add to saved remaining
      this.savedBlindTimerRemaining += addMs;
    } else if (this.blindTimer) {
      // Cancel current timer, recalculate with extra time
      const elapsed = Date.now() - this.blindTimerStartedAt;
      const blindStructure = this.tournamentCache?.blind_structure || [];
      const currentLevelData = blindStructure[Math.min(this.currentLevel, blindStructure.length - 1)];
      const totalMs = (currentLevelData?.durationMinutes || 10) * 60 * 1000;
      const remaining = Math.max(totalMs - elapsed, 0) + addMs;
    ...
    await this.broadcast('time_added', { minutes, level: this.currentLevel, added_at: new Date().toISOString() });
    console.log(`[Tournament:${this.tournamentId.slice(0, 8)}] TD added ${minutes} min to level ${this.currentLevel}`);
    return { success: true };
  }

  /** TD: Skip to next blind level immediately */
  async tdSkipLevel(): Promise<{ success: boolean; error?: string }> {
    if (!this.running) return { success: false, error: 'Tournament not running' };
    ...
    if (this.currentLevel >= blindStructure.length) {
      ...
      blindStructure.push({
        ...
        durationMinutes: Math.max(lastLevel.durationMinutes || 3, 2),
      });
    }
    ...
    await supabase.from('tournaments').update({ current_level: this.currentLevel }).eq('id', this.tournamentId);
    await this.broadcast('level_changed', {
      level: this.currentLevel,
    ...
    // Start timer for new level (unless paused)
    if (!this.onBreak) {
      this.startBlindTimer(blindStructure);
    }

    console.log(`[Tournament:${this.tournamentId.slice(0, 8)}] TD skipped to level ${this.currentLevel}: ${level.smallBlind}/${level.bigBlind}`);
    return { success: true };
  }

  /** TD: Go back one blind level */
  async tdPrevLevel(): Promise<{ success: boolean; error?: string }> {
    if (!this.running) return { success: false, error: 'Tournament not running' };
    if (this.currentLevel <= 0) return { success: false, error: 'Already at level 0' };
    ...
    console.log(`[Tournament:${this.tournamentId.slice(0, 8)}] TD reverted to level ${this.currentLevel}: ${level.smallBlind}/${level.bigBlind}`);
    return { success: true };
  }

  /** TD: Force a break */
  async tdForceBreak(durationMinutes: number = 5): Promise<{ success: boolean; error?: string }> {
    if (!this.running) return { success: false, error: 'Tournament not running' };
    if (this.onBreak) return { success: false, error: 'Already on break' };

    await this.pauseForBreak(durationMinutes * 60 * 1000);
    console.log(`[Tournament:${this.tournamentId.slice(0, 8)}] TD forced ${durationMinutes}-min break`);
    return { success: true };
  }

  /** TD: Cancel tournament with optional refunds */
  async tdCancel(applyRefunds: boolean = false): Promise<{ success: boolean; error?: string }> {
    if (!this.running) return { success: false, error: 'Tournament not running' };

    this.running = false;
    if (this.blindTimer) { clearTimeout(this.blindTimer); this.blindTimer = null; }
    if (this.eliminationTimer) { clearTimeout(this.eliminationTimer); this.eliminationTimer = null; }
    ...
    if (applyRefunds) {
      // Get all registered/playing players and refund buy-ins
      const { data: players } = await supabase
        .from('tournament_players')
      ...
      for (const player of players || []) {
        if (player.buy_in_amount && player.buy_in_amount > 0) {
          await supabase.rpc('credit_wallet', {
            p_user_id: player.user_id,
            p_amount: player.buy_in_amount,
            p_type: 'tournament_refund',
            p_ref: `refund-cancel-${this.tournamentId}`,
            p_note: 'Tournament cancelled — refund',
          });
        }
      }
    }
    ...
    await this.broadcast('tournament_cancelled', {
      refunds_applied: applyRefunds,
      cancelled_at: new Date().toISOString(),
    });
    await this.cleanupBroadcastChannel();

    console.log(`[Tournament:${this.tournamentId.slice(0, 8)}] TD CANCELLED tournament (refunds: ${applyRefunds})`);
    return { success: true };
  }

  /** TD: Adjust current blind level values */
  async tdAdjustBlinds(smallBlind: number, bigBlind: number, ante: number): Promise<{ success: boolean; error?: string }> {
    if (!this.running) return { success: false, error: 'Tournament not running' };
    ...
    await this.broadcast('blinds_adjusted', {
      level: this.currentLevel,
      smallBlind,
      bigBlind,
      ante,
    ...
    console.log(`[Tournament:${this.tournamentId.slice(0, 8)}] TD adjusted blinds: ${smallBlind}/${bigBlind} ante ${ante}`);
    return { success: true };
  }

  /** Deal-making: Get current state for deal calculation */
  async getDealState(): Promise<{ success: boolean; players?: any[]; prizePool?: number; error?: string }> {
    if (!this.running) return { success: false, error: 'Tournament not running' };

    const { data: players } = await supabase
      .from('tournament_players')
      .select('user_id, chip_count, profiles!inner(username)')
      .eq('tournament_id', this.tournamentId)
      .eq('status', 'playing');
    ...
    return {
      ...
      players: (players || []).map((p: any) => ({
        ...
        chips: p.chip_count || 0,
      })),
      prizePool: tournament?.prize_pool || 0,
    };
  }

  /** Deal-making: Execute an agreed deal (distribute prizes and end tournament) */
  async executeDeal(payouts: { userId: string; amount: number }[]): Promise<{ success: boolean; error?: string }> {
    if (!this.running) return { success: false, error: 'Tournament not running' };
    ...
    // Distribute prizes per the deal
    for (const payout of payouts) {
      if (payout.amount > 0) {
        await supabase.rpc('credit_wallet', {
          p_user_id: payout.userId,
          p_amount: Math.trunc(payout.amount * 100) / 100,
          p_type: 'tournament_prize',
          p_ref: `deal-${this.tournamentId}`,
          p_note: 'Tournament deal payout',
        });

        // Update player status
        await supabase.from('tournament_players').update({
          status: 'finished',
          prize_won: Math.trunc(payout.amount * 100) / 100,
          finished_at: new Date().toISOString(),
        }).eq('tournament_id', this.tournamentId).eq('user_id', payout.userId);
      }
    }
    ...
    await this.broadcast('deal_completed', {
      payouts,
      completed_at: new Date().toISOString(),
    });
    await this.cleanupBroadcastChannel();

    console.log(`[Tournament:${this.tournamentId.slice(0, 8)}] Deal executed — ${payouts.length} players paid out`);
    return { success: true };
  }

  /** Get current tournament info for TD panel */
  getTournamentInfo(): { id: string; level: number; onBreak: boolean; running: boolean } {
    return {
...
const httpServer = createServer(async (req, res) => {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // POST /tournament/pause — TD: Pause a tournament
  // Body: { tournamentId }
  // ─────────────────────────────────────────────────────────────────────────
  if (method === 'POST' && url === '/tournament/pause') {
    try {
      const body = JSON.parse(await readBody(req));
      const tm = gameServer.getTournamentManager(body.tournamentId);
      if (!tm) return sendJSON(res, 404, { success: false, error: 'Tournament not found on this server' });
      const result = await tm.tdPause();
      return sendJSON(res, result.success ? 200 : 400, result);
    } catch (err: any) {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // POST /tournament/resume — TD: Resume a paused tournament
  // Body: { tournamentId }
  // ─────────────────────────────────────────────────────────────────────────
  if (method === 'POST' && url === '/tournament/resume') {
    try {
      const body = JSON.parse(await readBody(req));
      const tm = gameServer.getTournamentManager(body.tournamentId);
      if (!tm) return sendJSON(res, 404, { success: false, error: 'Tournament not found on this server' });
      const result = await tm.tdResume();
      return sendJSON(res, result.success ? 200 : 400, result);
    } catch (err: any) {
      return sendJSON(res, 500, { success: false, error: err.message });
    }
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // POST /tournament/add-time — TD: Add time to current level
  // Body: { tournamentId, minutes }
  // ─────────────────────────────────────────────────────────────────────────
  if (method === 'POST' && url === '/tournament/add-time') {
    try {
      const body = JSON.parse(await readBody(req));
      const tm = gameServer.getTournamentManager(body.tournamentId);
      if (!tm) return sendJSON(res, 404, { success: false, error: 'Tournament not found on this server' });
      const result = await tm.tdAddTime(body.minutes || 5);
      return sendJSON(res, result.success ? 200 : 400, result);
    } catch (err: any) {
      return sendJSON(res, 500, { success: false, error: err.message });
    }
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // POST /tournament/skip-level — TD: Skip to next blind level
  // Body: { tournamentId }
  // ─────────────────────────────────────────────────────────────────────────
  if (method === 'POST' && url === '/tournament/skip-level') {
    try {
      const body = JSON.parse(await readBody(req));
      const tm = gameServer.getTournamentManager(body.tournamentId);
      if (!tm) return sendJSON(res, 404, { success: false, error: 'Tournament not found on this server' });
      const result = await tm.tdSkipLevel();
      return sendJSON(res, result.success ? 200 : 400, result);
    } catch (err: any) {
      return sendJSON(res, 500, { success: false, error: err.message });
    }
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // POST /tournament/prev-level — TD: Go back one blind level
  // Body: { tournamentId }
  // ─────────────────────────────────────────────────────────────────────────
  if (method === 'POST' && url === '/tournament/prev-level') {
    try {
      const body = JSON.parse(await readBody(req));
      const tm = gameServer.getTournamentManager(body.tournamentId);
      if (!tm) return sendJSON(res, 404, { success: false, error: 'Tournament not found on this server' });
      const result = await tm.tdPrevLevel();
      return sendJSON(res, result.success ? 200 : 400, result);
    } catch (err: any) {
      return sendJSON(res, 500, { success: false, error: err.message });
    }
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // POST /tournament/force-break — TD: Force a break
  // Body: { tournamentId, durationMinutes? }
  // ─────────────────────────────────────────────────────────────────────────
  if (method === 'POST' && url === '/tournament/force-break') {
    try {
      const body = JSON.parse(await readBody(req));
      const tm = gameServer.getTournamentManager(body.tournamentId);
      if (!tm) return sendJSON(res, 404, { success: false, error: 'Tournament not found on this server' });
      const result = await tm.tdForceBreak(body.durationMinutes || 5);
      return sendJSON(res, result.success ? 200 : 400, result);
    } catch (err: any) {
      return sendJSON(res, 500, { success: false, error: err.message });
    }
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // POST /tournament/adjust-blinds — TD: Adjust blind values for current level
  // Body: { tournamentId, smallBlind, bigBlind, ante }
  // ─────────────────────────────────────────────────────────────────────────
  if (method === 'POST' && url === '/tournament/adjust-blinds') {
    try {
      const body = JSON.parse(await readBody(req));
      const tm = gameServer.getTournamentManager(body.tournamentId);
      if (!tm) return sendJSON(res, 404, { success: false, error: 'Tournament not found on this server' });
      const result = await tm.tdAdjustBlinds(body.smallBlind, body.bigBlind, body.ante || 0);
      return sendJSON(res, result.success ? 200 : 400, result);
    } catch (err: any) {
      return sendJSON(res, 500, { success: false, error: err.message });
    }
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // POST /tournament/cancel — TD: Cancel tournament
  // Body: { tournamentId, applyRefunds? }
  // ─────────────────────────────────────────────────────────────────────────
  if (method === 'POST' && url === '/tournament/cancel') {
    try {
      const body = JSON.parse(await readBody(req));
      const tm = gameServer.getTournamentManager(body.tournamentId);
      if (!tm) return sendJSON(res, 404, { success: false, error: 'Tournament not found on this server' });
      const result = await tm.tdCancel(body.applyRefunds || false);
      return sendJSON(res, result.success ? 200 : 400, result);
    } catch (err: any) {
      return sendJSON(res, 500, { success: false, error: err.message });
    }
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // GET /tournament/deal-state/:tournamentId — Get current deal-making state
  // ─────────────────────────────────────────────────────────────────────────
  const dealStateMatch = url.match(/^\/tournament\/deal-state\/([^/]+)$/);
  if (method === 'GET' && dealStateMatch) {
    try {
      const tm = gameServer.getTournamentManager(dealStateMatch[1]);
      if (!tm) return sendJSON(res, 404, { success: false, error: 'Tournament not found on this server' });
      const result = await tm.getDealState();
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // POST /tournament/execute-deal — Execute an agreed deal
  // Body: { tournamentId, payouts: [{ userId, amount }] }
  // ─────────────────────────────────────────────────────────────────────────
  if (method === 'POST' && url === '/tournament/execute-deal') {
    try {
      const body = JSON.parse(await readBody(req));
      const tm = gameServer.getTournamentManager(body.tournamentId);
      if (!tm) return sendJSON(res, 404, { success: false, error: 'Tournament not found on this server' });
      const result = await tm.executeDeal(body.payouts || []);
      return sendJSON(res, result.success ? 200 : 400, result);
    } catch (err: any) {
      return sendJSON(res, 500, { success: false, error: err.message });
    }
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // GET /tournament/info/:tournamentId — Get tournament engine info
  // ─────────────────────────────────────────────────────────────────────────
  const infoMatch = url.match(/^\/tournament\/info\/([^/]+)$/);
  if (method === 'GET' && infoMatch) {
    const tm = gameServer.getTournamentManager(infoMatch[1]);
    if (!tm) return sendJSON(res, 404, { success: false, error: 'Tournament not found on this server' });
    return sendJSON(res, 200, tm.getTournamentInfo());
  }
