 ...
 *
 * Reworked 2026-09-26 (review of #5152): pins the added dispatcher_path
 * column and its value too, since that is what lets
 * check-retired-cron-jobs-match-dispatcher.mjs tell this job apart from a
 * job that quietly came back to life in the dispatcher.
 */
...
const MIGRATION = readFileSync(
  resolve(
    __dirname,
    '../../../supabase/migrations/20260926171100_a_retired_error_providers_orchestrator_stays_retired.sql'
  ),
  'utf8'
);
...
describe('a retired error provider\'s orchestrator stays retired', () => {
  ...
  it('adds dispatcher_path without touching any other column', () => {
    expect(SQL).toMatch(
      /ALTER TABLE public\.ca_retired_cron_jobs\s*\n\s*ADD COLUMN IF NOT EXISTS dispatcher_path text;/
    );
  });
  ...
  it('records the literal dispatcher path the job was actually dispatched under', () => {
    expect(SQL).toMatch(/'\/api\/clawbot\/orchestrator'\s*\n\)\s*\nON CONFLICT/);
  });
  ...
  it('leaves replaced_by null -- the feature was decommissioned, not swapped for something else', () => {
    expect(SQL).toMatch(/\n\s*null,\s*\n\s*'\/api\/clawbot\/orchestrator'/);
  });

  it('asserts the row installed with its dispatcher_path and that the staleness view now excludes the job', () => {
    expect(SQL).toMatch(
      /SELECT 1 FROM public\.ca_retired_cron_jobs\s*\n\s*WHERE job_name = '\/cron\/clawbot-orchestrator'\s*\n\s*AND dispatcher_path = '\/api\/clawbot\/orchestrator'/
    );
    expect(SQL).toMatch(
      /SELECT 1 FROM public\.v_openclaw_job_staleness\s*\n\s*WHERE job_name = '\/cron\/clawbot-orchestrator'/
    );
    expect(SQL).toMatch(/RAISE EXCEPTION 'clawbot-orchestrator retirement row did not install with its dispatcher_path';/);
    expect(SQL).toMatch(
