      ...
      for (const table of orderedTables) {
        try {
          ...
          const candidateHorses = validHorses.filter((h) => {
            /**
             * HORSES BELONG TO CLUBS (Dan 2026-09-02): "HORSES ARE ASSIGNED
             * TO SPECIFIC CLUBS, NOT 'GLOBAL'." A horse that is not a member
             * of THIS table's club is not a candidate for THIS table. The DB
             * entry gate refuses the buy-in anyway, so this is not what keeps
             * a Shark Club horse out of Deep Stack Society; it is what stops
             * the fleet spending its whole seat budget on refused attempts
             * and reporting them as unknown rolls. Only decidable when the
             * bankroll map loaded completely - an incomplete map is no
             * membership opinion, exactly like it is no bankroll opinion.
             */
