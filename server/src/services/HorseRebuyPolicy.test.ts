...
describe('WIRING - both engine sites ask, and a zero is a decision to leave', () => {
  ...
  for (const [name, SRC] of [
    ...
    it(`the ${name} site distinguishes a refusal from a non-authoritative transaction`, () => {
      expect(SRC).toMatch(/if \(funding\.status === 'unknown'\) return;/);
      expect(SRC).toMatch(/if \(funding\.status === 'funded'\) \{/);
      expect(SRC).toMatch(/horse\.stack = funding\.stack;/);
    });
