...
describe('lease heartbeat locks remain live without weakening exact ownership', () => {
  it('is a fail-closed post-Stage-B stopped-engine cutover', () => {
    ...
    expect(keySharePath).toMatch(/20260910060008_stage_b_lease_keyshare_once\.sql(?:\.pending)?$/);
  ...
  it('has a PostgreSQL 17 two-session proof for both tables and every effective function', () => {
    ...
    for (const migrationName of [
      'tournament_leases_have_fencing_generations.sql',
      'table_leases_and_hand_commits_have_generations.sql',
      'tournament_table_break_close_is_atomic.sql',
      'tournament_manager_requests_carry_lease_authority.sql',
      'post_commit_obligations_are_atomic_and_resumable.sql',
      'a_busy_manager_keeps_its_lease.sql',
      'a_hand_commit_does_not_hold_the_lease_against_its_own_heartb.sql',
      'stage_b_current_postimage_contraction.sql',
    ]) {
      expect(RUNNER).toContain(`migration_by_suffix ${migrationName}`);
    }
    ...
    expect(FIXTURE).toContain('public.fn_platform_frozen()');
    expect(FIXTURE).toContain('public.fn_serialize_engine_maintenance_break_write()');
    expect(FIXTURE).toContain('CREATE TRIGGER aa_serialize_maintenance_break_write');
    expect(FIXTURE).toContain("clock_timestamp()-interval '2 minutes'");
    expect(RUNNER).toContain(
      'UPDATE public.engine_table_leases SET heartbeat_at=clock_timestamp()'
    );
    ...
    for (const exactPreimageHash of [
      ...
      '7e3a52e636fdf400589523ab966fb8dc',
