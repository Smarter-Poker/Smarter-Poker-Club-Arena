...
describe('SentryEventBudget', () => {
  ...
  it('the global cap holds even when every event has a different fingerprint', () => {
    ...
    const allowed = Array.from({ length: 20 }, (_, i) => b.admit(`Table.${i}`)).filter(
      (v) => v.allow
    ).length;
  ...
  it('reads env overrides and ignores garbage', () => {
    expect(
      budgetFromEnv({
        ...
        SENTRY_BUDGET_WINDOW_MS: '-1',
      } as NodeJS.ProcessEnv)
    ).toMatchObject({
      perKeyLimit: 5,
      globalLimit: DEFAULT_BUDGET.globalLimit,
      windowMs: DEFAULT_BUDGET.windowMs,
    });
  });
});
