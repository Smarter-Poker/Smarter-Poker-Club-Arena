...
import { processBBJPayout, processMiniBBJPayout, setBBJPayoutQueue, resolveJackpotSiblingClubIds } from './bbj.js';
...
describe('Mini payouts retain the original durable jackpot operation', () => {
  ...
  it('claims before attempting, retries transport failure and invokes only the Mini RPC', async () => {
    const order: string[] = [];
    const claim = vi.fn(async (_params: unknown, _note: string) => { order.push('claim'); });
    const settle = vi.fn(async () => { order.push('settle'); });
    setBBJPayoutQueue({ claim, settle });
    rpc.mockImplementationOnce(async () => { order.push('rpc'); return { data: null, error: { message: 'fetch failed' } }; })
      .mockImplementationOnce(async () => { order.push('rpc'); return { data: [appliedRow()], error: null }; });
  ...
  it('keeps database failures pending instead of calling them skipped', async () => {
    const claim = vi.fn(); const settle = vi.fn(); setBBJPayoutQueue({ claim, settle });
    rpc.mockResolvedValue({data: null, error: {message: 'fetch failed'}});
    const pending = processMiniBBJPayout(miniParams);
  ...
  it('closes a persisted business-rule refusal without retrying it', async () => {
    const claim = vi.fn(); const settle = vi.fn(); setBBJPayoutQueue({ claim, settle });
    rpc.mockResolvedValue({data: [{applied:false,already_paid:false,refused:'reserve_at_floor'}],error:null});
    expect(await processMiniBBJPayout(miniParams)).toEqual({status:'skipped',reason:'reserve_at_floor'});
    expect(rpc).toHaveBeenCalledTimes(1); expect(settle).toHaveBeenCalledTimes(1);
  });
  it('lets the RPC replay an existing hand even when the current Main bank is empty', async () => {
    from.mockImplementation((name: string) => name === 'clubs'
      ? table({data:{union_id:null},error:null}) : table({data:{id:POOL,main_balance:0,backup_balance:1000},error:null}));
    rpc.mockResolvedValue({data:[appliedRow({applied:false,already_paid:true})],error:null});
