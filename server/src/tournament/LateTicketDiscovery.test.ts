...
function harness(rows = [target()]) {
  ...
  const window = vi.fn(async (): Promise<any> => ({ data: true, error: null }));
  ...
  const run = () => server.recoverLateSatelliteTickets(new Set(rows.map((r) => r.id)), pass, 7);
  return { server, board, window, pass, run };
}
describe('awarded ticket delivery during the actual late registration window', () => {
  ...
  it('retains four unresolved entries and retires without launching successors', async () => {
    const { server, run, pass } = harness(Array.from({ length: 6 }, (_, i) => target(String(i))));
