...
describe('TournamentSweepWorkCursor', () => {
  it('keeps successful 5.1-second stages and eventually reaches every tail stage', async () => {
    ...
    try {
      ...
      // Model nine scheduler admissions whose one successful request takes
      ...
      for (let admission = 0; admission < 9; admission++) {
      ...
      expect(executed).toEqual([0, 1, 2, 3, 4, 5, 6, 7, 8]);
      expect(cursor.nextStage).toBe(9);
      cursor.reset();
