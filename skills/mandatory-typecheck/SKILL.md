...
The branch push starts Club Arena's gated release; it is not completion.
Require the owning Hetzner workflow and exact live SHA before reporting a
deployment.
...
**Step 4: Certify the owning Hetzner publication:**

- Confirm the pull request merged and `publish-club-arena.yml` ended in
  success for the exact Club Arena `main` SHA.
- Confirm cache-busted `build-info.json` responses from both
  `ca-static.smarter.poker` and the public `/hub/club-arena/` rewrite report
  that same SHA.
- For `server/**` changes, also require the sealed engine workflow and exact
  cache-busted engine health SHA.
...
✅ Hetzner publication → exact Club Arena main SHA at both endpoints
```
