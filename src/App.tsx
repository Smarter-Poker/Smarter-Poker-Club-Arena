...
/* THE APP ONLY (store readiness, phase 3). Both are loaded through a dynamic
   import behind the compile-time constant, so the web bundle carries neither
   the gate, the consent sheet nor their stylesheets: on the web AgeGate is a
   pass-through and ConsentPrompt is not rendered. entry-chunk-delta.mjs is
   the gate that would otherwise notice them in first paint. */
const PassThroughGate = ({ children }: { children: ReactNode }) => <>{children}</>;
const AgeGate = IS_NATIVE_BUILD
  ? lazyWithRetry(() => import('./components/legal/AgeGate'))
  : PassThroughGate;
const ConsentPrompt = IS_NATIVE_BUILD
  ? lazyWithRetry(() => import('./components/legal/ConsentPrompt'))
  : () => null;
    ...
    <ErrorBoundary>
      <ToastProvider>
        ...
        <TOSGuard>
          <Suspense fallback={null}>
            <AgeGate>
              {/* THE APP: product analytics is opt-in; asked once (src/lib/consent.ts). Nothing on the web. */}
              {IS_NATIVE_BUILD && (
                <Suspense fallback={null}>
                  <ConsentPrompt />
                </Suspense>
                   ...
                   route or the component comes back. */}
                    ...
                    <Route
                      ...
                      element={
                        <AuthGuard>
                          <PageErrorBoundary pageName="Agent Management">
                            <LegacyClubToolRedirect
                              destination="agents"
                              toolName="Agent Management"
                            />
                          </PageErrorBoundary>
                        </AuthGuard>
                      }
