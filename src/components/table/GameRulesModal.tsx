/**
 * ═══════════════════════════════════════════════════════════════════════════════
 *  GAME RULES MODAL — Table Rules, Limits, and Hand Rankings
 * ═══════════════════════════════════════════════════════════════════════════════
 */
...
export function GameRulesModal({
  ...
  /* #ClubArenaConsole (2026-09-14): the rules sheet wears the shark frame -
     a page of content with one action, Close, on the shark's single plate.
     Re-rendered, not rewritten: every sentence, every row and every gate
     below is as it was; the tabs keep their own active-class expressions
     (the bomb-pot guard anchors on them). The variant is the title, the
     stakes the pill, the kind of table the eyebrow. The content area is the
     one part that scrolls, capped to what the viewport leaves after the head,
     the tabs and the foot, so Close is on screen from the first frame. */
  return (
    <div className="rules-overlay" onClick={onClose}>
      <div
        className="rules-modal sc-dialog"
        role="dialog"
        aria-modal="true"
        aria-label={`${variant} Rules`}
      ...
      >
        <SpadeConsole
          family="shark"
          eyebrow={isCashTable ? 'Cash Game Rules' : 'Table Rules'}
          title={variant}
          pill={stakes}
          pillInk="blue"
          plates={{
              ...
              <div className="rules-modal__content">
                ...
                <div className="rules-modal__section">
                  ...
                  <div className="rules-modal__features">
                    <div
                      className={`rules-modal__feature ${isStraddleEnabled ? 'rules-modal__feature--active sc-ink--green' : ''}`}
                    >
                      <span className="rules-modal__feature-text">Straddle</span>
                    </div>
                    <div
                      className={`rules-modal__feature ${isRunItTwiceEnabled ? 'rules-modal__feature--active sc-ink--green' : ''}`}
                    >
                      <span className="rules-modal__feature-text">Run It Twice</span>
                    </div>
                    <div
                      className={`rules-modal__feature ${isInsuranceEnabled ? 'rules-modal__feature--active sc-ink--green' : ''}`}
                    >
                      <span className="rules-modal__feature-text">Insurance</span>
                    </div>
                    <div
                      className={`rules-modal__feature ${bombPotRules?.enabled ? 'rules-modal__feature--active sc-ink--green' : ''}`}
                    >
