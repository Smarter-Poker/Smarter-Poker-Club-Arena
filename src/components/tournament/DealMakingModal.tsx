...
import { tournamentExecuteDeal } from '../../services/GameServerAPI';
...
const DealMakingModal: React.FC<DealMakingModalProps> = ({
  ...
  const handleAcceptDeal = async (userId: string) => {
    ...
    try {
      ...
      if (allAccepted) {
        // All players accepted — call the SERVER to execute the deal.
        // The server stops engines, distributes wallet credits, and updates DB.
        const amounts = getDealAmounts();
        ...
        const result = await tournamentExecuteDeal(tournamentId, payouts);
        if (!result.success) {
          // Fallback: if server endpoint fails (e.g. tournament not on this server instance),
          // update Supabase directly so the deal isn't lost
          console.warn('Server deal execution failed, falling back to Supabase:', result.error);
          await supabase
            .from('tournaments')
            .update({
              status: 'COMPLETED',
