...
import {
  captureCashoutReceiptCheck, recoverCashoutOperation, assertCashoutStartCurrent,
  type CashoutStart,
...
export function useCashoutReceiptChecks(isCurrent: () => boolean, onFound: () => void) {
  ...
  const refresh = () => { if (current()) redraw(value => value + 1); };
  return {
    cards: current() ? [...records.current.values()].map(entry => ({ ...entry, busy: busy.current.has(entry.key) })) : [],
    retain: (key: string, amount: number, action: string, original: CashoutStart) => {
      ...
      records.current.set(key, { key, amount, action, original,
    ...
    check: async (key: string) => {
      ...
      busy.current.set(key, token); refresh();
      try {
        ...
        if (outcome.found) {
          ...
          retained.message = 'No Verified Receipt Was Found For This Operation. No New Payment Was Sent.';
        }
      } catch {
        if (current() && records.current.get(key) === retained) {
          retained.message = 'The Receipt Could Not Be Verified. The Original Operation Is Still Unconfirmed.';
        }
      } finally {
...
export function CashoutReceiptChecks({ checks }: { checks: ReturnType<typeof useCashoutReceiptChecks> }) {
  if (!checks.cards.length) return null;
  return <section aria-label="Unconfirmed Cashout Outcomes">
    {checks.cards.map(card => <div key={card.key} role="status">
      <strong>{card.action}: {card.amount.toLocaleString()} Chips</strong>
      <p>{card.message}</p>
