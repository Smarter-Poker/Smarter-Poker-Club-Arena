...
export class TournamentEngine {
  ...
  private async resumeRunning(): Promise<void> {
    ...
    // If 0 or 1 active players left, tournament is over — finish it
    if (activePlayers.length <= 1) {
      console.log(
        `[TournamentEngine:${this.tournamentId.slice(0, 8)}] resumeRunning: ${activePlayers.length} active players — finishing tournament`
      );
      if (activePlayers.length === 1) {
        // Award the winner
        ...
        const payouts = resolvePayoutStructure(
          ...
          this.tournamentInfo.current_players
        );
        const firstPrize =
          payouts.length > 0
            ? Math.floor(this.tournamentInfo.prize_pool * (payouts[0].percentage / 100))
            : this.tournamentInfo.prize_pool;
        await this.supabase
          .from('tournament_players')
          .update({ status: 'winner', position: 1, prize: firstPrize })
          .eq('tournament_id', this.tournamentId)
      ...
      // Close all tables
      const { data: openTables } = await this.supabase
        .from('tables')
        .select('id')
        .eq('tournament_id', this.tournamentId)
        .neq('status', 'closed');
