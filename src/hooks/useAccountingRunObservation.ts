...
export function useAccountingRunObservation(
  ...
  const [state, setState] = useState<{
    ...
    revision: number;
    loading: boolean;
    observation: AccountingRunObservation | null;
  }>({
    token: isCurrent,
    revision: -1,
    loading: true,
  ...
  useEffect(() => {
    ...
    const finish = (observation: AccountingRunObservation | null) => {
      if (active && isCurrent())
        setState({ token: isCurrent, revision, loading: false, observation });
    };
    if (!input || !isCurrent()) {
  ...
  return {
    observation: current ? state.observation : null,
    loading: !!input && isCurrent() && (!current || state.loading),
    refresh,
  };
}
