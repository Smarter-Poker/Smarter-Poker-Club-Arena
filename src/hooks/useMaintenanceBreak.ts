...
export function useMaintenanceBreak() {
   ...
   *
   * THE 4404 HANDLER IN TablePage.tsx STILL TREATS 'unknown' AS 'idle' and
   * announces "This Table Is No Longer Running". That file is owned elsewhere
   * in this pass and was deliberately not edited; the one-line change it needs
   * is to hold the toast when this returns 'unknown' and let the next 4404 ask
   * again, exactly as it already holds it when a break is active.
   */
