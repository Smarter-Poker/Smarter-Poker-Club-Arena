...
export function isCashoutStartCurrent(start: CashoutStart | null): boolean {
  ...
  try { assertCashoutStartCurrent(start); return true; } catch { return false; }
}
...
export function usePreparedCashoutOperations(
  ...
  const signature = JSON.stringify(inputs.map(({ key, intent }) => [key, intent.userId,
    intent.clubId, intent.targetId, intent.playerId, intent.kind, intent.amount, intent.note ?? '']));
  const tokenRef = useRef<{ signature: string; current: () => boolean; revision: unknown } | null>(null);
  const [, refresh] = useState(0);
  if (!tokenRef.current || tokenRef.current.signature !== signature ||
      tokenRef.current.current !== current || tokenRef.current.revision !== sourceRevision) {
  ...
  useEffect(() => {
    ...
    const snapshot = latestInputs.current.map(entry => ({ key: entry.key, intent: { ...entry.intent } }));
    if (!ownsToken()) return () => { active = false; };
    if (new Set(snapshot.map(entry => entry.key)).size !== snapshot.length) {
      setResolved({ token, isCurrent: ownsToken, prepared: new Map(),
        errors: new Map([['', 'The Cashout Actions Could Not Be Verified']]) });
      return () => { active = false; };
    }
    void Promise.all(snapshot.map(async entry => {
      try {
        const prepared = await prepareCashoutOperation({ ...entry.intent, isCurrent: ownsToken });
        return { key: entry.key, prepared, error: null };
      } catch (error) {
        return { key: entry.key, prepared: null,
          error: error instanceof Error ? error.message : 'The Cashout Request Could Not Be Prepared' };
      }
    })).then(results => {
      if (!ownsToken()) return;
      setResolved({ token, isCurrent: ownsToken,
        prepared: new Map(results.filter(entry => entry.prepared !== null).map(entry => [entry.key, entry.prepared!])),
        errors: new Map(results.filter(entry => entry.error !== null).map(entry => [entry.key, entry.error!])),
      });
    });
    return () => { active = false; };
  }, [token]);

  const ready = () => resolved?.token === token && tokenRef.current === token && resolved.isCurrent();
  return {
