...
interface RakebackState {
  ...
  capturedPeriods: CapturedRakebackPeriod[];
...
export function useRakeback(userId: string | null) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      patch.capturedPeriods = source.value.periods;
  ...
  const claim = useCallback(async () => {
    ...
    try {
      ...
      if (!request) {
        ...
        const source = await getCapturedRakeback();
        if (!current()) return;
        const sourceActive = observeRakebackActivation(userId, source.source_active);
        update({ sourceActive, capturedPeriods: source.periods });
      ...
      if (request) {
        ...
        if (!current()) return;
        confirmedTotal = result.total_payout;
        deferred = result.periods.some((period) => period.deferred.length > 0);
        ...
        total = await claimLegacyRakeback(clubId);
        if (!current()) return;
      }
      update({
        claimStatus: 'success',
        claimMessage:
          total > 0
            ? `Claimed ${total.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} Chips.` +
      ...
      if (total > 0) {
        masterBus.emit('RAKEBACK_CLAIMED', { clubId: clubId ?? '', amount: total, userId });
      }
      void refresh();
    } catch (error) {
      if (!current()) return;
      ...
      if (confirmedTotal !== undefined && confirmedTotal > 0) {
