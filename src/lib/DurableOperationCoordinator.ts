...
export function createGenerationCoordinator(config: GenerationCoordinatorConfig) {
  ...
  const capturedOperations = new WeakMap<CapturedOperation, OperationSelection & { startId: string }>();
  const admittedOperations = new WeakMap<AdmittedOperation, OperationSelection & { startId: string }>();
  ...
  function readOperationHistory(raw: string | null, hash: string, repeatable: boolean): OperationHistory | null {
    if (raw === null) return null;
    let value: unknown;
    try { value = JSON.parse(raw); } catch { return identityError(); }
    if (!operationObject(value, ['version', 'historyId', 'hash', 'repeatable', 'state', 'head', 'generations']) ||
        value.version !== 2 || !operationUUID(value.historyId) || value.hash !== hash ||
        value.repeatable !== repeatable || !['initializing', 'active'].includes(value.state as string) ||
        !Number.isSafeInteger(value.head) || (value.head as number) < 1 ||
        !Array.isArray(value.generations) || value.generations.length !== value.head ||
        (!repeatable && value.head !== 1)) return identityError();
    const operations = new Set<string>();
    const starts = new Set<string>();
    for (const [index, item] of value.generations.entries()) {
      if (!operationObject(item, ['generation', 'operationId', 'acknowledged', 'starts']) ||
          item.generation !== index + 1 || !operationUUID(item.operationId) ||
          operations.has(item.operationId) || typeof item.acknowledged !== 'boolean' ||
          !Array.isArray(item.starts)) return identityError();
      operations.add(item.operationId);
      let acknowledged = false;
      for (const start of item.starts) {
        if (!operationObject(start, ['startId', 'acknowledged']) || !operationUUID(start.startId) ||
            starts.has(start.startId) || typeof start.acknowledged !== 'boolean') return identityError();
        starts.add(start.startId);
        acknowledged ||= start.acknowledged;
      }
      if (acknowledged !== item.acknowledged ||
          (index < value.generations.length - 1 && !acknowledged)) return identityError();
    }
    const history = value as unknown as OperationHistory;
    if (history.state === 'initializing' && (history.head !== 1 || history.generations[0].starts.length)) {
      return identityError();
    }
    return history;
  }
  function readOperationPointer(raw: string | null, history: OperationHistory): OperationPointer | null {
    if (raw === null) return null;
    let value: unknown;
    try { value = JSON.parse(raw); } catch { return identityError(); }
    if (!operationObject(value, ['version', 'historyId', 'generation', 'operationId', 'startId']) ||
        value.version !== 2 || value.historyId !== history.historyId ||
        !Number.isSafeInteger(value.generation) || (value.generation as number) < 1 ||
        !operationUUID(value.operationId) || !operationUUID(value.startId)) return identityError();
    const generation = history.generations[(value.generation as number) - 1];
    if (!generation || generation.operationId !== value.operationId ||
        !generation.starts.some(start => start.startId === value.startId)) return identityError();
  ...
  function operationMarker(history: OperationHistory): string { return MARKER_PREFIX + history.historyId; }
  function loadActiveOperation(selection: OperationSelection, allowOtherPointer = false): {
    history: OperationHistory; shared: Storage; session: Storage; pointer: OperationPointer | null;
  } {
    const { shared, session } = operationStorage();
    const history = readOperationHistory(shared.getItem(HISTORY_PREFIX + selection.hash), selection.hash, selection.repeatable);
    if (!history || history.state !== 'active' || history.historyId !== selection.historyId ||
        shared.getItem(PREFIX + selection.hash) !== operationMarker(history)) return identityError();
    ...
    if (prior !== null && !history.generations.some(item => item.operationId === prior)) return identityError();
    // Another saved, uncertain intent must not be displaced by this delayed start.
    if (!allowOtherPointer && ((prior !== null && prior !== selection.operationId) ||
        (pointer !== null && (pointer.generation !== selection.generation ||
          pointer.operationId !== selection.operationId)))) return identityError();
  ...
  async function prepare(hash: string, repeatable: boolean, isCurrent: () => boolean): Promise<PreparedOperation> {
    operationStorage();
    ...
    return navigator.locks.request(PREFIX + hash, { mode: 'exclusive' }, () => {
      ...
      if (!config.allowLegacyAdoption && (savedLegacy !== null || (!history && rawLegacy !== null))) return identityError();
      if (!history) {
        ...
        if (savedLegacy !== null && sharedLegacy !== null && savedLegacy !== sharedLegacy) return identityError();
        history = { version: 2, historyId: operationId(), hash, repeatable, state: 'initializing', head: 1,
          generations: [{ generation: 1, operationId: savedLegacy ?? sharedLegacy ?? operationId(),
            acknowledged: false, starts: [] }] };
      ...
      if (history.state === 'initializing') {
        ...
        if ((prior !== null && prior !== history.generations[0].operationId) ||
            (savedLegacy !== null && savedLegacy !== history.generations[0].operationId)) return identityError();
      ...
      const legacyGeneration = savedLegacy === null ? undefined : history.generations.find(item => item.operationId === savedLegacy);
      if ((savedLegacy !== null && !legacyGeneration) ||
          (pointer && legacyGeneration && pointer.operationId !== legacyGeneration.operationId)) return identityError();
      let selected = pointer ? history.generations[pointer.generation - 1] : legacyGeneration ?? history.generations[history.head - 1];
      if (!pointer && !legacyGeneration && repeatable && selected.acknowledged) {
        if (!Number.isSafeInteger(history.head + 1)) return identityError();
        selected = { generation: history.head + 1, operationId: operationId(), acknowledged: false, starts: [] };
        if (history.generations.some(item => item.operationId === selected.operationId)) return identityError();
      ...
      preparedOperations.set(prepared, { hash, historyId: history.historyId, generation: selected.generation,
        operationId: selected.operationId, repeatable, isCurrent });
      return prepared;
    });
  }
  ...
  function capture(
    prepared: PreparedOperation, isCurrent: () => boolean
  ): CapturedOperation {
  ...
  function recapture(
    original: CapturedOperation, isCurrent: () => boolean
  ): CapturedOperation {
  ...
  async function admit(start: CapturedOperation): Promise<AdmittedOperation> {
    ...
    return navigator.locks.request(PREFIX + selection.hash, { mode: 'exclusive' }, () => {
      ...
      if (!existing) {
        if (history.generations.some(item => item.starts.some(saved => saved.startId === selection.startId))) return identityError();
      ...
      const pointer: OperationPointer = { version: 2, historyId: history.historyId,
        generation: generation.generation, operationId: generation.operationId, startId: selection.startId };
  ...
  function reference(value: PreparedOperation | CapturedOperation | AdmittedOperation): OperationReference {
    const selection = preparedOperations.get(value as PreparedOperation) ??
      capturedOperations.get(value as CapturedOperation) ?? admittedOperations.get(value as AdmittedOperation);
    if (!selection) return identityError();
  ...
  async function restore(ref: OperationReference, isCurrent: () => boolean): Promise<PreparedOperation> {
    operationStorage(); assertCurrent(isCurrent);
    if (!operationObject(ref, ['hash','historyId','generation','operationId','repeatable']) ||
        !/^[a-f0-9]{64}$/.test(ref.hash) || !operationUUID(ref.historyId) || !operationUUID(ref.operationId) ||
        !Number.isSafeInteger(ref.generation) || ref.generation < 1 || typeof ref.repeatable !== 'boolean') return identityError();
  ...
  async function wasAcknowledged(value: AdmittedOperation): Promise<boolean> {
    ...
    if (!selection) return identityError();
    operationStorage(); assertCurrent(selection.isCurrent);
    return navigator.locks.request(PREFIX + selection.hash, { mode: 'exclusive' }, () => {
      assertCurrent(selection.isCurrent);
      return loadActiveOperation(selection, true).history.generations[selection.generation - 1].acknowledged;
    });
  }
