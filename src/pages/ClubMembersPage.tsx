 ...
 *     belong to Midway Union above it. Zero tables came back, the function
 *     returned before it ever looked at a seat, and the count fell through to
 *     browser presence: one, the person reading the screen. The seat row knows
 *     what the table does not - table_seats.club_id is the player's home club -
 *     and ca_club_members_overview reads it that way round. Anyone at a live
 *     table is online, horses included.
 *
 ...
 *
 *  8. TITLE CASE, including the search placeholder, via utils/titleCase.
 *
  ...
  const loadMembers = useCallback(
    async (getIsMounted?: () => boolean) => {
      ...
      try {
        ...
        // already in the roster, so this costs nothing.
        if (user?.id) {
        ...
        // Fees come from a rollup that is fed forward from a watermark. Nudge it
        // and move on; the roster is correct either way.
      ...
      <div className="member-main">
        ...
        <span className="member-subline">
          ...
          {member.home_club_name && (
            <span className="member-club">{member.home_club_name}</span>
          )}
        </span>
