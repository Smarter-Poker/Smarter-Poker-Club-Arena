    ...
    switch (evt.type) {
      ...
      case 'POT_WIN': {
        ...
        if (winnerIds.length > 0) {
          ...
          //
          // ROUND 3 ITEM 7: the merge is fenced by the HAND the award belongs
          // to, not by the resets having already run. The engine stamps
          // POT_WIN; where it does not, the hand on the table is the answer.
          // Merging only with a record carrying the SAME stamp is what stops a
          // late or duplicate POT_WIN for the finished hand resurrecting its
          // "Wins The Pot" label and its winners — the comment above says the
          // hand-start resets fence this, and they did not: they are state
          // writes, and this reads the ref, which still holds the old value
          // until React commits.
