 ...
 * presentation.
 *
 * #ClubArenaConsole: one flow, one console. The table's name in the subtitle,
 * On / Off in the pill, the trigger as lit words with their hints, every
 * number printed on the black glass over an engraved rule with its unit
 * beside it, and Back / Save Settings on the two painted plates. Every read,
 * clamp, rounding rule and pinned literal is kept; only the paint changed.
 */
...
/**
 * What the interval MEANS, in the engine's own terms. BombPotScheduler's
 * noteHandStart runs once per hand, before it is dealt, and counts that hand
 * (`handsSinceBomb++`); the hand on which the count reaches the frequency is
 * the bomb, and the count resets to zero when the bomb is dealt. So N = 10 is
 * nine ordinary hands and then a bomb: every 10th hand. N = 1 is a bomb on
 * every hand. (A due bomb still waits for the "Needs" player count below,
 * which has its own row and its own words.)
 */
...
/**
 * Why the number in force is not the number that was typed, or null when it
 * is. Said inline, under the field, at the moment it happens.
 */
...
export default function TableBombSettingsPage() {
  ...
  /* What is IN the interval box, which while a host is typing is not always a
     ...
     number that text means; the box snaps to it on blur. The refusal says why
     the two differ, and stays after the snap so the snap is never silent. */
    ...
    return (
      <div className={styles.page}>
        <SpadeConsole
          ...
          title="Bomb Pots"
          titleId="table-bomb-settings-title"
          pill="Loading"
          pillInk="muted"
          foot="foot"
        >
  ...
  return (
    <div className={styles.page}>
      <SpadeConsole
        ...
        title="Bomb Pots"
        titleId="table-bomb-settings-title"
        subtitle={tableName ? titleCase(tableName) : undefined}
        pill={draft.enabled ? 'On' : 'Off'}
        pillInk={draft.enabled ? 'green' : 'muted'}
        plates={{
        ...
        {draft.enabled && (
          <>
            <div className={styles.field}>
              <span className={`${styles.label} sc-ink--blue`}>Trigger</span>
              <div className={styles.modes} role="group" aria-label="Trigger">
                {MODES.map((m) => (
                  <button
                    key={m.value}
                    type="button"
                  ...
                  >
                    <span
                      className={`${styles.modeLabel} ${draft.mode === m.value ? 'sc-ink--white' : 'sc-ink--muted'}`}
                    >
                      {m.label}
                    </span>
                    <span className={`${styles.modeHint} sc-ink--muted`}>{m.hint}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            {draft.mode === 'every_n_hands' && (
              <div className={styles.intervalGroup}>
                <label className={styles.field}>
                  <span className={`${styles.label} sc-ink--blue`}>Bomb Pot Hand Interval</span>
                  <span className={styles.control}>
                    {/* A text box with the numeric keypad, not type="number":
                        a number input reports "" for a half-typed value, so the
                        page could not tell an empty box from a decimal in
                        progress and had nothing true to say about either. */}
                    <input
                      type="text"
                      inputMode="numeric"
                      autoComplete="off"
                      ...
                      value={frequencyText}
                      aria-invalid={
                        frequencyRefusal !== null &&
                        frequencyText.trim() !== String(draft.frequency)
                          ? true
                          : undefined
                      }
                      ...
                      onBlur={() => {
                        ...
                        // correction stays to explain the snap; "enter a
                    ...
                    />
                    <span className={`${styles.suffix} sc-ink--muted`}>Hands</span>
                  </span>
                </label>
                <div id={frequencyHintId} className={styles.hint}>
                  <p className={`sc-copy ${styles.sentence}`} data-testid="bomb-interval-sentence">
                    {bombHandIntervalSentence(draft.frequency)}
                  </p>
                  {frequencyRefusal && (
                    <p
                      className={`sc-copy ${styles.refusal} ${
                        frequencyText.trim() !== String(draft.frequency)
                          ? 'sc-ink--red'
                          : 'sc-ink--muted'
                      }`}
                      role="alert"
                      data-testid="bomb-interval-refusal"
                    >
                      {frequencyRefusal}
                    </p>
                  )}
            ...
            {draft.mode === 'timed' && (
              <>
                <label className={styles.field}>
                  <span className={`${styles.label} sc-ink--blue`}>A Bomb Every</span>
                  <span className={styles.control}>
                    <input
                      type="number"
                      className={styles.input}
                      ...
                      onChange={(e) =>
                        set('intervalMinutes', Math.max(1, Number(e.target.value) || 1))
                      }
                    />
                    <span className={`${styles.suffix} sc-ink--muted`}>Minutes</span>
                  </span>
                </label>
                <label className={styles.field}>
                  <span className={`${styles.label} sc-ink--blue`}>Show The Clock Within</span>
                  <span className={styles.control}>
                    <input
                      type="number"
                      className={styles.input}
                      ...
                      onChange={(e) =>
                        set('announceMinutes', Math.max(0, Number(e.target.value) || 0))
                      }
                    />
                    <span className={`${styles.suffix} sc-ink--muted`}>Minutes (0 = Always)</span>
                  </span>
                </label>
              </>
            )}
            ...
            <label className={styles.field}>
              <span className={`${styles.label} sc-ink--blue`}>Boards</span>
              <select
                className={`${styles.input} ${styles.select}`}
                value={draft.boards}
                onChange={(e) => set('boards', Number(e.target.value))}
              >
            ...
            <label className={styles.field}>
              <span className={`${styles.label} sc-ink--blue`}>Ante</span>
              ...
              <span className={styles.control}>
                <input
                  ...
                  value={draft.anteBB}
                  onChange={(e) =>
                    set('anteBB', Math.max(0, Math.round(Number(e.target.value) || 0)))
                  }
                />
                <span className={`${styles.suffix} sc-ink--muted`}>X BB (Whole Blinds)</span>
              </span>
            </label>

            <label className={styles.field}>
              <span className={`${styles.label} sc-ink--blue`}>Or A Fixed Ante</span>
              <span className={styles.control}>
                <input
                  type="number"
                  className={styles.input}
                ...
                />
                <span className={`${styles.suffix} sc-ink--muted`}>
                  Chips (0 = Use The Multiple)
                </span>
              </span>
            </label>

            <label className={styles.field}>
              <span className={`${styles.label} sc-ink--blue`}>Needs</span>
              <span className={styles.control}>
                <input
                  type="number"
                  className={styles.input}
                ...
                />
                <span className={`${styles.suffix} sc-ink--muted`}>Players Before It Fires</span>
              </span>
            </label>

            <label className={styles.field}>
              <span className={`${styles.label} sc-ink--blue`}>Played As</span>
              <select
                className={`${styles.input} ${styles.select}`}
                value={draft.variant}
            ...
            <label className={styles.toggleRow}>
              <span className={`${styles.label} sc-ink--blue`}>Separate Bomb Button</span>
              <input
                type="checkbox"
                className={styles.check}
