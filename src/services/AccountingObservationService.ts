...
const FIELDS = [
  ...
  'record_found',
  'state',
  'recorded_scheduled_at',
  'attempts',
  'started_at',
  'finished_at',
  'posted',
];
export const isAccountingUUID = (value: unknown): value is string =>
  typeof value === 'string' &&
...
export function accountingWeekEndingOn(day: string): AccountingWeek {
  ...
  return {
    periodStart: localInstant(start.toISOString().slice(0, 10), 0, 'America/Los_Angeles'),
...
export function accountingInstant(value: unknown): bigint {
  ...
  const match =
    /^(\d{4}-\d{2}-\d{2})T([01]\d|2[0-3]):([0-5]\d):([0-5]\d)(?:\.(\d{1,6}))?(?:Z|\+00:00)$/.exec(
      value
    );
...
export function validateAccountingScope(
  input: AccountingObservationInput
): AccountingObservationInput {
  if (
    !isAccountingUUID(input.actorId) ||
    !isAccountingUUID(input.scopeId) ||
    !['club', 'union'].includes(input.scopeKind)
  )
    throw unavailable();
  const end = accountingInstant(input.periodEnd),
    start = accountingInstant(input.periodStart);
  const week = accountingWeekEndingOn(zonedDate(new Date(input.periodEnd), 'America/Los_Angeles'));
  if (start !== accountingInstant(week.periodStart) || end !== accountingInstant(week.periodEnd))
    throw unavailable();
  return { ...input, scopeId: input.scopeId.toLowerCase(), ...week };
}

export function parseAccountingRunObservation(
  value: unknown,
  requested: AccountingObservationInput
  ...
  if (
    ...
    FIELDS.some((key) => !Object.prototype.hasOwnProperty.call(row, key)) ||
    row.contract_version !== 1 ||
    row.actor_user_id !== input.actorId ||
    row.scope_kind !== input.scopeKind ||
    row.scope_id !== input.scopeId ||
    typeof row.record_found !== 'boolean' ||
    typeof row.posted !== 'boolean' ||
    typeof row.state !== 'string' ||
    accountingInstant(row.period_start) !== accountingInstant(input.periodStart) ||
    accountingInstant(row.period_end) !== accountingInstant(input.periodEnd) ||
    accountingInstant(row.expected_run_at) !== accountingInstant(accountingRunExpectedAt(input))
  )
    throw unavailable();
  const observed = accountingInstant(row.observed_at);
  const empty =
    row.recorded_scheduled_at === null &&
    row.attempts === null &&
    row.started_at === null &&
  ...
  if (!row.record_found) {
    if (
      row.posted ||
      !empty ||
      row.state !== (input.scopeKind === 'club' ? 'unavailable' : 'no_recorded_run')
    )
      throw unavailable();
    ...
    if (
      !['running', 'incomplete', 'posted'].includes(String(row.state)) ||
      typeof row.attempts !== 'number' ||
      !Number.isSafeInteger(row.attempts) ||
      (row.attempts as number) < 1 ||
      accountingInstant(row.recorded_scheduled_at) !== accountingInstant(row.expected_run_at)
    )
      throw unavailable();
    ...
    if (row.state === 'running') {
      ...
      accountingInstant(row.finished_at) < started ||
      accountingInstant(row.finished_at) > observed ||
      row.posted !== (row.state === 'posted')
    )
      throw unavailable();
  }
...
export async function readAccountingRunObservation(
  requested: AccountingObservationInput
): Promise<AccountingRunObservation> {
  const input = validateAccountingScope({ ...requested });
  const account = captureWeeklyAccountingAccount(input.actorId);
  const current = () => {
    if (!account.isCurrent() || (input.isCurrent && input.isCurrent() !== true))
      throw unavailable();
  };
  current();
