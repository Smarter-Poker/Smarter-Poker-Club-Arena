...
export async function searchClubs(query: string): Promise<Club[]> {
  const { data, error } = await supabase
    .from('clubs')
    .select(
      'id, club_id, name, slug, description, avatar_url, banner_url, color_theme, member_count, table_count, chip_treasury, is_public, requires_approval, owner_id, union_id, settings, created_at, updated_at'
    )
    .ilike('name', `%${query}%`)
    .eq('is_public', true)
...
export async function getUserMemberships(
  ...
  const { data, error } = await supabase
    ...
    .select(
      ...
      club:clubs(id, club_id, name, slug, description, avatar_url, banner_url, color_theme, member_count, table_count, chip_treasury, is_public, requires_approval, owner_id, union_id, settings, created_at, updated_at)
    `
    )
    .eq('user_id', userId);
