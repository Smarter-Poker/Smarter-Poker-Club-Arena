...
export async function searchClubs(query: string): Promise<Club[]> {
  const { data, error } = await supabase
    .from('clubs')
    .select(
      'id, club_id, name, slug, description, avatar_url, banner_url, color_theme, member_count, table_count, chip_treasury, is_public, requires_approval, owner_id, settings, created_at, updated_at'
    )
    .ilike('name', `%${query}%`)
    .eq('is_public', true)
