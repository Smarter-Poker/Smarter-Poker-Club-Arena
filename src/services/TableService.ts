...
class TableService {
  ...
  async createTable(
    ...
    // FIX 111: Populate BOTH the JSONB `settings` blob AND the top-level columns
    // that the server reads via loadTable(). Without this, RIT/Insurance/Straddle
    // etc. would always be disabled on the server (top-level columns default FALSE).
    const { data, error } = await supabase
      .from('tables')
      .insert({
