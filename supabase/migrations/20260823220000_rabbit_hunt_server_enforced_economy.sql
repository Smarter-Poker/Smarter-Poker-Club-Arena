-- 20260823220000_rabbit_hunt_server_enforced_economy.sql
--
...
-- ServerTableEngineSettlement broadcast the five remaining deck cards to EVERY
-- client at the table the moment a hand ended before the river. TablePage stored
-- them in serverRabbitCardsRef and RabbitHunt then "charged" diamonds to display
-- cards the browser already held. Anyone with devtools read them free, and
-- because it was a table-wide broadcast, so did the opponents. Charging
-- 1 diamond for that would be theatre.
--
...
--
-- Verified on production against a real player and a real hand:
--   VIP 1st hunt      -> charged 0, free_remaining 99, cards returned
--   VIP 101st hunt    -> charged 1
--   player not in hand-> "you did not play that hand"
--   unknown hand      -> "no rabbit hunt available for that hand"
--   no auth.uid()     -> "authentication required"
...
-- Offers are worthless once the player moves on; keep the table tiny.
-- Scheduled at minutes 9 and 39, which no other maintenance job uses, so
-- fn_db_saturation_selftest CHECK 7 stays clean.
DO $sched$
BEGIN
  IF EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'sp-prune-rabbit-hunt-offers') THEN
    PERFORM cron.unschedule('sp-prune-rabbit-hunt-offers');
  END IF;
  PERFORM cron.schedule('sp-prune-rabbit-hunt-offers', '9,39 * * * *', $cmd$
select case
         when pg_try_advisory_lock(hashtext('sp-prune-rabbit-hunt-offers'))
           then (select set_config('statement_timeout','30s',true) is not null
                    and public.sp_prune_rabbit_hunt_offers(5000) >= 0)::text
...
-- ROLLBACK
--   select cron.unschedule('sp-prune-rabbit-hunt-offers');
