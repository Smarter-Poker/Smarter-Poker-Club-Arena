-- ── Fix Roster Query Performance & Add Downline Fees ────────────────────────
-- The previous JOIN optimization caused the planner to re-evaluate the base CTE
-- multiple times, turning an 8.5s query into a 40s+ timeout. Adding MATERIALIZED
-- fixes the execution plan. We also add `downline_fees` per user request.
...
BEGIN
  ...
  df_fees AS MATERIALIZED (
    SELECT c.root AS d_user_id,
           sum(r.fees) AS d_fees
    FROM closure c
    JOIN public.member_fee_rollup r ON r.user_id = c.descendant
    GROUP BY c.root
  )
  SELECT
    b.m_user_id,
    ...
    round(coalesce(f.f_fees, 0) + coalesce(df.d_fees, 0), 2),
  ...
  LEFT JOIN df_fees   df ON df.d_user_id = b.m_user_id
