...
-- Applied to production 20260825013028; the .sql file was never committed at the
-- time (see docs/changelog and issue: unrecorded-migration backfill). Content is
-- byte-exact to what ran. Do NOT re-apply; it is already live.

-- PERFORMANCE 2026-08-25. fn_apply_rakeback_player_stats_batch: replace the
-- per-item loop with a set-based path, keeping the original loop as a fallback.
--
-- Applied at Dan's explicit instruction after review.
--
-- EVIDENCE (pg_stat_statements, 2026-08-24 09:00 -> 21:35 UTC):
--   139 calls, mean 39,074 ms, MAX 380,366 ms (6.3 minutes), 1.51 core-hours.
-- On a 2 vCPU instance one 380-second call holds a core for over six minutes,
-- halving platform query capacity while live play competes for the other one.
--
-- ROOT CAUSE. The wrapper looped over p_items and for EACH item ran
...
--   * an UPSERT on player_stats keyed (user_id, club_id) taking a ROW LOCK, so
--     items for the same player serialised against each other in one batch.
--
-- FIX. Three statements regardless of batch size:
...
--      genuinely new - this is what preserves the apply-once guarantee and
--      stops a replayed batch double-counting rake;
--   3. one aggregated UPSERT into player_stats, touching each (user_id, club_id)
--      row ONCE per batch instead of once per item.
--
-- DISTINCT ON is required, not cosmetic: ON CONFLICT cannot handle two rows with
-- the same conflict key in one INSERT, and a batch can legally contain the same
-- rake_record/user twice.
--
-- RESILIENCE. The original per-item loop is retained verbatim as an EXCEPTION
-- fallback. The old code could absorb one bad item and report it in `failed`; a
-- single set-based statement cannot. If the fast path raises for any reason we
-- fall back to row-by-row and the caller sees exactly the old behaviour and the
-- old {ok, failed, first_error} shape.
--
-- ROUNDING. The old code applied
...
-- on sub-cent inputs, where rounding ONCE is strictly LESS lossy than rounding
-- repeatedly - the same accumulated-rounding drift that left the BBJ promo bank
-- 1,456.30 short on 2026-08-23. No rake is created or destroyed either way.
--
-- ROLLBACK: restore the previous body, which is the FOR..LOOP retained below,
...
-- POST-APPLY ASSERTIONS. Static only: this function moves money-adjacent
-- counters, so the assertions must never invoke it against production data.
DO $$
DECLARE v_src text;
BEGIN
  ...
  IF v_src IS NULL THEN RAISE EXCEPTION 'function is missing'; END IF;
  IF position('fast path' in v_src) = 0 THEN
    RAISE EXCEPTION 'function was not replaced with the set-based body';
  END IF;
  IF position('fallback: original loop' in v_src) = 0 THEN
    RAISE EXCEPTION 'the per-item fallback loop was lost';
  END IF;
  IF position('ON CONFLICT (rake_record_id, user_id) DO NOTHING' in v_src) = 0 THEN
    RAISE EXCEPTION 'the apply-once idempotency guard was lost';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_index WHERE indrelid='public.rakeback_stats_applied'::regclass AND indisunique) THEN
    RAISE EXCEPTION 'rakeback_stats_applied has no unique index - ON CONFLICT would fail';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_index WHERE indrelid='public.player_stats'::regclass AND indisunique) THEN
    RAISE EXCEPTION 'player_stats has no unique index - the aggregated UPSERT would fail';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
                  WHERE n.nspname='public' AND p.proname='apply_rakeback_player_stats') THEN
