...
-- Applied to production 20260825081617; the .sql file was never committed at the
...
-- ═══════════════════════════════════════════════════════════════════════════
--
-- Data repair for two engine defects fixed in the same PR
...
--      LAST_HAND_GRACE_MS later). A restart inside that window left
--      `this.onBreak` false against a row saying true, which guarantees
--      resumeFromBreak can never clear it.
--
...
-- alone deliberately -- the engine fix above heals those on their next resume,
-- and a running row is the only one where the flag could still be true.
--
-- ROLLBACK: none needed (idempotent, and re-setting a finished tournament's
-- on_break to true would itself be the defect). To identify what was touched:
