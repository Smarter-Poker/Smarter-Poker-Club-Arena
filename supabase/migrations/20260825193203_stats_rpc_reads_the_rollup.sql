...
-- Applied to production 20260825193203; the .sql file was never committed at the
...
-- same keys, same rounding. The only thing that changed is where the 750 rows
-- come from:
--
...
--
-- plus the not-yet-rolled tail, computed live from the same facts function so a
-- player's newest hands are never missing while the rollup catches up. Once
-- rolled_floor reaches the oldest hand, that tail is only the last cron
-- interval; during the initial backfill it is longer and the page is no worse
-- than it was before this change.
--
-- Both UNION branches are parenthesised because the first carries ORDER BY and
-- LIMIT, which Postgres will not accept bare inside a set operation.
...
BEGIN
  -- Lifetime volume is still the TRUE number, independent of the 750-hand
  -- analysis window: an index-only scan over ca_hand_player_idx.
...
WITH scored AS MATERIALIZED (
  SELECT * FROM (
    ...
    (
      ...
      FROM ca_hand_player_facts(coalesce(v_ceil, now()), now()) f
