...
-- Applied to production 20260825214012; the .sql file was never committed at the
-- time (see docs/changelog and issue: unrecorded-migration backfill). Content is
-- byte-exact to what ran. Do NOT re-apply; it is already live.

-- One line changes: the tail now calls ca_hand_player_facts(..., p_user) instead
-- of computing every player in the window and filtering afterwards. A 15-minute
-- tail is ~2,100 hands, which was ~27,000 (player, hand) rows produced so that
-- ~30 survived the WHERE. Verified bit-identical before rewiring: 62 rows either
-- way, zero rows in either direction of an EXCEPT.
--
-- The redundant `WHERE f.user_id = p_user` is KEPT deliberately. It costs
-- nothing now that the set is already narrowed, and it means a future change to
-- the argument list cannot silently widen what this branch contributes to a
-- player's stats.
