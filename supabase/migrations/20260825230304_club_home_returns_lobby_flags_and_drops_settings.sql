...
-- Applied to production 20260825230304; the .sql file was never committed at the
...
-- else -- which is exactly what had happened to five flags the table creation
-- page has been writing all along:
--
...
--
-- The same list was also missing every cash feature added since it was
-- written: Cap, No Rathole, Pineapple, Anonymous, Ban Chat, Restrict
-- Observers, the RIT mode and the auto-start threshold. The engine reads them
-- and enforces them; the lobby could not say so.
--
-- And it drops `settings`. All 50 live tables carry `{}` there -- it is a
-- legacy blob that predates the columns, and shipping it to every client on
-- every lobby load is bytes spent to say nothing. The client reads columns.
--
-- Tournaments gain the three flags they were missing plus is_pinned, which is
-- what actually pins a tournament to the top of the board.
...
BEGIN
  ...
  FROM (
    SELECT id, name, game_variant, stakes, current_players, max_players, status,
           small_blind, big_blind, min_buy_in, max_buy_in, created_at,
           -- Rule medallions
...
BEGIN
  ...
  IF COALESCE((v->>'found')::boolean, false) THEN
    ...
    IF t IS NOT NULL THEN
      ...
      END IF;
      IF NOT (t ? 'is_vip_only' AND t ? 'label_as_new'
