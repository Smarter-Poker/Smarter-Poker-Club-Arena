...
-- Applied to production 20260825232537; the .sql file was never committed at the
-- time (see docs/changelog and issue: unrecorded-migration backfill). Content is
-- byte-exact to what ran. Do NOT re-apply; it is already live.

-- A SAVED TEMPLATE IS NOT A LIVE TABLE (2026-08-25)
--
...
-- differently: the RPC filter keeps it off the board, and the buy-in guard
-- makes it unsittable even if some other surface links straight to it.
--
-- Both halves PATCH the deployed body rather than restating it, so this
-- migration cannot silently revert anything applied to those functions
-- earlier the same day. Each half is a no-op if already present, and the
-- assertion at the foot re-checks every guard that came before.
...
BEGIN
  ...
  END IF;
  -- Everything applied earlier today must have survived both patches.
