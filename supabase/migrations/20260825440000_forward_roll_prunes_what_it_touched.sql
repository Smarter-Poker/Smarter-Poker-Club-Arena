...
-- this whole design exists to avoid inside a week - and the "~20 pages instead
-- of 3,730" claim in 20260825400000 would have quietly stopped being true, with
...
-- scheduled job somebody has to remember to add. A full-table sweep is a window
...
-- without the newest hands and would delete exactly the wrong ones.
--
-- A player who stops playing keeps whatever they had at the time, slightly over
-- the retention if they were mid-window. That is bounded and harmless: the next
-- hand they play prunes them. ca_prune_hand_player_stat stays for full sweeps
-- and for repairing drift like that.
--
-- Verified after applying: 585 players, max 1,000 rows per player, 0 players
-- over retention, 584,889 rows total.
...
BEGIN
  ...
  -- Separate statement, so the ranking below sees the rows just written above.
