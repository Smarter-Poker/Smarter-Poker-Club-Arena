...
-- call and it did not touch either real problem: there were still three scans,
-- and the numbers were still the viewer's view rather than the club's.
--
...
-- family's rule makes them agree tomorrow too, and the assertion below pins it
-- against fn_get_club_member_count on every club rather than trusting it.
--
...
BEGIN
  ...
  -- And total must never be less than one of its own subsets, which would mean
  -- the FILTER is counting rows the unfiltered count does not see.
  IF EXISTS (
    SELECT 1 FROM (SELECT club_id FROM club_members GROUP BY club_id) c
