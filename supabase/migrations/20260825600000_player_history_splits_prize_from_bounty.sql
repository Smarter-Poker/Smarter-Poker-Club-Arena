-- ═══════════════════════════════════════════════════════════════════════════════
--  PLAYER HISTORY: SPLIT THE PLACEMENT PRIZE FROM THE BOUNTY EARNINGS
-- ═══════════════════════════════════════════════════════════════════════════════
--
-- Dan section 45: a past event has to be able to show Finish / Prize / Bounties /
-- Bounty Earnings / Total Won. Section 37: the categories stay separate.
--
-- `ca_player_stats_full` is the ONE function the player stats page reads, and its
-- `recent_tournaments` rows carried a single field:
--
--     coalesce(tp.prize, 0) + coalesce(tp.bounty_winnings, 0) AS prize
--
-- One number, named after one of the two things inside it. A mystery bounty
-- event where a player min-cashed for 40 and opened a 5,000 chest showed
-- "prize 5,040" and there was no way to get the split back out of it.
--
-- This replaces that with the parts AND the total, so nothing has to be
-- reconstructed by subtraction:
--
--     prize             the placement prize only
--     bounty_winnings   every bounty collected, mystery and flat alike
--     bounties          how many knockouts paid
--     total_won         prize + bounty_winnings, the old `prize` exactly
--
-- `total_won` is the old value under an honest name, so the page's net figure
-- (winnings minus buy-in) is unchanged to the chip. The summary block gains the
-- same two extra fields; its `total_winnings` is deliberately left alone, since
-- it already meant prize + bounty and several surfaces read it.
--
-- The tournament id and the mystery flag come along so history can link back to
-- the event and label the format.
--
-- NO SCHEMA CHANGE. This is a function body only: no table, no column, no RLS.

CREATE OR REPLACE FUNCTION public.ca_player_stats_full(p_user uuid, p_days integer DEFAULT NULL::integer)
 RETURNS jsonb
 LANGUAGE plpgsql
...
tourn AS (
  SELECT count(*)::int AS entries,
         ...
         -- Section 37: the two halves, kept apart. total_winnings above is the
         -- sum and stays exactly as it was for every existing reader.
...
tourn_recent AS (
  SELECT t.id AS tournament_id, t.name, t.start_time, t.variant,
         coalesce(t.is_mystery_bounty, false) AS is_mystery_bounty,
         tp.position AS finish_rank, tp.status,
         -- PLACEMENT ONLY. This field used to be prize + bounty_winnings.
         ...
         -- The old `prize`, under the name that describes it.
...
COMMENT ON FUNCTION public.ca_player_stats_full(uuid, integer) IS
  'Player stats for the stats page. recent_tournaments.prize is the PLACEMENT prize only; bounty_winnings and bounties are the bounty half; total_won is the sum (Dan sections 37 and 45).';
