...
-- transaction-scoped and taken in a fixed order (user then table), so two
-- callers cannot deadlock.
--
-- Rewritten in place from pg_get_functiondef, asserting every anchor, so a
-- silent no-op replace cannot look like success.
--
-- APPLIED to production 2026-08-25 and probed on a live 6-max table:
--   seat 7        -> 'TABLE_SIZE: seat 7 does not exist at this table (6 max)'
--   full table    -> 'TABLE_SIZE: table is full (6 of 6 seats taken)'
-- ═══════════════════════════════════════════════════════════════════════════
