...
-- the client is a courtesy a modified client ignores.
--
...
--
-- Reactions and gift throws ride this same table encoded as [REACTION:...]
-- and [THROW:...] messages, so they fall silent too. That is the correct
-- reading of "Ban Chat": an emoji spammed at a player is the behaviour the
-- switch exists to stop.
--
-- APPLIED to production 2026-08-25 and the predicate probed both ways.
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  IF v_check IS NULL THEN RAISE EXCEPTION 'the replacement policy did not land'; END IF;
  IF position('auth.uid()' IN v_check) = 0 THEN RAISE EXCEPTION 'the identity rule was lost'; END IF;
  IF position('message_type' IN v_check) = 0 THEN RAISE EXCEPTION 'the message_type rule was lost'; END IF;
  IF position('ban_chat' IN v_check) = 0 THEN RAISE EXCEPTION 'ban_chat is not being checked'; END IF;
END
$verify$;
