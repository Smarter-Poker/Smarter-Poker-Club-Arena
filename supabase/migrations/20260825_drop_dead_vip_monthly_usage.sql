...
--
-- The two diamond-balance tables (`diamond_wallets`, 416 rows;
-- `user_diamond_balance`, 5 rows) are deliberately NOT touched here. They are
-- also superseded by `profiles.diamonds`, but they hold data and are still read
-- by RPCs, so retiring them is a migration of its own with a real plan.
--
-- ROLLBACK
