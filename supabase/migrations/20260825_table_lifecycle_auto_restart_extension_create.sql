...
--   table_lifecycle_auto_restart_extension_create
--
-- Auto Restart, Auto Extension and Auto Create Table are three switches on the
-- table creation page with three tooltips and ZERO readers. `auto_restart` was
-- worse than dead: CreateTableModal's checkbox landed in the `settings` JSONB
-- blob nothing reads, so it never reached the column at all -- the same class
-- of bug as the straddle / bomb-pot / ante mirrors beside it in TableService.
--
...
--                     being closed for going quiet. Enforced on the TypeScript
--                     side, in the UPDATE that does the closing, because a
--                     check anywhere else could be raced past.
--
...
--
-- WHY SQL AND NOT spawnOverflowTables. That method only knows the fleet's own
-- DEFAULT_TABLES, matched by NAME PREFIX. A host's table is not in that list
-- and never could be, so widening it was never an option. This pass asks the
...
--   a closed table WITH auto_restart          -> reopened to 'waiting'
--   a closed table WITHOUT it                 -> left closed (0 touched)
--   a 2-seat table with auto_create_table and
--   one seat taken                            -> spawned "PROBE OVERFLOW #2"
--                                                carrying seven_deuce_enabled,
--                                                cap_enabled, cap_bb 77,
