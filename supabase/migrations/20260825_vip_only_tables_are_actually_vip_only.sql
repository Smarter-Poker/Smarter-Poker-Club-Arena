...
--   vip_only_tables_are_actually_vip_only
--
-- `tables.is_vip_only` has been a toggle on the table creation page since it
-- was written and NOTHING has ever read it -- not the engine, not the lobby,
-- not the seat sale. A host who switched it on got a table anyone could sit at.
--
-- The gate belongs HERE, in the function that actually sells the seat, for the
-- same reason the table-size and no-rathole checks landed here: this is the one
-- door every path goes through, and a check in the client is a suggestion.
--
-- VIP is `profiles.is_vip` AND an unexpired `vip_expires_at`. An expired VIP is
-- not a VIP; leaving them able to enter would make the expiry meaningless.
--
-- THE CLUB OWNER AND ITS STAFF ARE EXEMPT. A host locked out of their own
-- VIP-only table cannot moderate it, which is a worse failure than a non-VIP
-- taking a seat.
--
-- Placed AFTER the blacklist check and BEFORE the seat locks, so a player who
-- may not be here at all is turned away before any lock is taken.
--
-- This file is the FULL deployed body rather than a string patch, because it
-- is now the fourth rewrite of this function in one day and a patch that has to
-- find its own anchors is a patch that can silently half-apply. The assertion
-- at the foot re-checks that the three guards added before it all survived.
--
-- VERIFIED against production inside a rolled-back transaction (CLAUDE.md 11.5,
-- never call a money RPC outside one):
--   non-VIP      -> VIP_ONLY: this table is open to VIP members only
--   real VIP     -> passed the gate (stopped later by TABLE_SIZE, table full)
--   expired VIP  -> VIP_ONLY: this table is open to VIP members only
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  END IF;
  -- The three guards added earlier the same day must all have survived.
