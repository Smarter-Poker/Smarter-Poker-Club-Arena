...
AS $fn$
  ...
  -- tuples per page measured only over pages that hold rows, i.e. how densely
  -- this table packs when it does pack. Empty pages are excluded on purpose.
  d AS (
  ...
  IF EXISTS (
    SELECT 1 FROM pg_trigger t
     WHERE t.tgrelid = 'public.hand_history'::regclass
       ...
       AND (t.tgtype & 16) <> 0            -- TRIGGER_TYPE_UPDATE
  ) THEN
...
-- ── Schedule ────────────────────────────────────────────────────────────────
-- Every 5 minutes, deliberately out of phase with sp_prune_hand_history_10m
-- (which runs at :03,:13,...) so the two never contend for the same rows.
SELECT cron.unschedule('hand-history-compact')
...
-- VACUUM is what actually returns the pages; the compactor only makes the tail
-- empty. SKIP_LOCKED so it never queues behind anything, and it yields on its
-- own if a backend waits on the brief truncate lock.
SELECT cron.unschedule('hand-history-compact-vacuum')
...
BEGIN
  ...
  -- The compactor must be a no-op while disabled. Prove it here rather than
  -- trusting the branch.
  UPDATE public.hand_history_compaction_policy SET enabled = false;
