...
--
--   But the "exact" basis is Δtotal_winnings − Δtotal_losses from
...
--   hook. Measured per club-day, (Δlosses − Δwinnings) / house_cut should be
...
AS $fn$
  ...
  SELECT jsonb_build_object(
    ...
    -- 1.0 means the ledger balances. Negative means the players finished the
    -- day up while paying rake, which is impossible and is the tell.
...
DECLARE v jsonb;
BEGIN
  -- A day that is not finished must never be touched.
  ...
  -- Days before the ledger cutover carry a partial basis and must stay refused.
