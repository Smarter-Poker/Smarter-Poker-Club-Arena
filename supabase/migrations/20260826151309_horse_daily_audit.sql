...
--
-- "I need the daily audit and review added into the admin panel, where I can
--  take the results and have you analyze them daily, searching for any and
--  all flaws that need correcting, via schema, logic, GTO etc."
--
-- One row per day in horse_daily_audit: machine-computed FINDINGS (leak-tag
-- spikes vs the trailing week, worst horses, significant-negative league
-- layers, capture-coverage gaps, accounting outliers) plus an
-- agent_analysis column the daily Claude review writes its deep analysis
-- into. The engine's nightly job (HorseDailyAudit.ts, claim key
-- 'daily_audit' in horse_job_runs) calls fn_run_horse_daily_audit for the
-- completed previous day; the admin panel reads ca_horse_daily_audit.
-- ═══════════════════════════════════════════════════════════════════════════
...
-- No policies: service_role writes, admins read via the RPC below.

-- ───────────────────────────────────────────────────────────────────────────
-- The audit computation. Idempotent: re-running a day overwrites its row
-- (preserving any agent_analysis already written for it).
-- ───────────────────────────────────────────────────────────────────────────
...
begin
  ...
  -- ── FINDING: leak-tag spike vs the trailing 7 days ──
  for r in
    with day_tags as (
  ...
  -- ── FINDING: a horse bleeding hard in big pots ──
  for r in
  ...
  -- ── FINDING: league layer measuring SIGNIFICANT NEGATIVE ──
  for r in
  ...
  -- ── FINDING (schema): capture coverage gap ──
  -- A variant dealing big pots in hand_history but writing ZERO review rows
  -- means the capture path is blind to it.
  for r in
  ...
  -- ── FINDING (schema): rows missing their evidence payloads ──
  ...
  -- ── FINDING (schema/accounting): absurd nets ──
  for r in
  ...
  -- ── FINDING: losses carrying no tags at all (blind detectors) ──
  select count(*) into r from horse_hand_reviews
  where played_at >= p_day and played_at < p_day + 1
...
-- The daily Claude analysis lands here.
