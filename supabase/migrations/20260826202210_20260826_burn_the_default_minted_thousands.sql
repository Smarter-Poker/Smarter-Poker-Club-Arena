...
-- Applied to production 20260826202210; the .sql file was never committed at the
...
--
-- These are the balances club_members.chip_balance's old DEFAULT 1000 created
-- out of nothing. Nobody transferred them, nobody bought them, and no ledger
-- row explains them, which is exactly why they had to go: chip_balance is one
-- of the two pools fn_club_chip_circulation() counts and reconcile_ledger_
-- nightly compares against ledger movement.
--
-- This is a BURN, not a transfer. Chips that were never funded have no source
-- to return to; sending them to a club treasury would move the same unbacked
-- thousand somewhere else and call it settled.
--
-- Deliberately narrow. Only rows that are ALL of:
--   * exactly 1000.00 -- the default, to the cent
--   * zero hands played -- untouched since the row was created
--   * nothing held, locked or owed
...
-- reconciliation can see the money leave rather than find a gap.
--
-- ROLLBACK: read the rows back out of chip_transactions and re-credit them --
...
BEGIN
  ...
  ELSIF v_expected > 25 THEN
    ...
    -- the mint came back and this migration is the wrong tool.
