...
-- Applied to production 20260826204054; the .sql file was never committed at the
...
-- satellite winners, seated by fn_award_satellite_seat: the chain Dan asked
-- for is working. So the flagship was going to run the SHORT structure while
-- every future edition got the long one.
--
...
-- under a proof that it is an append: it aborts unless the first 20 levels of
-- the target match the first 20 of the replacement exactly.
...
BEGIN
  ...
  -- and not an extension, and it must not happen under registered players.
...
BEGIN
  ...
  IF v_bad > 0 THEN
    RAISE EXCEPTION '% pre-start Deep Stack(s) still on the short structure', v_bad;
  END IF;
  ...
  IF v_bad > 0 THEN
    RAISE EXCEPTION 'the level-12 rebuy blind changed - that is a rewrite, not an append';
  END IF;

  -- Still every level 10 minutes.
  SELECT count(*) INTO v_bad
  FROM public.tournaments t,
       LATERAL jsonb_array_elements(t.blind_structure::jsonb) lv
  WHERE t.name = 'Sunday $200 Deep Stack'
    AND t.status IN ('ANNOUNCED', 'REGISTERING')
...
-- ROLLBACK: restore the 20-level array from migration 20260826175613 onto the
-- schedule and re-run; the append proof above will then refuse, which is
-- correct - shortening a structure under registered players is a rewrite.
