...
-- is nowhere near over. What happens then is TournamentManagerBase
-- auto-escalation, which DOUBLES the last level's blinds every level
-- thereafter (16k, 32k, 64k, 128k...). That backstop is correct and stays - it
-- is what stops a structure running out entirely - but it is a 2.0x cliff
-- bolted onto a ramp that had been climbing at 1.20x-1.33x. A flagship deep
-- stack should not change character the moment it gets interesting.
--
...
-- Any realistic field now plays its endgame inside a hand-written structure,
-- and auto-escalation goes back to being a backstop rather than the referee.
--
...
BEGIN
  ...
  SELECT count(*) INTO v_bad FROM jsonb_array_elements(v_cfg->'blindStructure') lv
  ...
  FROM jsonb_array_elements(v_cfg->'blindStructure') lv WHERE (lv->>'level')::numeric = 12;
  IF 30000 / v_l12 < 25 THEN RAISE EXCEPTION 'a level-12 rebuy would be under 25 big blinds'; END IF;

  SELECT (lv->>'bigBlind')::numeric INTO v_last
  FROM jsonb_array_elements(v_cfg->'blindStructure') lv WHERE (lv->>'level')::numeric = 32;
  IF (100 * 30000) / v_last > 60 THEN
    RAISE EXCEPTION 'at the last defined level a 100-runner field still has %bb - too short', (100*30000)/v_last;
  END IF;
END $$;

-- ROLLBACK: restore the 20-level array from migration 20260826175613.
