...
-- Applied to production 20260826210835; the .sql file was never committed at the
...
--
-- public.pb_hands has ZERO rows and has never been written to. Its only caller,
-- Smarter-Poker-World-Hub/src/lib/poker-brain/storage.js:243 and :390, has been
-- failing and re-queueing offline for the life of the feature. It was broken in
-- two places at once:
--
...
-- lastDecision.action is an action label ('fold', 'raise', ...), so the column
-- is text and nullable, matching the sibling `decision` column.
--
-- Deliberately NOT added: `actionContext`, which HUD.jsx:884 passes to
-- storage.logHand() and which storage.js then drops on the floor -- it never
-- reaches an RPC argument. Wiring that through is a product decision about what
-- opponent-action summary to persist and in what shape. Left alone.
--
-- Dropping the 16-argument overload is safe: every parameter the 17-argument
-- version adds has a DEFAULT, so a caller passing the original 16 positional
-- arguments still resolves to it unchanged.
--
-- ROLLBACK:
...
-- Remove the ambiguity: keep one signature only.
...
BEGIN
  ...
  RAISE NOTICE 'pb_log_hand: exactly one overload, engine_suggestion column present, anon guard intact';
END $$;
