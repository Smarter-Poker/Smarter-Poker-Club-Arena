...
-- the payout path ignores is worse than none at all - it advertises money that
-- never arrives. `effectivePrizePool(prize_pool, guaranteed_prize)`
-- (server/src/tournament/startRules.ts) is `max()` of the two, and it is
-- applied at BOTH points where the pool becomes final: TournamentManagerBase
-- at late-registration close, and again at prize pool finalisation. So 20,000
-- is a floor the engine actually pays.
--
...
--   field cap            1,000
--
-- The guarantee is covered at 112 entries; every entry past that grows the
-- pool beyond it. Below that the club pays the difference:
--
...
--     13 satellites feeding this event are pool contributors, not a drain.
--
-- Five players are already registered (satellite winners). RAISING a guarantee
-- under registered players only ever increases what they play for, so unlike a
-- structure rewrite this is safe to apply to the live instance - and an
-- assertion below refuses to let this migration LOWER an existing guarantee,
-- which would not be.
...
DO $$
DECLARE
  v_cfg_gtd numeric; v_bad integer; v_entries numeric;
BEGIN
  ...
  -- A GUARANTEE MUST NEVER BE LOWERED UNDER REGISTERED PLAYERS.
  ...
  SELECT ceil(20000 / buy_in_amount) INTO v_entries FROM public.tournaments
...
-- ROLLBACK
--   UPDATE public.tournament_schedules SET config = config - 'guaranteedPrize'
--   WHERE name = 'Sunday $200 Deep Stack';
