...
-- Applied to production 20260826212740; the .sql file was never committed at the
...
--
-- ClubsService says, twice: "clubs.member_count is auto-synced by the
-- trg_sync_club_member_count trigger". There is no such trigger, and there
-- never was - `select count(*) from pg_trigger where tgname =
-- 'trg_sync_club_member_count'` returned 0. Nothing in fn_join_club touches the
-- column either. The only writer in the whole join path was one hand-rolled
-- `increment_member_count` call on InvitePage, so joining through the lobby
-- modal or the Discover tab never moved it, and NOTHING ever decremented it on
-- a leave.
--
...
--
-- Monotonically upward, exactly as a counter that is only ever incremented by
-- one of three join paths and never decremented would drift.
--
-- It is DERIVED from now on: recomputed from the rows, not adjusted by whoever
-- remembers to. A counter that is added to drifts; a counter that is recomputed
-- cannot. `increment_member_count` is left in place so the existing call site
-- keeps working, but it is now redundant rather than load-bearing.
--
-- Membership means status IN ('active','approved') - the same definition
-- getUserMemberships and the 4-club limit already use. A pending request is not
-- a member.
--
-- 2. THE COLUMN DEFAULT MINTED A DIFFERENT SHAPE OF CODE THAN THE CODE DOES.
--
-- clubs.club_id defaulted to (100000 + floor(random() * 900000))::integer - six
-- digits - while all three creation paths write Math.floor(10000 + random() *
-- 90000) - five. Every club that exists is five-digit. Any insert path that
-- omitted club_id would have minted a six-digit club, and the Discover tab's
-- join gate demanded exactly six until today, so the two shapes were each
-- unjoinable from one screen. Both lengths are accepted by the client now
-- (src/utils/clubCode.ts); this makes the default agree with what the product
-- actually generates.
--
-- ROLLBACK
...
BEGIN
  ...
  -- Recomputed, never incremented. See the header for the drift that the
  ...
  -- A member moving between clubs fires this once per row; the OLD club needs
  -- its own recount.
