...
--
-- The order in that second sentence is the whole design. The club settles up
-- FIRST, under its own control, cashing its players and agents down to zero.
-- The join is then REFUSED until the club is actually empty. Nothing here
-- reaches in and wipes a player's balance on the club's behalf -- a rule that
-- confiscates 48 million chips as a side effect of an admin clicking "join" is
-- not a rule, it is an accident waiting to be reported as theft.
--
-- WHAT ALREADY EXISTED, AND IS NOT DUPLICATED HERE
--
-- The "funded from the union only" half is already enforced, by Dan's own rule
-- of 2026-08-22: `fn_mint_club_chips` and `fn_mint_chips_from_diamonds` both
-- refuse a club whose `clubs.union_id` is set --
--
--   "Chip Mint is revoked for clubs in a union - chips are minted in the union
--    and sent to the club. Ask your union owner."
--
-- so once a club is in, `fn_union_send_to_club_atomic` is the only way chips
-- reach it. This migration adds the missing half: that it walks in at zero.
--
-- WHERE THE GUARD SITS
--
-- On `union_clubs` INSERT. That table is the authoritative membership record --
-- `trg_union_clubs_sync_mirror` is what copies it onto `clubs.union_id` -- so a
-- BEFORE INSERT trigger there catches every path into a union, including any
-- future admin screen nobody has written yet.
--
-- WHAT COUNTS AS "NOT EMPTY"
--
-- `fn_club_union_join_blockers` itemises it, and the refusal message names
-- every line so an owner knows exactly what to clear:
--
--   * the club treasury and its promo float
--   * member wallets -- players AND agents, held and locked chips included
--   * horse bankrolls, listed separately: they are the club's own stock, not a
--     person's money, but they are still chips the union would inherit
--   * agent float wallets (business / player / promo)
--   * chips sitting on the felt in an open seat
--   * the club's BBJ pool
--
-- FORWARD ONLY, on Dan's instruction. Club JAQK and SHARK CLUB are already in
-- Midway Union holding 1,051,817.71 and 1,476,644.87 in treasury; the trigger
-- fires on INSERT, so neither is touched and neither is re-validated.
--
-- ROLLBACK
--
--   DROP TRIGGER IF EXISTS trg_club_enters_a_union_empty ON public.union_clubs;
--   DROP FUNCTION IF EXISTS public.fn_enforce_club_enters_union_empty();
--   DROP FUNCTION IF EXISTS public.fn_club_union_join_blockers(uuid);
--   -- and for the creation grant:
--   -- restore the 10000 literals in fn_grant_first_club_bonus.
-- ═══════════════════════════════════════════════════════════════════════════════

-- ── The itemised list. Callable on its own, so a UI can show an owner what is
--    left to clear BEFORE they try to join and get refused. ──────────────────
...
BEGIN
  ...
  -- Member wallets. Horses are counted apart because they are the club's own
  -- stock rather than a person's money, but they are still chips.
...
DECLARE
  b jsonb;
BEGIN
  -- A union's own house club is not joining itself.
...
-- ── The creation grant: 10,000 -> 100,000, still to the founder ─────────────
-- Dan: "YOU CAN GIVE CLUBS 100,000 CHIPS NOT JUST 10,000. CLUBS THAT RUN THEIR
-- SELF CAN USE THE CHIPS INSIDE THERE ECO SYSTEM." A self-run club mints its
-- own chips anyway; the grant is a starting float, and it is only a problem
-- once the club is in a union, which the guard above now prevents.
...
BEGIN
  ...
  -- SHARK CLUB is not empty, so the blocker must say so. If this ever reports
  -- empty, the accounting below it has broken, not the club.
  ...
  IF (SELECT prosrc FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
       WHERE n.nspname='public' AND p.proname='fn_grant_first_club_bonus') LIKE '%10000)%' THEN
    RAISE EXCEPTION 'fn_grant_first_club_bonus still carries the old 10000 grant';
  END IF;
END $$;
