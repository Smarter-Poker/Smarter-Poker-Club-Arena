...
--
--  1. FUNDING A NEVER-FUNDED AGENT CRASHED (23502). fn_agent_wallet_send,
--     fn_club_bank_send and fn_promo_wallet_send all auto-created the
--     recipient's agents row WITHOUT commission_rate / player_rakeback_rate,
--     which are NOT NULL with no default. The first send to any agent who has
--     never held a float raised a raw not-null violation. fn_wallet_claim_back
--     learned this exact lesson on 2026-08-24; the three send paths did not.
--     Fixed with one helper, fn_ensure_agent_row, which mints the row at the
--     union's minimum permitted commission (trg_agents_commission_bounds
--     refuses anything below it) or 0 for a standalone club.
--
--  2. APPROVING A CHIP REQUEST COULD DESTROY CHIPS. If the requester had left
--     the club, fn_respond_chip_request debited the approver, credited nobody
--     (the UPDATE matched no row), wrote a ledger row with a NULL balance and
--     reported success. Probe: approver 500.00 -> 480.00, credit NULL.
--     Approval now delegates to fn_agent_wallet_send, which refuses a
--     recipient who is not an active member BEFORE any money moves — and, per
--     Dan 2026-08-25 ("any chips sent or claimed back transact from the Agent
--     Wallet"), the approval now spends the approver's AGENT WALLET rather
--     than their personal playing balance, with the same idempotency, ledger
--     row and downline guard as every other send.
--
--  3. CANCELLING A TICKET COULD DESTROY CHIPS. fn_cancel_tournament_ticket
--     marked the ticket cancelled and reported success even when the escrow
--     refund landed nowhere (issuer no longer a member). Probe: success=true,
--     refund balance NULL, ticket cancelled, 25 chips gone. It now refuses,
--     leaving the ticket issued, exactly as fn_redeem already did.
--
--  4. THE LEGACY MEMBERSHIP STATUS LOCKED MOST MEMBERS OUT. Rows created
--     before 2026-07-22 say status='approved' (~98% of production);
--     fn_request_chips, fn_respond_chip_request and fn_wallet_claim_back all
--     filtered on status='active' alone, so an 'approved' member could not
--     request chips and an 'approved' approver could not answer. Probe:
--     "you are not an active member of this club". Every membership test now
--     uses the same set the rest of the platform uses:
--     coalesce(status,'active') in ('active','approved').
--
--  5. SEND TICKET DISAGREED WITH ITS OWN ROSTER. The cashier offers the whole
--     recursive downline (fn_club_cashier_members), but
--     fn_issue_tournament_ticket only accepted DIRECT assignees, so a super
--     agent picking a depth-2 player collected "recipient is not in your
--     downline" for a target the grid offered. It now uses
--     fn_club_cashier_can_transact — the SAME edge the roster is built from.
--
--  6. PROMO SEND HAD NO DOWNLINE RULE AND REFUSED STAFF FLOATS. Any agent
--     could promo-send to ANY member of the club (probe: success), and an
--     owner / co-owner / admin could not be sent promo float at all, though
--     the wallet law says staff hold promo wallets and the roster offers
--     them. Both aligned with fn_agent_wallet_send.
--
--  7. TICKET REDEMPTION WAS INVISIBLE TO THE CLUB LEDGER. fn_redeem wrote
--     only wallet_transactions, so the club's chip history showed the escrow
--     leaving on issue and nothing ever arriving. It now writes the matching
--     chip_transactions row.
--
--  8. REPLAY LOOKUPS WERE NOT PINNED TO THE CALLER. The op_id idempotency
--     pre-checks matched on (club, type, op_id) alone, so a caller replaying
--     SOMEBODY ELSE'S op_id (readable off a chip_transactions row they were
--     party to) was handed a confident "success, replayed" receipt for money
--     they never sent. Every replay lookup now also matches the caller, and
--     the unique-violation handlers re-raise when the winning row is not the
--     caller's own.
--
--  9. AMOUNT SCALE GUARDS were missing from fn_request_chips and
--     fn_issue_tournament_ticket (a 10.005 request could be approved as a
--     rounded debit against an unrounded credit). Both now refuse sub-cent
--     amounts, matching the three send RPCs.
--
-- ROLLBACK: every statement is CREATE OR REPLACE FUNCTION or a grant change;
-- restore the previous definitions from the 2026-08-25 migrations
-- (20260825_agent_wallet_cashier, 20260821_cashier_requests_tickets,
-- 20260824_club_bank_cashier) to revert.
-- ═══════════════════════════════════════════════════════════════════════════
...
begin
  ...
  -- commission_rate and player_rakeback_rate are NOT NULL with no default,
  -- and trg_agents_commission_bounds refuses a rate outside the union policy
  -- band, so the row is minted at the union's minimum permitted commission —
  -- never a fabricated one, and never a bare insert that 23502s.
...
-- Internal plumbing for the money RPCs, never a client entry point.
...
begin
  ...
  -- A replay is only a replay when it is THIS caller's earlier attempt.
  -- Matching on op_id alone handed anyone who could read a transaction row
  -- (the recipient can) a confident receipt for money they never sent.
  ...
  -- Two decimal places is the coarser of the two stores either side of this
  -- transfer; anything finer is silently destroyed by column scale rounding.
  ...
  -- Derived from the recipient, never trusted from the caller.
  ...
  if v_dest = 'player_wallet' then
    ...
    -- THE 23502 FIX. The bare insert here omitted commission_rate and
    -- player_rakeback_rate (NOT NULL, no default), so the FIRST send to any
    -- agent who had never held a float crashed with a raw not-null violation.
...
exception
  when unique_violation then
    ...
    if v_prior.id is null then
      -- The winning row is not this caller's. That is a stolen op_id, not a
      -- replay; refuse rather than hand out someone else's receipt.
      raise;
    end if;
    return jsonb_build_object(
      'success', true, 'replayed', true,
...
begin
  ...
  if v_dest = 'player_wallet' then
    ...
    -- THE 23502 FIX (see fn_ensure_agent_row).
...
begin
  ...
  -- THE SAME DOWNLINE EDGE AS EVERY OTHER SEND (2026-08-26). Promo money had
  -- no scope rule at all: any agent could promo-send to any member of the
  -- club, which contradicts "agents only ever see their downlines". Staff
  ...
  -- Staff hold promo wallets too (wallet visibility law, 2026-08-23); the old
  -- agent-tier-only set refused an owner or admin the roster had offered.
  if v_dest = 'agent_wallet'
     and v_to_role not in ('owner', 'co_owner', 'admin', 'super_agent', 'agent', 'sub_agent') then
    return jsonb_build_object('success', false,
  ...
  if v_dest = 'player_wallet' then
    ...
    -- AS CASH — they credit chip_balance, spendable in any game or tournament.
    update club_members
       set chip_balance = coalesce(chip_balance, 0) + p_amount,
           updated_at = now()
     where club_id = p_club_id and user_id = p_to_user_id
...
begin
  ...
  -- BOTH membership words. status='active' alone refused the ~98% of
  -- production rows that still carry the pre-2026-07-22 'approved'.
...
begin
  ...
  -- fn_club_bank_role, not a raw club_members read: it resolves the club
  -- OWNER even without a membership row, and accepts both membership words.
  ...
  -- APPROVE IS A SEND OUT, AND SPENDS WHAT A SEND OUT SPENDS (2026-08-26).
  --
  -- The old body moved club_members.chip_balance from the approver — the
  -- personal playing balance, not the agent wallet the cashier law names —
  -- and if the requester had LEFT the club it debited the approver, credited
  -- nobody, and reported success: chips destroyed, invisibly.
  --
  -- fn_agent_wallet_send already holds every rule an approval needs: the role
  -- gate, the downline edge, the funded-float check, the membership check
  -- that refuses a vanished requester BEFORE money moves, the hundredths
  -- guard, one ledger row, and idempotency. Delegating is the fix — two code
  -- paths for the same conserved move is how they drift apart again.
...
begin
  ...
  -- THE ROSTER'S EDGE, NOT A PRIVATE ONE (2026-08-26). The cashier offers the
  -- whole recursive downline; the direct-parent test here refused a depth-2
  -- player the grid had offered. fn_club_cashier_can_transact is the SAME
  -- walk fn_agent_wallet_send refuses on, so list and refusal cannot disagree.
...
begin
  ...
  -- THE GUARD fn_redeem ALWAYS HAD AND THIS FUNCTION DID NOT (2026-08-26).
  -- With no membership row the UPDATE above matches nothing, v_after stays
  -- NULL, and the old body cancelled the ticket anyway and reported success —
  -- the escrowed chips left the ticket and landed nowhere. Refuse instead;
  -- the ticket stays issued and the holder can still redeem it.
...
begin
  ...
  -- BOTH ledgers (2026-08-26). Only wallet_transactions was written before,
  -- so the club's own chip history showed the escrow leaving on issue and
  -- nothing ever arriving anywhere.
  insert into chip_transactions (club_id, from_user_id, to_user_id, amount, transaction_type, notes, balance_after)
...
-- Only the two defective clauses change; the body is otherwise the 2026-08-24
-- version verbatim.
...
begin
  ...
  -- BOTH membership words (2026-08-26): status='active' alone refused the
...
exception
  when unique_violation then
    -- THE HANDLER THE 2026-08-24 COMMENT PROMISED AND NEVER HAD. Two
    -- concurrent identical submissions both passed the pre-check; the loser
    -- hit ux_chip_transactions_idempotency_key and surfaced a raw error
    -- (money-safe — its writes rolled back — but the client retried blind).
