...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'club_chat SELECT policy "Members can read club chat" is missing — refusing to guess at the shape';
  END IF;
END
$guard$;
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'club_chat INSERT policy "Members can insert club chat" is missing — refusing to guess at the shape';
  END IF;
END
$guard2$;
...
-- PART 3: Verify both policies landed correctly
-- ─────────────────────────────────────────────────────────────────────────
...
-- PART 4: Add table_seats to realtime publication
-- ─────────────────────────────────────────────────────────────────────────
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'table_seats is still NOT in the supabase_realtime publication after ALTER — something is wrong';
  END IF;
