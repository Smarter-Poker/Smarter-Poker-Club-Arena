...
-- One that writes without ever consulting auth.uid() acts on whatever arguments
-- it is handed, for any logged-in caller. All 15 are maintenance and rollup jobs
-- meant to be driven by cron or the engine under service_role -- not things a
-- browser should be able to fire.
--
...
--
-- VERIFIED AFTER APPLY, over PostgREST with the publishable key:
--   POST /rpc/verify_home_games_invariants -> HTTP 401  42501 permission denied
--   POST /rpc/fn_refresh_player_stats      -> HTTP 401  42501 permission denied
--   POST /rpc/check_username_available     -> HTTP 200  (control, still works)
-- Blind writers among authenticated SECURITY DEFINER functions: 15 -> 3.
--
-- THREE ARE DELIBERATELY LEFT ALONE -- each has a live caller that would break:
--
--   get_current_settlement_period()
--     Called from the browser: club-arena/src/services/SettlementService.ts:122
--     via supabase.rpc(), reached by SettlementDashboardPage, SettlementPage,
--     UnionDashboardPage and useUnionStore. It DOES insert into
--     settlement_periods with no identity check, so any authenticated user can
--     create a period. Worth fixing -- but the fix is deciding WHO may create
--     one, a product call, not one to make blind from the database side.
--
--   reserve_clip(...), reserve_sports_clip(...)
--     World-Hub src/services/{Clip,SportsClip}DeduplicationService.js build a
--     client as SUPABASE_SERVICE_ROLE_KEY || NEXT_PUBLIC_SUPABASE_ANON_KEY, so
--     with no service-role key present they run as anon/authenticated. A revoke
--     would break them in exactly the environment where that fallback is
--     load-bearing. The fallback is the latent bug; fix it first, then revoke.
--
-- NOTE ON ca_refresh_hand_player_index: its browser call was removed 2026-08-24
-- after pg_stat_statements measured a 73,901 ms MEAN and it was caught live at
-- 26.9s in IO/DataFileRead dragging the 10GB hand_history table off disk.
-- tests/no-maintenance-from-pageview.test.ts guards that at source level; this
-- revoke enforces the same rule at the database, where a source test cannot
-- reach.
--
...
DO $$
DECLARE v_still int; v_kept int; v_svc int; v_list text[] := ARRAY[
...
BEGIN
  SELECT count(*) INTO v_still FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
   WHERE n.nspname='public' AND p.oid::regprocedure::text = ANY(v_list)
  ...
  SELECT count(*) INTO v_svc FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
   WHERE n.nspname='public' AND p.oid::regprocedure::text = ANY(v_list)
