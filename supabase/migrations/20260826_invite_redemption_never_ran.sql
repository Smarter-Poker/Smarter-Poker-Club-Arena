...
BEGIN
  -- ── Authorization ────────────────────────────────────────────────────────
  -- A logged-in caller may only redeem for themselves. service_role has no
  -- auth.uid() and is allowed through for server-side backfills.
  ...
  -- player_number is TEXT. Match it as text, then fall back to a numeric
  -- comparison so a code with different leading-zero padding still resolves.
  ...
  -- ── 3. Effective upline: the inviter if they are an agent, else theirs ────
  ...
  -- A banned or removed member does not walk back in through an invite link.
  ...
  -- ── 5. Attach and admit ──────────────────────────────────────────────────
  -- agent_id is only filled when empty: a player already in a downline is not
  -- silently moved by someone else's link, and an inviter with no upline does
  -- not blank out an existing one.
  -- status is only promoted out of 'pending': an invite link admits an invited
  -- player, it does not change anything about a member who is already in.
...
COMMENT ON FUNCTION public.fn_redeem_club_invite_code(uuid, uuid, text) IS
  'Redeems a club invite/referral code for the CALLING user: attaches the upline '
  'agent (the inviter if they are an active agent, otherwise the inviter''s own '
  'agent), records invited_by, and promotes a pending membership to active. '
  'Never overwrites an existing agent_id and never changes a settled status. '
  'Fixed 2026-08-26: the previous body compared text player_number to an integer '
  'and therefore raised 42883 on every call since it was written.';

-- Post-apply assertions: the exact expression that used to fail must now plan,
-- and the function must refuse to redeem on someone else's behalf.
DO $$
DECLARE
  v_ok boolean;
BEGIN
