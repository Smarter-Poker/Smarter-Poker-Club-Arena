-- TIER 1. Comments only. Nothing dropped, nothing read differently.
-- Applied to production 2026-08-26.
--
-- EIGHT one-off backup tables sit in public, ~16 MB together, and
-- pg_stat_user_tables reports seq_scan = 0 AND idx_scan = 0 for every one.
-- Nothing has ever read any of them.
--
-- They are NOT dropped here, deliberately. Dropping is irreversible, one was
-- created TODAY by another agent mid-flight, and 16 MB does not justify an
-- agent guessing which migration each still insures. What they lacked is not
-- deletion, it is a stated expiry -- so the next sweep acts on a date instead
-- of a guess:
--
