...
-- functions. No function body is changed. Applied to production 2026-08-26.
--
-- PART A -- union_rake_weekly had no primary key. Verified safe before adding:
-- 17 rows, 17 distinct (union_id, club_id, week_start), 0 NULLs in any key
-- column. The other three no_primary_key findings are all backup tables and are
-- deliberately left alone.
--
-- PART B -- ten functions ran with a mutable search_path.
-- Worth stating plainly: NONE of these ten is SECURITY DEFINER (prosecdef
-- checked for every one). A mutable search_path on a SECURITY INVOKER function
-- is a correctness and lint issue, not the privilege-escalation vector it would
-- be on a definer function -- lower severity than the advisor WARN suggests.
--
-- Pinning to 'public' alone is safe here because none of the ten references an
-- extension-schema object (each body checked for gen_random_uuid, pgcrypto,
-- unaccent, similarity, citext and friends -- all clean). Functions elsewhere
-- that DO need them are pinned 'public, extensions'; do not copy the bare
-- 'public' below onto one of those.
--
-- ROLLBACK:
...
BEGIN
  ...
  END IF;
  SELECT count(*) INTO v_unpinned FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
  ...
  IF v_unpinned <> 0 THEN
    RAISE EXCEPTION 'assertion failed: % of the ten still has a mutable search_path', v_unpinned;
  END IF;
