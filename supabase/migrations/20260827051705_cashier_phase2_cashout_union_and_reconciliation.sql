...
-- Applied to production 20260827051705; the .sql file was never committed at the
-- time (see docs/changelog and issue: unrecorded-migration backfill). Content is
-- byte-exact to what ran. Do NOT re-apply; it is already live.

-- ═══════════════════════════════════════════════════════════════════════════
-- CASHIER PHASE 2 — CASH OUT LIFECYCLE, UNION SENDS, RECONCILIATION (2026-08-27)
-- Full narrative in the repo migration file of the same name and the phase-2
-- section of .agent/audits/2026-08-26-cashier-send-out-claim-back-audit.md.
--  1. fn_cashout_approve: 23502 on a never-funded approver -> fn_ensure_agent_row.
--  2. fn_expire_stale_cashouts could burn escrow when the player had left ->
--     recreates the membership row like fn_cashout_release.
--  3. fn_cashout_request: hundredths guard + 1e9 cap.
--  4. Caller-pinned op_id replays in request / approve / release.
--  5. fn_union_send_to_member: chips credited via home-club resolution that
--     could leave the union; promo credited the FROZEN public.wallets pool;
--     ledger rows violated the clubs FK. All rewired in-union; anon revoked.
--     (Zero rows of either tx type exist — purely preventive.)
--  6. fn_union_distribute_promo (agent) credited club_members.promo_balance,
--     which nothing reads -> agents.promo_wallet_balance via fn_ensure_agent_row.
--  7. reconcile_ledger_nightly: cashier integrity section (stuck escrow,
--     negative balances, over-claimed sends) filed as critical.
