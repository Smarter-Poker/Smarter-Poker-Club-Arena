...
-- Applied to production 20260827051853; the .sql file was never committed at the
...
-- (probed 2026-08-27: fn_cashout_approve by an owner without an agents row
-- raised 23514). The membership row stays the source of truth for the real
-- role; agents.role is float bookkeeping, so staff are stored at the closest
-- permitted tier rather than widening a constraint every commission and
-- rakeback query was written against.
