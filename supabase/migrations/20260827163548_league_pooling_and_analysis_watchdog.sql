...
--
-- TWO CONSEQUENCES OF YESTERDAY'S FIXES, HANDLED BEFORE THEY BITE:
--
-- 1. The card now ROTATES, so a given matchup is measured every few nights
--    rather than nightly, at 4000 pairs (stderr ~4.5 bb/100). Two problems
--    follow: a single night rarely resolves a real edge, and the daily
--    audit only inspected run_date = max(run_date) - so a layer measured
--    three nights ago could resolve significantly NEGATIVE and never be
--    seen again. fn_league_pooled combines every run of a matchup inside a
--    window by INVERSE-VARIANCE weighting (the correct pooling for
--    independent estimates of the same quantity: each night is weighted by
--    1/stderr^2, and the pooled stderr is 1/sqrt(sum of weights)). Three
--    nights at 4000 pairs pool to roughly the precision one night at 12000
--    would have had - the rotation costs coverage latency, not power.
--
-- 2. The 5 AM Claude analysis fired on 2026-08-27 and wrote nothing to
--    horse_daily_audit.agent_analysis. Nothing noticed. That is the same
--    class of failure as the audit job that never ran, one layer up, so it
--    gets the same treatment: a finding that fires when the analysis is
--    missing or stale for a day whose data is complete.
-- ═══════════════════════════════════════════════════════════════════════════
...
-- Pooled league findings: a negative layer is caught no matter WHICH night
-- it was measured, and only claimed once the pooled sample resolves it.
...
-- The watchdog on the 5 AM analysis itself.
...
begin
  ...
  -- Only judge a day whose analysis window has fully passed (the 5 AM local
  -- run lands ~10:00-11:00 UTC; give it until 14:00 UTC the following day).
...
-- Fold both into the composite the daily audit already calls.
