...
-- Applied to production 20260827174656; the .sql file was never committed at the
-- time (see docs/changelog and issue: unrecorded-migration backfill). Content is
-- byte-exact to what ran. Do NOT re-apply; it is already live.

-- OVERLAY IN THE DAILY P&L — 2026-08-27. An overlay is a bank EXPENSE (chips
-- leaving the union, or the standalone club's own bank, to honour a
-- guarantee), so it belongs beside rake, BBJ and insurance in the same
-- reports. Source of truth is tournament_overlay_funding.
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    -- OVERLAY 2026-08-27: guarantees this union's bank covered.
    'overlay', (
      SELECT jsonb_build_object(
        'events', coalesce(count(*), 0),
        'funded', round(coalesce(sum(f.overlay_amount), 0), 2),
        'underfunded_events', coalesce(count(*) FILTER (WHERE f.underfunded), 0))
      FROM tournament_overlay_funding f
      WHERE f.bank_type = 'union' AND f.bank_entity_id = p_union_id
        AND f.created_at >= v_from::timestamptz
    ),
    'daily', coalesce((
      ...
      LEFT JOIN (
        SELECT (f.created_at AT TIME ZONE 'UTC')::date AS d,
               round(sum(f.overlay_amount), 2) AS funded
          FROM tournament_overlay_funding f
         WHERE f.bank_type = 'union' AND f.bank_entity_id = p_union_id
           AND f.created_at >= v_from::timestamptz
         GROUP BY 1
...
-- Club daily revenue gains the same line, attributed to the bank that paid.
  ...
  SELECT jsonb_build_object(
    ...
    'totals', (
      SELECT jsonb_build_object(
        'events', coalesce(count(*), 0),
        'funded', round(coalesce(sum(f.overlay_amount), 0), 2),
        'bank', coalesce(max(f.bank_type), (SELECT CASE WHEN c.union_id IS NULL THEN 'club' ELSE 'union' END
                                              FROM clubs c WHERE c.id = p_club_id)),
        'underfunded_events', coalesce(count(*) FILTER (WHERE f.underfunded), 0))
      FROM tournament_overlay_funding f
      WHERE f.club_id = p_club_id AND f.created_at >= v_from::timestamptz
    ),
    'events', coalesce((
      SELECT jsonb_agg(jsonb_build_object(
               ...
               'pool_before', f.pool_before,
               'overlay', f.overlay_amount,
               'bank', f.bank_type,
               'underfunded', f.underfunded,
               'at', f.created_at)
