...
-- Applied to production 20260827174657; the .sql file was never committed at the
-- time (see docs/changelog and issue: unrecorded-migration backfill). Content is
-- byte-exact to what ran. Do NOT re-apply; it is already live.

-- FREEROLL FILL (Dan 2026-08-27): "all horses, if they are playing, should
-- play the freeroll - all real players would."
--
-- The overlay guard's first cut EXCLUDED freerolls on the reasoning that
-- entrants cannot fund a club-funded prize. True, and beside the point: a
-- freeroll at 4 of 50 reads as a dead room to the human deciding whether this
-- site has players. Measured when this was raised: $100 Freeroll 24/500,
-- Coffee Break Freeroll 4/50.
--
-- Returns open freerolls that still have room. The engine fills them from
-- EVERY lane (free money is not a cash-versus-events personality), gated on
-- each horse being inside its activity window.
...
-- Audit: a freeroll that started nearly empty is a visible-quality failure,
-- not an accounting one - so it is reported separately from overlays.
