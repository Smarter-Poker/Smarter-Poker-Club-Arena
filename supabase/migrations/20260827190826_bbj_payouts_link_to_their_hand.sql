...
-- Applied to production 20260827190826; the .sql file was never committed at the
...
-- ============================================================================
--
--  `bbj_atomic_payout_v2` inserts `hand_id` as a hardcoded NULL, and always
--  has. The only link between a jackpot and the hand that won it is
...
--  Two changes:
--
--  1. RESOLVE IT ON WRITE. hand_history is written before the payout
--     (ServerTableEngineSettlement.ts:1009 vs :1407), so the row is there to
--     be found. If it is not — `runStep` continues after a failed step by
--     design — hand_id stays NULL and nothing is worse than before.
--
--  2. BACKFILL what is still resolvable.
--
--  This does NOT make the pair redundant. The join stays as the fallback for
--  every row written before today, and `fn_bbj_hand_detail` is unchanged. What
--  it buys is a real reference: an id that a foreign key could one day enforce,
--  and a way to tell "the hand was never written" apart from "the hand was
--  written and later pruned", which today are indistinguishable.
--
--  Deliberately NOT a foreign key yet. `hand_history` rows are pruned, and an
--  ON DELETE RESTRICT would make the pruner fail on a jackpot hand while
--  ON DELETE SET NULL would quietly erase the very evidence this adds. The
--  prune exemption (20260827d) is what keeps them alive; a constraint on top
--  of it can come once that has run for a while without incident.
-- ============================================================================
...
-- which is now a readable fact rather than the default for every row.
...
DECLARE
  ...
  v_orphan integer;
BEGIN
  SELECT count(*) FILTER (WHERE hand_id IS NOT NULL),
         count(*) FILTER (WHERE hand_id IS NULL)
    INTO v_linked, v_orphan
    FROM public.bbj_payouts;

  -- Every payout whose hand we still hold must now be linked. If one is not,
  -- the backfill predicate does not match the join fn_bbj_hand_detail uses and
  -- the two would disagree about which hand belongs to which payout.
  IF EXISTS (
  ...
  RAISE NOTICE 'bbj_payouts linked=% orphaned=%', v_linked, v_orphan;
END $$;
