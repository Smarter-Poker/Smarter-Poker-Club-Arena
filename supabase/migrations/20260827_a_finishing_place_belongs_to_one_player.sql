...
-- by two players and both collected. Population at the time of this migration:
-- 206 duplicate (tournament_id, position) rows across 138 tournaments.
--
-- A guard for this already existed and it did not guard. trg_tournament_place_
-- collision detected the collision perfectly and then INSERTed a log row and
-- returned NEW - it watched the money leave. The table it writes to,
-- tournament_place_collisions, had ZERO rows, because the trigger post-dates
-- every collision it was built for. It had never even observed one.
--
-- Detection without enforcement is not a guard. It raises now.
--
...
-- read before a rebuy filter removes somebody from the busted set. The
-- elimination write is already a CAS with an explicit failure path -
-- 'Tournament.elimination_write_failed' reports loudly and the comment there
-- says "the next sweep sees the player still 'playing' with 0 chips and
-- retries". That retry re-reads the field and produces a distinct place. So the
-- failure mode of raising is one logged retry; the failure mode of logging is
-- paying a prize twice out of a pool that does not contain it.
--
...
BEGIN
  ...
  IF v_existing IS NOT NULL THEN
    -- The forensic row rolls back with the statement; that is acceptable,
    -- because the RAISE below carries both user ids and the elimination path
    -- reports it as elimination_write_failed - a louder signal than a row in a
