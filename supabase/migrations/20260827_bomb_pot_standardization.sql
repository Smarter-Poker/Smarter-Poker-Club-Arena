...
--                                every normal hand.
--
-- Tier 2: additive columns only, all nullable or defaulted. No backfill
-- besides the board-count/double-board reconciliation; no RLS change
-- (columns inherit the row policies of their tables).
-- ═══════════════════════════════════════════════════════════════════════════
...
-- Value guards. Named constraints inside a DO block because Postgres has no
...
-- Reconcile the legacy boolean with the canonical count: every table that
-- opted into double-board bomb pots is a 2-board table.
...
-- Post-apply assertions: every column exists with the expected type, and no
-- row violates the board-count/double-board reconciliation.
DO $$
BEGIN
  IF NOT EXISTS (
