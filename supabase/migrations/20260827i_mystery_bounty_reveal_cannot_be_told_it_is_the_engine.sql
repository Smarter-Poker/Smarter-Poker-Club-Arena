...
-- "harden_mystery_bounty_reveal_authorization"). This file is the record of
-- why, per CLAUDE.md.
--
-- THE HOLE. The authorization line read:
--
...
-- caller-supplied too, so even without p_auto an attacker could simply pass
-- the designated revealer's id and be believed.
--
-- PROVEN, not assumed. Inside a transaction that rolled itself back, as an
-- ordinary authenticated user who was NOT the designated revealer:
--
--   PROBE1 ok=true amount_cents=1234500 tier=jackpot
--          award_status=revealed chest_status=revealed
--
-- AFTER THIS MIGRATION, same probe, same fixture:
--
--   A(attacker, p_auto => true)          = {"ok": false, "reason": "not_the_revealer"}
--   B(attacker, impersonating revealer)  = {"ok": false, "reason": "not_the_revealer"}
--   award status after both attacks      = reserved
--   C(the real revealer's own tap)       = ok=true amount=1234500 status=revealed
--   D(the engine, service_role, p_auto)  = ok=true amount=1234500 status=revealed
--
-- THE FIX. Trust for these two inputs now comes from the request, not from the
-- argument list:
...
-- earlier guard on club_members a silent no-op (20260827g_close_browser_chip_mint).
-- auth.role() reads the verified request JWT claim, which SECURITY DEFINER
-- does not rewrite.
--
-- WHY COALESCE(auth.role(), 'service_role'). NULL means there is no PostgREST
...
-- produce NULL here, because reaching the `authenticated` role requires a
-- verified JWT and PostgREST always sets request.jwt.claims from it. `anon`
-- holds no EXECUTE on this function.
--
-- CALLERS, UNCHANGED:
--   src/pages/TablePage.tsx:1171                     p_auto: false, actor: userId
--   server/src/tournament/TournamentManagerEliminations.ts:1892
--                                                    p_auto: true (service key)
