...
-- Applied to production 20260828013431; the .sql file was never committed at the
...
-- which nobody re-checked, and it has been running there ever since:
--
--     v_roll := random() * 100;
...
-- last two weeks carries more head than pool. Measured today:
--
...
--
-- Those events structurally cannot pay their own heads. The 13x tail is the
...
-- already do on the paths they own. The mystery value is drawn from the funded
-- inventory, where it belongs.
--
-- SAFE TO APPLY NOW: no live mystery tournament currently has an over-funded
-- head (2 REGISTERING, mystery_bounty_stage 'pending', 0 rows over). This is a
-- pure prevention change; finished events are left exactly as they are, because
-- rewriting the recorded heads of a settled tournament would be inventing
-- history rather than fixing code.
--
-- NOT CHANGED, ON PURPOSE: the `bounty_pool` increment below. It is the ONLY
-- funding for entrants who arrive through a path other than the two register
-- RPCs (satellite seat awards, ticket redemptions) - the register RPCs pre-set
-- current_bounty, so the guard at the top of this function returns early for
-- them and there is no double count. Whether a free seat should fund a bounty
-- pool at all is a pricing question for Dan, not a bug an agent gets to decide
