...
-- "reconcile_pays_by_a_structure_the_field_can_fill"). This file is the record
...
--
-- BEFORE, on Sunday Midway Major:
--
--   issues: [{ issue: no_finisher_recorded, place: 9, expected: 250.00 }]
--   total_top_up: 0.00   total_paid_to_known_holders: 9750.00
--
-- AFTER:
--
--   issues: []   field_size: 8   paid_places: 8
--   total_top_up: 250.00, spread across all eight finishers
--   (place 1 expected 3076.92, place 8 expected 358.99, sum 10000.00)
--
...
-- ── AND STATE THE GRANTS, BECAUSE SILENCE IS NOT SAFETY ────────────────────
--
-- This function PAYS CHIPS when p_apply is true. Production has never granted
-- it to a browser role, but the migration that defined it said nothing about
-- grants at all, and a rebuild from this repository would therefore create it
-- with the Postgres default of EXECUTE to PUBLIC, which every browser role
-- inherits.
--
-- scripts/ci/check-definer-authorization.mjs refused this very migration for
-- that reason, on the push that was fixing the short-field residual. Applied
-- separately as "payout_reconcile_states_its_own_grants" and recorded here.
