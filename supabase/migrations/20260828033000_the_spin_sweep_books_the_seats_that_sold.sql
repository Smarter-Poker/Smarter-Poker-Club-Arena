...
-- "spin_sweep_seats_sold_with_cast_and_visible_failures"). This file is the
-- record of why, per CLAUDE.md.
--
-- ─── THE DEFECT ─────────────────────────────────────────────────────────────
--
...
-- drained the way a finished Spin drains it, inside a transaction that rolled
-- itself back:
--
...
-- number.
--
-- AFTER, same Spin, both drained states:
--
--   REPROBE5 A(current_players=1) seats=3 collected=30.00 rake=2.10
--            B(current_players=0) seats=3
--            failed=0, failures=[]
--
-- ─── THE FIRST ATTEMPT WAS WRONG, AND SILENTLY ──────────────────────────────
--
-- Worth recording because the shape repeats. The first rewrite passed
-- count(*) - which is bigint - to fn_spin_settle_game(p_seats integer). There
-- is no implicit bigint parameter, so every call failed function resolution,
-- `EXCEPTION WHEN OTHERS` swallowed it, and the sweep reported
-- {"settled": 0, "failed": 1} with no reason attached. The code read as
-- correct. Only re-running the probe caught it.
--
-- So this ships two things: the ::int cast, and the removal of the silence that
-- hid the need for it. Failures now carry their SQLSTATE and message.
--
-- ─── LIVE EXPOSURE, AND WHY IT IS SMALL FOR A BAD REASON ───────────────────
--
-- spin_reserve_ledger holds 2 rows booked at seats=2 whose events really had 3,
-- against 47,608 correct rows at seats=3: about 2.00 of collected and 0.16 of
-- rake. The incidence is low because the old WHERE clause ALSO required
...
-- question - did anybody actually enter.
--
...
-- duplicate-place work is that a retrospective charge is a decision, not a side
-- effect of a repair.
--
...
BEGIN
  ...
  FOR v_t IN
    SELECT t.id, t.club_id, t.buy_in_amount, t.spin_multiplier,
           ...
           -- The ::int is load-bearing: count(*) is bigint, and
           -- fn_spin_settle_game(p_seats integer) will not resolve against it.
           (SELECT count(*)::int FROM public.tournament_players tp
  ...
  LOOP
    ...
    BEGIN
      ...
      END IF;
    EXCEPTION WHEN OTHERS THEN
      -- A bare counter told nobody WHY, and that silence is what let a wrong
      -- parameter type read as a working sweep for the length of one probe.
      v_fail := v_fail + 1;
      v_failures := v_failures || jsonb_build_object(
...
COMMENT ON FUNCTION public.fn_spin_sweep_unbooked(integer) IS
  'Books any Spin that finished without a spin_reserve_ledger row, settling on the seats that were SOLD (rows in tournament_players) rather than on current_players, which drains as players bust. Reports why each failure failed. Fixed 2026-08-27.';
