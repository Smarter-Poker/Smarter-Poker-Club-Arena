...
-- Applied to production 20260828033311; the .sql file was never committed at the
...
-- ═══════════════════════════════════════════════════════════════════════════
--
-- fn_unregister_from_tournament refunded through credit_player_wallet, which
-- calls fn_credit_player_wallet_once, which moves chips and writes NO
-- wallet_transactions row. The registration's `tournament_buyin` debit therefore
-- survived with nothing offsetting it.
--
...
-- is callable by any club admin on a RUNNING event, and the wallet key dedupes
-- a repeated USER, not a repeated CHARGE, so nothing downstream caught it.
--
-- PROVEN, not assumed. register, unregister, register as one funded member, in
-- a transaction that rolled itself back:
--
...
--
-- The player is out 5.00 and the cancel would have paid them 10.00.
--
-- AFTER THIS MIGRATION, same probe, same event:
--
--   REPROBE4 ledger rows=3 | cancel would refund 5.00 | balance 5000.00->4995.00
--
-- THE FIX. Refund through fn_credit_and_log, which is the same helper
-- atomic_cancel_tournament itself uses: it credits once, and only if the credit
-- actually happened does it write the matching `refund` row against the
-- tournament. The money movement is unchanged; what changes is that the ledger
-- now says so.
--
-- THE IDEMPOTENCY KEY IS DELIBERATELY UNCHANGED. It stays
-- 'tourn_unreg:<registration_id>', so any unregistration already refunded under
-- the old code cannot be paid a second time by the new code, and the club
-- resolution inside fn_credit_player_wallet_once behaves exactly as before. A
-- new key would have been a second bug wearing the first one's clothes.
--
-- NO BACKFILL IS NEEDED AND NONE IS DONE. wallet_credit_idempotency holds zero
-- keys matching 'tourn_unreg:%': the path was open and never taken. There is no
-- historical over-refund to unwind.
...
BEGIN
  ...
  IF v_amount > 0 THEN
    ...
    -- cancel pay for a registration that had already been refunded here.
