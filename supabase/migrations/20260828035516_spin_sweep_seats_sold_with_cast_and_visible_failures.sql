...
-- Applied to production 20260828035516; the .sql file was never committed at the
...
-- ═══════════════════════════════════════════════════════════════════════════
--
-- Supersedes the first attempt at this fix, applied minutes earlier, which was
-- WRONG in a way worth recording: count(*) is bigint and
-- fn_spin_settle_game(p_seats integer) has no implicit bigint parameter, so
-- every call failed function resolution. `EXCEPTION WHEN OTHERS` then swallowed
-- it and the sweep reported {"settled": 0, "failed": 1} with no reason
-- attached. The rewrite read as correct. Only re-running the probe caught it:
--
--   REPROBE5 (first attempt) A booked seats=<NULL>  B booked seats=<NULL>
--                            sweep={"settled": 0, "failed": 1}
--
-- So this migration does two things: the cast, and the removal of the silence
-- that hid the need for it.
--
-- ─── THE ORIGINAL DEFECT ────────────────────────────────────────────────────
--
...
--
-- PROVEN. A real 3-seat 10.00 Spin, unbooked and its counter drained the way a
-- finished Spin drains it, inside a transaction that rolled itself back:
--
