...
-- Applied to production 20260828052043; the .sql file was never committed at the
...
-- ═══════════════════════════════════════════════════════════════════════════
--
-- Supabase's linter reported 793 findings on 2026-08-28. Exactly one was ERROR
-- level and four more were the kind that only get worse with time.
--
-- ─── 1. Four functions with a mutable search_path ──────────────────────────
--
-- fn_horse_hash(text), fn_horse_hash_fnv(text), fn_horse_lane(text) and
-- fn_place_entitlement(numeric, anyelement, integer) had no search_path pinned.
--
...
-- a horse plays in.
--
-- Pinned with ALTER FUNCTION rather than by rewriting four bodies, because the
-- bodies are correct and retyping them would risk more than it fixes.
--
-- ─── 2. spatial_ref_sys: CONFIRMED EXPOSED, AND I CANNOT CLOSE IT ──────────
--
-- The single ERROR-level advisory. PostGIS's reference-system table, 8,500 rows
-- of SRID definitions, sits in `public` with RLS disabled, and `anon` and
...
--
-- That is not academic: find_live_games_nearby resolves coordinates through
-- PostGIS, so an anonymous caller corrupting or deleting SRID 4326 breaks venue
-- search for everybody.
--
-- I COULD NOT FIX IT, and this migration says so rather than pretending. The
-- table is owned by `supabase_admin` and the grants were made BY supabase_admin.
-- `postgres` holds the privileges but is not the grantor, so its REVOKE is a
-- silent no-op - the exact shape of the column-level REVOKE that read as a fix
-- and did nothing on 2026-08-27. Proven:
--
--   REVOKE ... FROM anon, authenticated  -> assertion still found the writes
--   SET LOCAL ROLE supabase_admin        -> 42501 permission denied
--
-- It needs an owner-level action (Supabase support, or the dashboard). Until
-- then it must not be quietly forgotten, so the daily auditor gains a third
-- question and the finding becomes a standing, self-closing alarm instead of a
