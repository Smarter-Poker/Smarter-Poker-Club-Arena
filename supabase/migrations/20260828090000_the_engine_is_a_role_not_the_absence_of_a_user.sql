...
-- "the_engine_is_a_role_not_the_absence_of_a_user"). This file is the record.
--
...
--
-- which skips the check entirely when there is no auth.uid(). Every one was
-- written that way for the same honest reason: the ENGINE calls them on the
...
-- (#1570): with nothing but the public anon key that ships inside the client
-- bundle, an unauthenticated call bought a rebuy for a real seated player and
-- took the chips out of their balance. That one had the grant; the other
-- thirty-eight do not, so they are not bugs today. Their safety rests on a GRANT
-- rather than on their own GUARD, and a grant is one migration away from
-- changing. Among them: atomic_table_buyin, atomic_table_rebuy,
-- atomic_seat_cashout_locked, fn_apply_credit_payment, fn_generate_credit_invoice,
-- fn_assign_player_to_agent, fn_union_expel_club, fn_union_settlement_cascade.
--
...
--
-- PROVEN on a real money path, atomic_table_buyin, in a rolled-back transaction:
--
--   PROBE11  A(anon)   = refused at the gate      <- was: straight through
--            B(other)  = refused at the gate      <- unchanged
--            C(self)   = past the gate            <- unchanged
--            D(engine) = past the gate            <- unchanged
--
...
-- remain, so a future pass cannot quietly leave a third behind.
--
...
-- transcription is the greater risk. It is one transaction: all thirty-seven
-- land or none do.
