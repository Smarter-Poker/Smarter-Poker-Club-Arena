-- ═══════════════════════════════════════════════════════════════════════════
-- BOMB POT MAX (2026-08-28) — the final spec items + operator tooling
--
-- tables:
--   1. bomb_pot_sched_state      jsonb — the BombPotScheduler's full trigger
--                                state (every-N counter, orbit anchor,
--                                pending token, timed clock, separate bomb
--                                button). Engine-written; read once at boot,
--                                so a deploy costs NO trigger mode anything.
--   2. bomb_pot_manual_pending   MANUAL_NEXT_HAND (spec §2.1): set by the
--                                role-gated fn below, consumed and cleared by
--                                the engine at the next valid hand boundary.
--   3. bomb_pot_button_policy    'regular' (default) | 'separate' (§5.3).
--   4. bomb_pot_announce_seconds timed announce window (§3): the felt clock
--                                shows only within this many seconds of the
--                                due time. NULL = always show.
--
-- bomb_pot_award_units (spec §16.2/§17): one row per (pot layer, board,
--   hi/lo side, winner) for every multi-board bomb hand, written at
--   settlement with a UNIQUE idempotency key — a replayed write conflicts
--   and does nothing.
--
-- bomb_pot_manual_requests: the audit trail behind the manual trigger —
--   who asked, which table, when (spec §15.3 "must be permissioned and
--   logged").
--
-- fn_request_manual_bomb_pot: the ONLY path that sets manual_pending.
--   SECURITY DEFINER; requires club owner / co_owner / admin on the table's
--   club; refuses tables without bomb pots enabled; writes the audit row.
--
-- get_club_home: now also ships bomb_pot_ante_multiplier + bomb_pot_ante_fixed
--   so the lobby medallion can print the ante (spec §15.1).
--
-- Views (spec §22.3 analytics): v_bomb_pot_daily, v_bomb_pot_outcomes.
-- ═══════════════════════════════════════════════════════════════════════════
...
-- Award units are the same public information the table broadcast at
-- showdown; readable by any signed-in player, written only by the engine
-- (service_role bypasses RLS — no write policy exists on purpose).
DO $$
BEGIN
  IF NOT EXISTS (
...
-- No policies: only the SECURITY DEFINER function and service_role touch it.

-- ── The one sanctioned path to a manual bomb ────────────────────────────────
...
-- ── Lobby ante disclosure (spec §15.1) ──────────────────────────────────────
DO $$
DECLARE
  v_def text;
BEGIN
