...
-- PostgREST error. Proven live against production in a rolled-back probe:
--
...
--
-- Two things were wrong with the before.
--
...
--    24 hours across 131 distinct players.
...
--
-- The cap itself is CORRECT and is not touched: it is the house rule, enforced
-- in a trigger under an advisory lock so it cannot be raced, and it applies to
-- horses exactly as it applies to humans (HORSES ARE PLAYERS). The only change
-- is that the seat path now ANSWERS with it instead of crashing on it.
--
-- Narrow on purpose: 23514 is check_violation generally, so ONLY the four-table
-- message is converted. Any other check violation is re-raised untouched - a
-- bare RAISE preserves the original error, sqlstate and context. Swallowing
...
-- `the_four_table_limit_is_an_answer_not_a_crash`, post-assertions green.
-- This file is the repo's record of that change; the body is identical.
--
-- ROLLBACK
--   Re-apply the fn_take_seat_and_buy_in body from 20260828
--   'reopen_the_seat_first_front_door' - it is this function minus the 23514
--   handler and minus v_err.
...
BEGIN
  ...
  -- REOPENED 2026-08-28: `true` is p_seat_first_internal. Without it the
  -- seat-first guard inside fn_register_for_tournament refuses THIS caller too
  -- and no seat-first seat can be bought by anyone.
