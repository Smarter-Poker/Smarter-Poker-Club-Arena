...
-- ═══════════════════════════════════════════════════════════════════════════
--
-- 2026-08-28. Dan taps a seat on a Spin table, confirms the buy-in, and gets
-- "Could Not Take That Seat, Please Try Again". Every attempt. Every table.
--
-- WHAT BROKE. The 2026-08-27 money-path audit added a guard to
-- fn_register_for_tournament: seat-first variants must not be REGISTERED into,
-- because a direct registration debits the player for a seat that is never
-- allocated. Correct rule — wrong door. fn_take_seat_and_buy_in, the ONE
-- legitimate seat-first entry path, itself registers the player by calling
-- fn_register_for_tournament. The guard cannot tell that internal call from a
-- stray lobby registration, so it refused both:
--
--     fn_take_seat_and_buy_in
--       └─ fn_register_for_tournament            → {ok:false, seat_first_variant}
--     └─ returns {ok:false, reason:'seat_first_variant'}
--       └─ client regexes match nothing          → the generic error toast
--
-- Verified against production: ZERO human seat-first joins since the guard
-- landed (02:35 UTC today); the only join in 3 days predates it. The RPC
-- answers HTTP 200 with ok:false, so nothing errored anywhere — the feature
-- simply refused everyone. Horses kept playing (their path is
-- fn_register_horse_for_tournament, unguarded), which inverts the
-- HORSES-ARE-PLAYERS law in the one direction it never anticipated: humans
-- denied what horses get.
--
-- THE SECOND REGRESSION, same guard. It blocked variant IN ('spin','sng') —
-- ALL sngs. But only heads-up (max_players <= 2) sngs are seat-first; the
-- 6-max and 9-max SNGs this platform also runs (80 + 24 completed events)
-- register through the lobby like any MTT. The guard left them with NO entry
-- path at all: registration refused (seat_first_variant), seat-first refused
-- (not_a_seat_first_game).
--
-- THE FIX, two halves:
--
--   1. fn_register_for_tournament grows `p_seat_first_internal boolean
--      DEFAULT false`. The guard fires only when it is false, and its
--      predicate becomes the CANONICAL seat-first test used by
--      fn_take_seat_and_buy_in and fn_sync_seat_first_player_count:
--      variant = 'spin' OR max_players <= 2. Multi-seat SNGs register
--      normally again; the lobby still cannot register into a Spin or a
--      Heads-Up; the seat path passes true and is let through.
--
--   2. fn_take_seat_and_buy_in calls it with true.
--
-- The old 1-arg function is DROPPED, not overloaded: a 1-arg twin beside a
-- 2-arg-with-default twin makes every PostgREST call ambiguous (PGRST203).
-- DROP loses the ACL, so the grants are restated verbatim below
-- (postgres/authenticated/service_role held EXECUTE before; anon did not).
--
-- ROLLBACK
--   Re-run the fn_register_for_tournament body from migration
--   20260827 (money-path audit) as CREATE OR REPLACE with the single uuid
--   parameter after DROPping the 2-arg version, restore the same grants, and
--   re-run the 20260825 fn_take_seat_and_buy_in body. Both bodies are in
--   supabase_migrations.schema_migrations.statements.

-- ── 1. fn_register_for_tournament: internal flag + corrected predicate ──────
...
-- DROP discarded the ACL; restate it. anon stays out (auth.uid() is required
-- anyway). PUBLIC and anon are BOTH revoked: this project's ALTER DEFAULT
-- PRIVILEGES hands anon a direct EXECUTE grant on every new function, so
-- revoking PUBLIC alone leaves anon in — the first apply of this migration
-- proved it when its own post-assertion refused to land it.
...
-- ── 2. fn_take_seat_and_buy_in: identify itself as the seat path ────────────
...
DECLARE v_n int;
BEGIN
  -- Exactly ONE fn_register_for_tournament, the 2-arg one. A surviving 1-arg
  -- twin would make every PostgREST call to it ambiguous (PGRST203), which is
  -- a worse outage than the one this migration fixes.
  ...
  -- The seat path actually passes the flag.
  IF NOT EXISTS (
    SELECT 1 FROM pg_proc p
     WHERE p.pronamespace = 'public'::regnamespace
  ...
  -- The guard still stands for external callers, with the canonical predicate.
  IF NOT EXISTS (
    SELECT 1 FROM pg_proc p
     WHERE p.pronamespace = 'public'::regnamespace
       AND p.proname = 'fn_register_for_tournament'
