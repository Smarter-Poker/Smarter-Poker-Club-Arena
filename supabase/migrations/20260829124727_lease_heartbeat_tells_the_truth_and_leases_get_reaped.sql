...
-- Applied to production 20260829124727; the .sql file was never committed at the
-- time (see docs/changelog and issue: unrecorded-migration backfill). Content is
-- byte-exact to what ran. Do NOT re-apply; it is already live.

-- The lease heartbeat could only say "kept", so it said "stolen" for everything
-- else — and nothing ever reaped a dead lease row. See the repo migration
-- 20260829120000_lease_heartbeat_tells_the_truth_and_leases_get_reaped.sql for
-- the full incident notes. Measured 2026-08-29: 204 teardowns in one hour while
-- the "stolen" tables were held by that very instance with a 2.8s heartbeat;
-- 1,987 dead table-lease rows against 263 live, 3,015 dead tournament rows
-- against 39 live. Additive only: the v1 functions stay, because the engine
-- running right now calls them.
