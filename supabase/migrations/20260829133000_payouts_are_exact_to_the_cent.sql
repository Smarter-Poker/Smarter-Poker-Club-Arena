...
-- pay their pool exactly, and since 2026-08-24 every single one is this cent.
--
-- THE RULE, now identical here and in server/src/tournament/payoutMath.ts
-- (and src/lib/payoutMath.ts, which is byte-identical to it):
--
--   * work in integer CENTS and integer BASIS POINTS -- no fraction exists to
--     be rounded away, so the implementations cannot drift;
...
-- at zero -- so on any pool smaller than the number of places it was paying,
...
--
-- Everything else is unchanged: the satellite skip, the COMPLETED gate, the
-- short-field trim, the debit-aware paid total, and the accepted-finding
-- suppression added earlier the same day.
--
-- ── ROLLBACK ──────────────────────────────────────────────────────────────
