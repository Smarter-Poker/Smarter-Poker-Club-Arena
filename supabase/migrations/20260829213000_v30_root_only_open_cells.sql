-- ════════════════════════════════════════════════════════════════════════
-- V30 CORRECTION — the warehouse only holds OPEN nodes; facing cells were
-- built from contaminated numbers and are purged (Dan 2026-08-29)
-- ════════════════════════════════════════════════════════════════════════
--
-- Found while probing the first V30 turn batches, verified against 1,200
-- sampled rows across all three streets:
--
--   1. EVERY solved tree is an open node. The root actions (tree_lines
--      matching '^r:0:X') are uniformly {c, b16} — no tree anywhere starts
--      with hero facing a bet. The 'actions' array lists DEEP-tree labels
--      (f, b45 live at depth >= 1, often as the opponent's node), so V29's
--      "actions contains f => facing node" classification was wrong for
--      every row it matched.
--
--   2. The exported per-hand numbers are trustworthy ONLY where the label
--      is clean at the root. Measured: 'c' and 'b16' values all lie in
--      [0,1]; 'f' averages 299 (max 544) and 'b45' averages 10.8 — those
--      are contaminated deep-node exports (EV-like magnitudes), not
--      frequencies. Any cell whose mix includes them is garbage: rollMix
--      normalizes by total, so fold~400 vs call~0.9 read as "fold 99%".
--
-- CONSEQUENCE FOR SHIPPED V29: the flop 'facing' cells had exactly this
-- shape and have been steering the facing-a-bet consult toward folds since
-- they went live. They are deleted here (the loader's next refresh drops
-- them from the brain; the facing consult itself is removed from
-- HorseLogic in the same PR). The flop 'open' cells are UNAFFECTED — flop
-- rows export root-only frequencies (all in [0,1], per-hand sums ~1.0,
-- measured mean 0.999) and stay as shipped.
--
-- THE V30 SPEC, RESTATED: aggregate turn/river as OPEN cells only, from
-- ROOT actions only, with per-hand validation — every root value in
-- [0, 1.001] and the root sum in [0.95, 1.05] (measured: 56% of hand-rows
-- pass; the rest are blend-contaminated and contribute NOTHING rather than
-- noise). Valid hands are renormalized to exactly 1 before averaging.
-- ════════════════════════════════════════════════════════════════════════

-- ── purge: facing cells (all streets) + the turn cells built pre-spec ────
delete from public.gto_postflop_compact where facing = 'facing';
delete from public.gto_postflop_compact where street in ('turn', 'river');

update public.gto_agg_progress
   set last_id = null, rows_done = 0, done = false, updated_at = now()
 where street in ('turn', 'river');

-- ── the corrected cursor-driven aggregator ──────────────────────────────
...
comment on function public.fn_aggregate_gto_street_next(text, integer) is
  'V30 cursor-driven batch aggregator for turn/river OPEN cells. Root-node actions only (tree_lines r:0:X); per-hand validated (all values in [0,1.001], sum in [0.95,1.05]) and renormalized — contaminated deep-node exports (f, b45) contribute nothing. Paced by the engine''s GtoAggregationDriver; restart-safe; permanently silent once done=true.';
...
-- ── assertions ──────────────────────────────────────────────────────────
do $$
declare v_facing integer; v_turn integer; v_cur record;
begin
  select count(*) into v_facing from public.gto_postflop_compact where facing = 'facing';
  if v_facing <> 0 then
    raise exception 'facing cells survived the purge: %', v_facing;
  end if;
  select count(*) into v_turn from public.gto_postflop_compact where street in ('turn','river');
  if v_turn <> 0 then
    raise exception 'pre-spec turn/river cells survived the purge: %', v_turn;
  end if;
  select * into v_cur from public.gto_agg_progress where street = 'turn';
  if v_cur.last_id is not null or v_cur.done then
    raise exception 'turn cursor did not reset';
  end if;
end;
$$;
