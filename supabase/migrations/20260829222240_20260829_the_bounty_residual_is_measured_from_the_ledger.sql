...
-- Applied to production 20260829222240; the .sql file was never committed at the
...
-- completed this afternoon paid out MORE than their bounty pool held:
--
...
-- created from nothing. The ordering question is recorded in the changelog
-- for a follow-up; this migration closes the minting.
--
...
BEGIN
  ...
  END IF;
  IF position('bounty_pool,0) - COALESCE(v_paid,0)' in v_def) = 0 THEN
    RAISE EXCEPTION 'the residual is still computed from the counter';
  END IF;

  -- Re-run it against one of the events that overpaid. It is idempotent (the
  -- credit key is per-tournament-per-winner), and it must now report a
  -- residual of 0 because the ledger already shows more paid than the pool.
  SELECT id INTO v_tid FROM tournaments
   WHERE name LIKE 'Evening Mystery Bounty%' AND status = 'COMPLETED'
     AND ended_at = '2026-08-29 21:30:12.866+00';

  IF v_tid IS NULL THEN
    RAISE NOTICE 'sample event not found; function replaced, assertion skipped';
    RETURN;
  END IF;

  SELECT fn_finalize_bounty_pool(v_tid, NULL) INTO v_res;
  IF (v_res->>'residual')::numeric <> 0 THEN
    RAISE EXCEPTION 'expected no residual on an already-overpaid event, got %', v_res->>'residual';
  END IF;

  RAISE NOTICE 'verified: an exhausted pool now yields residual 0 (ledger_paid %)', v_res->>'ledger_paid';
END $$;
