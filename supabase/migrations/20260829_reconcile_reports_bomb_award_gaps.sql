-- 2026-08-29: reconcile_ledger_nightly learns about bomb-pot award-unit gaps.
--
-- Companion to 20260829_bomb_award_ledger_gaps_are_loud.sql, which creates
-- fn_bomb_pot_ledger_gaps and widens the entity_type CHECK. That function is
-- only a query until something calls it every night; this is the caller.
--
-- Filed as CRITICAL, not warn. A missing award unit does not mean money moved
-- wrongly — logHandHistory records the settlement independently, and every gap
-- found so far paid out correctly to the cent. It means the platform's record
-- of HOW a bomb pot was split has a hole in it, which is precisely the thing
-- the ledger exists to prevent, and the only way anyone learns is if it shouts.
--
-- The body below is the 2026-08-28 function verbatim with one section added
-- immediately before the summary rollup. Nothing else is changed.
--
-- Tier 2: CREATE OR REPLACE of a reporting function. Writes only to
-- ledger_reconcile_log, which the function already truncates for the day.
...
-- Post-apply assertion: the new section must be present in the stored body.
DO $$
BEGIN
...
-- ROLLBACK: CREATE OR REPLACE this function again with the
-- 'BOMB-POT AWARD LEDGER' INSERT block removed. Everything else in the body is
-- the 2026-08-28 version verbatim.
