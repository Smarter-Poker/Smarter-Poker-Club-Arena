...
-- Applied to production 20260830032832; the .sql file was never committed at the
...
-- ════════════════════════════════════════════════════════════════════════
--
-- SYMPTOM: the engine deployed at 22:18 UTC and gto_agg_progress never
-- moved off the 500 rows I had folded by hand. The driver was ticking and
-- silently reporting the same error every 20 seconds:
--
--     21000  DELETE requires a WHERE clause
--
-- CAUSE: fn_aggregate_gto_street_next clears its temp table with an
...
-- function itself was never wrong in SQL — I probed it through the MCP as
-- the postgres role, which has no such guard, so it passed. The engine
...
--
-- FIX: TRUNCATE instead. It is not a DELETE, so no qual is required, and on
-- a temp table it is cheaper anyway. (The temp table is ON COMMIT DROP and
-- each RPC call is its own transaction, so the clear is belt-and-braces to
-- begin with — but a function that assumes a fresh table and does not
-- enforce it is a trap for the next caller.)
...
begin
  ...
  -- which refuses an unqualified DELETE (error 21000). See header.
