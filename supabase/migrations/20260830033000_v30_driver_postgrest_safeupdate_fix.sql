-- ════════════════════════════════════════════════════════════════════════
-- V30 FIX — the driver could not call its own RPC (Dan 2026-08-29)
-- ════════════════════════════════════════════════════════════════════════
--
-- SYMPTOM: the engine deployed at 22:18 UTC and gto_agg_progress never moved
-- off the 500 rows folded by hand. The driver was ticking and silently
-- failing every 20 seconds with:
--
--     21000  DELETE requires a WHERE clause
--
-- CAUSE: fn_aggregate_gto_street_next cleared its temp table with an
-- unqualified `delete from tmp_agg30`. PostgREST's API roles run with the
-- safe-update guard on, which refuses any DELETE without a qual. The
-- function was never wrong in SQL — it was probed through the Supabase MCP
-- as the postgres role, which has no such guard, so it passed. The engine
-- reaches it through PostgREST, where it could never have worked.
--
-- THE LESSON, WRITTEN DOWN: an RPC the engine calls must be probed THE WAY
-- THE ENGINE CALLS IT (POST /rest/v1/rpc/... as service_role), not only
-- through a privileged SQL session. "It runs in psql" is not evidence that
-- it runs in production.
--
-- FIX: TRUNCATE. Not a DELETE, so no qual is required, and cheaper on a temp
-- table. (The table is ON COMMIT DROP and each call is its own transaction,
-- so the clear is belt-and-braces — but a function that assumes a fresh
-- table without enforcing it is a trap for the next caller.)
--
-- Applied to production via the Supabase MCP on 2026-08-30, then verified
-- through the engine's own path: the cursor advanced 500 -> 1500 -> 2000 on
-- successive minutes with updated_at seconds old.
...
comment on function public.fn_aggregate_gto_street_next(text, integer) is
  'V30 cursor-driven batch aggregator for turn/river OPEN cells. Root-node actions only (tree_lines r:0:X); per-hand validated and renormalized. Clears its temp table with TRUNCATE, never DELETE - PostgREST roles refuse an unqualified DELETE (21000). Paced by the engine''s GtoAggregationDriver at a measured 200-row batch (300+ exceeds the API statement budget).';
