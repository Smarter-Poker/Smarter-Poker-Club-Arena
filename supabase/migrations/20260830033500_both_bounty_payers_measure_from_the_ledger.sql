...
-- ═══════════════════════════════════════════════════════════════════════════
--
-- Earlier the same day `fn_finalize_bounty_pool` was changed to compute its
-- residual from the wallet ledger instead of the `bounty_pool_paid` counter,
-- after eight mystery bounty events paid 140.60 more than their pools held.
--
...
--
-- The reason is that there are TWO payers and only one was fixed.
-- `fn_collect_bounty` decided what was left with the identical stale read:
--
...
--
-- HOW THIS IS APPLIED. The single availability line is replaced inside the
-- live function definition, and the migration REFUSES to run if that line is
-- not exactly where it expects -- rather than re-stating the whole function
-- and risking silently dropping one of its guards. The assertions at the foot
-- then confirm the hybrid tripwire, the mystery-phase handoff, the
-- already-collected dedupe and the exhaustion guard all survived.
--
-- ── ROLLBACK ──────────────────────────────────────────────────────────────
...
BEGIN
  ...
  IF position('v_available := round(COALESCE(v_t.bounty_pool,0) - COALESCE(v_t.bounty_pool_paid,0), 2);' in v_src) = 0 THEN
    -- Already patched, or the function moved on. Either way, do not patch blind.
    IF position('FROM wallet_transactions wt' in v_src) > 0 THEN
      RAISE NOTICE 'fn_collect_bounty already measures from the ledger - nothing to do';
      RETURN;
    END IF;
