-- ════════════════════════════════════════════════════════════════════════
-- V30 — the batch floor was blocking the only tuning knob (2026-08-30)
-- ════════════════════════════════════════════════════════════════════════
--
-- WHAT WAS MEASURED, 04:00 UTC, through the engine's own path
-- (POST /rest/v1/rpc as service_role, three consecutive calls):
--
--     call 1  http=500  8.27s  57014 canceling statement due to timeout
--     call 2  http=200  8.15s  processed=200
--     call 3  http=200  7.54s  processed=200
--
-- A 200-row batch now costs ~8s, and the API roles' statement_timeout is
-- exactly 8s (authenticator: statement_timeout=8s). The batch sits ON the
-- cliff edge, so roughly every other call burns eight seconds of database
-- CPU and then rolls back having moved the cursor nowhere. Observed
-- throughput was 200 rows per 20s tick — ~10 rows/s, one QUARTER of the
-- ~40 rows/s the driver's own header projects, because only one call per
-- tick was landing.
--
-- The driver header's "~3.5s for 200 rows" was true when it was measured
-- and is not true now: gto_postflop_compact has accumulated cells, so
-- nearly every batch takes the expensive ON CONFLICT merge path rather
-- than a plain insert.
--
-- WHY THE FLOOR HAD TO MOVE: the SQL clamped p_batch to [200, 5000], so
-- 200 was simultaneously the smallest batch the caller could ask for and
-- the largest that fits the 8s budget. There was no room left to tune —
-- the driver's only control was pinned against a wall. This lowers the
-- floor to 25. Nothing else in the function changes: the aggregation body
-- below is byte-identical to 20260830033000, TRUNCATE and all, so the
-- root-only 'open' cells and the check/bet_small/bet_big bucket set are
-- untouched.
--
-- Widening a clamp cannot change behaviour for any existing caller: every
-- caller today passes 200, and greatest(25, 200) is still 200.
--
-- ROLLBACK: re-apply 20260830033000_v30_driver_postgrest_safeupdate_fix.sql
-- verbatim; it is the same function with the floor back at 200.
