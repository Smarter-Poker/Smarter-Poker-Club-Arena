...
-- Applied to production 20260830053658; the .sql file was never committed at the
-- time (see docs/changelog and issue: unrecorded-migration backfill). Content is
-- byte-exact to what ran. Do NOT re-apply; it is already live.

-- ════════════════════════════════════════════════════════════════════════
-- V30 OPTIMIZATION — the aggregator was re-deriving its own root actions
-- once per row of the batch (Dan 2026-08-30)
-- ════════════════════════════════════════════════════════════════════════
--
-- EXPLAIN (ANALYZE) on a live 200-row turn batch showed the cost was not
-- where it looked. Of ~3.5s per call:
--
--   * ~530ms  fetching the batch (index scan on the PK, filtering street --
--             341 of 541 rows read are discarded because no composite
--             (street, id) index exists);
--   * ~860ms  RE-COMPUTING THE `roots` CTE, EIGHTY TIMES. The plan read
--             `Group ... rows=176 loops=80`: because a CTE Scan reports an
--             estimate of 1 row, the planner chose a nested loop with a
--             join filter for `src JOIN roots ON r.id = s.id` and removed
--             13,920 rows by that filter -- an O(n^2) self-join over data
--             the outer row already had in hand.
--
-- FIX: derive the root actions with a LATERAL against the row itself, so
-- there is no self-join, no repeated Group, and no dependence on the
-- planner's estimate of a CTE. The per-row bet count and max bet size come
-- from the same array, so `rootbets` disappears too.
--
-- PROVEN EQUIVALENT BEFORE APPLYING: both formulations were run over the
-- SAME 200-row window and their cell sets compared with EXCEPT in both
-- directions -- 6,883 cells each, only_in_old = 0, only_in_new = 0. This
-- changes speed, not a single frequency.
--
-- NOT DONE, DELIBERATELY: a composite (street, id) index would reclaim the
-- other ~530ms, but building one on a 79 GB / 8.8M-row table is exactly the
-- class of heavy operation that caused the 2026-08-15 liveness incident.
-- The payoff (~15%) does not justify that risk while the fleet is dealing.
--
-- ALSO MEASURED, worth knowing: only 88 of every 200 turn rows carry both
-- `frequencies` and `tree_lines`. The other 56% are older exports with
-- nothing to aggregate. They still advance the cursor (correctness), but
-- the real yield per batch is ~44%.
...
begin
  ...
  -- ROOT ACTIONS, derived per row with a LATERAL. No self-join: this is the
  -- whole optimization. The array also answers "how many root bets" and
  -- "how big is the largest", which used to need a second CTE.
  base as (
    select s.id, s.fam, s.pos, s.depth_bucket, s.tex, s.freqs, r.acts
    from src s
    cross join lateral (
      select array_agg(distinct replace(l.line, 'r:0:', '')) as acts
      from jsonb_array_elements_text(s.tree_lines) l(line)
      where l.line ~ '^r:0:[^:]+$'
    ) r
    where s.pos is not null and s.tex is not null and r.acts is not null
  ),
  handvals as (
    select b.id, b.fam, b.pos, b.depth_bucket, b.tex, a.act,
           h.key as hand, (h.value)::numeric as v,
           (select count(*) from unnest(b.acts) x
             where x like 'b%' and substr(x, 2) ~ '^[0-9]+$') as nb,
           (select max((nullif(substr(x, 2), ''))::numeric) from unnest(b.acts) x
             where x like 'b%' and substr(x, 2) ~ '^[0-9]+$') as mx
    from base b
    cross join lateral unnest(b.acts) a(act)
    cross join lateral jsonb_each_text(b.freqs -> a.act) h
    where h.value ~ '^-?[0-9.eE+]+$'
  ),
  validated as (
  ...
  normed as (
    select hv.id, hv.fam, hv.pos, hv.depth_bucket, hv.tex, hv.hand,
      case
        ...
        when coalesce(hv.nb, 0) = 1 then
          case when (nullif(substr(hv.act, 2), ''))::numeric >= 100
            then 'bet_big' else 'bet_small' end
        when (nullif(substr(hv.act, 2), ''))::numeric >= hv.mx then 'bet_big'
        else 'bet_small'
      end as bucket,
      hv.v / va.tot as freq
    from handvals hv
