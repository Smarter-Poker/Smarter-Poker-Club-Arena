...
-- Applied to production 20260830053917; the .sql file was never committed at the
-- time (see docs/changelog and issue: unrecorded-migration backfill). Content is
-- byte-exact to what ran. Do NOT re-apply; it is already live.

-- ════════════════════════════════════════════════════════════════════════
-- REVERT v30_aggregator_lateral_optimization (Dan 2026-08-30)
-- ════════════════════════════════════════════════════════════════════════
--
-- I replaced the roots self-join with a LATERAL because EXPLAIN showed the
-- roots CTE being recomputed 80 times (~860ms of a ~3.5s call). The rewrite
-- was proven to produce byte-identical cells (6,883 both ways, EXCEPT empty
-- in both directions) -- but it was SLOWER IN PRACTICE, and that is the only
-- measurement that counts.
--
-- What actually happened: pushing the pos/tex predicates into `src` made the
-- planner evaluate fn_gto_texture_class_any (a plpgsql function) inside the
-- filter for all 200 rows before anything could be discarded, and re-measured
-- end to end the pipeline went 977ms -> 1252ms. Live, every driver call then
-- exceeded the ~8s API budget: the cursor sat at 167,950 rows for 93 seconds
-- with no movement, after having advanced steadily all hour.
--
-- So: back to the formulation that demonstrably moves 28 rows/second in
-- production. The nested-loop waste is real and still worth fixing one day --
-- but with a composite (street, id) index and a SQL-language texture
-- function, measured under load, not with a clever rewrite pushed on the
-- strength of a plan node.
--
-- THE RULE THIS COST ME: an optimization is not proven by EXPLAIN. It is
-- proven by the throughput of the thing it was meant to speed up.
