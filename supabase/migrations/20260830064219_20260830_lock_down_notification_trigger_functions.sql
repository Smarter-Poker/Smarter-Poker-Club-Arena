...
-- Applied to production 20260830064219; the .sql file was never committed at the
...
-- REVOKEs on only two of them. The other three inherited PostgreSQL's default
...
--
-- NOT EXPLOITABLE TODAY, and I checked rather than assumed:
--
--   * The four notify_* are TRIGGER functions. They take no arguments and read
...
--     SECURITY DEFINER, bypasses RLS on profiles. That discloses nothing new:
--     the `profiles_select` policy is already `FOR SELECT TO authenticated
--     USING (true)`, so any signed-in user can read every profile anyway.
--   * fn_raise_notification is the one that inserts arbitrary notifications and
--     it was locked correctly from the start. Probed as `authenticated`: the
--     call fails and zero rows are created.
--
...
BEGIN
  ...
  IF v_bad <> '' THEN
    RAISE EXCEPTION 'still executable by a client role:%', v_bad;
  END IF;
