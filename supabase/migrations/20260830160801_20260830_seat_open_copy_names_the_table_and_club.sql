...
-- Applied to production 20260830160801; the .sql file was never committed at the
...
-- three active subscriptions for one phone). But the two "Seat Open"
...
-- They read as a repeat. A notification that a person mistakes for a duplicate
-- has failed even when the delivery was perfect.
--
-- So the distinguishing token moves into the title, where it is actually read,
-- and the club is added to the body for anyone who is a member of more than
-- one:
--
--   title:   Seat Open: Bomb Pot NLH 0.25/0.50
--   body:    A Seat Just Opened At Bomb Pot NLH 0.25/0.50 In Shark Club.
--            Sit Down Now To Claim It.
--
-- The per-table `tag` already collapses REPEAT offers for one table into a
-- single shade entry, so two entries for two tables is correct behaviour and
-- stays. This only makes the difference legible.
--
-- REWRITTEN FROM THE DEPLOYED SOURCE, not retyped. The function is 2,823
-- characters of queue logic with FOR UPDATE SKIP LOCKED and TTL reclamation
-- that has nothing to do with copy; transcribing it by hand to change two
-- string literals is how a working queue acquires a silent bug. This reads
-- pg_get_functiondef, applies three targeted replacements, asserts every one
-- of them matched, and executes the result.
DO $migration$
DECLARE
  v_def  text;
  v_new  text;
BEGIN
  SELECT pg_get_functiondef(oid) INTO v_def
    FROM pg_proc
   WHERE proname = 'fn_offer_open_seat'
     AND pronamespace = 'public'::regnamespace;

  IF v_def IS NULL THEN
    RAISE EXCEPTION 'fn_offer_open_seat not found';
  END IF;
...
BEGIN
  ...
  IF position('Seat Open: ' in v_src) = 0 THEN
    RAISE EXCEPTION 'title does not name the table'; END IF;
  IF position('club_name' in v_src) = 0 THEN
    RAISE EXCEPTION 'club name was not carried through'; END IF;
  -- the queue logic must be untouched
