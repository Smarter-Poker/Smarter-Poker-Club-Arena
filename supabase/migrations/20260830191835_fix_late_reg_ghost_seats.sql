-- ============================================================================
-- 20260830191835_fix_late_reg_ghost_seats.sql
-- TIER: 2  |  AFFECTS: fn_seat_late_registrant
--
-- FIX THE GHOST SEAT MULTIPLIER (11 PLAYERS ON A 9-MAX TABLE)
--
-- When a table engine crashes, GameServer sweeps ghost seats by setting
-- left_at = now(). These vacated seats are aggressively reused by
-- fn_seat_late_registrant and ensureLateRegSeated to prevent unique_violation
-- races.
--
-- The bug: Both processes successfully overwrote the table_seats row with the
-- NEW user, but NEITHER cleared the table_id / seat_number columns from the
-- OLD user's tournament_players row. Both the old user and new user now claim
-- to sit at that same seat in their roster record.
--
-- When 2 ghost seats were swept and reused on a 9-max table, 11 users
-- held a table_id pointing to it. This crashes ServerTableEngineDealing
-- with deck_capacity_exceeded.
-- ============================================================================
...
BEGIN
  ...
  IF FOUND THEN
    ...
    -- row still falsely points to this table. This causes ghost assignments. Clear it.
    UPDATE public.tournament_players
       SET table_id = NULL, seat_number = NULL
     WHERE tournament_id = p_tournament_id
       AND table_id = v_table
