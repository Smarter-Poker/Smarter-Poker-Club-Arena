...
-- Applied to production 20260830205523; the .sql file was never committed at the
-- time (see docs/changelog and issue: unrecorded-migration backfill). Content is
-- byte-exact to what ran. Do NOT re-apply; it is already live.

-- V31 cells must carry the SIZE, not just the bucket.
--
-- hand_matrix says how often each holding takes each bucket. It does not say
-- what the bucket IS in chips, and the buckets are wide: bet_big is ">=110%
-- of pot" and the turn's actual mean is 246.8% (range 117-263, sd 36.6).
-- A reader guessing the midpoint would size the solver's overbet at roughly
-- a third of what it is, which throws away the entire reason V31 exists -
-- v1 cannot express a large turn bet at all.
--
-- Sizes are near-deterministic per tree, so a per-cell mean is faithful:
-- flop bet_small is exactly 33 (sd 0) and flop bet_mid exactly 75 (sd 0).
--
-- The cursor is RESET and the table truncated. Only 1,350 of ~1.89M rows
-- were folded (77 cells), so redoing them costs minutes, and a table where
...
begin
  ...
  -- the size the bucket actually means, per cell, from the solves that built it
  sizecells as (
  ...
  cells as (
    select c.street, c.fam, c.pos, c.depth_bucket, c.tex,
           ...
           coalesce(max(z.size_map), '{}'::jsonb) as size_map
    from (
  ...
  on conflict (street, game_family, position, depth_bucket, texture_class) do update
     set hand_matrix = (
         ...
         -- the size merges the same weighted way as the frequencies
         size_pct = coalesce((
           select jsonb_object_agg(bk, round(
