...
-- grants, so the scheduled path is untouched. Verified before applying, and
-- the 23:45:00 run after it reported `succeeded`.
--
...
--
-- HOW IT WAS FOUND: the Supabase security advisor reports 645 + 77 SECURITY
-- DEFINER grant warnings, which is far too many to read. Filtering pg_proc to
-- functions that are SECURITY DEFINER *and* VOLATILE (can write) *and* not a
-- trigger *and* actually EXECUTE-able by anon reduces 722 warnings to NINE,
-- of which six are public reads the signup and lobby pages need.
--
