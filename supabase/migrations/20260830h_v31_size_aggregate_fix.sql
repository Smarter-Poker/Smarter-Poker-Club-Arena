-- ═══════════════════════════════════════════════════════════════════════════
-- V31 aggregator — carry the bet size, and fix the max(jsonb) that 20260830g
-- shipped. (2026-08-30)
-- ═══════════════════════════════════════════════════════════════════════════
--
-- 20260830g added gto_postflop_v31.size_pct and an aggregator that filled it,
-- but carried the size through the `cells` CTE with `max(z.size_map)`. There
-- is no max(jsonb) in Postgres. The DDL applied cleanly and the function
-- failed on its FIRST CALL, which is the whole lesson: a migration succeeding
-- is not the same as the code in it working, and this class of defect is
-- invisible until something executes it.
--
-- The fix is not a cast. `sizecells` yields exactly one row per
-- (street, fam, pos, depth_bucket, tex) and `cells` groups by those same five
-- columns, so size_map is functionally dependent on the group key and simply
-- belongs IN the GROUP BY. Postgres cannot infer that across a join, which is
-- why it demanded an aggregate. Grouping by it is provably identical, because
-- the value cannot vary within a group.
--
-- WHY THE SIZE MATTERS AT ALL: `bet_big` means ">=110% of pot" and the turn
-- mean is 246.8% (117-263, sd 36.6). A reader guessing the bucket midpoint
-- would size the solver overbet at about a third of what it is — and playing
-- that overbet is the one thing the v1 warehouse cannot do, since every v1
-- tree offers exactly one bet size (b16, 16% of pot) on every street at every
-- stack depth.
--
-- The body below is the definition PostgreSQL itself reports for the live
-- function, so this file and production cannot drift.
--
-- TIER 2: replaces one function body. No schema, no data, no cursor change.
...
-- ── WHO MAY CALL IT ──────────────────────────────────────────────────────
--
-- pg_get_functiondef does NOT emit grants, so the body above — dumped from
-- the live function to guarantee this file matches production — would, on a
-- standalone apply, leave a SECURITY DEFINER WRITER that any logged-in
-- browser role could execute. The live function has been locked down since
-- 20260830e; this file had not said so, and a migration that cannot be
-- replayed safely is not a migration.
--
-- Nobody in a browser should ever call this: it is a one-time backfill of
-- the solver warehouse, driven by the engine. PUBLIC is named as well as the
-- roles, because revoking a role while PUBLIC still holds the grant reads as
...
begin
  ...
  if v_n <> 0 then
    raise exception 'fn_aggregate_gto_v31_next is still reachable by a browser role';
  end if;
end $$;

-- ROLLBACK: re-apply the aggregator body from
-- 20260830e_v31_aggregator_rpc_budget.sql (which predates the size column, so
-- size_pct would then stay {} on every newly folded cell).
