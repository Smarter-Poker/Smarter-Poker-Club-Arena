...
-- Applied to production 20260831082249; the .sql file was never committed at the
-- time (see docs/changelog and issue: unrecorded-migration backfill). Content is
-- byte-exact to what ran. Do NOT re-apply; it is already live.

-- ============================================================================
-- 20260831030000_run_it_mode_describes_what_the_table_actually_does.sql
-- TIER: 2 | AFFECTS: tables.run_it_mode (descriptive only), and a new guard
-- rule on auto_start_players.
--
...
-- were RUNNING run-it-twice while their run_it_mode read 'none'. The page is
-- fixed to write all three columns.
--
...
--    and the booleans are NOT touched, so no table changes behaviour. This
--    only stops the row from describing itself as something it is not, now
--    that 'none' genuinely means off.
--
-- 2. NEW GUARD RULE. auto_start_players above the seat count is a table that
--    can never deal and is not even flagged as stuck (the same number is
--    dealThreshold, which the zombie reaper reads). The UI now clamps it;
--    the database refuses it, the way it already refuses the seat law.
--
...
BEGIN
  UPDATE public.tables
   ...
   WHERE tournament_id IS NULL
     ...
     -- the engine's own gate: this table IS running it twice today
...
-- 2. auto_start_players may never exceed the seats.
...
-- Assert the repair landed and that nothing violates the new rule.
DO $$
