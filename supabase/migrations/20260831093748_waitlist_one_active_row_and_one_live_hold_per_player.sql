...
-- Applied to production 20260831093748; the .sql file was never committed at the
...
--    client's existing recovery handles the refusal correctly with no change.
--
...
--    defaulting to 1. Setting it to a larger number restores the old
--    behaviour without a code change; 0 would be read as "no cap".
--
...
-- active states, and 0 players currently holding more than one live offer, so
-- neither change can fail on existing data.
--
-- ROLLBACK:
...
-- ── 1. The index now covers a live offer as well as a place in line ─────────
DROP INDEX IF EXISTS public.table_waitlist_one_active_per_player_uidx;
CREATE UNIQUE INDEX table_waitlist_one_active_per_player_uidx
  ON public.table_waitlist (table_id, user_id)
  WHERE status IN ('waiting', 'notified');

-- ── 2. The cap, as a config row Dan owns ───────────────────────────────────
