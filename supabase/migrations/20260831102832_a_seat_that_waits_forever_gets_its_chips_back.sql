...
-- Applied to production 20260831102832; the .sql file was never committed at the
...
-- when the third seat is sold. Nothing anywhere bounds the wait in between.
-- If the third player never arrives, the chips of everyone already seated are
-- locked with no timeout, no refund and no way out but a human noticing.
--
...
-- 21 HOURS. Nine spins waited over six hours. Every one of those was
-- horse-seated, so no human has been harmed yet; the fleet fills tiers fast
-- enough that this has stayed invisible. It is latent, not theoretical: the
-- first human to sit at a thin stake inherits exactly that 21-hour wait.
--
...
-- terms, which also returns the fleet capacity those 21 hours were consuming.
--
-- THE RULE. A spin is expired when it is still open, has not started, has at
-- least one seat that has been waiting longer than the policy, and is NOT
-- full (a full-but-unstarted game is about to deal - never cancel that).
-- Cancellation goes through atomic_cancel_tournament, the sanctioned refund
-- path, which is what trg_tournaments_cancel_must_refund insists on.
--
-- THE TIMEOUT IS A CONFIG ROW, NOT A CONSTANT: p90 is under 7 minutes, so the
-- 30-minute default is ~4.4x the slowest normal fill. It is Dan's to tune, and
-- 0 disables the sweep entirely without a deploy.
--
-- ROLLBACK
...
-- Observability: what the sweep would act on right now, and what is waiting.
