...
-- Applied to production 20260831104152; the .sql file was never committed at the
-- time (see docs/changelog and issue: unrecorded-migration backfill). Content is
-- byte-exact to what ran. Do NOT re-apply; it is already live.

-- 20260831_journal_the_real_treasury_writers.sql
-- TIER 3. 23 functions write clubs.chip_treasury; ZERO wrote chip_ledger.
-- This journals the four with real live volume. Every ledger INSERT is wrapped
-- so it can NEVER block the money. HORSES ARE PLAYERS: no is_horse branch.
-- See repo file for full rationale + ROLLBACK.
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint
    ...
    RAISE EXCEPTION 'PRE-FLIGHT: chip_ledger does not admit table_stack yet. Apply migration 1 first.';
  END IF;
