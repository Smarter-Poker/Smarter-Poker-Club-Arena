...
-- Applied to production 20260831110515; the .sql file was never committed at the
-- time (see docs/changelog and issue: unrecorded-migration backfill). Content is
-- byte-exact to what ran. Do NOT re-apply; it is already live.

-- See supabase/migrations/20260831_anon_cannot_name_itself_the_actor.sql for the
-- full reasoning. fn_club_set_member_role took its actor from
-- COALESCE(auth.uid(), p_actor_user_id); auth.uid() is NULL for anon, so an
-- unauthenticated caller could name any club owner as the actor and set any
-- member's role. Two layers: the function refuses a caller-supplied actor
-- unless the caller is a trusted backend, and PUBLIC/anon lose EXECUTE.
