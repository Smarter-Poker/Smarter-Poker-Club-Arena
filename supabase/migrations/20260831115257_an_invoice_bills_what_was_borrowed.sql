...
-- Applied to production 20260831115257; the .sql file was never committed at the
-- time (see docs/changelog and issue: unrecorded-migration backfill). Content is
-- byte-exact to what ran. Do NOT re-apply; it is already live.

-- Phase 1 of 7. fn_generate_all_credit_invoices billed credit_limit minus
-- agent_wallet_balance - the UNUSED part of the line, not a debt. 224 invoices,
-- 18,047,771 chips, against an estate-wide credit_used of 0.00. And
-- fn_apply_credit_payment never paid the debt down.
-- Full narrative in
-- supabase/migrations/20260901000001_an_invoice_bills_what_was_borrowed.sql

BEGIN;

ALTER TABLE public.credit_invoices ADD COLUMN IF NOT EXISTS void_reason text;

ALTER TABLE public.credit_invoices DROP CONSTRAINT IF EXISTS credit_invoices_status_check;
ALTER TABLE public.credit_invoices ADD CONSTRAINT credit_invoices_status_check
  CHECK (status = ANY (ARRAY['pending','partial','paid','overdue','disputed','void']));

COMMENT ON COLUMN public.credit_invoices.void_reason IS
  'Why this invoice was cancelled. A void invoice is never payable; fn_apply_credit_payment refuses one before any wallet is touched.';
...
CREATE OR REPLACE FUNCTION public.fn_apply_credit_payment(
  p_invoice_id uuid, p_amount numeric, p_method text
) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER
SET search_path TO 'public', 'extensions'
AS $function$
...
UPDATE public.credit_invoices ci
   SET status = 'void',
       amount_remaining = 0,
       void_reason = 'Billed the unused portion of the credit line rather than the credit drawn. Corrected 2026-08-31; no payment was ever made against this invoice.'
...
DO $verify$
DECLARE v_bad int; v_voided int;
BEGIN
  ...
  IF pg_get_functiondef('public.fn_apply_credit_payment(uuid,numeric,text)'::regprocedure) !~ 'credit_used' THEN
    RAISE EXCEPTION 'fn_apply_credit_payment still does not pay down agents.credit_used';
  END IF;
  IF pg_get_functiondef('public.fn_generate_all_credit_invoices(timestamptz)'::regprocedure) ~ 'credit_limit' THEN
