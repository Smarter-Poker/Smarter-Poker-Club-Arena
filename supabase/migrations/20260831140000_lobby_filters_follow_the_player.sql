...
-- their filters on a laptop and opens the lobby on their phone got the
...
-- a BROWSER writes directly, so the row is locked to its owner: USING and
-- WITH CHECK auth.uid() = user_id on all four verbs. Without WITH CHECK on
-- insert and update, a player could write filters onto somebody else's
-- account - harmless-looking, but it is somebody else's lobby.
--
