...
-- 110% of its pool. Morning Grinder (PLO) b687e4aa did the same on place 1,
-- 96.00 against 48.00. Those two are the LAST overpaid MTTs that the Phase 3
...
-- moved at all. Summing it to answer "what has this player already been paid?"
-- asks a best-effort record a question only an authoritative one can answer.
-- Under-report and the reconciler pays twice; over-report and it hides a
-- genuine shortfall forever.
--
...
-- and only if money moved once. Phase 3 backfilled every tournament in the
-- platform's history into it, so it can be relied on for the back catalogue as
-- well as for today.
--
...
-- structure money already paid, and could conclude a player was square when
-- the structure still owed them. The record distinguishes the two by `source`,
-- so this now counts only what the prize pool is meant to fund.
--
...
--
-- SAFETY, MEASURED BEFORE TRUSTING IT. The dangerous direction is the record
-- reporting LESS than the ledger, because that is what would propose a top-up
-- the old logic did not. Across every COMPLETED non-satellite tournament of
-- the last 30 days -- 163,302 (tournament, finisher) rows:
--
--     163,286  the two sources agree
--          15  record HIGHER than ledger   (the safe direction; these are the
--              double-pay cases this migration prevents)
--           1  record LOWER than ledger    (by 0.31 chips, total)
--
-- That single row is "The Daily Big Mini" place 3, and it cannot double-pay
-- even in principle: its `:reconcile` key is already consumed, so a second
-- top-up for that place returns false from fn_credit_and_log and is reported
-- as `top_up_refused_by_idempotency` instead of being paid. The reconcile key
-- is scoped per (tournament, user, place), which bounds this failure mode to
-- one top-up per place for all time.
--
-- VERIFIED ON THE TWO EVENTS THAT CAUSED IT (dry run, p_apply => false):
...
--
-- BEHAVIOUR OTHERWISE UNCHANGED: the trim-to-field rule, the basis-point
-- rounding with the residual on the last paid place, the duplicate-finisher
-- and no-finisher refusals, the overpayment-is-reported-never-clawed-back
-- stance, the alert de-duplication and its acceptance handshake, and the
-- prize-column stamp all behave exactly as before. `paid_from` is added to the
-- returned jsonb so a reader can tell which source answered.
--
