...
-- Applied to production 20260831161500; the .sql file was never committed at the
-- time (see docs/changelog and issue: unrecorded-migration backfill). Content is
-- byte-exact to what ran. Do NOT re-apply; it is already live.

-- ═══════════════════════════════════════════════════════════════════════════
-- PGRST RELOAD WATCHDOG (2026-08-31)
--
-- Companion to fix_pgrst002_schema_cache_timeout. That migration made schema-
-- cache reloads survivable; this one makes the next reload storm LOUD instead
-- of a silent afternoon of 503s. Modeled on fn_db_saturation_selftest:
-- selftest function -> log table -> RAISE WARNING, scheduled via pg_cron with
-- an advisory-lock overlap guard.
--
-- Three parts:
--   1. ca_ddl_events + two event triggers: an audit log of every DDL command,
--      flagged with whether it makes PostgREST reload its schema cache
--      (mirrors extensions.pgrst_ddl_watch's tag list and pg_temp filter),
--      with role/application attribution so a storm names its source.
--   2. fn_pgrst_reload_watchdog(): breaches when reload-triggering DDL
--      exceeds a threshold per hour, when the authenticator statement_timeout
--      fix has been reverted (the exact regression that caused the outage),
--      or when the pgrst watch triggers are disabled.
--   3. pg_cron job every 15 minutes, offset minutes to avoid shared slots.
-- ═══════════════════════════════════════════════════════════════════════════

-- 1a. The DDL event log ------------------------------------------------------
...
-- No policies: not readable through the API. postgres and service-role paths
-- that bypass RLS can read it; nothing else needs to.
...
-- 1b. Event-trigger loggers --------------------------------------------------
-- These must NEVER raise: an event trigger that errors breaks every DDL
-- statement on the platform. Everything is wrapped, and failure is silence.
...
DECLARE
  ...
  v_timeout interval;
  r record;
  c_reloads_per_hour_breach constant int := 60;  -- one reload lasts ~28s; 60/h means near-continuous reloading
  c_reloads_per_hour_warn   constant int := 20;
BEGIN
  -- CHECK 1: reload-triggering DDL rate. The 2026-08-31 outage hours ran at
  -- hundreds per hour; overnight baseline is ~4.
  ...
  -- CHECK 2: the outage fix itself. authenticator statement_timeout below the
  -- schema-cache load time (~28s) recreates the outage the moment DDL flows.
  ...
  BEGIN
    IF v_timeout_raw IS NULL THEN
      v_timeout := NULL; -- no per-role setting: falls to the global default (2min) - acceptable
  ...
  -- CHECK 3: the pgrst watch triggers and this watchdog''s own loggers exist
  -- and are enabled. A disabled pgrst watch means schema changes silently stop
  -- reaching PostgREST (stale-cache errors); a missing logger blinds CHECK 1.
  FOR r IN
  ...
  -- Retention: watchdog log 30 days, DDL event log 14 days.
  DELETE FROM public.pgrst_reload_watchdog_log WHERE ran_at < now() - interval '30 days';
  DELETE FROM public.ca_ddl_events WHERE occurred_at < now() - interval '14 days';
...
-- 3. Schedule ----------------------------------------------------------------
-- Offset minutes (6,21,36,51) so it never shares a firing slot with the
-- maintenance jobs fn_db_saturation_selftest warns about.

SELECT cron.schedule(
  'pgrst-reload-watchdog',
  '6,21,36,51 * * * *',
  $job$
  SELECT CASE
