...
-- Applied to production 20260831190443; the .sql file was never committed at the
...
-- Register both under guard-integrity protection.
INSERT INTO public.ca_guard_inventory (kind, object_a, object_b, note, active)
VALUES
  ('function', 'public', 'fn_ca_suspense_regression_check', 'phase 2: suspense regression watchdog', true),
  ('cron', 'ca-suspense-regression-15m', NULL, 'phase 2: suspense regression check every 15 min', true)
ON CONFLICT DO NOTHING;

-- And in the money-RPC registry (it reads chip_ledger only, but keep the
-- inventory of standing financial functions complete).
