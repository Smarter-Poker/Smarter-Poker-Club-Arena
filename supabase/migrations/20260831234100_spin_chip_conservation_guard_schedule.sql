-- Schedule the spin/sng chip conservation guard, hourly at :49.
--
...
-- tournaments plus the correlated seat counts. Measured over a 6h window on
-- 2026-08-31: 2.3s and 7.5s on two scheduled runs of ~1,300 games, and one 8s
-- outlier under lock contention. Budget for seconds, not milliseconds.
--
...
DO $$
BEGIN
  PERFORM cron.unschedule('spin_chip_conservation_hourly')
    WHERE EXISTS (SELECT 1 FROM cron.job
                   WHERE jobname = 'spin_chip_conservation_hourly');
