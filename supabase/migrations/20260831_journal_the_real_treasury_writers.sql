-- 20260831_journal_the_real_treasury_writers.sql
--
-- TIER 3 (three money-path functions replaced). ROLLBACK pasted at the bottom.
--
-- WHAT WAS WRONG
--
-- 23 functions in this database write clubs.chip_treasury. ZERO of them write
-- chip_ledger. The only chip_ledger writer on the platform is the club_members
-- trigger, and migration 1 has just stopped that trigger pretending to be the
-- treasury's counterparty. So as of migration 1 the treasury has an honest but
-- EMPTY ledger: real money moves in and out of it and nothing records that.
--
-- This migration journals the treasury movements with actual live volume:
--
--   fn_horse_fund_from_treasury   9,076,675 chips over 20,806 calls for one
--                                 club -- by far the biggest real mover
--   fn_horse_seat_from_treasury   the same movement by a different door (it
--                                 creates the seat instead of topping it up).
--                                 Journalling one and not the other would put
--                                 a hole in the ledger the exact shape of
--                                 which door the engine happened to use
--   credit_club_rake_to_treasury  rake credited straight to the treasury
--   atomic_distribute_rake        its v_route = 'club_chip_treasury' branch,
--                                 i.e. rake for a club with no union
--
-- HOW IT IS SAFE
--
-- Every ledger INSERT is wrapped in its own BEGIN / EXCEPTION WHEN OTHERS
-- block that logs to ca_ledger_write_failures and swallows. This mirrors the
-- trigger's pattern exactly, and it is the whole point: A RECORD OF THE MONEY
-- MOVING MUST NEVER BE ABLE TO STOP THE MONEY MOVING. If the ledger write
-- fails, the buy-in still happens, the rake is still credited, and the failure
-- is loud in ca_ledger_write_failures instead of silent.
--
-- The atomic_distribute_rake insert sits INSIDE the `IF v_n > 0` first-claim
-- guard, so a replayed hand does not double-journal -- the same idempotency
-- the rake_distribution_legs leg_key already provides for the money itself.
--
-- HORSES ARE PLAYERS (CLAUDE.md 10.5). fn_horse_fund_from_treasury and
-- fn_horse_seat_from_treasury are horse PLUMBING -- the engine supplying what
-- a browser would -- which is the one sanctioned kind of horse branch. No
-- is_horse test is added anywhere here, and nothing in this migration gives a
-- horse a different deal from a human: a horse's treasury-funded buy-in is now
-- journaled exactly as any other treasury-funded buy-in would be.
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
                 WHERE n.nspname='public' AND p.proname='fn_horse_fund_from_treasury'
                   AND p.prosrc LIKE '%horse_treasury_funding%') THEN
    RAISE EXCEPTION 'PRE-FLIGHT: fn_horse_fund_from_treasury() is not the version this migration was written against.';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
                 WHERE n.nspname='public' AND p.proname='fn_horse_seat_from_treasury'
                   AND p.prosrc LIKE '%horse_treasury_funding%') THEN
    RAISE EXCEPTION 'PRE-FLIGHT: fn_horse_seat_from_treasury() is not the version this migration was written against.';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
                 WHERE n.nspname='public' AND p.proname='credit_club_rake_to_treasury'
                   AND p.prosrc LIKE '%total_rake%') THEN
    RAISE EXCEPTION 'PRE-FLIGHT: credit_club_rake_to_treasury() is not the version this migration was written against.';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
                 WHERE n.nspname='public' AND p.proname='atomic_distribute_rake'
                   AND p.prosrc LIKE '%club_chip_treasury%') THEN
    RAISE EXCEPTION 'PRE-FLIGHT: atomic_distribute_rake() has no club_chip_treasury branch to journal.';
  END IF;

  -- Migration 1 must be in first: without table_stack in the domain every
  -- INSERT below would fail the CHECK and land in the failure log.
  IF NOT EXISTS (SELECT 1 FROM pg_constraint
    ...
    RAISE EXCEPTION 'PRE-FLIGHT: chip_ledger does not admit table_stack yet. Apply 20260831_ledger_trigger_stops_inventing_the_treasury.sql first.';
  END IF;
...
-- ---------------------------------------------------------------------------
-- 1. Horse buy-in / rebuy funded from the treasury.  club_treasury -> table_stack
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  /* THE TREASURY GETS A LEDGER ROW (2026-08-31). This is a REAL treasury
     movement -- chips leave clubs.chip_treasury and arrive on the felt -- and
     until today nothing recorded it. Never allowed to block the money: a
     failed audit row is logged and swallowed, exactly as the club_members
     trigger does. */
  BEGIN
    ...
    VALUES (
      ...
      p_amount, 'buyin', v_club_id, p_table_id,
...
-- ---------------------------------------------------------------------------
-- 2. The same movement through the other door: seat created AND funded.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  /* Journaled for the same reason as fn_horse_fund_from_treasury: this is a
     real treasury outflow onto the felt, and which door it came through is
     not a reason for the ledger to have a hole in it. Never blocks. */
  BEGIN
    INSERT INTO public.chip_ledger
      (performed_by, from_type, from_entity_id, to_type, to_entity_id,
       amount, category, club_id, table_id, description)
...
-- ---------------------------------------------------------------------------
-- 3. Rake credited straight to the treasury.  table_stack -> club_treasury
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4. atomic_distribute_rake, v_route = 'club_chip_treasury' branch ONLY.
--    Everything else in this 200-line function is byte-for-byte unchanged.
-- ---------------------------------------------------------------------------
  ...
  ELSE
    ...
    IF v_n > 0 THEN
      ...
      /* JOURNAL THE TREASURY LEG (2026-08-31). Inside the leg_key first-claim
         guard, so a replayed hand credits the treasury once and journals it
         once. Never allowed to block the rake. */
      BEGIN
        INSERT INTO public.chip_ledger
          (performed_by, from_type, from_entity_id, to_type, to_entity_id,
...
BEGIN
  FOR r IN
    SELECT unnest(ARRAY['fn_horse_fund_from_treasury','fn_horse_seat_from_treasury',
                        'credit_club_rake_to_treasury','atomic_distribute_rake']) AS fname
  LOOP
    IF NOT EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
                   WHERE n.nspname='public' AND p.proname = r.fname
                     AND p.prosrc LIKE '%INSERT INTO public.chip_ledger%') THEN
      RAISE EXCEPTION 'POST-APPLY: %() does not write chip_ledger.', r.fname;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
                   WHERE n.nspname='public' AND p.proname = r.fname
                     AND p.prosrc LIKE '%ca_ledger_write_failures%') THEN
      RAISE EXCEPTION 'POST-APPLY: %() journals without a never-block failure sink.', r.fname;
    END IF;
    -- HORSES ARE PLAYERS: no new is_horse discrimination anywhere in these.
    IF EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
               WHERE n.nspname='public' AND p.proname = r.fname
                 AND p.prosrc LIKE '%is_horse%') THEN
      RAISE EXCEPTION 'POST-APPLY: %() gained an is_horse branch. CLAUDE.md 10.5 forbids it.', r.fname;
    END IF;
  END LOOP;

  -- The union branch of atomic_distribute_rake must NOT have been journaled as
  -- a treasury movement: the union rake wallet is not the club treasury.
  IF (SELECT count(*) FROM regexp_matches(
        (SELECT prosrc FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
          WHERE n.nspname='public' AND p.proname='atomic_distribute_rake'),
        'INSERT INTO public\.chip_ledger', 'g')) <> 1 THEN
    RAISE EXCEPTION 'POST-APPLY: atomic_distribute_rake should journal chip_ledger exactly once (the club_chip_treasury branch only).';
  END IF;

  RAISE NOTICE 'POST-APPLY OK: four real treasury writers now journal, none can block, none discriminates.';
END;
$postapply$;

-- ---------------------------------------------------------------------------
-- ROLLBACK (Tier 3). Restore the four bodies exactly as they were before this
-- migration: each is this file's version with the BEGIN/EXCEPTION chip_ledger
-- block (and its v_st / v_msg declarations) deleted. Nothing else differs, so
-- the rollback is a deletion, not a rewrite. Recover the pre-change source with
--
--   SELECT pg_get_functiondef(oid) FROM pg_proc
--    WHERE proname IN ('fn_horse_fund_from_treasury','fn_horse_seat_from_treasury',
--                      'credit_club_rake_to_treasury','atomic_distribute_rake');
--
-- against a PITR snapshot from before 2026-08-31, or delete the four marked
-- blocks from this file and re-run it. Ledger rows already written by these
-- functions are HISTORY and must not be deleted on rollback -- they record
-- movements that really happened.
-- ---------------------------------------------------------------------------
