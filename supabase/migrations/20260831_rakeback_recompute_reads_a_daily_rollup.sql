-- Repo copy of production migration `rakeback_recompute_reads_a_daily_rollup`
...
--
-- fn_rakeback_recompute_periods rebuilds a (club, week) FROM SOURCE — the right
-- design, and the fix for an older bug where incrementing rake_generated
-- double-counted on every engine restart. But the settler calls it once per
-- (club, week) PER PAGE of 1,000 rake_records, and a week of the busy club is
-- 274,783 records. Measured: 28.4s for ONE club-DAY through the old per-row
-- path, ~400s for the week, against a 60s engine client deadline (15s before
-- the outage widened it). Every call failed with supabase_timeout, 221 buckets
-- failed per cycle, and the settler correctly refused to advance its watermark
-- past records it could not settle. That refusal is right and stays.
--
-- 1. SET-BASED SHARES. The old body called fn_rake_shares_for_record per record
--    through a LATERAL, and that wrapper hits rake_attributions TWICE per hand.
--    Reading the ledger as ONE grouped join and falling back to the allocator
--    only for hands it does not cover is identical and ~7x faster (400s -> 54s).
--    Verified before shipping: 6h window, 406 users, ZERO differing cents,
--    totals equal (595,843); full day 564 users / 6,089,636 cents both paths.
--
-- 2. A DAILY ROLLUP, which is what makes it fit. A week is now the SUM OF ITS
--    DAYS: rakeback_daily_user holds integer cents per (club, day, user), a day
--    is recomputed only when its record count changed, and the week is a
--    trivial aggregate. Measured after: a day 4.6s, a full week with every day
--    fresh 7.5s — an 8x margin on the live deadline, and inside the original
--    15s one. Cents are integers and addition is associative, so summing days
--    then the week is EXACTLY summing the week. The rollup is derived data with
--    no authority: drop it and the next call rebuilds it from rake_records.
--    Cross-checked after building: the rollup's 2026-08-24 totals are 564 users
--    and 6,089,636 cents — the same numbers the old path produced.
...
BEGIN
  ...
  -- Rebuild the day wholesale: DELETE then INSERT, so a user who no longer has
  -- a share for the day does not linger with a stale one.
