...
--
-- TIER 3 (new table + reconcile_ledger_nightly replaced). ROLLBACK at the bottom.
--
-- WHAT WAS WRONG
--
-- reconcile_ledger_nightly compared an ALL-TIME sum of chip_ledger movement
-- against an ALL-TIME stored clubs.chip_treasury. The earliest auto-audited
-- ledger row is 2026-08-27. So the check was measuring four days of flow
-- against a balance accumulated over the platform's whole life, and calling
-- the difference drift. It was never going to be zero, and it never was.
--
-- The player-wallet side of the house already solved exactly this on
-- 2026-08-26 (20260826_baseline_the_unledgered_gap.sql): what cannot be
-- reconstructed is WRITTEN DOWN as an opening balance rather than invented,
-- and reconciliation runs forward from the line. The club_treasury block never
-- got the same treatment. This is that treatment.
--
-- WHY THE LINE IS DRAWN NOW AND NOT EARLIER
--
-- Migrations 1 and 2 had to land first. Before migration 1 the ledger's
-- treasury rows were ~62k phantoms invented by the club_members trigger, so a
-- baseline taken then would have been baselining a lie and every row after it
-- would have kept lying. The cutover is taken here, after the trigger stopped
-- inventing a counterparty and after the four real treasury writers started
-- journaling. Everything before the line is acknowledged, not reconstructed;
-- everything after it is measured.
--
-- NOTHING ELSE IN THE FUNCTION IS TOUCHED. frozen_wallets_pool,
-- insurance_bank, insurance_offer_unresolved, seat_stack_exit,
-- chip_circulation, cashout_escrow_stuck, negative_balance, over_claimed_send
-- and bomb_award_ledger_gap are reproduced byte-for-byte, and a post-apply
-- assertion fails the migration if any of them went missing.
--
-- THE SEVERITY THRESHOLDS ARE UNCHANGED: 0 is ok, <= 1.00 is warn, anything
-- more is critical. Widening them to manufacture a pass would be the same
-- class of mistake as the one being fixed.
--
-- THE HONEST RESIDUAL
--
-- Only 4 of the 23 functions that write clubs.chip_treasury journal to
-- chip_ledger today. From the cutover forward this check will therefore go
-- critical again the first time one of the other 19 moves treasury money --
-- fn_club_bank_send, fn_mint_club_chips, fn_union_send_to_club_atomic,
-- fn_member_leave_to_treasury and the rest. THAT IS THE CHECK WORKING. It will
-- now name a real unjournaled writer instead of drowning in phantom buy-ins,
-- and the fix is to journal that writer, not to move this line again.
...
BEGIN
  ...
  -- Every other check must be present BEFORE, so the post-apply comparison means something.
  IF v_src NOT LIKE '%frozen_wallets_pool%' OR v_src NOT LIKE '%insurance_bank%'
    ...
    RAISE EXCEPTION 'PRE-FLIGHT: reconcile_ledger_nightly() is missing a check this migration expects to preserve. Re-read prosrc before proceeding.';
  END IF;

  -- Migrations 1 and 2 must be in, or the line would be drawn under a lie.
  ...
  IF to_regclass('public.ca_treasury_baseline') IS NOT NULL THEN
    RAISE EXCEPTION 'PRE-FLIGHT: ca_treasury_baseline already exists. The line is drawn once; moving it later would erase real drift.';
  END IF;
END;
$preflight$;

-- ---------------------------------------------------------------------------
-- 1. Write the gap down. Do not smear it into the ledger.
-- ---------------------------------------------------------------------------
...
/* FOR UPDATE serialises against the treasury writers, which all take the same
   row lock, so the snapshot and the line are taken against a settled table
   rather than one mid-write. */
DO $baseline$
BEGIN
...
-- ---------------------------------------------------------------------------
-- 2. reconcile_ledger_nightly. ONLY the club_treasury block differs.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  /* CLUB TREASURY, MEASURED FROM THE LINE (2026-08-31).
     Was: all-time ledger flow vs all-time stored balance, which compared four
     days against forever and was permanently critical by construction.
     Now: opening balance at the cutover, plus only the flow recorded AFTER it.
     Same shape as fn_chip_drift_since_baseline does for player wallets. */
  WITH cut AS (
...
BEGIN
  ...
  -- Nothing else may have been lost in the re-install.
  IF v_src NOT LIKE '%frozen_wallets_pool%' OR v_src NOT LIKE '%insurance_bank%'
     OR v_src NOT LIKE '%insurance_offer_unresolved%' OR v_src NOT LIKE '%seat_stack_exit%'
     OR v_src NOT LIKE '%chip_circulation%' OR v_src NOT LIKE '%cashout_escrow_stuck%'
     OR v_src NOT LIKE '%negative_balance%' OR v_src NOT LIKE '%over_claimed_send%'
  ...
  -- The thresholds must be exactly what they were. No widening.
  ...
  -- REPORTED, NOT ENFORCED. If a treasury moved through one of the 19 still
  -- unjournaled writers between the baseline and this run, that is a REAL
  -- finding and the honest thing is to surface it, not to loosen the check
  -- until it disappears.
  IF v_crit > 0 THEN
...
--
-- BEGIN;
-- -- Restore the all-time club_treasury CTE (this is the pre-2026-08-31 block;
-- -- paste it back into the function in place of the baseline block above):
-- --
-- --   WITH credits AS (
...
-- --   WHERE club_id IS NOT NULL;
-- --
-- DROP TABLE public.ca_treasury_baseline;
-- COMMIT;
-- ---------------------------------------------------------------------------
