-- =============================================================================
-- THE AGENT PANEL CANNOT STRAND AN AGENT
-- =============================================================================
-- 2026-08-31, phase 2 of 7, follow-up to 20260901000002 in the same session.
-- Found by auditing every caller of the functions that migration changed, and
-- proved against production inside a transaction that was rolled back.
--
-- TWO GAPS IN fn_admin_update_agent, one of them mine.
--
-- G1, A REGRESSION 20260901000002 INTRODUCED. That migration added a coherence
-- check refusing "prepaid AND a credit line", which is a real contradiction.
-- But it read the pair as
--
--     COALESCE(p_is_prepaid, is_prepaid), COALESCE(p_credit_limit, credit_limit)
--
-- so raising a limit WITHOUT saying anything about is_prepaid inherited the
-- stored `true` and was refused. Two live callers do exactly that -
-- CreditRequestService.raiseAgentCreditLimit and AgentService.setCreditLimit -
-- and 63 of 111 active agents are prepaid. Probed on production, rolled back:
--
--   G1 raise limit on a prepaid agent: success=false
--      err=[a prepaid agent carries no credit line. Move them to credit first,
--           or set the limit to 0.]
--
-- and there is no screen that "moves them to credit first", so the credit panel
-- dead-ended for the majority of agents. An operator who types a credit limit
-- has already said which funding they mean. The limit IS the choice, so an
-- unstated is_prepaid now follows it instead of refusing the call. Stating both
-- and contradicting yourself is still refused, because that is a real mistake.
--
-- G2, PRE-EXISTING AND UNGUARDED. Setting the limit to 0 on an agent who is not
-- prepaid produced (not prepaid, no line) - the one combination that can send
-- nothing at all, which is the exact state 20260901000002 exists to stop the
-- PROMOTION from creating. The agent panel could still create it afterwards:
--
--   G2 set limit 0 on a credit agent: success=true
--      -> resulting row prepaid=false limit=0.00
--
-- One active agent is in that state on production right now. It is left alone:
-- prepaid-or-credit and how much is a commercial term, and picking one for
-- somebody is the "a rate nobody chose" bug this programme keeps removing.
-- Reported to Dan instead.
--
-- TWO MORE HOLES CLOSED WHILE HERE, both of which reached the client as a raw
-- Postgres error rather than a sentence:
--   - moving an agent to prepaid while they still owe credit_used would leave a
--     debt with no line behind it;
--   - lowering a limit below what has already been drawn would violate the
--     check_credit CHECK and surface as 23514.
--
-- THE GUARDS ONLY FIRE WHEN THE CALL TOUCHES FUNDING. This matters: one active
-- agent is already in the send-nothing state, and four callers update only a
-- status or a role. Validating the resulting pair unconditionally would have
-- made that agent impossible to suspend, reinstate or re-grade - a second,
-- worse regression hiding inside the fix for the first.
--
-- NO SCHEMA CHANGE. One function body. No table lock.
-- =============================================================================
...
BEGIN
  IF pg_get_functiondef(
     ...
     ) !~ 'v_touches_funding' THEN
    RAISE EXCEPTION 'fn_admin_update_agent still validates funding on calls that do not touch it';
  END IF;
  IF pg_get_functiondef(
       'public.fn_admin_update_agent(uuid,text,text,numeric,numeric,numeric,uuid,text,boolean)'::regprocedure
     ) ~ 'Move them to credit first' THEN
    RAISE EXCEPTION 'fn_admin_update_agent still dead-ends a limit raise on a prepaid agent';
  END IF;
  RAISE NOTICE 'the agent panel can no longer strand an agent';
END
$verify$;
...
-- ROLLBACK
-- =============================================================================
-- One function body, no schema change, no data written. To revert, re-create
-- fn_admin_update_agent from 20260901000002. Note that doing so restores BOTH
-- gaps: the credit panel dead-ends again for prepaid agents, and the agent
-- panel can strand an agent with no funding.
