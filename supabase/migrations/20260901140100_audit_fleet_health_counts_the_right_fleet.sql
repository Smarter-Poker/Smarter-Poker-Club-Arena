...
--
-- RECORDED AFTER THE FACT. Applied to production via the Supabase MCP
-- alongside PR #2479; the file was not committed with it, and the omission
-- was caught by the verification pass the same day. The transformation below
-- is idempotent - it returns early if the fix is already present - so
-- re-running it against the live database is a no-op.
--
-- WHY IT EXISTS. fn_audit_fleet_health counted `profiles where is_horse =
-- true` as the fleet and reported everyone who dealt no hands as idle. On
-- 2026-08-31 that read "417 of 1000 horses dealt no hands", severity
-- CRITICAL, recommending an investigation of the seeding cycles. Split by
-- horse_status:
--
...
--
-- 416 of the 417 are DISABLED - idle because they are switched off. The
-- seeder was doing its job: 583 of 584 available horses dealt hands.
--
-- The open-seat check had the opposite failure: it compared raw open seats
-- against a fixed 150, so its meaning drifts with the size of the floor.
-- Measured while writing this, the real state was far worse than the number
-- suggested - 97 cash tables, 767 seats, 88 filled, ELEVEN PERCENT
-- occupancy - and the finding still read as a mild "156 open seats" warning.
-- An occupancy ratio does not move when the table count does, and the
-- evidence now carries the supply arithmetic so a reader can see whether the
-- constraint is seeding or simply too few enabled horses for too many tables.
--
...
begin
  ...
  -- Every anchor must be present, or the function has drifted from what this
  -- migration was written against and a blind replace would be a guess.
...
begin
  ...
  -- Behavioural: on the day that produced the false alarm, the idle finding
  -- must now be absent or count only enabled horses.
  select f into v_idle
    from jsonb_array_elements(fn_audit_fleet_health('2026-08-31'::date)) f
...
-- ROLLBACK
-- ===========================================================================
-- This function writes no data, so a revert loses nothing but the two
-- corrections above. To revert, re-run the CREATE OR REPLACE FUNCTION
-- public.fn_audit_fleet_health(date) body from the migration that last
-- touched it before 2026-09-01 (20260828f_wire_fleet_health and successors),
-- then:
