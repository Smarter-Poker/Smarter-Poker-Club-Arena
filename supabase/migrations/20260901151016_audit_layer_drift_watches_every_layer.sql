...
-- Applied to production 20260901151016; the .sql file was never committed at the
...
-- layer ships, and it has not been since 2026-08-27.
--
-- MEASURED: of the 60 features that fired in the last seven days, FORTY are
...
--
-- TUNED, NOT GUESSED. Backtested over three days before shipping:
--   2026-08-29  1 alarm   icm_legacy 96%
--   2026-08-30  1 alarm   icm_legacy 94%
--   2026-08-31  4 alarms  icm_legacy 92%, v16_unblocker 77%,
--                         v18_self_image 65%, v15_nut_status 60%
-- One alarm on a quiet day, and on the day the V29-V32 solver stack displaced
...
begin
  select pg_get_functiondef(p.oid) into v_def
   ...
   where n.nspname = 'public' and p.proname = 'fn_run_horse_daily_audit';
  for v_n in
    select 1 where position('fn_audit_layer_drift(p_day)' in v_def) = 0
  loop
    raise exception 'POST-APPLY: the audit does not call the layer-drift check';
  end loop;
  -- every earlier hook survives the splice
