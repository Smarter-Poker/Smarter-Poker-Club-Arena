...
-- Applied to production via Supabase MCP 2026-09-01 16:56 UTC.
-- Dan: "WIPE AWAY ANY CURRENT PLAYER FEES THAT WERE PREVIOUSLY GENERATED, AS
-- WELL AS ALL RAKE BBJ ETC. KEEP JUST THE CHIPS IN PLAY."
-- Clears the last fee/rake residue of pre-green-light play: agent rake
-- counters (57.44), clubs.total_rake (868.24), 558 rake_records, 112
-- tournament rake settlements; re-clamps two wallets that caught straggler
-- backpays at 16:11:08 UTC (+9.51 / +6.80) from wiped-history games (the
-- unpaid queue was verified EMPTY first). Chips in play asserted at exactly
-- 4,160,000 / 3,340,000 on exit.
-- Mechanics: DELETE on rake_records fires the per-change reporting-repair
-- trigger (platform-wide rollup rebuild each time; 558 timed out attempt
-- one). Triggers disabled around the delete; the rollup repair runs ONCE
