...
-- Applied to production 20260901172428; the .sql file was never committed at the
...
-- of those families: flh (Fixed Limit Hold'em) and flo8 (Fixed Limit Omaha
-- Hi-Lo, seat law 8) -- stud, razz, badugi and 2-7 triple draw do not exist
-- as variants, and Kill / Half-Kill pots have no engine support, so seeding
-- them would list games that cannot deal (the crazy-pineapple lesson).
--
-- This seeds what can actually run: FLH and FLO8 at all 13 locked limit
-- stakes in the three engine-real variations -- Classic 6-Max, Classic Full
-- Ring, Deep Buy-In. 2 families x 13 stakes x 3 = 78 tables. Fixed-limit
-- stakes name the SMALL BET / BIG BET; blinds derive as sb = small bet / 2,
-- bb = small bet, which is how the engine's BettingStructure reads them.
-- Buy-ins: 20 big bets min / 100 big bets max (200/500 for Deep).
-- Idempotent by settings->>'seed_key' (dss:flh:2-4:classic6 style).
-- =============================================================================
DO $$
DECLARE
