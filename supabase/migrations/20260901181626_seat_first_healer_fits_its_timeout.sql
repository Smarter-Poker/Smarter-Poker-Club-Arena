...
-- Applied to production 20260901181626; the .sql file was never committed at the
-- time (see docs/changelog and issue: unrecorded-migration backfill). Content is
-- byte-exact to what ran. Do NOT re-apply; it is already live.

-- SEAT-FIRST HEALER FITS ITS TIMEOUT (2026-09-01, follow-up to
-- seat_first_openers_can_be_reseated). The engine calls this via PostgREST as
-- service_role, whose statement_timeout is pinned to 8s (the PGRST002 fix).
-- With 96 husks on the board, one call tried to heal 25 of them - up to 50
-- horse registrations plus a scope-checked random pick each - and was
-- cancelled mid-flight every tick: "seat-first repair failed: canceling
-- statement due to statement timeout" in the engine log, and zero husks
-- healed. Two changes, same transaction:
--   1. Function-level SET statement_timeout = '60s' - a per-function
--      override that applies at entry whatever the caller's role setting is.
--   2. The husk batch is clamped to 6 per call. The engine calls every 30
--      seconds; healing 6 per tick clears a 96-husk board in under ten
--      minutes without ever running long. p_limit still caps below 6.
...
BEGIN
  FOR v_t IN
    SELECT t.id, t.name, t.club_id, t.game_type, t.max_players, t.blind_structure,
     ...
     -- timeout; the 30-second tick supplies the throughput.
...
BEGIN
  ...
  IF v_def NOT LIKE '%statement_timeout%' THEN
    RAISE EXCEPTION 'healer carries no timeout override';
  END IF;
  ...
  IF v_def NOT LIKE '%fn_ca_entry_scope_ok%' THEN
    RAISE EXCEPTION 'healer lost its club scope';
  END IF;
END $$;
