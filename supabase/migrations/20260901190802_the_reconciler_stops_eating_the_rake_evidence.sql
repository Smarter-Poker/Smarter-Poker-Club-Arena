...
-- Applied to production 20260901190802; the .sql file was never committed at the
...
--   1. The reconciler deletes only the entity types it writes.
--   2. An illegal rake is classified as an illegal rake. Every rake_law row
--      was filed as 'reporting_mismatch' at layer 'reporting' regardless of
--      kind, which is exactly why a hand raked with no flop read as a
--      harmless evidence gap and got shrugged off.
--   3. A daily wide scan, so a violation missed while a job was down is
--      still found instead of ageing out of the two-hour window forever.
--   4. The rake law joins the hourly ratchet at a baseline of zero, so the
--      NEXT violation raises an incident by itself.
-- ═══════════════════════════════════════════════════════════════════════════

-- ── 1. The reconciler deletes only what it owns ────────────────────────────
...
-- The hourly job keeps its 2h window (cheap, catches things fast). This one
-- re-reads a full day, and the hand_id dedupe in fn_rake_law_check means a
-- hand already filed is never filed twice.
