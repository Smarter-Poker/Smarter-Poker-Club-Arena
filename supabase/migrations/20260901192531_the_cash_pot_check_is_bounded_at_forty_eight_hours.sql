...
-- Applied to production 20260901192531; the .sql file was never committed at the
-- time (chip-std phase 1.5 mirror, docs/changelog/2026-09-02-chip-std-p1-mirror.md).
-- Content is byte-exact to what ran. Do NOT re-apply; it is already live.

-- A second correction to 20260901190656, folded into that file. Its window
-- ceiling was 720 hours. Cash hands arrive at roughly 59,000 a day and the
-- per-row cost is a jsonb_array_elements sum over `winners`: a 24-hour window
-- reads 58,932 rows and returns comfortably; a 168-hour window read 463,506
-- rows and returned once, then hit the statement timeout on the very next call
-- an hour later when the database was busier. That is the shape of every check
-- on this platform that quietly stopped working, and fn_spin_unpaid_check
-- learned it the same way on 2026-08-31.
--
-- 48 hours is double what the engine asks for and half of what has been seen
-- to fail. Only the LEAST() ceiling changes.
