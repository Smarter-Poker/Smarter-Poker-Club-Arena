...
-- Applied to production 20260901193055; the .sql file was never committed at the
...
-- HORSE JUMPS IN AND 'STARTS THE SPIN'"). The engine-side window constants
-- move to 45-90s in the same change; the healer's fresh window must match or
-- a repaired game waits longer than a fresh one.
...
BEGIN
  ...
  IF v_def LIKE '%fn_ca_entry_scope_ok(%' THEN
    RAISE EXCEPTION 'per-horse scope fn call reappeared';
  END IF;
  IF v_def NOT LIKE '%LEAST(GREATEST(p_limit, 0), 6)%' THEN
    RAISE EXCEPTION 'healer batch clamp lost';
  END IF;
END $$;
