-- HEALER HUMAN WINDOW IS 45-90 SECONDS (Dan 2026-09-01, verbatim: "THEY ARE
-- SUPPOSED TO SEAT 2, WAIT ANYWHERE FROM 45-90 SECONDS FOR A HUMAN, BEFORE A
-- HORSE JUMPS IN AND 'STARTS THE SPIN'"). Applied to production as
-- healer_human_window_is_45_to_90_seconds. Identical to
-- 20260901181911_seat_first_healer_inside_eight_seconds except the fresh
-- human window: 45 + floor(random() * 46) seconds (45-90s) instead of
-- 60-180s, matching the engine-side SEAT_FIRST_HUMAN_WINDOW constants moved
-- in the same PR. Assertion added for the window literal.
BEGIN;
...
BEGIN
  ...
  LOOP
    ...
    -- game. Only when the repair actually changed something.
...
BEGIN
  ...
  IF v_def NOT LIKE '%45 + floor(random() * 46)%' THEN
    RAISE EXCEPTION 'healer window is not 45-90s';
  END IF;
