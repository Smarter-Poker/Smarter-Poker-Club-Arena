...
-- Applied to production 20260901194301; the .sql file was never committed at the
...
--
-- That is the same shape as every bug this whole sweep has been about: a fix
-- with nothing holding it in place. So the ratchet holds it. baseline 0, and
-- the hourly watcher raises a critical incident the moment the scoping is
-- gone -- whoever removed it, however they removed it.
...
AS $fn$
  -- 1 when reconcile_ledger_nightly clears rows it does not own, 0 when the
  -- DELETE is scoped to its own entity types.
  SELECT CASE
           WHEN NOT EXISTS (
