...
--     pool paying four 200 seats is the club funding the seats, as advertised.
...
--
-- CARE WITH THE SPIN TEST: `spin_type` is the string 'standard' on every row
-- in the table, MTT freezeouts included. Excluding on `spin_type IS NULL`
-- reports ZERO unexplained events and silently buries an 18,381.60 chip
-- discrepancy on an MTT. The honest discriminator is tournament_type = 'SPIN',
-- which matches spin_multiplier > 0 exactly (28,710 both ways).
--
-- With satellites, Spins and guarantees accounted for, ten MTT events remain
-- unexplained for 18,899.68 chips, 97% of it a single event (Sunday $200 Deep
-- Stack: 115 entrants, 133 rebuys, pool 44,640, guarantee 20,000, paid
-- 63,021.60). Those are the ones worth a person's attention, and now they are
-- the only ones that will reach one.
-- ═══════════════════════════════════════════════════════════════════════════
...
-- 48 hours, not ten days: a ten-day scan cost 31 seconds of the hourly watch
-- (20260901231551). The ten-day view stays available to a person by passing
-- the interval. Rolling window, so the count falls as events age out; a RISE
-- means settlement is creating chips again.
...
SELECT 'prize_overpay_unexplained_10d',
       ...
       'Completed tournaments paying beyond both pool and guarantee, excluding satellites and Spins, in a rolling 48 HOURS. 4 events / 18,394.61 chips on 2026-09-01, 97% of it one MTT: Sunday $200 Deep Stack, 115 entrants, 133 rebuys, pool 44,640, guarantee 20,000, paid 63,021.60. Use fn_ca_prize_overpay_unexplained(''10 days'') for the wider view.'
