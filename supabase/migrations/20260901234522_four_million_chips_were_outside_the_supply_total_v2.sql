...
-- Applied to production 20260901234522; the .sql file was never committed at the
...
-- either, so it is circulating money by the system's own definition.
--
-- TWO CONSEQUENCES, both live:
--   1. Reported total chip supply has been understated by ~4.35 million.
--   2. Every chip moving into or out of club_wallets - which is where rake
--      lands, continuously - registers as unexplained drift. That is the
--      persistent hourly noise: mean +160 with a standard deviation of 970
--      over 24 hours, 10 intervals positive and 12 negative. The 23:05
--      critical said "SAME-SIGN across consecutive intervals (a leak
--      persists)" about a series whose sign flips almost every hour.
--
-- Verified by arithmetic before changing anything: at 23:05 the component
-- deltas sum to 2,944.34 and `unexplained` recorded 1,102.07. The difference
-- is 1,842.27, exactly that interval's club_wallets movement.
--
...
-- critical and poison the engine deploy gate's trailing-4h window - the same
-- way my own corrections did earlier today (see
-- the_two_intervals_a_correction_made_unmeasurable). So this migration writes
-- the first snapshot under the new definition itself, with unexplained NULL
-- and the reason recorded in ca_supply_snapshot_classifications. The next
-- scheduled run compares against that row and sees an ordinary interval.
-- ═══════════════════════════════════════════════════════════════════════════
