...
-- reports a defect already fixed is the same disease as one that misses a real
-- defect: both teach people to stop reading it.
--
...
-- the rake taken from the pot. Measured after: 0% of 232 raked hands.
...
COMMENT ON FUNCTION public.fn_ca_rake_distribution_mismatch(interval) IS
  'Hands where the CHIP-MOVING rake legs sum to more than the rake taken from the pot. club_accumulator is excluded: since the 2026-09-02 ruling it records the club share accruing toward the weekly settlement and moves no chips.';
