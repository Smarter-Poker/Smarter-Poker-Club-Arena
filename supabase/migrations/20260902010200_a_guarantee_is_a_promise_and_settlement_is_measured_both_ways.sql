...
--
-- Dan asked: "we never have to worry about wrong amounts getting paid out or
-- users not getting paid out?!" The honest answer was no, and the reason the
-- platform could not answer it is that every settlement check built so far -
-- mine included - only looked at OVERpayment. Nothing watched the direction
-- that hurts a player.
--
...
-- UNDERPAID by 1,703.00 chips. All six share one signature -
--
...
--
-- A guarantee is a promise that the pool is topped up to a floor if the field
-- falls short. These events paid out only what the field put in. The
-- advertised number and the paid number are different, and the difference is
-- owed to players.
--
-- financial_alerts already carries a Tournament.guarantee_not_met source. It
-- fired once, on 2026-08-31, and nothing acted on it - which is the theme of
-- this whole sweep, and why this becomes a ratchet instead of another log line.
--
-- This moves no money. Funding the overlay is Dan's call.
-- ═══════════════════════════════════════════════════════════════════════════
...
COMMENT ON FUNCTION public.fn_ca_tournament_settlement_mismatch(interval) IS
  'Completed tournaments where credits do not match what the event owed: GREATEST(prize_pool, guaranteed_prize). Reports BOTH directions - underpaid is a player who did not get what was promised, overpaid is chips created. Excludes satellites (paid in seats) and Spins (club-funded multiplier).';
