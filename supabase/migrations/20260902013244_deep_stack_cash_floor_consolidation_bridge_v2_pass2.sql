...
-- Applied to production as 20260902013244; recorded so the repo matches the
-- migration list. Do NOT re-apply; it is already live.
-- Pass 2 of the v2 consolidation bridge (see
-- deep_stack_cash_floor_consolidation_bridge_v2): identical body, run again
-- because pass 1 filled the 45 most-seated tables to target (fields of 4+
-- went 2 -> 26, 74 horses seated) and the remaining lonely single-horse
-- tables sat beyond the LIMIT. Idempotent: tables at target are skipped.
DO $$
DECLARE
...
BEGIN
  ...
  FOR v_t IN
    SELECT t.id, t.name, t.max_players, t.big_blind, t.min_buy_in, t.max_buy_in,
     ...
     WHERE t.club_id = v_club AND t.tournament_id IS NULL
       ...
       AND (SELECT count(*) FROM table_seats ts WHERE ts.table_id=t.id AND ts.left_at IS NULL)
           < LEAST(t.max_players - 1, 5)
  ...
  RAISE NOTICE 'consolidation v2 pass2: % horses seated, % refused; real fields % -> %',
