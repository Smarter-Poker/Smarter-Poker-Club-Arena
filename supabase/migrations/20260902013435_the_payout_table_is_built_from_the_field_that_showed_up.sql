...
-- Applied to production 20260902013435; the .sql file was never committed at the
-- time (chip-std phase 1.5 mirror, docs/changelog/2026-09-02-chip-std-p1-mirror.md).
-- Content is byte-exact to what ran. Do NOT re-apply; it is already live.

-- The payout structure has to be built from the field that ACTUALLY showed up,
-- not the one hoped for at creation - which is why it is set in the same write
-- that locks registration, alongside the overlay. One atomic statement now
-- decides three things that must agree: the field is closed, the pool meets
-- the guarantee, and the payout table covers the top payout_percent of exactly
-- the players who entered.
--
-- Existing structures are NOT overwritten unless they are the placeholder. A
-- club that has deliberately configured its own table keeps it; the 53,293
-- events carrying the default [{"place":1,"percentage":100}] regardless of
-- field size get a real one.
...
BEGIN
  ...
  IF v_entrants > 0 THEN
    ...
    -- only replace the placeholder; a deliberately configured table stands
    IF NEW.payout_structure IS NULL
       OR jsonb_array_length(COALESCE(NEW.payout_structure,'[]'::jsonb)) = 0
       OR NEW.payout_structure = '[{"place":1,"percentage":100}]'::jsonb
       OR jsonb_array_length(NEW.payout_structure) <> v_places
    THEN
      NEW.payout_structure := public.fn_ca_payout_structure(
                                v_entrants, COALESCE(NEW.payout_percent,10));
    END IF;
  END IF;
...
COMMENT ON FUNCTION public.fn_ca_fund_overlay_on_lock() IS
  'Fires in the same write that closes registration and settles three things that must agree: the payout table is rebuilt for the field that actually entered (top payout_percent of them), the prize pool is topped to the guarantee out of the main bank, and the draw is journalled once, naming the tournament and the shortfall. Idempotent; all-or-nothing on the bank draw.';
