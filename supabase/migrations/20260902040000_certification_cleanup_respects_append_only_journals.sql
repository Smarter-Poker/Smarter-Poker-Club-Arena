-- The production certification sweeps use short-lived identities whose email
-- marker is checked while the Auth row is locked. Diamond transactions became
-- append-only after this cleanup function was introduced, so cleanup began
-- failing at its first journal DELETE and left every later fixture row behind.
--
-- This does not relax the append-only trigger. It supplies the trigger's
-- required, transaction-local maintenance reason only after the function has
-- proved that the target is a reserved certification identity.
...
BEGIN
  ...
  -- `fn_ca_journal_append_only` requires a non-empty incident reference for
  -- every authorized UPDATE/DELETE. `true` makes this transaction-local, so
  -- the permission cannot leak into the next PostgREST request on the session.
  PERFORM set_config(
    'app.ledger_maintenance',
    'certification-cleanup:' || p_user_id::text,
    true
  );
  ...
  -- audit_trail.actor_id intentionally restricts Auth deletion for real users.
  -- The locked reserved marker above is the narrow certification exception.
...
COMMENT ON FUNCTION public.cleanup_reserved_certification_account(uuid) IS
  'Removes Only Reserved ca-customization-cert-* Test Identities; Journal Maintenance Context Is Transaction-Local.';
