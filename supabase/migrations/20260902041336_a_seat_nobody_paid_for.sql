...
-- ---------------------------------------------------------------------------
-- MEASURED AFTER APPLY, and appended rather than folded into the text above so
-- that everything above this line is what actually ran.
--
-- Applied to production as 20260902041336. Function body verified against
-- pg_proc.prosrc by md5: 525d8b466979e1eb5cb764be8e867973, 5912 bytes.
--
--   fn_uncollected_entry_check(24)  1453 ms   12,678 seats checked   0 unfunded
--   fn_uncollected_entry_check(48)  1386 ms                          0 unfunded
--
-- THE INDEX HELPED LESS THAN SECTION 2 PREDICTS, and the honest number belongs
-- next to the prediction. The planner does now take
-- idx_tournament_players_registered_at (Index Scan, 14,489 rows in 132ms
-- instead of a parallel seq scan discarding 93,727), but total runtime moved
-- only 1.85s -> 1.45s: the remaining second is the wallet_transactions read,
-- not the roster read. The index is still worth keeping - it is the term that
-- grows with the roster - but it is not what makes this check fast, and a
-- future hand hunting a slow run should look at wallet_transactions first.
--
-- Backfill result: 23 satellite seat records, 4,600.00 chips, every position
-- reconstructed (none left NULL).
