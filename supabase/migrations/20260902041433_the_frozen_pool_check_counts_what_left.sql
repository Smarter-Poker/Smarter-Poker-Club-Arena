...
-- Operator telemetry, never a browser call, and not an RLS policy helper
-- (verified: no pg_policy expression references it). Production already holds
-- exactly these grants - CREATE OR REPLACE does not touch an ACL - but the
-- definer gate reads this file, and a bare SECURITY DEFINER declaration reads
-- as default PUBLIC EXECUTE. Stated explicitly so the file says what the
-- database already does. GRANT/REVOKE do not fire pgrst_ddl_watch, so this
