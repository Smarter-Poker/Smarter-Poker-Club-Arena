-- RAKE LEDGER CLUB ISOLATION — HARD GUARD (Dan 2026-09-02). Recorded
-- byte-exact from schema_migrations. Applied to production 20260902044316.
-- Dan, verbatim: "NO OTHER CLUBS DATA CAN EVER EVER EVER CREEP IN OR BE
-- ENTERED INTO ANY OTHER CLUBS LEDGERS! ... LAYERS OF PROTECTION". A
-- BEFORE INSERT/UPDATE trigger refuses any rake row whose club_id is not
-- the club/union that owns the game (union-shared tables may attribute to
-- member clubs; a club-owned table's rake is sealed to that club, so a
-- standalone club like Deep Stack is fully isolated). fn_rake_club_
-- isolation_violations() is the standing audit (0 = healthy). Audit at
-- install: 0 real violations; the 950k union-shared-table rows are the
-- legitimate union model, not contamination.
