...
-- Applied to production 20260902050738; the .sql file was never committed at the
-- time (chip-std phase 1.5 mirror, docs/changelog/2026-09-02-chip-std-p1-mirror.md).
-- Content is byte-exact to what ran. Do NOT re-apply; it is already live.

-- The dry run returned zero events against a set I had just measured at 34.
-- The LIMIT was bounding the CANDIDATE rows - every completed MTT carrying a
-- guarantee, ordered by ended_at - and the 34 that are actually short fall
-- outside the first 500 by date. The shortfall test ran afterwards, on rows
-- that had already been discarded.
--
-- This is the identical mistake as "a limit must bound the work not the view",
-- which I fixed in another function earlier tonight and then wrote again here
-- four hours later. The rule: a LIMIT belongs on the rows that QUALIFY, never
-- on the rows you are about to test.
...
BEGIN
  FOR t IN
    ...
    SELECT * FROM candidates
     WHERE paid_all < guaranteed_prize - 0.01     -- qualify FIRST
     ORDER BY ended_at
     LIMIT GREATEST(p_limit,1)                    -- then bound
  LOOP
