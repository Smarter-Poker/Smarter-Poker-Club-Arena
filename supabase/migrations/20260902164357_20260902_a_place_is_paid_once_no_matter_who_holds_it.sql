...
-- Applied to production 20260902164357; the .sql file was never committed at the
...
-- paid PER USER. Those agree only while a finishing place never changes hands.
-- When it does -- a force-complete restating the order, a recovery sweep
-- re-deriving ranks, an agent correcting a finish -- the function looks up who
-- holds place N NOW, sees that this user has been paid nothing at all, and pays
-- them the whole place in full. The player paid earlier keeps theirs. The place
-- is paid twice and the pool is breached.
--
-- Measured before this was written:
--
--   81 places across 49 tournaments paid to two different users
--   21,206.93 chips of excess
--   every credit landed between 2026-08-31 13:37 and today
--
-- The worst single event is "Sunday $200 Deep Stack" (dfae9288): 44,640.00 pool,
-- 62,841.60 paid. All nine paying places were paid twice -- structure to one
-- player, reconcile to another, the second exactly 2.1379x the first, because
-- the structure had run against a 20,880.00 pool and the reconciler against the
-- full one. Place 1: 6,264.00 to be61d864, then 13,392.00 to 2d48c8ed.
--
-- THE FIX. The reconciler now caps a top-up by the GREATER of what the current
-- holder was paid and what the PLACE was paid to anybody. A place can never be
-- paid more than the structure says it is worth, whoever ends up holding it.
--
-- The case is reported rather than silently absorbed: a new issue,
-- 'place_paid_to_a_different_player', names both users and both amounts so the
-- next reader can see a place changed hands after it was settled. Paying the
-- new holder anyway is sometimes the right answer -- that is a decision under
-- CLAUDE.md 10.9, made deliberately with the old payment in view, and never a
-- side effect of an hourly sweep.
--
-- That issue joins 'overpaid' and 'no_finisher_recorded' in the accepted set,
-- so an event whose alert has been resolved with a written resolution stays
-- quiet instead of re-raising critical every hour.
--
-- NOT CLAWED BACK: the 21,206.93 already out the door stays with the players
-- who received it. CLAUDE.md 10.9 rule 3 -- nothing is taken back from a player
-- for our defect. This stops the bleed; it does not reopen what has been paid.
--
