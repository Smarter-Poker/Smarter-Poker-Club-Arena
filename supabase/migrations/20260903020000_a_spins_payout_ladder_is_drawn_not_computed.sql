...
--
-- LIVE MONEY BUG. 32 games, 1,592 chips, from 2026-09-02 04:13 UTC until this
-- was applied at 17:09. Second place was paid nothing on every high
-- multiplier. ALREADY APPLIED TO PRODUCTION as
-- `a_spins_payout_ladder_is_drawn_not_computed_from_the_field`; this file is
-- the record of it.
--
-- WHAT HAPPENED
--
-- fn_ca_fund_overlay_on_lock is a BEFORE UPDATE trigger on tournaments that
-- fires when status moves REGISTERING -> RUNNING. That is exactly the moment
-- the engine writes the Spin draw, so the trigger saw the drawn ladder and
-- rewrote it.
--
...
--
-- On a three-handed Spin that rounds to ONE paid place. The condition
-- `jsonb_array_length(v_existing) <> v_places` is then TRUE for every high
-- multiplier, because a 10x pays TWO. So the engine's
-- [{place:1,80},{place:2,20}] was replaced by fn_ca_payout_structure(3, 10),
-- which returns exactly:
--
--   [{"place": 1, "percentage": 100.0000000000000000}]
--
-- That string is BYTE-IDENTICAL to what sat on every affected row. The root
-- cause was confirmed against production by calling the function, not
-- inferred from reading it.
--
-- WHY IT STARTED SUDDENLY, AT 04:13 ON 2026-09-02
--
-- The trigger's own comment says the payout rewrite stood down while it
-- sorted BEFORE fn_guard_managed_game_lifecycle, and was to be "re-enabled
-- once the trigger is renamed to sort after the guard". It is now named
-- `zz_ca_fund_overlay_on_lock`. The rename re-enabled the rewrite. Every 10x
-- since paid 100/0 instead of 80/20. Day by day at 10x: 08-21 to 09-01 was
-- 246 games, all split correctly; 09-02 was 33 games, 29 of them wrong.
--
-- WHY #2645 DID NOT FIX IT
--
-- #2645 was a real bug and a real fix - the engine's per-field copy dropped
-- payout_structure out of tournamentCache. But it is a different bug. The
-- engine writes the correct ladder to the row; this trigger overwrites it
-- afterwards, in the same statement. A 10x that started at 17:07:42, five
-- minutes AFTER #2645 deployed, still carried the 100% stamp. That is what
-- sent me looking past the engine.
--
-- THE FIX
--
-- A Spin's ladder is a property of the multiplier the wheel drew, from
-- SPIN_TIERS: 2x/3x/4x/5x pay one place, 10x pays 80/20, the higher tiers pay
-- three. It is not a percentage of a three-player field and never was.
--
-- The overlay half of the trigger is deliberately untouched - a Spin carries
-- no guarantee, so it returns at the `v_short <= 0` line exactly as before.
-- The only change is the `NEW.variant IS DISTINCT FROM 'spin'` guard around
-- section 1; everything else is byte-identical to the deployed function.
--
-- HORSES: nothing here reads is_horse. Every seat is paid from the same
-- drawn ladder (CLAUDE.md 10.5).
--
-- ROLLBACK: re-apply the previous definition, dropping the variant guard.
-- Do not do that without replacing it with another way to keep the trigger
-- off a Spin's ladder.
--

-- Body below is pg_get_functiondef() of the deployed function, md5
-- c48478752a8b74b22317048a310f8661, 6104 bytes, read back from production
-- after applying. The ONLY difference from the previous definition is the
-- `IF NEW.variant IS DISTINCT FROM 'spin' THEN` wrapper on section 1 and its
-- comment.
