-- FIND A PLAYER, ROUND THREE: DROP THE COUNT, AND KNOW YOUR OWN SEAT
--
-- Two corrections to 20260902214500.
--
-- 1. hidden_count becomes has_hidden.
--
--    The panel needs to tell the viewer that the club list is incomplete,
--    otherwise it lies by omission. It does NOT need to say how many clubs it
--    withheld. A count is a membership-existence oracle: it is differential
--    across viewers, so two accounts comparing what each of them sees can
--    subtract and infer which specific gated clubs a stranger belongs to. On a
--    platform with agent hierarchies competing for the same players, "this
--    player is in three private clubs" is itself worth money.
--
--    Rendering the count is not the leak - RETURNING it is, because any viewer
--    can read the network response. So the fix has to be here, in the payload,
--    not in the component. A boolean carries the whole of the honest signal
--    ("there is more you cannot see") and none of the arithmetic.
--
-- 2. access_action learns 'play'.
--
--    fn_get_table_watch_access has always returned 'play' when the caller is
--    seated at the table, but fn_search_players never did: it derived the
--    action from club membership alone, so your own live game came back as
--    'watch'. The card therefore read "Observe Table" for a seat you were
--    sitting in, and the client - which navigates on can_watch - sent you to
--    your own table as a spectator. Now the search payload agrees with the
--    click-time check, and the client has a 'play' branch to route on.
...
BEGIN
  ...
  viewer_clubs AS MATERIALIZED (
    ...
    -- Tables the VIEWER is sitting at. Used to mark their own game 'play'
    -- instead of 'watch', so the card offers a seat back rather than a
    -- spectator slot on a hand they are in.
    SELECT ts.table_id
      ...
      FROM (
        SELECT DISTINCT ON (live.table_id) live.table_id,
          jsonb_build_object(
            ...
            -- A seat you are sitting in is always openable.
    ...
    LEFT JOIN LATERAL (
      ...
      SELECT
        ...
        -- Boolean, never a count. See the header.
...
BEGIN
  ...
  -- The count must not come back: a boolean is the whole of the honest signal.
  IF position('hidden_count' IN v_src) > 0 THEN
    RAISE EXCEPTION 'affiliations returns a count of hidden clubs again';
  END IF;
  IF position('has_hidden' IN v_src) = 0 THEN
    RAISE EXCEPTION 'affiliations lost has_hidden, so the panel cannot admit it is incomplete';
  END IF;

  -- Everything 20260902214500 fixed must still hold.
  ...
  -- Your own seat must be openable as a seat, not as a spectator slot.
