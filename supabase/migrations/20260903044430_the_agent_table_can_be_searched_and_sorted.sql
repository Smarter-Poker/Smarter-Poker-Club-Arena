-- 20260903044430_the_agent_table_can_be_searched_and_sorted.sql
-- RECOVERED 2026-09-03 from supabase_migrations.schema_migrations.
-- This migration was APPLIED to production on 2026-09-03 at 04:44:30 UTC but was never committed, so the repo
...
-- THE AGENT TABLE CAN BE SEARCHED AND SORTED.
--
-- Same split as the other two helpers: totals BEFORE the filter, count AFTER
-- it. A share stays a share of the club while the operator types.
--
-- Two details that were load-bearing here and are easy to lose:
--
--   The synthetic rows (Unassigned, Unlisted Recipients) still sink to the
--   bottom under EVERY sort. They are not agents; sorting by name must not
--   file "Unassigned" under U among real people.
--
--   The final jsonb_agg re-sorts the page. It used to repeat the ORDER BY,
--   which meant the page order and the output order were two copies of one
--   rule that could drift apart. It now orders by the row number the page
--   assigned, so there is exactly one sort in this function.
...
BEGIN
  ...
  WITH RECURSIVE ok_days AS MATERIALIZED (
    ...
    SELECT p.* FROM page p ORDER BY p.rn
     OFFSET GREATEST(COALESCE(p_offset,0),0)
      LIMIT GREATEST(LEAST(COALESCE(p_limit,50),200),1)
  )
  SELECT jsonb_build_object(
    'rows', COALESCE((SELECT jsonb_agg((to_jsonb(s) - 'total_rows' - 'rn') ORDER BY s.rn)
                        FROM sliced s), '[]'::jsonb),
    'total', COALESCE((SELECT MAX(s.total_rows) FROM sliced s), 0),
