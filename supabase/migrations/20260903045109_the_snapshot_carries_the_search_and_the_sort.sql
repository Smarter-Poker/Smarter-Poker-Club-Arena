-- 20260903045109_the_snapshot_carries_the_search_and_the_sort.sql
-- RECOVERED 2026-09-03 from supabase_migrations.schema_migrations.
-- This migration was APPLIED to production on 2026-09-03 at 04:51:09 UTC but was never committed, so the repo
...
-- it to reach the helper that does the work. They are echoed back in the
-- payload as applied_search / applied_sort: the client can then tell "the
-- server ran my search and three rows matched" apart from "the server ignored
-- a parameter it did not understand and returned everything".
--
-- The old eight-argument version is dropped rather than left beside this one.
