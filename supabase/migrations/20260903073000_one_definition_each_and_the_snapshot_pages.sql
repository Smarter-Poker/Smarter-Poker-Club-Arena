-- ONE DEFINITION EACH, AND THE SNAPSHOT PAGES.
--
-- ca_rake_snapshot now takes a page offset and reads {rows,total,total_direct}
-- from the helpers instead of a bare array.
--
-- breakdown_total comes from total_direct - the sum over EVERY row - and NOT
-- from summing the page. That distinction is the whole risk in this change: the
-- old code summed the array it was handed, which was right while the array was
-- the entire list, and becomes silently wrong the moment it is a page. Shares
-- would have been percentages of the first fifty rows and would have moved
-- every time the operator pressed Load More.
--
-- The previous seven-argument arity is dropped at the end for the same reason
-- the helpers' were: adding a defaulted parameter creates a SECOND definition
-- rather than replacing the first, and the old call shape then matches both.
...
BEGIN
  ...
  ELSE
    ...
    -- The agent table reads live for anything the rollup has not finished; the
    -- headline is an hourly job. Between runs the column can exceed the total.
