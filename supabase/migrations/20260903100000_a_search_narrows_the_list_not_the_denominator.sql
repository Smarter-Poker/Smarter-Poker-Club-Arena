...
--
-- Search and sort have to be server-side because the lists are paged.
-- Filtering the fifty rows that happen to be loaded, out of two hundred, and
-- calling it a search finds only what was already fetched.
--
-- THE DECISION THAT MATTERS is what a share means once a filter is on. Two
-- totals stop being the same number here, and this is where a filter quietly
-- corrupts a page:
--
...
--                  OF THE CLUB. Were it the filtered sum, searching a single
--                  agent would put them at 100 percent - true "of the rows
...
-- they can no longer both come from one window function.
--
-- The count is COUNTED rather than read from a window over the page. A window
-- reads correctly right up until the slice is empty: ask for offset 60 of a
-- 33 row result and it answers "total 0" - nothing matches, while matches
-- plainly exist. Paging made that reachable; search makes it easy, because a
-- filter can shrink the set under an offset the caller is still holding.
--
-- Sorting is a whitelist mapped in SQL and never interpolated. p_sort reaching
-- an ORDER BY as text would be an injection with extra steps.
...
-- The narrower versions are dropped rather than left beside these. Every
-- parameter past the first few carries a default, so an older call would match
-- both signatures and Postgres would refuse it as ambiguous.
