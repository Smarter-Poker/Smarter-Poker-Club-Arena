-- THE PROMO DISBURSEMENT SPEAKS THE LEDGER'S OWN WORD FOR PROMO.
--
-- fn_promo_disburse declared its legs as 'promo_disbursement', which is not in
-- chip_ledger_category_check, so fn_ca_declare_ledger refused it before a chip
-- moved: "category promo_disbursement is not in the ledger vocabulary - add it
-- to chip_ledger_category_check FIRST". The vocabulary guard did exactly what
-- it exists to do.
--
-- The ledger already has a word for this money: 'promo', the same word the BBJ
-- sweep uses when it moves the slice into the float. A disbursement is that
-- float going back out, so it is the same category with a different direction,
-- and widening the vocabulary would buy nothing but an ACCESS EXCLUSIVE lock on
-- chip_ledger during live play.
--
-- The human-readable distinction stays where people read it: the
-- chip_transactions receipt is still transaction_type 'promo_disbursement',
-- with the owner who authorised it, the op id, and both balances after.
