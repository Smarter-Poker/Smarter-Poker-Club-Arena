-- ═══════════════════════════════════════════════════════════════════════════
-- A DEPARTED SEAT IS SETTLED AGAINST A WALLET ON CASH TABLES ONLY, AND THE
-- ONE TOURNAMENT DEBIT IT MADE IS REVERSED (chip standard, 2026-09-04).
--
-- 20260904120601 taught the hand write to settle a seat that left mid-hand
-- against the club wallet the seat cashed out to. It did not say "cash
-- tables only". At 13:04:21 UTC, seven minutes after the delta-mode engine
-- began serving, a table balance moved a player off tournament table
-- 4adf3a95 mid-hand; the write found the seat gone and debited the player's
-- delta - 230 TOURNAMENT chips - from their SHARK CLUB wallet as 230 real
-- chips (key late_seat_settle:c525bb6c-...:2b5a987d-...). Tournament chips
-- are play chips; atomic_credit_wallet_and_log refuses a tournament table
-- for exactly this reason. One row, found by reading the departed register
-- in the first ten minutes.
--
-- 1. The function is re-created with the guard: the wallet settlement runs
--    only when tables.tournament_id IS NULL; a tournament table with a
--    missing seat is refused whole, as it was before 20260904120601 (the
--    engine's tournament conservation gate and the elimination path own it).
-- 2. The 230.00 is returned to the wallet it was taken from, keyed
--    late_seat_settle_reversal:<hand>:<user>, journalled table_stack ->
--    player_wallet as a correction, with a chip_transactions row.
-- 3. The ACL is restated on the branch for check-definer-authorization.
-- ═══════════════════════════════════════════════════════════════════════════
...
-- 2. The reversal, asserted against the one row it exists for.
DO $$
DECLARE
  v_user uuid := '2b5a987d-6c5e-4857-b817-dceb1c5e4e5a';
  v_club uuid := 'a41434bb-8d0c-400a-8f0d-e8b3d65afed4';
  v_table uuid := '4adf3a95-2840-4f3f-bb4e-62efea77a52a';
  v_key text := 'late_seat_settle_reversal:c525bb6c-8e47-dbcf-e946-048715be3977:2b5a987d-6c5e-4857-b817-dceb1c5e4e5a';
  v_n int; v_after numeric; v_claimed int;
BEGIN
  SELECT count(*) INTO v_n FROM public.chip_ledger
   WHERE idempotency_key LIKE 'late_seat_settle:%' AND from_type = 'player_wallet' AND to_type = 'table_stack'
     AND to_entity_id = v_table AND from_entity_id = v_user AND amount = 230.00;
  IF v_n <> 1 THEN
    RAISE EXCEPTION 'expected exactly one 230.00 late_seat_settle debit for this player on this table, found %', v_n;
  END IF;
  IF (SELECT count(*) FROM public.chip_ledger WHERE idempotency_key LIKE 'late_seat_settle:%'
        AND EXISTS (SELECT 1 FROM public.tables t WHERE t.id = chip_ledger.to_entity_id AND t.tournament_id IS NOT NULL)) <> 1 THEN
    RAISE EXCEPTION 'more than one tournament-table late_seat_settle row exists; this migration reverses exactly one';
  END IF;
  INSERT INTO public.wallet_credit_idempotency (key, user_id, amount) VALUES (v_key, v_user, 230.00)
  ON CONFLICT (key) DO NOTHING;
  GET DIAGNOSTICS v_claimed = ROW_COUNT;
  IF v_claimed = 0 THEN RAISE NOTICE 'already reversed'; RETURN; END IF;
  PERFORM public.fn_ca_declare_ledger('correction', 'table_stack', v_table, NULL, v_key, NULL);
  UPDATE public.club_members SET chip_balance = COALESCE(chip_balance, 0) + 230.00, updated_at = now()
   WHERE user_id = v_user AND club_id = v_club RETURNING chip_balance INTO v_after;
  IF v_after IS NULL THEN RAISE EXCEPTION 'wallet not found for the reversal'; END IF;
  PERFORM set_config('app.ledger_category', '', true); PERFORM set_config('app.ledger_counterparty', '', true);
  PERFORM set_config('app.ledger_counterparty_entity', '', true); PERFORM set_config('app.ledger_idempotency_key', '', true);
  INSERT INTO public.chip_transactions (club_id, to_user_id, amount, transaction_type, notes, table_id, balance_after, metadata)
  VALUES (v_club, v_user, 230.00, 'late_seat_debit_reversed',
          'Returned: 230.00 chips were taken from this wallet at 13:04 UTC for a tournament hand settled after a table move; tournament chips are play chips and nothing was owed',
          v_table, v_after, jsonb_build_object('key', v_key));
  IF NOT EXISTS (SELECT 1 FROM public.chip_ledger WHERE idempotency_key = v_key AND from_type = 'table_stack' AND to_type = 'player_wallet' AND amount = 230.00) THEN
    RAISE EXCEPTION 'the reversal left no journal row';
  END IF;
  IF position('tb.tournament_id IS NOT NULL' IN pg_get_functiondef('public.fn_ca_settle_hand_stacks_absolute'::regproc)) = 0 THEN
    RAISE EXCEPTION 'the tournament guard did not land';
  END IF;
END $$;
