-- 20260905083756_the_break_counts_orbits_not_only_minutes.sql
--
-- THE BREAK COUNTS ORBITS, NOT ONLY MINUTES (OPORD 1.4 section 18.3)
--
-- 18.3 CLOSE: "act only when the condition has held for the full hysteresis
-- window, which is the shorter of 2 completed orbits on the candidate table
-- or 5 minutes (tracked on the row as break_eligible_since, cleared the
-- moment the condition fails)." The tick shipped with the clock only. Read on
-- production 2026-09-05 02:35 CDT: 60 breaks completed in six hours against
-- 81 feeders opened, and the fleet refills a chair in about thirty seconds,
-- so a candidate that dealt twenty hands with the whole game fitting
-- elsewhere still waited the fifth minute, and usually lost its eligibility
-- to a refill before it got there. Games grew and rarely shrank.
--
-- WHAT CHANGES. In step 5 (BREAK) the second arm of the eligibility test
-- becomes: five minutes have passed, OR the candidate has dealt at least
-- 2 x (its seated count) hands since break_eligible_since. An orbit is one
-- hand per seated player. A candidate with fewer than two seated deals no
-- hands, so for it only the clock runs; nothing breaks faster than it can
-- deal. The hand count is one indexed read of hand_history
-- (idx_hand_history_table_created_id), once per tick, only for a game that
-- has an eligible candidate. The table_break_started payload now carries
-- eligible_since, hands_since_eligible and orbit so the event log says which
-- arm fired.
--
-- TWO REPAIRS RIDE ALONG, both found by probing this change against live
-- boards (rolled back). (1) RECONCILE: a cluster table with status =
-- 'closed' and lifecycle still 'live' (15 of them; the operator's
-- close-game action and the pre-controller close path write status only)
-- now has its lifecycle follow its status when nobody is on it, with a
-- 'lifecycle_followed_status' event. (2) ROLES: a game with a live feeder
-- and no live main (found: NLH 0.05/0.10 Classic, disabled 2026-09-04,
-- Main 1 closed by status, five players still on the feeder) promotes its
-- oldest live feeder to Main 1, per 1.3 s9.2 'oldest live table is Main 1'.
--
-- THE WHOLE FUNCTION IS RE-DECLARED, deliberately. The live body had drifted
-- from every file in the repo: 20260905060000 declared it, then 20260905041557
-- (applied later, lower version) and the Gate 5 snapshot block were applied
-- as string substitutions on the live source, and no file held the result.
-- This file does. The guard below asserts the live body is exactly the one
-- this edit was made against (md5 4aa56484ead103f09cf33a109f62621c) and
-- aborts otherwise, so a concurrent change to the tick cannot be silently
-- overwritten. The post-assert pins the new body's md5 (a7636d2912f09e74937c1b5a0331002f).
--
-- ROLLBACK: re-apply the body from 20260905060000, then the two substitutions
-- in 20260905041557 and 20260905033729 (or `git show` this file's parent and
-- take tick_body from it).
--
-- One transaction (production DDL policy, CLAUDE.md section 2). GRANT/REVOKE
-- do not reload the schema cache; the CREATE OR REPLACE does, once.
...
BEGIN
  ...
  IF v_md5 IS DISTINCT FROM '4aa56484ead103f09cf33a109f62621c' THEN
    RAISE EXCEPTION 'fn_cash_cluster_tick live body is % (expected 4aa56484ead103f09cf33a109f62621c) - the tick changed under this migration; rebase it on the live source', coalesce(v_md5, 'absent');
  END IF;
END
$guard$;
...
BEGIN
  SELECT md5(p.prosrc), p.prosrc INTO v_md5, v_src FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = 'fn_cash_cluster_tick';
  IF v_md5 <> 'a7636d2912f09e74937c1b5a0331002f' THEN
    RAISE EXCEPTION 'fn_cash_cluster_tick body after apply is %, expected a7636d2912f09e74937c1b5a0331002f', v_md5;
  END IF;
  IF position('v_hands >= 2 * v_orbit' IN v_src) = 0 THEN
    RAISE EXCEPTION 'fn_cash_cluster_tick does not count orbits';
  END IF;
END
$post$;
