...
--
-- ONE CHAIR PER PLAYER PER GAME, EVERY TICK (2026-09-05, follows 20260905083756)
--
-- READ, NOT ASSUMED (03:58 CDT): cash_game_roster live rows 192, seats on
-- cluster tables 193, and the one-row gap is one horse holding two chairs in
-- NLH 0.25/0.50 Classic - seat 2 on Main 1 since 2026-09-04 17:01 (59.70) and
-- seat 5 on the feeder since 19:17 (74.97), both tables `waiting`. Earlier in
-- the night the same measure read 298 vs 303: five such pairs. The roster is
-- per player, so every pair read as one row of "drift"; there is no roster
-- defect. The pairs predate the one-seat-per-game door (20260905053000 and
-- the fleet's ONE SEAT PER GAME gate), and the tick settled a second chair
-- only when its table was BREAKING.
--
-- WHAT CHANGES. RECONCILE gains a pass over every player with more than one
-- live chair in the game. The oldest chair is theirs. Each newer chair goes
-- home through the same door as a Leave: on a `waiting` table (between hands,
-- nothing dealing) it is cashed out now with the same three calls the breaking
-- branch and fn_cashout_seats_for_closing_table make
-- (fn_ensure_club_wallet, fn_ca_declare_ledger, atomic_seat_cashout_locked
-- 'forced' - idempotent by key, credits club_members.chip_balance); on a
-- running table it is flagged leave_pending and the engine cashes it out at
-- the hand boundary through its own settlement. Events:
-- second_chair_cashed_out {where: reconcile} / second_chair_leave_pending.
--
-- MONEY (CLAUDE.md 10.9). One player affected today: the horse above; 74.97
-- returns from the feeder chair to its club wallet by the platform's own
-- idempotent cash-out. Nothing is taken from anyone; the Main 1 chair and its
-- 59.70 are untouched. Proven in a rolled-back transaction first (the probe
-- is in docs/changelog/2026-09-05-the-break-counts-orbits-not-only-minutes.md).
--
-- The guard asserts the live body is 20260905083756's (a7636d2912f09e74937c1b5a0331002f) and the
-- post-assert pins the new one (7bd855d7d10b6517c013b63aef5b3b1c). One transaction.
...
BEGIN
  ...
  IF v_md5 IS DISTINCT FROM 'a7636d2912f09e74937c1b5a0331002f' THEN
    RAISE EXCEPTION 'fn_cash_cluster_tick live body is % (expected a7636d2912f09e74937c1b5a0331002f) - rebase this migration on the live source', coalesce(v_md5, 'absent');
  END IF;
END
$guard$;
...
BEGIN
  SELECT md5(p.prosrc), p.prosrc INTO v_md5, v_src FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = 'fn_cash_cluster_tick';
  IF v_md5 <> '7bd855d7d10b6517c013b63aef5b3b1c' THEN
    RAISE EXCEPTION 'fn_cash_cluster_tick body after apply is %, expected 7bd855d7d10b6517c013b63aef5b3b1c', v_md5;
  END IF;
  IF position('second_chair_leave_pending' IN v_src) = 0 THEN
    RAISE EXCEPTION 'fn_cash_cluster_tick does not reconcile a second chair';
  END IF;
END
$post$;
