...
-- Applied to production 20260905162114; the .sql file was never committed at the
...
--
-- LEDGER ROW
--   version     20260905162114   (the stamp IS the apply time, UTC: 2026-09-05 16:21:14)
--   name        add_index_drop_tracking
--   created_by  daniel@bekavactrading.com
--   statements  1 statement(s), 1181 bytes
--
-- WHY THE FILENAME VERSION WAS NOT RESERVED
--
-- CLAUDE.md 4.5 says never hand-pick a migration version and always run
-- scripts/new-migration.mjs. A mirror is the one sanctioned exception, and the
-- exception is what makes it safe: 20260905162114 IS ALREADY IN
...
alter table ops_rollback.index_drops_20260905
  add column if not exists status text default 'pending',
  add column if not exists err text;

update ops_rollback.index_drops_20260905
   set status='dropped', dropped_at=now()
 where idx in ('idx_user_progress_status');
