-- 20260905194840_the_break_consolidates_a_thin_game_the_floor_was_never_a_bar.sql
--
-- THE BREAK CONSOLIDATES A THIN GAME; THE FLOOR WAS NEVER A BAR (2026-09-05 14:50 CDT)
--
...
-- tables (main1=9 main2=7 ... main7=1 main9=1 main10=0 main11=1 ... feeder=0),
-- PLO6 0.50/1 Classic with 1 player on 4 tables (main1=1 main2=0 main3=0
-- feeder=0), and the same shape on PLO5 0.50/1, PLO4 1/2, PLO5 1/2. The card
-- says the game's players (cluster_players counts every seat in the game,
-- correctly); the table you land on has one. And break_eligible_since was
-- NULL on every one of those tables: the BREAK rule never armed.
--
-- WHY. Step 5 required `v_seated_total >= v_floor * v_remaining_tables`:
-- break only if the remaining tables would average at or above the maintain
-- floor (3 on 6-max, 4 on 9-max). Breaking a table never makes the rest
-- shorter, so the clause can only refuse the breaks that matter most: 1 >=
-- 3 x 3 is never true, 42 >= 4 x 15 is never true. Games grew (R3 loop,
-- feeder over-count) and could not shrink. OPORD 1.4 18.3's floor is the
-- fleet's fill target, and Dan's rule is "fewer tables, more players at
-- each table" (StableHand.ts, NIGHT_MIN_PLAYERS).
--
-- WHAT CHANGES. (1) The floor clause is removed; the STRICT fit stays
-- (everyone fits into the rest AND the rest keeps a seat open, so the OPEN
-- rule cannot fire on the same board - 20260905040500). (2) The candidate
-- order prefers an EMPTY table (nobody to move), then the feeder, then the
-- highest main. (3) A candidate with nobody or one player has no hand to
-- protect: its hysteresis is 60 seconds instead of five minutes; two-plus
-- seated keeps the shorter of two orbits or five minutes (20260905083756).
--
-- Guard: live body must be bf55d7a456afba498070a3d6e62c2002; post: a2e3f01531e3df4fcb962ba18e294808. One transaction.
...
BEGIN
  ...
  IF v_md5 IS DISTINCT FROM 'bf55d7a456afba498070a3d6e62c2002' THEN
    RAISE EXCEPTION 'fn_cash_cluster_tick live body is % (expected bf55d7a456afba498070a3d6e62c2002) - rebase this migration on the live source', coalesce(v_md5, 'absent');
  END IF;
END
$guard$;
...
BEGIN
  SELECT md5(p.prosrc), p.prosrc INTO v_md5, v_src FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = 'fn_cash_cluster_tick';
  IF v_md5 <> 'a2e3f01531e3df4fcb962ba18e294808' THEN
    RAISE EXCEPTION 'fn_cash_cluster_tick body after apply is %, expected a2e3f01531e3df4fcb962ba18e294808', v_md5;
  END IF;
  IF position('v_seated_total >= v_floor * v_remaining_tables' IN v_src) > 0 THEN
    RAISE EXCEPTION 'fn_cash_cluster_tick still bars the break on the floor';
  END IF;
END
$post$;
