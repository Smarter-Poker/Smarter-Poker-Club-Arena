-- 20260905202112_the_headcount_column_is_the_seats.sql
--
-- THE HEADCOUNT COLUMN IS THE SEATS (2026-09-05 15:05 CDT)
--
-- Read live: 22 open cash tables with one or more seated players carried
-- tables.current_players = 0 (Short Deck 0.25/0.50 Action, PLO5 0.50/1
-- Classic, PLO8 0.25/0.50 Classic, ...: each 1 seated, 0 painted). The
-- engine writes current_players when it loads seats to deal; a table with one
-- player has no dealer, so nothing ever wrote it. Every per-table surface
-- (the game lobby's table list, the must-move list, the felt's header) paints
-- that column, so a table with a player on it read as empty.
--
-- WHAT CHANGES. RECONCILE in fn_cash_cluster_tick sets current_players to the
-- live seat count for the game's open tables, only where they differ. The
-- engine's own writes are the same number, so the two never fight. The
-- game-level card was already right (cluster_players counts seats directly).
--
-- Guard: live body must be a2e3f01531e3df4fcb962ba18e294808; post: 5433f92b25cb592c5d9de007b4a94110. One transaction.
...
BEGIN
  ...
  IF v_md5 IS DISTINCT FROM 'a2e3f01531e3df4fcb962ba18e294808' THEN
    RAISE EXCEPTION 'fn_cash_cluster_tick live body is % (expected a2e3f01531e3df4fcb962ba18e294808) - rebase this migration on the live source', coalesce(v_md5, 'absent');
  END IF;
END
$guard$;
...
BEGIN
  SELECT md5(p.prosrc), p.prosrc INTO v_md5, v_src FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = 'fn_cash_cluster_tick';
  IF v_md5 <> '5433f92b25cb592c5d9de007b4a94110' THEN
    RAISE EXCEPTION 'fn_cash_cluster_tick body after apply is %, expected 5433f92b25cb592c5d9de007b4a94110', v_md5;
  END IF;
  IF position('THE HEADCOUNT COLUMN IS THE SEATS' IN v_src) = 0 THEN
    RAISE EXCEPTION 'fn_cash_cluster_tick does not reconcile current_players';
  END IF;
END
$post$;
