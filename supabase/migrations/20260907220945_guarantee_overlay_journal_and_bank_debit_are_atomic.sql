-- A failed guarantee journal insert previously committed the bank debit and pool update.
-- Preserve existing guarantee and bank selection policy; make the original transaction atomic.
-- Isolated actual-trigger probes proved rollback, retries, replay and bank routing.
-- No historical balances changed.
BEGIN;
DO $guard$ BEGIN
IF md5(pg_get_functiondef('public.fn_ca_fund_overlay_on_lock()'::regprocedure)) <> '65ed23c8f8826749ca09b71ffa67be31' THEN
RAISE EXCEPTION 'Overlay function changed; rebase this correction before applying';
END IF; END $guard$;
