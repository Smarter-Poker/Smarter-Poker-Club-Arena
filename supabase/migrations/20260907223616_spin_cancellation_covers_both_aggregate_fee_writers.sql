-- Complete aggregate Spin fee coverage for both active original writers.
-- The prior correction covered entry booking. Prize settlement can also book
-- the same fee when entry booking has not done so; retain its source identity.
-- Expanded actual-function rollback probes cover both writers and failures.
BEGIN;
DO $guard$ BEGIN
IF md5(pg_get_functiondef('public.atomic_cancel_tournament(uuid,uuid)'::regprocedure)) <> '4aa9881a5e81cc0459dc75c0bf910c45' THEN
RAISE EXCEPTION 'Atomic cancellation function changed; rebase both-writer correction';
END IF; END $guard$;
    ...
    IF v_t.variant = 'spin' AND v_t.club_id IS NOT NULL THEN
      FOR v_fee_row IN
        SELECT r.* FROM public.rake_records r
         WHERE r.tournament_id=p_tournament_id AND r.is_tournament
           AND r.source IN ('fn_spin_book_entry','fn_spin_settle_game') AND r.rake_amount>0
