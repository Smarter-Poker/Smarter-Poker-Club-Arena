BEGIN;
-- Reject a different recipient before mutating the original paid obligation.
-- The original function returned a refusal after increasing amount_owed.
-- Preserves payment policy, existing keys, amounts, privileges and all history.
DO $guard$
BEGIN
 IF md5(pg_get_functiondef('public.fn_settle_tournament_obligation(uuid,text,integer,uuid,numeric,text,text,uuid)'::regprocedure)) <> '0a80cabe92f3becd3baff7d910916d02' THEN
  RAISE EXCEPTION 'Settlement definition changed since audited baseline';
 END IF;
END $guard$;
