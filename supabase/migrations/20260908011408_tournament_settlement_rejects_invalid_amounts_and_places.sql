BEGIN;
DO $baseline$ BEGIN
IF md5(pg_get_functiondef('public.fn_settle_tournament_obligation(uuid,text,integer,uuid,numeric,text,text,uuid)'::regprocedure)) <> '16f7e2b5cea86f327956ea9cdb3250a6' THEN RAISE EXCEPTION 'settlement input baseline changed'; END IF;
END $baseline$;
