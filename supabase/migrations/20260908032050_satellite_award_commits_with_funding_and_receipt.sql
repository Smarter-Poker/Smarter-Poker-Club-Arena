...
DO $precondition$
BEGIN
  IF md5(pg_get_functiondef('public.fn_award_satellite_seat(uuid,uuid,uuid,text,integer)'::regprocedure)) <> 'ecb6fd235531d4790dff900e2e3b6caf' THEN
    RAISE EXCEPTION 'Satellite award definition changed since inspection';
  END IF;
END $precondition$;
