  ...
  LOOP
    ...
    UPDATE public.ca_drift_incidents i
       SET status = 'resolved',
           ...
           correction_ref =
             'migration 20260908035400_six_full_pool_events_retire_only_their_stale_obligation_metadata; obligation '
             || v_expected.obligation_id::text,
           resolution =
  ...
  UPDATE public.ca_drift_incidents i
     SET status = 'resolved',
         ...
         correction_ref = COALESCE(
           i.correction_ref,
           'migration 20260908035400_six_full_pool_events_retire_only_their_stale_obligation_metadata; canonical final-table payout evidence'),
         root_cause = COALESCE(
           i.root_cause,
