...
DO $precondition$ BEGIN
 IF md5(pg_get_functiondef('public.fn_ca_settle_hand_stacks_absolute(uuid,bigint,jsonb,numeric,numeric,text,numeric)'::regprocedure)) <> '82fa4ccea29144c1191f92660cd5c47c' THEN RAISE EXCEPTION 'Hand settlement changed since inspection'; END IF;
END $precondition$;
...
BEGIN
  ...
  BEGIN
    ...
    v_result := jsonb_build_object('success', true, 'players', v_n,
      ...
      'conservation_checked', v_delta_mode OR p_rake IS NOT NULL, 'request', v_request);
