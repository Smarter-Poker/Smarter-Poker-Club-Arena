-- 20260908045932_non_satellite_terminal_settlement_commits_one_stored_receipt.sql
--
...
-- During the rolling window the five component finish RPCs must remain
...
-- byte-for-byte except for one canonical bulk prelock immediately before its
-- existing loop. The definition hash makes upstream drift a migration stop,
-- never a best-effort textual patch.
...
DECLARE
  ...
  v_loop_needle text := $needle$    -- lock seats, compute deltas / targets
    FOR e IN SELECT * FROM jsonb_array_elements(p_stacks) LOOP$needle$;
  v_loop_replacement text := $replacement$    -- ONE LOCK ORDER WITH TERMINAL CLOSE (20260908): tournament,
    ...
    IF v_tournament_id IS NOT NULL THEN
      ...
      SELECT count(*),count(DISTINCT (x.value->>'user_id')::uuid)
        ...
        FROM jsonb_array_elements(p_stacks) AS x(value);
    ...
    -- reacquire locks that this transaction already owns, independent of the
    -- caller's JSON order.
    -- live target seat once in durable row-id order before the original loop.
    PERFORM 1
    ...
    FOR e IN SELECT * FROM jsonb_array_elements(p_stacks) LOOP$replacement$;
  ...
  v_result_needle text := $needle$      'conservation_checked', v_delta_mode OR p_rake IS NOT NULL);$needle$;
  v_result_replacement text := $replacement$      'conservation_checked', v_delta_mode OR p_rake IS NOT NULL,
      ...
      'tournament_table_live_seat_count', v_table_live_seat_count);$replacement$;
BEGIN
  SELECT pg_get_functiondef(p.oid) INTO v_definition
    FROM pg_proc p
   WHERE p.oid =
     'public.fn_ca_settle_hand_stacks_absolute(uuid,bigint,jsonb,numeric,numeric,text,numeric)'::regprocedure;
  IF v_definition IS NULL OR md5(v_definition) <> '82fa4ccea29144c1191f92660cd5c47c'
...
CREATE TABLE public.tournament_terminal_settlement_cutover (
  ...
  migration_version                 text NOT NULL
    CHECK (migration_version = '20260908045932'),
...
SELECT 'fn_complete_tournament_terminal:v1', '20260908045932',
