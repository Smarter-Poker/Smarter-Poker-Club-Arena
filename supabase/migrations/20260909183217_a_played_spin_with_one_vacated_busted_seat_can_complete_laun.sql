...
--
-- A Spin can have an immutable three-player reserve draw and a real persisted
-- hand while its launch receipt is still incomplete. Once one player busts and
-- the seat is vacated, the ordinary launch retry sees only two active roster
-- rows and stands down before it can adopt the table or complete the receipt.
-- That is the exact state observed in the stuck REGISTERING Spins; treating it
-- as a new two-player field would be unsafe.
--
-- This migration adds one service-only, evidence-only proof. It returns true
-- only for a paid 3-max Spin (never a Heads-Up SNG) with:
--   * exactly three original roster/payment/entitlement identities;
--   * exactly two playing players and one zero-stack eliminated player;
--   * exactly two matching live seats plus one vacated eliminated seat;
--   * at least one hand persisted on the tournament's one table;
--   * both the roster and felt conserving all three bought starting stacks;
--   * one exact contribution/draw and their exact chip-ledger/escrow evidence.
--
-- The manager uses this as a preflight hint. The RUNNING status transaction
-- invokes the same proof again after locking the tournament, and only then
-- lowers its active field requirement from three to two. A fresh two-player
-- Spin, a decided one-player Spin, a Heads-Up SNG, or incomplete money evidence
-- still follows the ordinary refusal path. No watcher, reconciler, refund, or
-- repair job is introduced.
--
-- ROLLBACK: reverse the one literal edit to the launch completion core, then
-- DROP FUNCTION public.fn_prove_played_spin_launch_recovery(uuid).
--
...
CREATE OR REPLACE FUNCTION public.fn_prove_played_spin_launch_recovery(
...
COMMENT ON FUNCTION public.fn_prove_played_spin_launch_recovery(uuid) IS
  'Service-only evidence proof for one already-played 3-max Spin with exactly one busted/vacated original paid seat. Never applies to Heads-Up SNG and moves no state.';

REVOKE ALL ON FUNCTION public.fn_prove_played_spin_launch_recovery(uuid)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.fn_prove_played_spin_launch_recovery(uuid)
  TO service_role;

DO $patch_launch_completion$
DECLARE
  v_src text;
  v_new text;
  v_old CONSTANT text :=
...
BEGIN
  ...
  IF position(v_old IN v_src) = 0
     OR (length(v_src) - length(replace(v_src, v_old, ''))) / length(v_old) <> 1 THEN
    RAISE EXCEPTION
      'launch completion field anchor is missing or ambiguous; read the deployed function before retrying';
  END IF;

  v_new := replace(v_src, v_old, v_replacement);
  IF position('fn_prove_played_spin_launch_recovery' IN v_new) = 0
     OR position('v_required_field := 2' IN v_new) = 0
    ...
    RAISE EXCEPTION 'played Spin launch edit dropped an existing completion proof';
  END IF;

  EXECUTE v_new;
END;
$patch_launch_completion$;

-- The completion core stays owner-only. The public service entry point retains
-- its lease-generation wrapper and invokes this core only after fencing.
