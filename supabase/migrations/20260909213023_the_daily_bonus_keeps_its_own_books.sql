...
-- Applied to production 20260909213023; the .sql file was never committed at the
...
--
-- LEDGER ROW
--   version     20260909213023   (the stamp IS the apply time, UTC: 2026-09-09 21:30:23)
--   name        the_daily_bonus_keeps_its_own_books
--   created_by  daniel@bekavactrading.com
--   statements  1 statement(s), 22171 bytes
--
-- WHY THE FILENAME VERSION WAS NOT RESERVED
--
-- CLAUDE.md 4.5 says never hand-pick a migration version and always run
-- scripts/new-migration.mjs. A mirror is the one sanctioned exception, and the
-- exception is what makes it safe: 20260909213023 IS ALREADY IN
...
--     TRIGGER        trg_ca_daily_bonus_refusals_append_only
--     FUNCTION       public.fn_ca_daily_bonus_claimed_from, public.fn_ca_daily_bonus_refuse, public.fn_ca_daily_bonus_velocity_check, public.fn_ca_daily_bonus_budget_check, public.fn_ca_daily_bonus_claim
--     TABLE          public.ca_daily_bonus_refusals
--     INDEX          ca_daily_bonus_refusals_date_idx, ca_daily_bonus_refusals_user_idx, ca_daily_bonus_claims_device_idx, ca_daily_bonus_claims_ip_idx
--     DROP           TRIGGER trg_ca_daily_bonus_refusals_append_only, FUNCTION public.fn_ca_daily_bonus_claim
--     RLS-ENABLE     
--
...
-- THE DAILY BONUS KEEPS ITS OWN BOOKS (Dan, 2026-09-09, phase 1).
-- 1. ca_daily_bonus_refusals: one row per refused claim.
-- 2. ca_daily_bonus_claims.claimed_from: device_id (client), ua + ip_class (server-read).
-- 3. Velocity rule (3 accounts / device, 6 / network per day) -> financial_alerts, once a day.
-- 4. Budget anomaly rule (today > 14-day p95, min 7 days) -> financial_alerts, once a day.
-- Also repairs the NULL 2026-10 club_arena_daily budget line and backfills
-- ca_diamond_engine_spend for the bonus journals. ONE BODY for the claim.
