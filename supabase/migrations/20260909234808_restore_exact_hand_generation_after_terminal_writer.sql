-- Restore the exact-seat expansion overwritten by the later terminal writer.
-- Inspected production definitions and both allowed end states are pinned.
-- Preserves terminal synchronization, zero-stack vacating, owners and ACLs.
-- Database-first additive deployment: whole legacy rosters remain supported.
-- Verified with real PostgreSQL stack and full hand-boundary regressions.
BEGIN;
DO $source_guard$
BEGIN
 IF NOT (
  (md5(pg_get_functiondef('public.fn_ca_settle_hand_stacks_absolute(uuid,bigint,jsonb,numeric,numeric,text,numeric)'::regprocedure))
      = '2e322bc7dfee3cf5cb6548ed3a587095'
   AND md5(pg_get_functiondef('public.fn_ca_commit_hand_settlement(uuid,bigint,jsonb,numeric,numeric,text,numeric,jsonb,jsonb,text,uuid,jsonb)'::regprocedure))
  ...
  (md5(pg_get_functiondef('public.fn_ca_settle_hand_stacks_absolute(uuid,bigint,jsonb,numeric,numeric,text,numeric)'::regprocedure))
   ...
   RAISE EXCEPTION 'Hand writer changed since inspected terminal/exact-seat composition';
 END IF;
END $source_guard$;
