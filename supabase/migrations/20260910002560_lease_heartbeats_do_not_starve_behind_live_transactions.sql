 ...
 *
 * Hand and manager transactions keep an exact lease row locked until their
 * mutation commits.  FOR SHARE protected that generation, but it also
 * conflicts with the heartbeat's FOR NO KEY UPDATE row lock.  Busy tournament
 * tables could therefore deny every heartbeat for a full proof window and
 * cause a healthy manager to fence and destroy all of its child engines.
 *
 * The two redundant ownership constraints below are deliberate.  PostgreSQL
 * treats every column belonging to a non-partial unique key as a key column.
 * Once instance_id and lease_generation are key columns, FOR KEY SHARE has the
 ...
 *
 * This is a stopped-engine cutover.  NOWAIT makes an unexpected live writer a
 * refusal, never a partially weakened authority boundary.
 */
BEGIN;
