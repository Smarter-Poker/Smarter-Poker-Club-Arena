...
--
-- Stage A deliberately left the strict tournament completion guards disabled
-- while the exact atomic engine rolled out. Every valid completion in that
-- compatibility window must own an immutable finish claim. The immediately
-- preceding forward repair supplies the one proven historical claim that an
-- out-of-band settlement omitted; this migration accepts no other omission.
--
-- This stopped-engine DML-only boundary certifies the finite cohort before the
-- destructive authority contraction. It neither creates a claim nor moves
-- money: a missing, conflicting or partial immutable receipt aborts the entire
-- transaction.
