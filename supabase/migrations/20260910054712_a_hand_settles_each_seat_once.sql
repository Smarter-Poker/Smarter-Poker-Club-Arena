-- 20260910054712_a_hand_settles_each_seat_once.sql
--
-- Version reserved by scripts/new-migration.mjs against origin/main and every
-- remote branch, so it cannot collide with another agent's in-flight work.
--
-- WHAT THIS CHANGES, AND WHY (swarm workstream I, 2026-09-10; analysis, the
-- rolled-back measurements and the self-test are in
-- docs/changelog/swarm-2026-09-10/I-one-seat-write-per-hand.md / .sql):
--
-- Per hand, every seat row in table_seats was written TWICE inside the
-- settlement transaction: fn_ca_settle_hand_stacks_absolute (the owner-only
-- core) wrote stack, then fn_ca_commit_hand_settlement (the service door)
-- wrote time_bank_uses_remaining / time_bank_remaining. The second write ran
-- the full trigger stack again plus three FK re-checks: 0.48-0.66 ms per seat
-- steady, 1.7-5 ms cold, ~2.0-2.6 ms per hand at 3.96 seats, and one extra
-- tuple version + WAL record per seat per hand (~1,200/min at 300 hands/min).
--
-- The door now publishes the hand's time-bank items in a transaction-local
-- setting (app.ca_hand_time_banks, keyed by table+hand+exact seat generation,
-- cleared right after the core returns) and the core folds them into its
-- stack UPDATE under the same guards. The door then PROVES the seat state with
-- the predicate its lawful-noop rule already used and writes only a seat the
-- core did not carry. Either side alone degrades to today's two writes, so
-- deploy order does not matter. All 43 table_seats triggers were read: none
-- reads the time-bank columns, none has UPDATE OF on them, so every trigger
-- sees identical inputs on the combined write.
--
-- PROVEN BEFORE APPLY (05:58 UTC, rolled back): both bodies created inside a
-- transaction that ended in ROLLBACK; the self-test on a live cash seat
-- reported seat_writes=1 with the envelope (stack unchanged, uses 1->2,
-- secs 20->27, door proof 1) and seat_writes=1 without it (stack-only),
-- VERDICT: PASS. Applied 05:59 UTC; first 5 minutes: 3,851 settlements
-- succeeded, 0 failed, 0 stuck, 0 ledger write failures.
--
-- Bases (md5 of pg_get_functiondef before this change):
--   fn_ca_commit_hand_settlement      44a02f56937b379e01ab2524c859c892
--   fn_ca_settle_hand_stacks_absolute 5e38ac8623ed5dc3eebe1d18f4269e8e
-- ROLLBACK: the originals, verbatim, are the commented block at the end of
-- docs/changelog/swarm-2026-09-10/I-one-seat-write-per-hand.sql.
--
...
DO $g$ BEGIN
  -- Idempotent: already applied on production 2026-09-10 05:59 UTC.
  IF pg_get_functiondef('public.fn_ca_settle_hand_stacks_absolute'::regproc) NOT LIKE '%app.ca_hand_time_banks%'
     AND (md5(pg_get_functiondef('public.fn_ca_commit_hand_settlement'::regproc)) <> '44a02f56937b379e01ab2524c859c892'
       OR md5(pg_get_functiondef('public.fn_ca_settle_hand_stacks_absolute'::regproc)) <> '5e38ac8623ed5dc3eebe1d18f4269e8e') THEN
    RAISE EXCEPTION 'settlement bodies changed since workstream I analysed them; re-derive before applying';
  END IF;
END $g$;
