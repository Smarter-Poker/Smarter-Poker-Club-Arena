-- Continue the already-approved short-format rules in every DB reader.
...
BEGIN
  IF v_oid IS NULL OR NOT EXISTS (
    ...
    RAISE EXCEPTION 'Short-format blind resolver preflight changed; review current authority'
      USING ERRCODE='55000';
  END IF;
END;
$preflight$;
...
BEGIN
    ...
    IF (v_levels->(v_len-1)) ? 'spinContinuation' THEN
      ...
      IF jsonb_typeof(v_continuation) IS DISTINCT FROM 'object'
        ...
        RAISE EXCEPTION 'Spin blind continuation is missing its frozen formula'
          USING ERRCODE='55000';
      END IF;
      IF (v_continuation->>'version')::numeric <> 1
         OR (v_continuation->>'anchorLevel')::numeric <= 0
         OR trunc((v_continuation->>'anchorLevel')::numeric)
              <> (v_continuation->>'anchorLevel')::numeric
         OR (v_continuation->>'anchorBigBlind')::numeric <= 0
         OR (v_continuation->>'growth')::numeric <= 1
         OR (v_continuation->>'roundBigTo')::numeric <= 0 THEN
        RAISE EXCEPTION 'Spin blind continuation is missing its frozen formula'
          USING ERRCODE='55000';
      END IF;
  ...
  -- SNG is the approved two-seat Heads-Up product. Its overflow is the
  -- headsUpSpec rule, not the generic MTT observed-ratio / chip-cap rule.
  IF lower(COALESCE(p_variant,''))='sng'
     OR upper(COALESCE(p_tournament_type,''))='SNG' THEN
    v_bb := CASE v_index
      WHEN 0 THEN 20 WHEN 1 THEN 30 WHEN 2 THEN 40 WHEN 3 THEN 60
      WHEN 4 THEN 80 WHEN 5 THEN 100 WHEN 6 THEN 120 WHEN 7 THEN 150
      WHEN 8 THEN 180 WHEN 9 THEN 210 WHEN 10 THEN 300 WHEN 11 THEN 400
      ELSE round((400::numeric*power(1.4::numeric,v_index-11))/10)*10
    END;
    RETURN jsonb_build_object(
      'small_blind',round(v_bb/2),'big_blind',v_bb,'ante',0,
      'level_index',v_index,'source','heads_up_overflow','blind_capped',false
    );
  END IF;
