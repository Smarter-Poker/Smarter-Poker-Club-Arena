-- Recompose the previously verified satellite funded-component correction onto
-- the live 2026-09-10 15:59 UTC reader, preserving ticket-redemption inflow.
-- Correct duplicate pre-start seat counts, duplicate version 2 fee subtraction,
-- and empty-target phantom bounty. Preserve current global settlement lock authority.
-- No historical row rewrite and no exact-K/Stage-B activation or new caller.
BEGIN;
SET LOCAL lock_timeout='1s';
SET LOCAL statement_timeout='10s';
DO $guard$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_proc p WHERE p.oid=to_regprocedure('public.fn_award_satellite_seat(uuid,uuid,uuid,text,integer)') AND p.proisstrict=false AND pg_get_function_arguments(p.oid)='p_satellite_id uuid, p_target_id uuid, p_user_id uuid, p_username text DEFAULT NULL::text, p_position integer DEFAULT NULL::integer' AND pg_get_function_result(p.oid)='jsonb' AND md5(p.prosrc)=ANY(ARRAY['cc53a9560c7b211c9c052a186dabc96c','2c21c56c6a9d4a2f8ee79082bd4fef57']) AND pg_get_userbyid(p.proowner)='postgres' AND p.proacl::text='{postgres=X/postgres,service_role=X/postgres}' AND p.proconfig IS NOT DISTINCT FROM ARRAY['search_path=public']::text[] AND p.prosecdef=true AND p.provolatile='v') THEN
    RAISE EXCEPTION 'Satellite correction preflight source or authority changed: fn_award_satellite_seat(uuid,uuid,uuid,text,integer)';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_proc p WHERE p.oid=to_regprocedure('public.fn_ca_escrow_on_rake_record()') AND p.proisstrict=false AND pg_get_function_arguments(p.oid)='' AND pg_get_function_result(p.oid)='trigger' AND md5(p.prosrc)=ANY(ARRAY['233661d2164a417c2a76c8e0cbfbe9cc','3e628d6a57a93eeb61d494ee33f989a3']) AND pg_get_userbyid(p.proowner)='postgres' AND p.proacl::text='{postgres=X/postgres,service_role=X/postgres}' AND p.proconfig IS NOT DISTINCT FROM ARRAY['search_path=public']::text[] AND p.prosecdef=true AND p.provolatile='v') THEN
    RAISE EXCEPTION 'Satellite correction preflight source or authority changed: fn_ca_escrow_on_rake_record()';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_proc p WHERE p.oid=to_regprocedure('public.fn_ca_lock_settlement_lane_global()') AND p.proisstrict=false AND pg_get_function_arguments(p.oid)='' AND pg_get_function_result(p.oid)='void' AND md5(p.prosrc)=ANY(ARRAY['343015440ea5c84ee4ca7ae583c73d30']) AND pg_get_userbyid(p.proowner)='postgres' AND p.proacl::text='{postgres=X/postgres,service_role=X/postgres}' AND p.proconfig IS NOT DISTINCT FROM ARRAY['search_path=public, pg_temp']::text[] AND p.prosecdef=false AND p.provolatile='v') THEN
    RAISE EXCEPTION 'Satellite correction preflight source or authority changed: fn_ca_lock_settlement_lane_global()';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_proc p WHERE p.oid=to_regprocedure('public.fn_ca_tournament_escrow(uuid)') AND p.proisstrict=false AND pg_get_function_arguments(p.oid)='p_tournament_id uuid' AND pg_get_function_result(p.oid)='TABLE(prize_in numeric, bounty_in numeric, fee_in numeric, overlay_in numeric, satellite_in numeric, prize_out numeric, bounty_out numeric, fee_out numeric, refund_out numeric, prize_balance numeric, bounty_balance numeric, fee_balance numeric)' AND md5(p.prosrc)=ANY(ARRAY['52c3b25be3904e86f1a8558ce542ec61','56663389f8348d2ab35a54460c0b7632']) AND pg_get_userbyid(p.proowner)='postgres' AND p.proacl::text='{postgres=X/postgres,service_role=X/postgres}' AND p.proconfig IS NOT DISTINCT FROM ARRAY['search_path=public']::text[] AND p.prosecdef=true AND p.provolatile='s') THEN
    RAISE EXCEPTION 'Satellite correction preflight source or authority changed: fn_ca_tournament_escrow(uuid)';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_proc p WHERE p.oid=to_regprocedure('public.fn_sync_tournament_current_players()') AND p.proisstrict=false AND pg_get_function_arguments(p.oid)='' AND pg_get_function_result(p.oid)='trigger' AND md5(p.prosrc)=ANY(ARRAY['ecb120c2c6a4ecee6c2e04d4c9b5ebc7']) AND pg_get_userbyid(p.proowner)='postgres' AND p.proacl::text='{postgres=X/postgres,service_role=X/postgres}' AND p.proconfig IS NOT DISTINCT FROM ARRAY['search_path=public']::text[] AND p.prosecdef=true AND p.provolatile='v') THEN
    RAISE EXCEPTION 'Satellite correction preflight source or authority changed: fn_sync_tournament_current_players()';
  END IF;
...
DO $guard$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_proc p WHERE p.oid=to_regprocedure('public.fn_award_satellite_seat(uuid,uuid,uuid,text,integer)') AND p.proisstrict=false AND pg_get_function_arguments(p.oid)='p_satellite_id uuid, p_target_id uuid, p_user_id uuid, p_username text DEFAULT NULL::text, p_position integer DEFAULT NULL::integer' AND pg_get_function_result(p.oid)='jsonb' AND md5(p.prosrc)=ANY(ARRAY['2c21c56c6a9d4a2f8ee79082bd4fef57']) AND pg_get_userbyid(p.proowner)='postgres' AND p.proacl::text='{postgres=X/postgres,service_role=X/postgres}' AND p.proconfig IS NOT DISTINCT FROM ARRAY['search_path=public']::text[] AND p.prosecdef=true AND p.provolatile='v') THEN
    RAISE EXCEPTION 'Satellite correction postflight source or authority changed: fn_award_satellite_seat(uuid,uuid,uuid,text,integer)';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_proc p WHERE p.oid=to_regprocedure('public.fn_ca_escrow_on_rake_record()') AND p.proisstrict=false AND pg_get_function_arguments(p.oid)='' AND pg_get_function_result(p.oid)='trigger' AND md5(p.prosrc)=ANY(ARRAY['3e628d6a57a93eeb61d494ee33f989a3']) AND pg_get_userbyid(p.proowner)='postgres' AND p.proacl::text='{postgres=X/postgres,service_role=X/postgres}' AND p.proconfig IS NOT DISTINCT FROM ARRAY['search_path=public']::text[] AND p.prosecdef=true AND p.provolatile='v') THEN
    RAISE EXCEPTION 'Satellite correction postflight source or authority changed: fn_ca_escrow_on_rake_record()';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_proc p WHERE p.oid=to_regprocedure('public.fn_ca_lock_settlement_lane_global()') AND p.proisstrict=false AND pg_get_function_arguments(p.oid)='' AND pg_get_function_result(p.oid)='void' AND md5(p.prosrc)=ANY(ARRAY['343015440ea5c84ee4ca7ae583c73d30']) AND pg_get_userbyid(p.proowner)='postgres' AND p.proacl::text='{postgres=X/postgres,service_role=X/postgres}' AND p.proconfig IS NOT DISTINCT FROM ARRAY['search_path=public, pg_temp']::text[] AND p.prosecdef=false AND p.provolatile='v') THEN
    RAISE EXCEPTION 'Satellite correction postflight source or authority changed: fn_ca_lock_settlement_lane_global()';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_proc p WHERE p.oid=to_regprocedure('public.fn_ca_tournament_escrow(uuid)') AND p.proisstrict=false AND pg_get_function_arguments(p.oid)='p_tournament_id uuid' AND pg_get_function_result(p.oid)='TABLE(prize_in numeric, bounty_in numeric, fee_in numeric, overlay_in numeric, satellite_in numeric, prize_out numeric, bounty_out numeric, fee_out numeric, refund_out numeric, prize_balance numeric, bounty_balance numeric, fee_balance numeric)' AND md5(p.prosrc)=ANY(ARRAY['56663389f8348d2ab35a54460c0b7632']) AND pg_get_userbyid(p.proowner)='postgres' AND p.proacl::text='{postgres=X/postgres,service_role=X/postgres}' AND p.proconfig IS NOT DISTINCT FROM ARRAY['search_path=public']::text[] AND p.prosecdef=true AND p.provolatile='s') THEN
    RAISE EXCEPTION 'Satellite correction postflight source or authority changed: fn_ca_tournament_escrow(uuid)';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_proc p WHERE p.oid=to_regprocedure('public.fn_sync_tournament_current_players()') AND p.proisstrict=false AND pg_get_function_arguments(p.oid)='' AND pg_get_function_result(p.oid)='trigger' AND md5(p.prosrc)=ANY(ARRAY['ecb120c2c6a4ecee6c2e04d4c9b5ebc7']) AND pg_get_userbyid(p.proowner)='postgres' AND p.proacl::text='{postgres=X/postgres,service_role=X/postgres}' AND p.proconfig IS NOT DISTINCT FROM ARRAY['search_path=public']::text[] AND p.prosecdef=true AND p.provolatile='v') THEN
    RAISE EXCEPTION 'Satellite correction postflight source or authority changed: fn_sync_tournament_current_players()';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger t WHERE t.tgrelid='public.tournament_players'::regclass AND t.tgname='trg_sync_tournament_current_players' AND t.tgfoid=to_regprocedure('public.fn_sync_tournament_current_players()') AND t.tgenabled='O' AND NOT t.tgisinternal AND t.tgtype=29 AND t.tgnargs=0 AND NOT t.tgdeferrable AND NOT t.tginitdeferred AND t.tgqual IS NULL AND ARRAY(SELECT a.attname::text FROM unnest(t.tgattr::smallint[]) WITH ORDINALITY x(num,ord) JOIN pg_attribute a ON a.attrelid=t.tgrelid AND a.attnum=x.num ORDER BY x.ord)=ARRAY['status','tournament_id']::text[]) THEN
    RAISE EXCEPTION 'Satellite correction requires the current enabled roster count authority';
  END IF;
