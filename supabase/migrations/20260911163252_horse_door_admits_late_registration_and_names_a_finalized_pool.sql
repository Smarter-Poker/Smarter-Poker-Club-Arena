-- ═══════════════════════════════════════════════════════════════════════════
--  THE HORSE DOOR IS THE HUMAN DOOR (lane D audit, 2026-09-11)
-- ═══════════════════════════════════════════════════════════════════════════
--
...
-- REBUILT 2026-09-11 BEFORE APPLYING: the two rewritten bodies below are
-- production's own text (pg_get_functiondef / pg_proc, same day) with only the
-- changes described above applied to them. The audit's draft had dropped four
-- comment blocks that carry incident history (ONE DEFINITION OF FULL, MYSTERY
-- BOUNTY, CHIP STANDARD 1.2, and the two lock-order notes in the terminal
-- gate); every one of them is back, and a line-by-line diff against pg_proc
...
-- The human lifecycle gate holds no grants at all (anon, authenticated and
-- service_role all read false, 2026-09-11). A newly created function inherits
-- PUBLIC EXECUTE, so say so rather than inherit it: this gate is reachable
-- only from the terminal gate above it.
REVOKE ALL ON FUNCTION public.fn_register_horse_for_tournament_before_atomic_lifecycle_gate(uuid, uuid)
...
-- THE GRANTS, STATED (2026-09-11). Production holds exactly these today:
-- {postgres=X/postgres} on all three inner gates, plus service_role on the
-- two public entry points, which this migration does not touch. CREATE
-- FUNCTION grants PUBLIC EXECUTE by default, so a rebuild from these files
-- would otherwise open three SECURITY DEFINER money doors to anon. Naming
-- PUBLIC as well as the roles matters: anon inherits whatever PUBLIC holds,
-- so revoking anon alone reads as a fix and does nothing.
REVOKE ALL ON FUNCTION public.fn_register_horse_for_tournament_before_maintenance_gate(uuid, uuid)
  FROM PUBLIC, anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.fn_register_horse_for_tournament_before_terminal_gate(uuid, uuid, boolean)
  FROM PUBLIC, anon, authenticated, service_role;

DO $check$
BEGIN
  IF (SELECT prosrc FROM pg_proc WHERE oid = 'public.fn_register_horse_for_tournament_before_maintenance_gate(uuid,uuid)'::regprocedure)
     NOT LIKE '%fn_tournament_late_registration_open%' THEN
    RAISE EXCEPTION 'the horse core still refuses late registration';
  END IF;
  IF (SELECT prosrc FROM pg_proc WHERE oid = 'public.fn_register_horse_for_tournament_before_maintenance_gate(uuid,uuid)'::regprocedure)
     NOT LIKE '%prize_pool_finalized%' THEN
    RAISE EXCEPTION 'the horse core still meets the finalized-pool trigger instead of reading the column';
  END IF;
  IF (SELECT prosrc FROM pg_proc WHERE oid = 'public.fn_register_horse_for_tournament_before_terminal_gate(uuid,uuid,boolean)'::regprocedure)
     NOT LIKE '%fn_register_horse_for_tournament_before_atomic_lifecycle_gate%' THEN
    RAISE EXCEPTION 'the terminal gate does not go through the lifecycle gate';
  END IF;
  IF has_function_privilege('anon',
       'public.fn_register_horse_for_tournament_before_maintenance_gate(uuid,uuid)', 'EXECUTE')
     OR has_function_privilege('authenticated',
       'public.fn_register_horse_for_tournament_before_maintenance_gate(uuid,uuid)', 'EXECUTE')
     OR has_function_privilege('anon',
       'public.fn_register_horse_for_tournament_before_atomic_lifecycle_gate(uuid,uuid)', 'EXECUTE')
     OR has_function_privilege('authenticated',
       'public.fn_register_horse_for_tournament_before_atomic_lifecycle_gate(uuid,uuid)', 'EXECUTE')
     OR has_function_privilege('anon',
       'public.fn_register_horse_for_tournament_before_terminal_gate(uuid,uuid,boolean)', 'EXECUTE')
     OR has_function_privilege('authenticated',
       'public.fn_register_horse_for_tournament_before_terminal_gate(uuid,uuid,boolean)', 'EXECUTE') THEN
    RAISE EXCEPTION 'a horse admission core became reachable from the client';
  END IF;
END
$check$;
