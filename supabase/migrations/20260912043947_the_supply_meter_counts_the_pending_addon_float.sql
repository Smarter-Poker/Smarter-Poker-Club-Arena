-- The supply meter counts the pending add-on float.
--
-- This is the platform's WORST DISCREPANCY as the ops board reads it: drift
-- incident b39556cb-1555-447e-b804-415729122db8, -118.78 club_chips at
-- 2026-09-11 17:05Z, "total chip supply changed beyond ledgered
-- issuance/retirement". Nothing was destroyed. Not one chip is owed to anybody.
-- The meter was looking at a table that does not hold all the chips.
--
-- atomic_table_addon declares 'table_stack' as its ledger counterparty and
-- debits the player's wallet in the SAME transaction, so the journal says the
-- chips are on the felt the instant the add-on is requested. But when the
-- request lands MID-HAND (p_apply_to_seat = false) the chips do not go to the
-- seat. They go to table_pending_addons and wait there until
-- resolve_pending_addon delivers them, which posts no further leg because the
-- leg was already posted. Between those two moments the chips are journalled
-- to a store, real, and invisible: table_pending_addons appears in none of the
-- fifteen components this function summed.
--
-- So every snapshot landing inside an open add-on window read the chips as
-- gone, and the next one read them as arriving. Measured on production:
--
--   16:11:03.291388Z  float   0.00   (previous snapshot)
--   17:05:00.754767Z  float 115.68   -> unexplained -118.78   THE INCIDENT
--   18:05:00.278901Z  float   0.00   -> unexplained +121.18   the rebound
--
-- One row did it: table_pending_addons 5c4a6eae-67c2-4fd9-bfca-c99ba9272683,
-- 115.68, created 17:04:44.069227Z, resolved 17:05:09.744632Z - open for 25.7
-- seconds, and the hourly meter fired 16.7 seconds into that window. Its
-- chip_ledger leg (category 'addon', player_wallet -> table_stack, 115.68)
-- carries the same microsecond as the pending row. The severity was decided by
-- whether a DIFFERENT add-on happened to be open an hour earlier.
--
-- Subtracting the float collapses the residual to 0.00 on every interval in 30
-- hours where the float moved (-224.41/+224.41, -135.82/+135.82, -64.45/+64.45,
-- -46.67/+47.00 and the pair above). Nothing is stranded and nothing leaks: all
-- 15,459 rows ever written are resolved, and sum(amount - applied_to_stack -
-- refunded) over the resolved ones is exactly 0.00. Refunds reverse against the
-- same account (table_stack -> player_wallet), so felt + unresolved float is a
-- closed basis in both directions.
--
-- This is the same defect the ticket float had on 2026-09-11 and is fixed the
-- same way. The basis version bump is load-bearing, not cosmetic: this function
-- NULLs unexplained when the previous snapshot carries a different basis, which
-- is what stops the changeover itself firing a phantom incident the size of
-- whatever float happens to be open at that moment.
--
-- The component is scoped to non-tournament tables to match `felt` exactly, so
-- the two can never disagree. Measured: zero pending add-ons have EVER been
-- written against a tournament table, so this changes no number today and
-- keeps the basis honest if that ever changes.
--
-- fn_ca_trial_balance is corrected in the same transaction for two reasons.
-- Its table_stack account reads s.felt alone, so it localised this whole
-- incident to table_stack (-118.78) while every other account balanced - true,
-- and useless, because the account was not actually short. And it carries NO
-- ticket_escrow row at all, so its total_supply has silently been missing the
-- ticket float since ticket-escrow-v3 landed yesterday. A diagnostic that
-- disagrees with the meter sends the next investigator after chips that are
-- exactly where they should be. It cost this investigation real time.
...
DECLARE
  ...
  v_outside text[] := public.fn_ca_noncirculating_chip_stores();
BEGIN
  /* THE WINDOW IS THE SNAPSHOTS, NOT A CLOCK GUESS (2026-09-10). See the
     migration that introduced this: asking for "the first snapshot after
     now() minus 75 minutes" produced SIX readings out of 168 cron runs. */
  ...
  WITH acct(account, ledger_types, bal_delta) AS (
    VALUES
      ...
      /* THE FELT IS OWED WHAT A MID-HAND ADD-ON ALREADY PAID FOR (2026-09-12).
         This read s.felt alone, so it localised incident b39556cb entirely to
         table_stack (-118.78) while every other account balanced to the cent -
         true, and useless, because the account was not short. The add-on's
         table_stack leg is posted at request time; the chips reach the seat up
         to minutes later. COALESCE keeps pre-v4 snapshots on the old basis
         rather than inventing a delta out of a NULL, exactly as promo does. */
      ...
      /* THE TICKET FLOAT HAD NO ACCOUNT HERE (2026-09-12). ticket-escrow-v3
         ...
         ticket issued or redeemed has shown up in this function as an
         unattributable total_supply difference ever since. */
