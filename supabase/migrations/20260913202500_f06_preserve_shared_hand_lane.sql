-- A no-op roster mirror during accepted hand settlement must not promote T shared.
-- Source custody changes retain the existing exclusive lane and receipt guards.
...
DO $preflight$
BEGIN
 IF md5(pg_get_functiondef('smarter_private.f06_source_guard()'::regprocedure)) NOT IN ('02764c094fbbb44cb83f50f0591d2141','2bfb53c26a9043bcb1cd015dfbebc8b7')
 OR md5(pg_get_functiondef('smarter_private.f06_try_lane(uuid)'::regprocedure)) <> '78a3a191b9991b0a3a343db39de335aa'
 OR md5(pg_get_functiondef('public.fn_ca_share_settlement_lane_for_table(uuid)'::regprocedure)) <> 'f1337496592df1b4e89cac9fc7ecfef8'
 ...
 OR NOT EXISTS(SELECT 1 FROM pg_trigger WHERE tgname='a00_f06_source_roster' AND tgenabled='O' AND pg_get_triggerdef(oid)='CREATE TRIGGER a00_f06_source_roster BEFORE INSERT OR DELETE OR UPDATE OF table_id, user_id, seat_number, status ON public.tournament_players FOR EACH ROW EXECUTE FUNCTION smarter_private.f06_source_guard()') THEN
  RAISE EXCEPTION 'F06 shared hand lane predecessor or binding changed; review before installation';
 END IF;
END $preflight$;
...
DO $postflight$
BEGIN
 IF md5(pg_get_functiondef('smarter_private.f06_source_guard()'::regprocedure)) <> '2bfb53c26a9043bcb1cd015dfbebc8b7'
