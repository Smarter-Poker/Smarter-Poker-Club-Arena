-- Reserved 2026-09-13 20:37:40 UTC. The nightly tuner wrote a profile and
-- audit in separate transactions from a profile read before a long study.
-- A later profile edit could be overwritten; an audit failure left the dial
-- change without its record. One service-only compare-and-set now commits
-- the existing update, audit and immutable request receipt together. No
-- strategy formula, sample floor, persona, wallet or settlement change.
BEGIN;
SET LOCAL lock_timeout='2s';
CREATE TABLE public.horse_tuner_write_receipts (
  horse_id uuid NOT NULL,
  run_date date NOT NULL,
  request_hash text NOT NULL CHECK(request_hash ~ '^[a-f0-9]{64}$'),
  request_payload text NOT NULL CHECK(octet_length(request_payload)<=65536),
  profile_changed boolean NOT NULL,
  audit_id bigint NOT NULL,
  created_at timestamptz NOT NULL DEFAULT clock_timestamp(),
  PRIMARY KEY(horse_id,run_date)
);
ALTER TABLE public.horse_tuner_write_receipts ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON TABLE public.horse_tuner_write_receipts FROM PUBLIC,anon,authenticated,service_role;

CREATE FUNCTION public.fn_record_horse_tuner_update(p_payload text)
...
BEGIN
  ...
  FOREACH k IN ARRAY ARRAY['tightness','aggression','bluffFreq'] LOOP
    ...
    END IF;
    IF (mods_after->>k)::numeric NOT BETWEEN 0.85 AND 1.18
  ...
  END IF;
  IF changed THEN UPDATE public.profiles SET horse_profile=next_profile WHERE id=horse AND is_horse IS TRUE; END IF;
...
COMMENT ON FUNCTION public.fn_record_horse_tuner_update(text) IS
  'Atomic legacy tuner profile compare-and-set plus audit and exact-request receipt. Replays do not retune; new profile edits and old unbound daily logs refuse. Does not grant causal learning activation.';
COMMIT;
