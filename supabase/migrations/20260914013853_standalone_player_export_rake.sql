...
-- Applied to production 20260914013853; the .sql file was never committed at the
-- time (standalone-club player export rake scoping; found by the two-way
-- migration diff of 2026-09-21). Content is byte-exact to what ran. Do NOT
-- re-apply; it is already live.
--
-- VERIFIED LIVE 2026-09-21 07:06:55 UTC, by the postimage this migration asserts
-- for itself: md5(pg_get_functiondef('public.ca_club_player_export_start(uuid,date,date,text,uuid)'))
-- = 563881446c064edd64de3b9013711726. It reads back exactly as written.
--
-- WHAT IT DOES. A standalone club's player export reads rake through the same
-- scoped branch the player page already uses, so the two cannot disagree about
-- one player's rake. It changes a reporting reader only; no financial source row
-- is rewritten.
--
-- @live-proof: md5(pg_get_functiondef('public.ca_club_player_export_start(uuid,date,date,text,uuid)'::regprocedure)) = '563881446c064edd64de3b9013711726'
