...
-- Applied to production 20260914060325; the .sql file was never committed at the
-- time (chip-statement keyset cursor; found by the two-way migration diff of
...
-- 'public.fn_ca_chip_statement_page(text,uuid,jsonb,integer)')) =
-- c706109ed8dab335912f046aaa6ee215 - the exact hash this migration's own guard
-- refuses to overwrite - the definition carries the next_cursor key it adds, and
-- its ACL is {postgres=X/postgres,authenticated=X/postgres,service_role=X/postgres},
-- anon revoked, as the GRANT at the end of the file states.
--
-- WHAT IT DOES. The paged chip statement returns a complete keyset cursor rather
-- than a bare timestamp, so a page boundary that shares a timestamp cannot drop
-- or repeat a ledger leg.
--
-- @live-proof: md5(pg_get_functiondef('public.fn_ca_chip_statement_page(text,uuid,jsonb,integer)'::regprocedure)) = 'c706109ed8dab335912f046aaa6ee215'
