...
-- Applied to production 20260914064415; the .sql file was never committed at the
-- time (legacy chip-statement cursor refusal; found by the two-way migration diff
-- of 2026-09-21). Content is byte-exact to what ran. Do NOT re-apply; it is
-- already live.
--
-- ITS OBJECT WAS SUPERSEDED IN PLACE, WHICH IS WHY THE BACKFILL MARKER MATTERS
-- HERE. This migration moved public.fn_ca_chip_statement(text,uuid,timestamptz,
-- integer) from 8e3c59ba71af74570be7daf1c99855b1 to
-- 0fc821ab5a709168c1c348aa15f32792 at 06:44 UTC. Fifty-nine minutes later
-- 20260914070338_chip_statement_cached_boundary_refusal replaced the same
-- function again, and the live definition is now
-- 48dd7c1b2b828579f04a45b84a6a6eee - which is the hash THAT migration's guard
-- names, and which it accepts only from 0fc821ab, this one's postimage. So the
-- chain proves this migration ran even though its own postimage is no longer the
-- live one. That is exactly the case the BACKFILLED exemption in
-- scripts/ci/check-migrations-applied.mjs was written for: the object genuinely
-- ran and is genuinely gone, the byte-exact record is correct, and the live
-- schema is correct.
--
-- WHAT IT DOES. The original fn_ca_chip_statement signature is kept for cached
-- clients, and refuses an ambiguous legacy continuation instead of guessing at a
-- page boundary.
--
-- @live-proof: md5(pg_get_functiondef('public.fn_ca_chip_statement(text,uuid,timestamptz,integer)'::regprocedure)) = '48dd7c1b2b828579f04a45b84a6a6eee'

-- Apply after the component using fn_ca_chip_statement_page is verified served.
DO $guard$
BEGIN
...
DO $ready$ BEGIN IF to_regprocedure('public.fn_ca_chip_statement_page(text,uuid,jsonb,integer)') IS NULL THEN RAISE EXCEPTION 'paged statement reader must be installed first'; END IF; END $ready$;

-- Keep the original signature for cached clients; ambiguous pages fail explicitly.
