-- Preserve tournament identity while repeated refusals remain deduplicated.
DO $guard$
DECLARE v_hash text;
BEGIN
  SELECT md5(pg_get_functiondef('public.fn_ca_financial_alert_to_incident()'::regprocedure)) INTO v_hash;
  IF v_hash NOT IN ('591d71ae6e8a7572640e531d247b13bc','3771aeef9008e35c6af3d03f220ebb3b') THEN
    RAISE EXCEPTION 'Financial incident trigger definition drift: %',v_hash;
  END IF;
END;
$guard$;
