-- Weekly union accounting: atomic failure and durable scheduler evidence.
-- Baseline read from production 2026-09-14. No historical money is changed.
BEGIN;
SET LOCAL lock_timeout = '3s';
SET LOCAL statement_timeout = '60s';
DO $baseline$
BEGIN
  IF md5(pg_get_functiondef('public.fn_union_settlement_cascade(uuid,timestamptz,timestamptz)'::regprocedure))
      <> 'c74bc3013f73455910ea2e133d30ca8c' THEN
    RAISE EXCEPTION 'union cascade changed since review; reconcile before applying';
  END IF;
  IF md5(pg_get_functiondef('public.fn_union_settlement_cascade_due()'::regprocedure))
      <> 'afaab7d0294db66ffd572c2b7c6e5c94' THEN
    RAISE EXCEPTION 'union due runner changed since review; reconcile before applying';
  END IF;
END $baseline$;
...
CREATE TABLE public.union_accounting_runs (
...
ALTER TABLE public.union_accounting_runs ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.union_accounting_runs FROM PUBLIC, anon, authenticated;
GRANT SELECT ON public.union_accounting_runs TO authenticated;
GRANT ALL ON public.union_accounting_runs TO service_role;
CREATE POLICY union_accounting_runs_scoped_read ON public.union_accounting_runs
  FOR SELECT TO authenticated USING (public.ca_can_oversee_union(union_id));
...
REVOKE ALL ON FUNCTION public.fn_union_accounting_run_at(timestamptz) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION public.fn_union_accounting_run_at(timestamptz) TO service_role,authenticated;
INSERT INTO public.ca_money_rpc_registry(proname,status,notes) VALUES
 ('fn_union_settlement_cascade_due','approved','Server-only weekly coordinator; per-union rollback and durable result, no independent balance writes'),
 ('fn_union_settlement_cascade','approved','Existing union waterfall; downstream refusal rolls back the entire attempt')
ON CONFLICT(proname) DO NOTHING;

-- Keep the existing job identity. The 04:00 local attempt is retried after the
-- hourly maintenance window at :05; later retries repair outages automatically.
-- The complete JSON result is stored in union_accounting_runs, not discarded
-- behind IS NOT NULL (which made a refused run look successful).
SELECT cron.schedule('union-weekly-rakeback-close','0,5,20,35 * * * *',
  $job$SET statement_timeout='2400s'; SELECT public.fn_union_settlement_cascade_due();$job$);
