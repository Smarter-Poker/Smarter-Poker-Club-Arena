-- Horse-only daily review population. Legacy abs(net)>=20BB review rates are
-- unchanged. Read immutable accepted contribution/refund facts after commit;
-- never reconstruct chips from mixed raise-to/increment action amounts.
-- This is eligibility capture and evidence triage, NOT a GTO verdict or policy
-- update. All reviews stay unverified until full decision replay and a matched
-- reference exist. Repeated three-day sweeps recover late visible rows, but
-- source timestamps/current profiles do not establish historical completeness.
BEGIN;
SET LOCAL lock_timeout = '250ms';
CREATE TABLE public.horse_commitment_audit_days (
  ...
  scanned_hands bigint NOT NULL DEFAULT 0,
  horse_hands bigint NOT NULL DEFAULT 0,
  flagged_horse_hands bigint NOT NULL DEFAULT 0,
  unknown_horse_hands bigint NOT NULL DEFAULT 0,
  ...
  CHECK (pass>0 AND scanned_hands>=0 AND horse_hands>=0 AND flagged_horse_hands>=0 AND unknown_horse_hands>=0 AND hand_gaps>=0)
);
CREATE TABLE public.horse_commitment_reviews (
  ...
  committed_bb numeric,
  eligibility text NOT NULL CHECK (eligibility IN ('over_10bb','unknown')),
  gto_verdict text NOT NULL DEFAULT 'unverified' CHECK (gto_verdict='unverified'),
  reasons text[] NOT NULL,
  PRIMARY KEY (hand_id,horse_user_id),
  CHECK (eligibility <> 'over_10bb' OR (big_blind IS NOT NULL AND net_contribution IS NOT NULL AND returned_uncalled IS NOT NULL AND committed_amount IS NOT NULL AND committed_bb IS NOT NULL AND big_blind>0 AND net_contribution>=0 AND returned_uncalled>=0 AND committed_amount=net_contribution+returned_uncalled AND committed_bb=committed_amount/big_blind AND committed_bb>10))
);
CREATE INDEX horse_commitment_reviews_day ON public.horse_commitment_reviews(played_at,hand_id);
CREATE TABLE public.horse_commitment_audit_gaps (
  hand_id uuid PRIMARY KEY,
  played_at timestamptz NOT NULL,
  reasons text[] NOT NULL,
  observed_at timestamptz NOT NULL DEFAULT transaction_timestamp()
);
CREATE INDEX horse_commitment_audit_gaps_day ON public.horse_commitment_audit_gaps(played_at,hand_id);
ALTER TABLE public.horse_commitment_audit_days ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.horse_commitment_reviews ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.horse_commitment_audit_gaps ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.horse_commitment_audit_days,public.horse_commitment_reviews,public.horse_commitment_audit_gaps FROM PUBLIC,anon,authenticated,service_role;

CREATE FUNCTION public.fn_horse_commitment_audit_step()
...
DECLARE
  ...
  last_created timestamptz; last_id uuid; old_hash text;
...
BEGIN
  ...
  FOR h IN
    SELECT x.id,x.table_id,x.created_at,x.game_variant,x.big_blind,x.players,x.tournament_id,
      t.tournament_type,c.payload_hash,c.post_commit_payload_hash,
    ...
    IF roster_ok THEN
      ...
      FOR actor IN SELECT pr.id FROM jsonb_array_elements(h.players) p JOIN public.profiles pr ON pr.id=(p->>'userId')::uuid WHERE pr.is_horse IS TRUE ORDER BY pr.id LOOP
        ...
        IF gross IS NULL OR gross>10*bb THEN
          ...
          SELECT source_payload_hash INTO old_hash FROM public.horse_commitment_reviews WHERE hand_id=h.id AND horse_user_id=actor.id;
