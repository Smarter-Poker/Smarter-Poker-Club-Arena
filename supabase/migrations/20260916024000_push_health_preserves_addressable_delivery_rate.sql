-- Add exact addressable delivery metrics without changing legacy snapshot fields.
-- This is a reader-only successor to the installed 20260914140459 definition.
BEGIN;
SET LOCAL lock_timeout='3s';
SET LOCAL statement_timeout='30s';
DO $guard$ BEGIN
 IF md5(pg_get_functiondef('public.fn_push_health_snapshot(uuid)'::regprocedure)) <> '7a869169a979bdd444ca2ecfac746f97'
 THEN RAISE EXCEPTION 'push health reader changed since addressable-rate review'; END IF;
END $guard$;

CREATE OR REPLACE FUNCTION public.fn_push_health_snapshot(p_user_id uuid) RETURNS jsonb
...
BEGIN
 ...
 WITH recent AS MATERIALIZED (
  ...
  SELECT count(*) FILTER(WHERE created_at>=stamp-interval '24 hours') queued,
   ...
   count(*) FILTER(WHERE created_at>=stamp-interval '24 hours' AND status='skipped' AND failure_reason='no_subscription') unreachable,
 ...
 SELECT jsonb_build_object(
  ...
  'funnel',jsonb_build_object('windowHours',24,'queued',f.queued,'sent',f.sent,'suppressed',f.suppressed,
   ...
   -- Preserve the legacy raw-queue rate for existing RPC readers.
   'deliveryRate',CASE WHEN f.queued>0 THEN round(100.0*f.sent/f.queued) ELSE NULL END,
   'unreachable',f.unreachable,'addressable',f.queued-f.unreachable,
   'addressableDeliveryRate',CASE WHEN f.queued>f.unreachable THEN round(100.0*f.sent/(f.queued-f.unreachable)) ELSE NULL END),
