...
BEGIN
 ...
 IF NOT EXISTS(SELECT 1 FROM pg_proc WHERE oid=to_regprocedure('public.fn_f06_abort_mixed_unsettled_generation(uuid,jsonb)') AND md5(pg_get_functiondef(oid))='9a655cf82edac7513824335a4091e3b8' AND pg_get_userbyid(proowner)='postgres' AND proacl::text='{postgres=X/postgres,service_role=X/postgres}') THEN RAISE EXCEPTION 'F06_SPIN_PRIOR_AUTHORITY_CHANGED: %','public.fn_f06_abort_mixed_unsettled_generation(uuid,jsonb)'; END IF;
