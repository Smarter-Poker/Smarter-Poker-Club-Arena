-- The existing disposition may consume one original historical projection after retention loss.
...
BEGIN
 ...
 IF NOT EXISTS(SELECT 1 FROM pg_proc WHERE oid=to_regprocedure('public.fn_f06_abort_mixed_unsettled_generation(uuid,jsonb)') AND md5(pg_get_functiondef(oid))='483b508311d233d3da73e55db17499ce' AND pg_get_userbyid(proowner)='postgres' AND proacl::text='{postgres=X/postgres,service_role=X/postgres}') THEN RAISE EXCEPTION 'F06_SPIN_PRIOR_AUTHORITY_CHANGED: %','public.fn_f06_abort_mixed_unsettled_generation(uuid,jsonb)'; END IF;
...
DECLARE
 ...
 original_pruned_witness CONSTANT jsonb := $earlybird_original${
  "schema": "earlybird-original-pruned-projection/v1",
  "observed_at": "2026-09-18T13:04:23.077577+00:00",
  "source_raw_sha256": "ca66ad34a31449009c5e381a252f87aa2271d010543e6acbd2a2dcb3212c0bb3",
  "source_parsed_sha256": "19873d0ce16e526b7e06cb37ca77ba7a7c84ee751cc96f6698ef250f91d5cc4f",
  "full_snapshot_row_available": false,
  "snapshot_hash_scope": "historical_database_whole_row",
  "original_permit": {
    "state": "reserved",
    "table_id": "f2ad5d92-6640-4e14-acd0-555222be14fe",
    "lifecycle": 224250,
    "permit_id": "332ef667-03f4-40cf-bd0a-15503ef80a47",
    "custody_id": "95f802d6-81f2-4941-a8df-806ba409d8b2",
    "generation": "b16bba80-2454-46d6-a80b-372bdcd93b0f",
    "evidence_id": null,
    "hand_number": 12636137,
    "tournament_id": "a5aa6984-6c1c-4b59-aeb7-9e7878853bdd"
  },
  "paid_receipt_id": "3f5e8bcc-4aeb-4f8b-ae1c-61ed5fb40fcf",
  "snapshot": {
    "id": "e7cf319f-2911-40e5-9544-a434f5bf8982",
    "pot": 26312,
    "stage": "preflop",
    "players": [
      {
        "seat": 1,
        "stack": 0,
        "user_id": "dca6c345-c2ab-456f-98d9-dfbca3a43f7d",
        "deadInvested": 2500,
        "totalInvested": 2500,
        "returnedUncalled": 0,
        "individualAnteInvested": 2500
      },
      {
        "seat": 2,
        "stack": 296188,
        "user_id": "4f7f8abb-ba34-464c-9ecb-0ab7a972a978",
        "deadInvested": 15875,
        "totalInvested": 23812,
        "returnedUncalled": 0,
        "individualAnteInvested": 15875
      }
    ],
    "row_hash": "e076344a1e32ff8969cd212f663751c1",
    "table_id": "f2ad5d92-6640-4e14-acd0-555222be14fe",
    "created_at": "2026-09-18T12:47:33.136119+00:00",
    "updated_at": "2026-09-18T12:47:36.386485+00:00",
    "hand_number": 12636137,
    "is_complete": true,
    "state_stage": "preflop",
    "action_count": 0
  }
}$earlybird_original$::jsonb;
...
SELECT array_agg(DISTINCT x ORDER BY x) INTO generations FROM (SELECT generation x FROM smarter_private.f06_hand_permits WHERE permit_id=ANY(reserved_ids) UNION SELECT g) q;
 ...
 prior_proof:=NULL; historical:=NULL; snapshot_projection:=NULL;
 ...
 IF snapshot_projection->>'stage' IS DISTINCT FROM 'preflop'
 OR snapshot_projection->>'state_stage' IS DISTINCT FROM 'preflop'
 OR jsonb_typeof(snapshot_projection->'players') IS DISTINCT FROM 'array'
 OR jsonb_array_length(snapshot_projection->'players') NOT BETWEEN 2 AND 10 THEN
 RAISE EXCEPTION 'F06_ABORT_SNAPSHOT_CHANGED' USING ERRCODE='55000'; END IF;
 player_count:=jsonb_array_length(roster);
 IF player_count IS DISTINCT FROM jsonb_array_length(snapshot_projection->'players')
 OR (SELECT count(DISTINCT x->>'user_id') FROM jsonb_array_elements(snapshot_projection->'players') x)<>player_count
 OR EXISTS(SELECT 1 FROM jsonb_array_elements(snapshot_projection->'players') x WHERE NOT EXISTS(
 ...
 OR (snapshot_projection->>'pot')::numeric IS DISTINCT FROM
 (SELECT sum((x->>'totalInvested')::numeric) FROM jsonb_array_elements(snapshot_projection->'players') x) THEN
 ...
 IF (snapshot_projection->>'is_complete')::boolean THEN
 ...
 OR (pruned_snapshot AND snapshot_projection->'action_count' IS DISTINCT FROM '0'::jsonb) THEN
 ...
 OR paid.completed_at IS NULL OR paid.completed_at>(snapshot_projection->>'created_at')::timestamptz
 ...
 AND (hand_number>h.hand_number OR (hand_number=h.hand_number AND id<>(snapshot_projection->>'id')::uuid)))
 ...
 END IF;
 hands:=hands||jsonb_build_array(jsonb_build_object('permit',to_jsonb(h),'snapshot_id',(snapshot_projection->>'id')::uuid,
 'snapshot_hash',snapshot_projection->>'row_hash','roster',roster,'break_id',v_break_id,'prior',prior_proof) || CASE WHEN historical IS NULL THEN '{}'::jsonb ELSE jsonb_build_object('interruption',historical) END);
 END LOOP;
 IF prior_based>1
