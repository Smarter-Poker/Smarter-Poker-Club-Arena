-- Retained MTT originals preserve chips, movement authority and their old lease.
...
IF NOT EXISTS(SELECT 1 FROM pg_proc WHERE oid=to_regprocedure('public.fn_engine_lease_stale_seconds()') AND md5(pg_get_functiondef(oid))='483a7e0ee940744fd557f1f2144d9eba' AND proowner='postgres'::regrole) THEN RAISE EXCEPTION 'F06_RETAINED_AUTHORITY_CHANGED: public.fn_engine_lease_stale_seconds()'; END IF;
IF NOT EXISTS(SELECT 1 FROM pg_proc WHERE oid=to_regprocedure('public.fn_f06_abort_mixed_unsettled_generation(uuid,jsonb)') AND md5(pg_get_functiondef(oid))='483b508311d233d3da73e55db17499ce' AND proowner='postgres'::regrole) THEN RAISE EXCEPTION 'F06_RETAINED_AUTHORITY_CHANGED: public.fn_f06_abort_mixed_unsettled_generation(uuid,jsonb)'; END IF;
IF NOT EXISTS(SELECT 1 FROM pg_proc WHERE oid=to_regprocedure('smarter_private.f06_prior_committed_stacks(uuid,jsonb,jsonb)') AND md5(pg_get_functiondef(oid))='4f914e43de444919d99229775a2ac2e5' AND proowner='postgres'::regrole) THEN RAISE EXCEPTION 'F06_RETAINED_AUTHORITY_CHANGED: smarter_private.f06_prior_committed_stacks(uuid,jsonb,jsonb)'; END IF;
IF NOT EXISTS(SELECT 1 FROM pg_proc WHERE oid=to_regprocedure('public.fn_f06_finish_hand(uuid,uuid,uuid,text,uuid)') AND md5(pg_get_functiondef(oid))='d5700b1c4e4663c5b9e12915a75d269b' AND proowner='postgres'::regrole) THEN RAISE EXCEPTION 'F06_RETAINED_AUTHORITY_CHANGED: public.fn_f06_finish_hand(uuid,uuid,uuid,text,uuid)'; END IF;
IF NOT EXISTS(SELECT 1 FROM pg_proc WHERE oid=to_regprocedure('smarter_private.f06_generation_aborted(uuid,uuid)') AND md5(pg_get_functiondef(oid))='3530559a94372866bf3baec006a8fd3c' AND proowner='postgres'::regrole) THEN RAISE EXCEPTION 'F06_RETAINED_AUTHORITY_CHANGED: smarter_private.f06_generation_aborted(uuid,uuid)'; END IF;
IF NOT EXISTS(SELECT 1 FROM pg_proc WHERE oid=to_regprocedure('smarter_private.f06_aborted_hand_guard()') AND md5(pg_get_functiondef(oid))='56c232740eb495dd2e2164159dfe782c' AND proowner='postgres'::regrole) THEN RAISE EXCEPTION 'F06_RETAINED_AUTHORITY_CHANGED: smarter_private.f06_aborted_hand_guard()'; END IF;
IF NOT EXISTS(SELECT 1 FROM pg_trigger WHERE tgrelid='public.hand_atomic_commits'::regclass AND tgname='a00_f06_aborted_hand' AND tgenabled='O' AND tgfoid=to_regprocedure('smarter_private.f06_aborted_hand_guard()')) THEN RAISE EXCEPTION 'F06_RETAINED_FENCE_BINDING_CHANGED: a00_f06_aborted_hand'; END IF;
IF NOT EXISTS(SELECT 1 FROM pg_trigger WHERE tgrelid='public.hand_history'::regclass AND tgname='a00_f06_aborted_history' AND tgenabled='O' AND tgfoid=to_regprocedure('smarter_private.f06_aborted_hand_guard()')) THEN RAISE EXCEPTION 'F06_RETAINED_FENCE_BINDING_CHANGED: a00_f06_aborted_history'; END IF;
IF NOT EXISTS(SELECT 1 FROM pg_trigger WHERE tgrelid='public.engine_tournament_leases'::regclass AND tgname='f06_aborted_generation' AND tgenabled='O' AND tgfoid=to_regprocedure('smarter_private.f06_aborted_generation_guard()')) THEN RAISE EXCEPTION 'F06_RETAINED_FENCE_BINDING_CHANGED: f06_aborted_generation'; END IF;
IF NOT EXISTS(SELECT 1 FROM pg_trigger WHERE tgrelid='smarter_private.f06_mixed_aborts'::regclass AND tgname='f06_mixed_receipt_immutable' AND tgenabled='O' AND tgfoid=to_regprocedure('smarter_private.f06_abort_receipt_immutable()')) THEN RAISE EXCEPTION 'F06_RETAINED_FENCE_BINDING_CHANGED: f06_mixed_receipt_immutable'; END IF;
IF NOT EXISTS(SELECT 1 FROM pg_trigger WHERE tgrelid='smarter_private.f06_mixed_abort_hands'::regclass AND tgname='f06_mixed_hands_immutable' AND tgenabled='O' AND tgfoid=to_regprocedure('smarter_private.f06_abort_receipt_immutable()')) THEN RAISE EXCEPTION 'F06_RETAINED_FENCE_BINDING_CHANGED: f06_mixed_hands_immutable'; END IF;
IF NOT EXISTS(SELECT 1 FROM pg_trigger WHERE tgrelid='smarter_private.f06_mixed_abort_generations'::regclass AND tgname='f06_mixed_generations_immutable' AND tgenabled='O' AND tgfoid=to_regprocedure('smarter_private.f06_abort_receipt_immutable()')) THEN RAISE EXCEPTION 'F06_RETAINED_FENCE_BINDING_CHANGED: f06_mixed_generations_immutable'; END IF;
END $pins$;
-- This is a zero-credit original-hand disposition, not a move, payout or restart.
-- The caller separately proves exact physical original-owner retirement. Expiry
-- and missing snapshots alone never establish that the original hand did not start.
CREATE FUNCTION smarter_private.f06_retained_mtt_abort_snapshot(p_input jsonb)
...
BEGIN
 ...
 RETURN actual;
END $function$;
REVOKE ALL ON FUNCTION smarter_private.f06_retained_mtt_abort_snapshot(jsonb) FROM PUBLIC,anon,authenticated,service_role;

CREATE FUNCTION public.fn_f06_abort_retained_mtt_hands(p_receipt_id uuid,p_expected jsonb)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path=pg_catalog,public,smarter_private AS $function$
...
END $function$;
REVOKE ALL ON FUNCTION public.fn_f06_abort_retained_mtt_hands(uuid,jsonb) FROM PUBLIC,anon,authenticated,service_role;
GRANT EXECUTE ON FUNCTION public.fn_f06_abort_retained_mtt_hands(uuid,jsonb) TO service_role;
