...
-- otherwise the repository's newest (20260917204152) byte for byte.
--
-- PREIMAGE. The pinned preimage md5s below are that migration's own asserted
-- postimage, reproduced from the repository body on a native cluster. The
-- owner must confirm the live md5(prosrc) and md5(pg_get_functiondef) of
...
-- @live-proof: (SELECT md5(p.prosrc) = '1362b029e14fc53524890b4f7e117181' FROM pg_proc p WHERE p.oid = to_regprocedure('public.fn_upsert_tournament_schedule(jsonb)'))
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_catalog.pg_proc p
    WHERE p.oid = to_regprocedure('public.fn_upsert_tournament_schedule(jsonb)')
      AND md5(p.prosrc) = '1362b029e14fc53524890b4f7e117181'
      AND md5(pg_get_functiondef(p.oid)) = 'b5ae29c619954f64648fd34c89d6625a'
      AND pg_get_userbyid(p.proowner) = 'postgres'
      AND p.proacl::text = '{postgres=X/postgres,authenticated=X/postgres,service_role=X/postgres}'
      AND p.proconfig = ARRAY['search_path=public']::text[]
  ) THEN
