-- A BUST AWAITING ITS ELIMINATION DOES NOT HOLD A DEAD HAND (2026-09-26)
--
-- What was wrong
-- --------------
-- public.fn_f06_abort_abandoned_generation voids the hand a dead lease
-- generation left reserved. Its roster proof refused
-- F06_ABANDONED_ROSTER_CHANGED whenever a 'playing' registration named the
-- table without a live chair there. It meant "a player's chips are somewhere
-- this door cannot see". The one shape that actually met it is a player who
-- busted in an EARLIER, committed hand: the registration holds 0 chips, the
-- chair was vacated holding 0, and the only thing missing is the elimination,
-- which the dead generation never got to write.
--
-- Measured on production, 2026-09-26 (engine f1d956c3, adoption at
-- 04:12:22 UTC): the door refused F06_ABANDONED_ROSTER_CHANGED for 48 RUNNING
-- events (96 refusals: the adoption ask plus the table-admission ask). In
-- every one of the 48 the blocking registration was a bust with chips = 0 and
-- no live chair anywhere in the event (96 such registrations, all 0 chips, all
-- horses). The adopting manager's own elimination pass marked them eliminated
-- from 04:12:42 onwards (union_pnl_inventory_events: status playing ->
-- eliminated, eliminated_at back-dated to the bust), and the table admission's
-- ten-minute re-ask then aborted 45 generations at 04:22 with the unchanged
-- door. So the door was not protecting a chip: it was waiting for a status
-- write that has nothing to do with the voided hand, and every adoption that
-- meets a pending bust stalls its event for the whole refused cooldown (on the
-- builds before that cooldown existed, for the life of the manager).
--
-- What changes
-- ------------
-- One refusal is narrowed to exactly what it has to prove, nothing else moves:
--
--   * a 'playing' registration at the table without its live chair is admitted
--     ONLY when it holds 0 chips AND no chair that player ever had in the event
--     is live or holds a chip (left_at IS NULL or stack IS DISTINCT FROM 0
--     anywhere refuses). The player's own vacated chair is not required to
--     still carry its user id, because a later seat at that table reuses the
--     row (20 of the 96 had been reused by 04:30).
--   * such a bust cannot have been dealt into the voided hand: a snapshot
--     player without a live chair still refuses F06_ABORT_SAVED_STACKS_CHANGED
--     and a hand without a snapshot still refuses if it has hole cards.
--   * every admitted bust is named on the receipt
--     (hands[].busts_pending_elimination / never_started[].busts_pending_elimination).
--   * any registration holding ANY chips without its live chair, and every
--     other refusal of the door, is exactly as before.
--
-- The void still credits nothing and debits nothing, and the elimination is
-- still written by the manager's own elimination pass, not here.
...
BEGIN
  SELECT md5(p.prosrc) INTO v_src
   ...
   WHERE p.oid = 'public.fn_f06_abort_abandoned_generation(uuid,uuid,uuid,text,boolean)'::regprocedure
     ...
     AND p.prosecdef
     AND p.provolatile = 'v';
  IF v_src IS NULL OR v_src NOT IN ('9dc6adcbe4ea282151638ce2b5fb4e5d', '53950f516ec6139b4ad30d7753ab47e9') THEN
    RAISE EXCEPTION 'PREIMAGE: fn_f06_abort_abandoned_generation is not the definition this migration narrows (md5 %)', v_src;
  END IF;
END
$pre$;
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'POSTIMAGE: fn_f06_abort_abandoned_generation is not the narrowed definition with its owner, grants and settings';
  END IF;
