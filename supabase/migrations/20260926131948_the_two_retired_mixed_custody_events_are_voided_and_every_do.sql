...
-- lock_timeout 5s and a statement cap on the void (15 s here, 20 s for the
-- 351-entrant event in 20260926142646; the lease-stale window is 30 s), so a
-- busy lane or a slow void refuses cleanly (the whole transaction rolls back,
-- nothing written) instead of stalling the fleet.
...
--
-- TWO TRANSACTIONS (2026-09-26 14:25 UTC). The first calm apply held the
-- settlement lane and was still stamping 5a387a75's 351 registrations when
-- its 10 s cap refused it (nothing written). So this migration creates the
-- door, opens the guard clause and voids 615783bf (195 entrants) only;
-- 20260926142646 voids 5a387a75 in its own transaction and restores the
-- guard byte for byte. Between the two the clause is inert: it lets a write
-- through only inside a transaction holding an uncompleted claim, and only
-- this postgres/service_role door for these two ids can write one. The door
-- also no longer rewrites vacated chairs that already read 'left' (all 534
-- of them here), which was most of the per-row trigger work.
--
...
--
-- @live-proof: (SELECT status FROM public.tournaments WHERE id = '615783bf-15e3-40b7-9368-75f21b6ac53b') = 'CANCELLED'
-- @live-proof: (SELECT count(*) FROM smarter_private.f06_retired_event_voids WHERE tournament_id = '615783bf-15e3-40b7-9368-75f21b6ac53b' AND completed_at IS NOT NULL) = 1
...
BEGIN
  ...
  UPDATE public.table_seats s
   ...
   WHERE s.id = ANY (v_source_seat_ids) AND s.left_at IS NOT NULL
     -- Only a vacated chair that still differs is written (2026-09-26 14:25):
     -- rewriting every historical chair of a 351-entrant event through the
     -- table_seats triggers is what ran the first calm apply past its cap.
     AND (s.status IS DISTINCT FROM 'left' OR s.leave_pending IS DISTINCT FROM false
       OR s.is_sitting_out IS DISTINCT FROM false OR s.is_away IS DISTINCT FROM false
       OR s.sit_out_at IS NOT NULL OR s.scheduled_leave_hands IS NOT NULL);
...
-- statement is capped (15 s here), so the lane is held at most ~18 s.
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'POSTIMAGE: f06_source_guard does not carry the void clause';
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_proc p
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.tournaments WHERE id = '5a387a75-754a-416e-8fee-b85b15fc2702' AND status = 'RUNNING') THEN
    RAISE EXCEPTION 'POSTIMAGE: 5a387a75 is voided by the next migration, not this one';
  END IF;
  FOREACH t IN ARRAY ARRAY['615783bf-15e3-40b7-9368-75f21b6ac53b'::uuid] LOOP
