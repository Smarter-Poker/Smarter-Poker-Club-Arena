...
--
-- REWORK (2026-09-26, PRIMARY-CHAT review of #5152): the original version
-- of this migration (20260923215300) fixed this one job by hand and built
-- no way to catch the next one -- if the World Hub dispatcher drops another
-- job from ALL_CRONS without a matching row here, v_openclaw_job_staleness
-- will treat it as newly stale again, exactly as it did for eight days
-- here. This version adds dispatcher_path: the literal ALL_CRONS path
-- string a job was dispatched under (which can differ from job_name --
-- clawbot's own job_name, /cron/clawbot-orchestrator, is a normalized log
-- identifier distinct from its actual dispatch path,
-- /api/clawbot/orchestrator), so
-- scripts/ci/check-retired-cron-jobs-match-dispatcher.mjs can read the live
-- dispatcher file and assert that path is truly gone, both now and on every
-- future retirement that records one. The migration's version was also
-- regenerated: the original timestamp was hand-picked rather than reserved
-- against origin/main and sibling branches per CLAUDE.md 4.5, and the
-- original was never installed against production (schema_migrations has no
-- 20260923215300 row), so replacing it here does not touch history.
--
...
--
-- @live-proof: EXISTS (SELECT 1 FROM public.ca_retired_cron_jobs WHERE job_name = '/cron/clawbot-orchestrator' AND dispatcher_path = '/api/clawbot/orchestrator')

BEGIN;

ALTER TABLE public.ca_retired_cron_jobs
  ADD COLUMN IF NOT EXISTS dispatcher_path text;

COMMENT ON COLUMN public.ca_retired_cron_jobs.dispatcher_path IS
  'The literal ALL_CRONS path string this job was dispatched under in '
  'Smarter-Poker-World-Hub/scripts/openclaw-cron-dispatcher.py before '
  'retirement -- job_name is sometimes a normalized log identifier that '
  'differs from it (e.g. /api/clawbot/orchestrator vs '
  '/cron/clawbot-orchestrator). Null for a row registered before this '
  'column existed. scripts/ci/check-retired-cron-jobs-match-dispatcher.mjs '
  'reads it to confirm a retired job never silently returns to the live '
  'dispatcher.';

INSERT INTO public.ca_retired_cron_jobs (job_name, retired_at, reason, replaced_by, dispatcher_path)
VALUES (
  ...
  IF NOT EXISTS (
    SELECT 1 FROM public.ca_retired_cron_jobs
     WHERE job_name = '/cron/clawbot-orchestrator'
       AND dispatcher_path = '/api/clawbot/orchestrator'
  ) THEN
    RAISE EXCEPTION 'clawbot-orchestrator retirement row did not install with its dispatcher_path';
  END IF;
  IF EXISTS (
