 ...
 * receipt and resolves its alert with the receipt ids. A human is never clawed
 * back: the reversal refuses a recipient that is not a horse.
 */
...
describe('the mystery-bounty make-good', () => {
  ...
  it('declares the treasury, skips the clubs trigger, and credits through the idempotent wallet door with no tournament', () => {
    expect(MAKE_GOOD).toContain("fn_ca_declare_ledger('settlement', 'club_treasury', c_dss, NULL, v_key, ARRAY['clubs'])");
    expect(MAKE_GOOD).toContain("fn_credit_and_log(r.user_id, round(r.cents / 100.0, 2), v_key, 'settlement', v_desc, NULL)");
...
describe('the duplicate prize reversal', () => {
  it('recovers only from horses', () => {
  ...
  it('re-proves the double payment before moving a chip', () => {
    expect(REVERSAL).toContain('a recipient no longer shows exactly two equal top-up credits and no reversal');
  ...
  it('returns each duplicate to the union bank in one reversal leg', () => {
    expect(REVERSAL).toContain("fn_ca_declare_ledger('reversal', 'union_bank', c_union, NULL, v_key, ARRAY['union_wallets'])");
...
describe('dispositions that pay nothing', () => {
  ...
  it('the rakeback disposition refuses to apply if any recipient is human', () => {
