...
vi.mock('../src/hooks/useAuthUser', () => ({
  useAuthUser: () => ({ user: state.userId ? { id: state.userId } : null, isHydrating: false }),
}));
...
function NavigateClub() {
  ...
  return (
    <button onClick={() => navigate(`/clubs/${ID.otherClub}/cashier`)}>Switch Fixture Club</button>
  );
}
function mountCashier() {
  return render(
    <MemoryRouter initialEntries={[`/clubs/${ID.club}/cashier`]}>
      <Routes>
        <Route
...
describe('cashier club weekly statement boundary', () => {
  it('shows only scoped weekly funding, club payments and neutral retained chips', async () => {
    state.rows = [
      ...
      weeklyStatementRow({ club_id: ID.otherClub, net_amount: '777.00' }),
      weeklyStatementRow(),
    ];
    mountCashier();
  ...
  it.each([
    ...
    'shows unavailable for a bad read instead of a zero balance or empty history: %j',
    async (reply) => {
  ...
  it('discards an old response through an auth ABA without an intermediate render', async () => {
    ...
    state.reply = () =>
      ++reads === 1
        ? old.promise
        : Promise.resolve({ data: null, error: { message: 'new epoch unavailable' } });
    mountCashier();
    const panel = await openWeeklyStatements();
