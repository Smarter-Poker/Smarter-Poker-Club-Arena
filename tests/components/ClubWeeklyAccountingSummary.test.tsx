...
vi.mock('../../src/hooks/useAuthUser', () => ({ useAuthUser: () => ({
  user: fixture.userId ? { id: fixture.userId } : null, isHydrating: false,
}) }));
vi.mock('../../src/core/IdentityDNA', () => ({ getIdentityDNAStatus: () => ({
  loaded: !!fixture.userId, authenticated: !!fixture.userId, userId: fixture.userId,
}) }));
...
beforeEach(() => {
  vi.clearAllMocks(); fixture.rows = []; fixture.reply = null; fixture.calls = [];
  ...
  vi.mocked(supabase.from).mockImplementation((table: string) => {
    ...
    query.eq = vi.fn((key: string, value: unknown) => { call.filters.push([key, value]); return query; });
    query.limit = vi.fn((cap: number) => { call.cap = cap; return query; });
    query.then = (done: (value: unknown) => unknown, fail: (error: unknown) => unknown) => {
      const reply = fixture.reply ? fixture.reply() : Promise.resolve({
        data: fixture.rows.filter(row => call.filters.every(([key, value]) => row[key] === value)).slice(0, call.cap),
        error: null,
      });
...
describe('issued club weekly summaries', () => {
  it('shows aggregate exact cents only and never queries individual commissions or a payer', async () => {
    fixture.rows = [weeklyStatementRow(), weeklyStatementRow({ invoice_type: 'transaction_receipt', gross_amount: '987654.32' })];
    ...
    expect(fixture.calls[0]).toEqual({ table: 'settlement_invoices', cap: 50,
      filters: [['club_id', ID.club], ['invoice_type', 'club_weekly_accounting']] });
  ...
  it('never publishes a late previous-club response over the next club', async () => {
    ...
    await screen.findByText('No Issued Weekly Summaries Were Found For This Club.');
    await act(async () => { old.resolve({ data: [weeklyStatementRow()], error: null }); });
    expect(screen.queryByRole('table')).toBeNull();
    expect(screen.queryByRole('alert')).toBeNull();
  });
