...
vi.mock('../../src/utils/strictClubIdResolver',() => ({ resolveClubUUIDStrict:vi.fn(async id => id) }));
vi.mock('../../src/utils/clubIdResolver',() => ({ resolveClubUUID:async (id:string) => id }));
vi.mock('../../src/utils/errorReporter',() => ({ reportError:vi.fn() }));
vi.mock('../../src/hooks/useAuthUser',() => ({ useAuthUser:() => ({ user:{ id:auth.user } }) }));
...
const lookup = vi.fn(),apply = vi.fn(),retire = vi.fn();
const view = () => true;
let revision = 0;
const props = () => ({ actorId:ID.actor,clubId:ID.club,targetUserId:ID.target,amount:'250',note:'',enabled:true,
  isCurrent:view,revision,getRevision:() => revision });
function deferred<T>() {
...
beforeEach(() => {
  revision = 0; signIn(null); signIn(ID.actor); localStorage.clear(); sessionStorage.clear(); vi.stubGlobal('crypto',webcrypto);
  vi.mocked(masterBus.emit).mockClear(); vi.mocked(CreditService.lowerCreditLine).mockClear();
  ...
  vi.mocked(supabase.rpc).mockReset().mockImplementation((name,args) => {
    if (name === 'fn_agent_credit_reduction_snapshot_v1') return Promise.resolve({ data:creditSnapshot(),error:null }) as never;
...
const button = () => screen.getByRole('button',{ name:'Reduce Credit Line' }) as HTMLButtonElement;
async function submit() { await waitFor(() => expect(button().disabled).toBe(false)); fireEvent.click(button()); }
...
describe('credit reduction controls and retained receipts',() => {
  it('debounces idle preparation while immediate invalidation keeps only the latest exact intent eligible',async () => {
    vi.useFakeTimers();
    const snapshots = () => vi.mocked(supabase.rpc).mock.calls.filter(([name]) => name === 'fn_agent_credit_reduction_snapshot_v1');
    const component = render(<CreditReductionControls {...props()} />);
    await act(async () => { await vi.advanceTimersByTimeAsync(200); });
    expect(snapshots()).toHaveLength(0); expect(button().disabled).toBe(true);
    revision += 1; component.rerender(<CreditReductionControls {...props()} amount="25" />);
    await act(async () => { await vi.advanceTimersByTimeAsync(200); });
    expect(snapshots()).toHaveLength(0);
    revision += 1; component.rerender(<CreditReductionControls {...props()} amount="27.15" note=" Updated " />);
    await act(async () => { await vi.advanceTimersByTimeAsync(299); });
    expect(snapshots()).toHaveLength(0); expect(button().disabled).toBe(true);
    await act(async () => { await vi.advanceTimersByTimeAsync(1); });
    await vi.waitFor(() => expect(snapshots()).toHaveLength(1));
    await vi.waitFor(() => expect(button().disabled).toBe(false));
    apply.mockImplementationOnce(async (_name,args) => ({ data:creditEnvelope(args,'recorded',{
      requested_reduction:'27.15',applied_reduction:'27.15',after_limit:'72.85',after_prepaid:false,
      reason:'Updated',assignment_reason:'Updated',
    },false),error:null }));
  ...
  it.each(['start','check','cancel'] as const)('shows a synchronous %s capture failure with no dispatch or stuck busy state',async action => {
    if (action !== 'start') apply.mockRejectedValueOnce(new Error('lost'));
    render(<CreditReductionControls {...props()} />);
    if (action !== 'start') { await submit(); await screen.findByText(/Response Was Lost/); await screen.findByRole('button',{ name:'Check Receipt' }); }
    else await waitFor(() => expect(button().disabled).toBe(false));
    const previousApplies = apply.mock.calls.length, previousLookups = lookup.mock.calls.length;
    vi.spyOn(crypto,'getRandomValues').mockImplementationOnce(() => { throw new Error('Random Source Unavailable'); });
    fireEvent.click(action === 'start' ? button() : screen.getByRole('button',{ name:action === 'check' ? 'Check Receipt' : 'Cancel Pending Change' }));
    ...
    expect(apply).toHaveBeenCalledTimes(previousApplies); expect(lookup).toHaveBeenCalledTimes(previousLookups);
    expect(retire).not.toHaveBeenCalled();
  });
  it('shows applied reduction and historical limit, never the requested amount as a successful payment',async () => {
    render(<CreditReductionControls {...props()} />); await submit();
    ...
    expect(masterBus.emit).toHaveBeenCalledWith('CREDIT_UPDATED',{ clubId:ID.club,userId:ID.target });
  });
  it('restores an unknown item after unmount, and Check Receipt cannot become another reduction',async () => {
    ...
    const first = render(<CreditReductionControls {...props()} />); await submit();
    await screen.findByText(/Response Was Lost/); const operation = apply.mock.calls[0][1].p_operation_id;
    first.unmount(); sessionStorage.clear();
    render(<CreditReductionControls {...props()} amount="1" />);
    const check = await screen.findByRole('button',{ name:'Check Receipt' }); fireEvent.click(check);
    expect(await screen.findByText('No Recorded Result Yet. This Change Remains Pending.')).toBeTruthy();
    expect(screen.getByText('Pending Credit Change 1')).toBeTruthy();
    expect(lookup.mock.calls.at(-1)![1].p_operation_id).toBe(operation); expect(apply).toHaveBeenCalledTimes(1);
    expect(button().disabled).toBe(true);
    fireEvent.click(screen.getByRole('button',{ name:'Cancel Pending Change' }));
    expect(await screen.findByText('Pending Change Cancelled. Its Original Request Cannot Apply.')).toBeTruthy();
  ...
  it('recovers an already-applied original result after reload without another apply',async () => {
    apply.mockRejectedValueOnce(new Error('lost')); const first = render(<CreditReductionControls {...props()} />);
    await submit(); await screen.findByText(/Response Was Lost/); first.unmount(); sessionStorage.clear();
    lookup.mockImplementation(async (_name,args) => ({ data:creditEnvelope(args,'recorded'),error:null }));
    ...
    expect(await screen.findByText('Credit Line Reduced By 100.00 Chips.')).toBeTruthy();
    expect(apply).toHaveBeenCalledTimes(1); expect(screen.queryByRole('button',{ name:'Reduce Credit Line' })).toBeNull();
  });
  it('retires a pending form gesture through amount ABA before its receipt lookup returns',async () => {
    ...
    const component = render(<CreditReductionControls {...props()} />); await submit();
    await waitFor(() => expect(lookup).toHaveBeenCalledTimes(1));
    act(() => { revision += 2; component.rerender(<CreditReductionControls {...props()} />); });
    await act(async () => { delayed.resolve({ data:creditEnvelope(lookup.mock.calls[0][1]),error:null }); });
    await waitFor(() => expect(screen.queryByText('Checking Credit Change...')).toBeNull());
    expect(apply).not.toHaveBeenCalled(); expect(screen.queryByText(/Credit Line Reduced By/)).toBeNull();
  ...
  it('remounts the actual account epoch while an old request remains pending, with no manual rerender',async () => {
    const delayed = deferred<any>(); lookup.mockReturnValueOnce(delayed.promise);
    render(<EpochWrapper />); await submit(); await waitFor(() => expect(lookup).toHaveBeenCalledTimes(1));
    act(() => { signIn(null); signIn(ID.actor); });
    await screen.findByRole('button',{ name:'Check Receipt' });
    await act(async () => { delayed.resolve({ data:creditEnvelope(lookup.mock.calls[0][1],'recorded'),error:null }); });
    expect(screen.queryByText(/Credit Line Reduced By/)).toBeNull(); expect(masterBus.emit).not.toHaveBeenCalled();
  ...
  it('blocks invalid precision during idle preparation without dispatching',async () => {
    ...
    expect(button().disabled).toBe(true); expect(apply).not.toHaveBeenCalled();
    expect(vi.mocked(supabase.rpc).mock.calls.filter(([name]) => name === 'fn_agent_credit_reduction_snapshot_v1')).toHaveLength(0);
  });
});
