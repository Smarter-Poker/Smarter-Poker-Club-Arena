...
/**
 * A BLANK SEED NEVER STALLS A WON GAME (review 2026-09-22, restated under
 * owner ruling 2026-09-21, R1). "Your Seed" is a free text field in Bonus
 * Proof. Emptying it used to strand a won game: pressing Drop made
 * DiamondBonusService throw BonusRefusal('Enter A Seed With 1 To 64
 * Characters') before anything was sent, which play()'s catch cannot tell
 * from a refusal the server gave - the ticket dropped while
 * useLiveBonusGuard still held every exit for the award. Donkey Cross was
 * fixed in #5096; Plinko drops a seed of its own the same way. The original
 * of this block also covered the five-second countdown that pressed Drop for
 * the player. That countdown is gone (R1: a game never starts itself), so
 * what the empty field must not break now is the player's own press.
 */
describe('a blank seed never stalls a won Plinko game', () => {
  ...
  it('waits for the press with the field cleared, then drops a seed of its own', async () => {
    fakeClock();
    won();
    ...
    await advance();
    // The award still holds every exit, and the empty field starts nothing:
    // owner ruling 2026-09-21, R1, a won game waits for Drop Diamonds.
    expect(holding()).toBe(true);
    await advance(60000);
    expect(backend.start).not.toHaveBeenCalled();
    fireEvent.click(drop());
    ...
    // The seed written back must not buy a second drop on the same award.
    await advance(60000);
    expect(backend.start).toHaveBeenCalledTimes(1);
  });
