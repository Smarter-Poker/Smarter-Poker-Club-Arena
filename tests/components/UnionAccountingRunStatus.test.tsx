  ...
  supabase: {
    ...
    from: (table: string) => {
      ...
      q.eq = (key: string, value: unknown) => {
        ...
        m.calls.push(['eq', key, value]);
        return q;
      };
      q.then = (yes: any, no: any) =>
        Promise.resolve({
          data: m.rows.filter((row) => filters.every(([key, value]) => row[key] === value)),
          error: null,
        }).then(yes, no);
      return q;
    },
...
it('shows incomplete canonical status without individual diagnostics or payer actions', async () => {
  m.rpc.mockResolvedValue({
    data: observationRow({ state: 'incomplete', posted: false }),
    error: null,
  });
  ...
  expect(m.rpc).toHaveBeenCalledWith(
    'fn_accounting_run_observation_v1',
    expect.objectContaining({ p_scope_id: ID.union, p_period_start: '2026-09-07T07:00:00.000Z' })
  );
});
it.each([
  { data: null, error: { message: 'offline' } },
  { data: { status: 'complete', result: { success: true, accounting_version: 3 } }, error: null },
])('makes legacy/unavailable evidence visibly unavailable', async (reply) => {
  m.rpc.mockResolvedValue(reply);
  render(<UnionAccountingRunStatus unionId={ID.union} periodEnd="2026-09-14" />);
...
it('does not display a late result after union A to B to A route changes', async () => {
  ...
  m.rpc
    ...
    .mockResolvedValue({ data: missingObservation(), error: null });
...
it('mounts the actual shared workspace and weekly reader even when club applicability is unknown', async () => {
  ...
  m.rows = [
    ...
    weeklyStatementRow({ club_id: WEEKLY_ID.otherClub }),
  ];
  m.rpc.mockImplementation(async (_name, args) => ({
    data: {
      ...missingObservation('club', WEEKLY_ID.club),
      actor_user_id: WEEKLY_ID.actor,
  ...
  render(
    <MemoryRouter>
      <WeeklyAccountingWorkspace scopeKind="club" scopeRef={WEEKLY_ID.club} />
    </MemoryRouter>
  );
