/**
 * Source-only routed accounting regressions, UNRUN.
...
import { OBSERVER_ID, missingObservation, observationRow, recordedPeriod } from '../helpers/accountingObservation';
...
const m = vi.hoisted(() => ({
  ...
  dashboard: { status: 'ready', clubId: null, platformWide: true, clubRole: null,
    isPlatformStaff: true, userId: null, message: null } as {
      status: 'loading' | 'ready' | 'denied' | 'error'; clubId: string | null;
      platformWide: boolean; clubRole: string | null; isPlatformStaff: boolean;
      userId: string | null; message: string | null;
    },
  reload: vi.fn(),
}));
vi.mock('../../src/core/IdentityDNA', () => ({ getIdentityDNAStatus: () => ({
  loaded: !m.hydrating, authenticated: !!m.actor, userId: m.actor,
}) }));
vi.mock('../../src/core/MasterBus', () => ({ masterBus: {
  emit: vi.fn(),
  subscribe: (name: string, callback: (event: { payload: { isAuthenticated: boolean; userId?: string } }) => void) => {
...
vi.mock('../../src/hooks/useAuthUser', async () => {
  ...
  return { useAuthUser: () => {
    ...
    useSyncExternalStore(callback => { m.authSubscribers.add(callback); return () => m.authSubscribers.delete(callback); },
      () => `${m.authRevision}:${m.actor ?? ''}:${String(m.hydrating)}`);
    return { user: m.actor ? { id: m.actor } : null, isHydrating: m.hydrating };
  } };
});
vi.mock('../../src/hooks/useFinancialAdminScope', () => ({ useFinancialAdminScope: () => ({ ...m.dashboard, reload: m.reload }) }));
...
vi.mock('../../src/components/common/TransactionLedgerView', () => ({
  default: (props: { clubId?: string; userId?: string; clubScoped?: boolean; limit?: number }) => (
    <div data-testid="transaction-ledger" data-club={props.clubId ?? ''} data-user={props.userId ?? ''}
      data-club-scoped={String(props.clubScoped === true)} data-limit={String(props.limit)} />
  ),
}));
vi.mock('../../src/lib/supabase', () => ({ supabase: {
  ...
  from: (table: string) => {
    ...
    query.select = (fields: string) => { call.fields = fields; return query; };
    for (const operation of ['eq', 'is', 'gte', 'lte']) {
      query[operation] = (column: string, value: unknown) => { call.filters.push({ operation, column, value }); return query; };
    }
    query.order = (column: string, options: { ascending: boolean }) => {
      call.orders.push({ column, ascending: options.ascending }); return query;
    };
    query.limit = (limit: number) => { call.limit = limit; return query; };
    query.then = (yes: (reply: Reply) => unknown, no: (reason: unknown) => unknown) => {
      let reply: Reply | Promise<Reply>;
      if (m.reply) reply = m.reply(call);
      else {
        const rows = (m.rows[table] ?? []).filter(row => call.filters.every(filter => {
          if (filter.operation === 'gte') return String(row[filter.column]) >= String(filter.value);
          if (filter.operation === 'lte') return String(row[filter.column]) <= String(filter.value);
...
function auth(actor: string | null) {
  m.actor = actor; m.authRevision += 1;
  for (const callback of [...m.handlers]) callback({ payload: { isAuthenticated: !!actor, userId: actor ?? undefined } });
...
function missingFor(args: Record<string, string>) {
  ...
  return { ...missingObservation(args.p_scope_kind as 'club' | 'union', args.p_scope_id),
    actor_user_id: args.p_expected_actor_id, period_start: week.periodStart, period_end: week.periodEnd,
    expected_run_at: accountingRunExpectedAt(week), observed_at: '2026-09-15T12:00:00.000001Z' };
}
function postedClub() {
...
function RoutedPages({ initial }: { initial: string }) {
  return <MemoryRouter initialEntries={[initial]}>
...
function expectNoManualAccountingControls() {
  ...
  expect(within(main).queryByRole('button', { name: /open (?:new )?period|start period|close|pay|distribute|execute|canary/i })).not.toBeInTheDocument();
}
beforeEach(() => {
  vi.useFakeTimers({ toFake: ['Date'] });
  vi.setSystemTime(new Date('2026-09-15T12:00:00Z'));
  m.rpc.mockReset(); m.resolveClub.mockReset(); m.resolveUnion.mockReset(); m.reload.mockReset();
  m.hydrating = false; m.rows = {}; m.calls = []; m.reply = null;
  ...
  m.dashboard = { status: 'ready', clubId: null, platformWide: true, clubRole: null,
    isPlatformStaff: true, userId: ID.actor, message: null };
  auth(null); auth(ID.actor);
  m.rpc.mockImplementation(async (_name: string, args: Record<string, string>) => ({ data: missingFor(args), error: null }));
});
...
describe('routed automatic weekly accounting', () => {
  it('keeps the actual club weekly-only reader and cashier link while a selected valid DST week changes only the observer request', async () => {
    ...
    expect(screen.queryByRole('heading', { name: 'Recorded Union Periods' })).not.toBeInTheDocument();
    const summaryReads = m.calls.filter(call => call.table === 'settlement_invoices').length;
    m.rpc.mockClear();
    fireEvent.change(screen.getByLabelText('Week Ending Monday'), { target: { value: '2026-03-09' } });
    await waitFor(() => expect(m.rpc).toHaveBeenCalledTimes(1));
    expect(m.rpc).toHaveBeenCalledWith('fn_accounting_run_observation_v1', {
      p_expected_actor_id: ID.actor, p_scope_kind: 'club', p_scope_id: ID.club,
      p_period_start: '2026-03-02T08:00:00.000Z', p_period_end: '2026-03-09T07:00:00.000Z',
    });
    expect(m.calls.filter(call => call.table === 'settlement_invoices')).toHaveLength(summaryReads);
    expect(m.calls.every(call => call.table === 'settlement_invoices')).toBe(true);
    const query = m.calls[0];
    expect(query.filters).toEqual([{ operation: 'eq', column: 'club_id', value: ID.club },
      { operation: 'eq', column: 'invoice_type', value: 'club_weekly_accounting' }]);
    expect(query.limit).toBe(50);
    expectNoManualAccountingControls(); expectOnlyObserverRPCs();
  ...
  it('uses actual bounded union aggregate period records and preserves the weekly-statement link without showing their amounts as payments', async () => {
    m.rows.settlement_periods = [recordedPeriod(),
    ...
    expect(within(history).getByText(/Recorded period status alone does not prove payment/)).toBeInTheDocument();
    expect(screen.queryByText('100.25 Chips')).not.toBeInTheDocument();
    expect(screen.queryByText('10.25')).not.toBeInTheDocument();
    expect(m.calls).toHaveLength(1);
    expect(m.calls[0]).toMatchObject({ table: 'settlement_periods', limit: 12,
      filters: [{ operation: 'eq', column: 'union_id', value: ID.union }, { operation: 'is', column: 'club_id', value: null }],
      orders: [{ column: 'start_at', ascending: false }, { column: 'id', ascending: false }] });
  ...
  it.each(['empty', 'error', 'malformed'] as const)('keeps a real %s union period result distinct from an unavailable read', async mode => {
    m.reply = () => mode === 'empty' ? { data: [], error: null }
      : mode === 'error' ? { data: null, error: { message: 'transport refused' } }
      : { data: [recordedPeriod({ union_id: ID.otherUnion })], error: null };
    render(<RoutedPages initial={`/unions/${ID.union}/settlement`} />);
    if (mode === 'empty') {
      expect(await screen.findByText('No Recorded Periods Were Found For This Union.')).toBeInTheDocument();
      ...
      expect(screen.queryByText('No Recorded Periods Were Found For This Union.')).not.toBeInTheDocument();
    }
    expectOnlyObserverRPCs();
  });

  it.each([
    { from: 'club', to: 'union', oldId: ID.club, targetId: ID.union, navigation: 'Test Open Union',
      heading: 'Union Weekly Accounting', empty: 'No Recorded Periods Were Found For This Union.', table: 'settlement_periods' },
    { from: 'union', to: 'club', oldId: ID.union, targetId: ID.club, navigation: 'Test Open Club',
      heading: 'Club Weekly Accounting', empty: 'No Issued Weekly Summaries Were Found For This Club.', table: 'settlement_invoices' },
  ] as const)('discards a delayed $from alias after navigating to $to before any old financial read dispatches', async route => {
    const oldAlias = deferred<string>();
    const resolver = route.from === 'club' ? m.resolveClub : m.resolveUnion;
    resolver.mockImplementation((reference: string) => reference === 'slow-scope' ? oldAlias.promise : Promise.resolve(reference));
    ...
    expect(m.rpc).not.toHaveBeenCalled(); expect(m.calls).toHaveLength(0);
    ...
    expect((await screen.findAllByRole('heading', { name: route.heading })).length).toBeGreaterThan(0);
    await screen.findByText(route.empty);
    await act(async () => { oldAlias.resolve(route.oldId); });
    expect(screen.queryByRole('heading', { name: route.from === 'club' ? 'Club Weekly Accounting' : 'Union Weekly Accounting' })).not.toBeInTheDocument();
    expect(m.calls.every(call => call.table === route.table)).toBe(true);
    expect(m.rpc.mock.calls.every(([, args]) => args.p_scope_kind === route.to && args.p_scope_id === route.targetId)).toBe(true);
  });
  ...
  it('retires delayed club receipts through club to union to club route changes instead of republishing the first club view', async () => {
    const oldStatus = deferred<Reply>(); const oldSummary = deferred<Reply>();
    m.rpc.mockReturnValueOnce(oldStatus.promise);
    let firstSummary = true;
    m.reply = call => {
      if (call.table === 'settlement_invoices' && firstSummary) { firstSummary = false; return oldSummary.promise; }
      return { data: [], error: null };
    };
    render(<RoutedPages initial={`/clubs/${ID.club}/settlement`} />);
    await waitFor(() => expect(m.calls.some(call => call.table === 'settlement_invoices')).toBe(true));
    await waitFor(() => expect(m.rpc).toHaveBeenCalledTimes(1));
    ...
    expect(screen.queryByText('100.25 Chips')).not.toBeInTheDocument();
    expect(screen.queryByRole('heading', { name: 'Recorded Union Periods' })).not.toBeInTheDocument();
    expect(screen.getByText('No Issued Weekly Summaries Were Found For This Club.')).toBeInTheDocument();
    expectOnlyObserverRPCs();
  });
  ...
  it('retires a delayed routed account A-B-A receipt even when both auth events precede the next render', async () => {
    ...
    expect(m.rpc).toHaveBeenLastCalledWith('fn_accounting_run_observation_v1', {
      p_expected_actor_id: ID.actor, p_scope_kind: 'club', p_scope_id: ID.club,
      p_period_start: WEEK_START, p_period_end: WEEK_END,
    });
    expectOnlyObserverRPCs();
  });
});
...
describe('routed settlement dashboard boundaries', () => {
  it.each([{ label: 'Open Clubs', path: '/clubs' }, { label: 'Open Unions', path: '/unions' }])(
    'preserves platform selection $label and the authenticated user ledger without inventing a global accounting run', async destination => {
      render(<RoutedPages initial="/settlement-dashboard" />);
      expect(screen.getByText(/Choose a club or union from its accounting page/)).toBeInTheDocument();
  ...
  it('uses the selected canonical club workspace and scopes the retained ledger to that club', async () => {
    ...
    expectNoManualAccountingControls(); expectOnlyObserverRPCs();
    expect(screen.getByRole('link', { name: 'Open Club Cashier And Records' })).toHaveAttribute('href', `/clubs/${ID.club}/cashier`);
  });
  ...
  it('removes the selected workspace and ledger when its account no longer matches the financial scope', async () => {
    ...
    act(() => { auth(ID.otherActor); });
    view.rerender(<RoutedPages initial="/settlement-dashboard" />);
    expect(screen.getByText('Accounting Is Unavailable Until This Account Is Ready.')).toBeInTheDocument();
