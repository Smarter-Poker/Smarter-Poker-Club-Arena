...
vi.mock('../../src/hooks/useAuthUser', () => ({
  useAuthUser: () => ({ user: { id: state.userId } }),
}));
vi.mock('../../src/services/CashoutService', () => ({
  captureCashoutAccountGuard: (actor: string) => {
    const epoch = state.epoch;
    return () => state.userId === actor && state.epoch === epoch;
  },
}));
...
vi.mock('../../src/services/CashoutOperation', () => ({
  ...
  captureCashoutStart: (prepared: any) => {
    ...
    state.capture(prepared);
    return prepared;
  },
  assertCashoutStartCurrent: (start: any) => {
...
vi.mock('../../src/utils/settlementLock', () => ({
  checkSettlementLock: (...args: unknown[]) => state.lock(...args),
}));
vi.mock('../../src/stores/useWalletStore', () => ({
  useWalletStore: () => ({ loadBalances: state.load, mintChips: vi.fn() }),
}));
...
vi.mock('../../src/hooks/useCashierHistory', () => ({
  useCashierHistory: () => ({ transactions: [], loading: false, error: null, load: state.history }),
}));
...
vi.mock('../../src/utils/clubQuickLink', () => ({
  CHIP_BALANCE_EVENTS: ['TEST_CHIP_BALANCE'],
  fetchClubChipBalances: async () =>
    state.balance === null
      ? null
      : new Map([['51000000-0000-4000-8000-000000000004', state.balance]]),
...
vi.mock('../../src/core/MasterBus', () => ({
  masterBus: {
    subscribe: (name: string, handler: () => void) => {
      ...
      return () => {
        state.authListeners.delete(handler);
      };
    },
    emit: vi.fn(),
...
function Navigation() {
  ...
  return (
    <button onClick={() => navigate('/cashier?club=51000000-0000-4000-8000-000000000005')}>
      Switch Club
    </button>
  );
}
...
beforeEach(() => {
  ...
  state.balance = 20000;
  state.recover.mockReset().mockResolvedValue({ found: false });
  ...
  state.run.mockReset().mockResolvedValue({ status: 'pending' });
...
it.each(['unknown', 'absent'] as const)(
  ...
  async (outcome) => {
    ...
    await act(async () => state.refreshBalance());
    if (outcome === 'unknown')
      state.confirm.mockRejectedValue(new Error('Receipt Lookup Unavailable'));
    fireEvent.click(confirm);
    await screen.findByText(
      outcome === 'unknown' ? /Receipt Lookup Unavailable/i : /Insufficient chips in this club/i
    );
...
it('hides already visible pending rows on a same-ID account epoch change until a fresh read returns', async () => {
  state.pendingRead.mockResolvedValue({
    data: [{ id: 'old', amount: 876, status: 'pending', created_at: '2026-09-15T00:00:00Z' }],
    error: null,
  });
  const view = render(
    <MemoryRouter initialEntries={[`/cashier?club=${CLUB}`]}>
  ...
  state.pendingRead.mockReturnValue(nextRead.promise);
  state.epoch += 2;
  view.rerender(
    <MemoryRouter initialEntries={[`/cashier?club=${CLUB}`]}>
      <CashierPage />
...
it('auth notification remounts the Cashier during an old pending action and its completion cannot unlock a fresh action', async () => {
  const oldResponse = deferred<{ status: string }>();
  const newResponse = deferred<{ status: string }>();
...
it.each([0, null])(
  'shows a recovered original hold with displayed balance %s without new-payment gates',
  async (balance) => {
...
it('unknown recovery and a changed account during lookup cannot reach payment checks', async () => {
  const gate = deferred<{ found: false }>();
  state.recover.mockReturnValueOnce(gate.promise);
  fireEvent.click(await mount('250'));
