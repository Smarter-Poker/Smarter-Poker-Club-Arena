...
vi.mock('../../src/hooks/useAuthUser', () => ({ useAuthUser: () => ({ user: { id: identity.userId } }) }));
...
vi.mock('../../src/utils/strictClubIdResolver', () => ({ resolveClubUUIDStrict: async (id: string) => id }));
...
vi.mock('../../src/utils/settlementLock', () => ({ checkSettlementLock: vi.fn(async () => ({ locked: false })) }));
vi.mock('../../src/hooks/useVisibilityRefresh', () => ({ useVisibilityRefresh: () => {} }));
vi.mock('../../src/utils/resolvePageClubId', () => ({ resolvePageClubId: async () => '51000000-0000-4000-8000-000000000004', pickPreferredClubId: () => null }));
vi.mock('../../src/utils/retryFetch', () => ({ retryFetch: (fn: () => unknown) => fn() }));
vi.mock('../../src/components/common/confirmDialog', () => ({ confirmDialog: vi.fn(async () => true) }));
...
const row = { id: ID.cashout, clubId: ID.club, playerId: ID.player, agentId: ID.agent,
  playerName: 'Original Player', amount: 250, status: 'pending' as const, createdAt: '2026-09-15T09:00:00+00:00' };
...
function retainedHistory() {
  const key = Object.keys(localStorage).find(key => key.startsWith('smarter-poker:cashout-generations:v2:') &&
    JSON.parse(localStorage.getItem(key)!).generations.some((generation: any) => generation.starts.length > 0));
...
beforeEach(() => {
  ...
  done = vi.fn(); dashboardReads = 0; dashboardReadGate = null; gateAcknowledgment = true; receiptReturned = false; ackWaiting = false;
  providerChange = () => { throw new Error('Channel Has Not Been Registered'); };
  ...
  Object.defineProperty(navigator, 'locks', { configurable: true, value: {
    request: (key: string, _options: unknown, callback: () => unknown) => {
      const next = (locks.get(key) ?? Promise.resolve()).then(async () => {
        if (gateAcknowledgment && receiptReturned && !ackWaiting) { ackWaiting = true; await ack; }
        return callback();
      });
  ...
  channel.on.mockImplementation((_event, _filter, callback) => { providerChange = callback; return channel; });
  ...
  vi.mocked(supabase.from).mockImplementation((table: string) => {
    ...
    query.then = (resolve: (value: unknown) => unknown) => {
      ...
      return Promise.resolve({ data: table === 'cashout_requests' ? [{ id: ID.cashout,
        player_id: ID.player, club_id: ID.club, amount: 250, status: 'pending', created_at: row.createdAt }] : [], error: null }).then(resolve);
    };
    return query as never;
  });
  vi.mocked(supabase.rpc).mockReset().mockImplementation(async (name, args) => {
    if (name === 'fn_cashout_operation_receipt_v2') return { data: cashoutLookupEnvelope(args!), error: null } as never;
    receiptReturned = true;
    return { data: { ...cashoutV2Receipt(name === 'fn_cashout_approve_v2' ? 'approval' : 'cancellation'), op_id: args!.p_op_id }, error: null } as never;
  });
});
afterEach(() => { cleanup(); releaseAck(); vi.useRealTimers(); vi.restoreAllMocks(); vi.unstubAllGlobals(); });

it.each(['panel', 'modal'] as const)('hides already visible %s rows on a batched real auth ABA while the new read is pending', async surface => {
  const actor = surface === 'panel' ? ID.agent : ID.player;
  signIn(actor);
  const element = () => surface === 'panel' ? <AgentCashoutPanel clubId={ID.club} />
    : <CashoutRequestModal isOpen playerId={ID.player} clubId={ID.club} currentBalance={500} onClose={() => {}} />;
  ...
  const rows = new Promise<typeof row[]>(resolve => { resolveRows = resolve; });
  const read = surface === 'panel' ? vi.mocked(cashoutService.getAgentPendingCashouts) : vi.mocked(cashoutService.getPlayerCashouts);
  read.mockReturnValue(rows);
  act(() => { signIn(ID.other); signIn(actor); view.rerender(element()); });
...
it('hides already visible dashboard cashout actions and totals on a batched real auth ABA before its new read returns', async () => {
  signIn(ID.agent);
  const element = () => <MemoryRouter initialEntries={[`/agent-dashboard?club=${ID.club}`]}><AgentDashboardPage /></MemoryRouter>;
  const view = render(element());
  ...
  dashboardReadGate = new Promise(resolve => { resolveRows = resolve; });
  act(() => { signIn(ID.other); signIn(ID.agent); view.rerender(element()); });
...
it.each(['panel', 'modal', 'dashboard'] as const)('a real auth ABA remounts %s while its old action is pending and preserves the operation for a fresh explicit recovery', async surface => {
  ...
  vi.mocked(supabase.rpc).mockImplementation((name, args) => {
    if (name === 'fn_cashout_operation_receipt_v2') return Promise.resolve({ data: cashoutLookupEnvelope(args!), error: null }) as never;
    return new Promise(resolve => pending.push({ args, resolve })) as never;
  });
  if (surface === 'panel') render(<AgentCashoutPanel clubId={ID.club} onCashoutProcessed={done} />);
  else if (surface === 'modal') render(<CashoutRequestModal isOpen playerId={ID.player} clubId={ID.club} currentBalance={500} onClose={() => {}} onComplete={done} />);
  else render(<MemoryRouter initialEntries={[`/agent-dashboard?club=${ID.club}`]}><AgentDashboardPage /></MemoryRouter>);
  const name = surface === 'panel' ? 'Approve Cashout' : surface === 'modal' ? 'Cancel' : 'Approve';
  if (surface === 'dashboard') fireEvent.click(await screen.findByRole('button', { name: /Cashouts/ }));
  const originalButton = await screen.findByRole('button', { name });
  await waitFor(() => expect(originalButton).not.toBeDisabled()); fireEvent.click(originalButton);
  await waitFor(() => expect(pending).toHaveLength(1));
  const operation = pending[0].args.p_op_id;
  let freshRows!: () => void;
  if (surface === 'dashboard') {
    dashboardReadGate = new Promise(resolve => { freshRows = () => resolve({ data: [{ id: row.id,
      club_id: row.clubId, player_id: row.playerId, amount: row.amount, status: row.status, created_at: row.createdAt }], error: null }); });
  } else {
    const read = surface === 'panel' ? vi.mocked(cashoutService.getAgentPendingCashouts) : vi.mocked(cashoutService.getPlayerCashouts);
    read.mockReturnValue(new Promise(resolve => { freshRows = () => resolve([row]); }));
  }
  const readsBefore = surface === 'dashboard' ? dashboardReads : surface === 'panel'
    ? vi.mocked(cashoutService.getAgentPendingCashouts).mock.calls.length : vi.mocked(cashoutService.getPlayerCashouts).mock.calls.length;
  // No caller rerender: the real auth bus must wake the outer scope-key hook.
  act(() => { signIn(ID.other); signIn(actor); });
  expect(screen.queryByRole('button', { name })).toBeNull();
  await waitFor(() => expect(surface === 'dashboard' ? dashboardReads : surface === 'panel'
    ? vi.mocked(cashoutService.getAgentPendingCashouts).mock.calls.length : vi.mocked(cashoutService.getPlayerCashouts).mock.calls.length).toBeGreaterThan(readsBefore));
  ...
  await act(async () => pending[0].resolve({ data: { ...cashoutV2Receipt(kind), op_id: operation }, error: null }));
  expect(freshButton).toBeDisabled();
  expect(done).not.toHaveBeenCalled();
  expect(screen.queryByText('Cashout approved. Invoice recorded.')).toBeNull();
  fireEvent.click(freshButton); expect(pending).toHaveLength(2);
  await act(async () => pending[1].resolve({ data: { ...cashoutV2Receipt(kind, { replayed: true }), op_id: operation }, error: null }));
...
it.each(['approval', 'cancellation'] as const)('real %s bus/realtime refresh waits for receipt acknowledgment', async kind => {
  signIn(kind === 'approval' ? ID.agent : ID.player);
  if (kind === 'approval') render(<AgentCashoutPanel clubId={ID.club} onCashoutProcessed={done} />);
  else render(<CashoutRequestModal isOpen playerId={ID.player} clubId={ID.club} currentBalance={500} onClose={() => {}} onComplete={done} />);
  const button = await screen.findByRole('button', { name: kind === 'approval' ? 'Approve Cashout' : 'Cancel' });
  await waitFor(() => expect(button).not.toBeDisabled());
  const read = kind === 'approval' ? vi.mocked(cashoutService.getAgentPendingCashouts) : vi.mocked(cashoutService.getPlayerCashouts);
  expect(read).toHaveBeenCalledOnce();
  vi.useFakeTimers();
  await act(async () => { fireEvent.click(button); });
  expect(ackWaiting).toBe(true);
  ...
  await act(async () => { providerChange(); await vi.advanceTimersByTimeAsync(500); });
  expect(read).toHaveBeenCalledOnce();
  expect(done).not.toHaveBeenCalled();
  ...
  expect(vi.mocked(supabase.rpc).mock.calls.filter(([name]) => name !== 'fn_cashout_operation_receipt_v2')).toHaveLength(1);
});
...
it('a real auth A to B to A event still retires the accepted action while ordinary refresh is deferred', async () => {
  ...
  expect(ackWaiting).toBe(true);
  await act(async () => {
    providerChange(); signIn(ID.other); signIn(ID.agent); releaseAck();
...
it('retains an unknown terminal operation after the pending row disappears, then checks its exact receipt without payment', async () => {
  ...
  vi.mocked(supabase.rpc).mockImplementation(async (name, args) => {
    ...
    return new Promise(resolve => { failResponse = () => resolve({ data: null, error: { message: 'Lost After Commit' } } as never); });
  });
  render(<AgentCashoutPanel clubId={ID.club} onCashoutProcessed={done} />);
  const approve = await screen.findByRole('button', { name: 'Approve Cashout' });
  await waitFor(() => expect(approve).not.toBeDisabled()); fireEvent.click(approve);
  await waitFor(() => expect(originalOperation).not.toBe(''));
  await act(async () => { providerChange(); failResponse(); });
  ...
  vi.mocked(supabase.rpc).mockImplementation(async (name, args) => {
    ...
    return { data: cashoutLookupEnvelope(args!, { ...cashoutV2Receipt('approval', { replayed: true }), op_id: originalOperation }), error: null } as never;
  });
  fireEvent.click(check);
  await waitFor(() => expect(done).toHaveBeenCalledOnce());
  expect(screen.queryByRole('button', { name: 'Check Receipt' })).toBeNull();
  expect(vi.mocked(supabase.rpc).mock.calls.slice(callsBeforeCheck).map(([name]) => name)).toEqual(['fn_cashout_operation_receipt_v2']);
});

it('an unknown receipt check stays unresolved and an account change removes the in-view card', async () => {
  gateAcknowledgment = false;
  signIn(ID.agent);
  vi.mocked(supabase.rpc).mockResolvedValue({ data: null, error: { message: 'Lookup Unavailable' } } as never);
  const view = render(<AgentCashoutPanel clubId={ID.club} />);
  const approve = await screen.findByRole('button', { name: 'Approve Cashout' });
  await waitFor(() => expect(approve).not.toBeDisabled()); fireEvent.click(approve);
  const check = await screen.findByRole('button', { name: 'Check Receipt' });
  fireEvent.click(check);
  await screen.findByText('The Receipt Could Not Be Verified. The Original Operation Is Still Unconfirmed.');
  expect(screen.getByRole('button', { name: 'Check Receipt' })).not.toBeDisabled();
  expect(vi.mocked(supabase.rpc).mock.calls.every(([name]) => name === 'fn_cashout_operation_receipt_v2')).toBe(true);
  act(() => signIn(ID.other));
...
it('the real dashboard bus listeners defer their full reload until its approval receipt is acknowledged', async () => {
  signIn(ID.agent);
  render(<MemoryRouter initialEntries={[`/agent-dashboard?club=${ID.club}`]}><AgentDashboardPage /></MemoryRouter>);
