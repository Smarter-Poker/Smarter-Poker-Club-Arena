...
describe('horses take seats, not just places on a list', () => {
  ...
  it('reads the busy set ONCE per call, not once per horse', () => {
    ...
    /* THE SLICE ENDS AT THE NEXT METHOD, WHICHEVER IT IS (2026-08-25).
       It used to run all the way to `createSpin`, and #805 inserted
       `unseatedRegistrantHorses` in between - a DIFFERENT call, with its own
       single batched read of the same three tables. Counting across both made
       the total 2 and left main red, blocking the World Hub sync for every
       agent. What this test protects is that ONE call does not loop a query
       per horse, so it is scoped to one call, and the new method is held to
       the same rule separately below. */
    const pick = recurring.slice(
      recurring.indexOf('private async horseLoadMap'),
