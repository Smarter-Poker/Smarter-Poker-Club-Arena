...
type Reply = { data: Record<string, unknown>[] | null; error: { message: string; code?: string } | null };
function deferred<T>() {
...
const mocks = vi.hoisted(() => ({
  ...
  queries: [] as { table: string; userId: string; filters: Map<string, unknown>; limit?: number; columns?: string }[],
...
vi.mock('../src/hooks/useFinancialAdminScope', async (original) => {
  ...
  return {
    ...
    useFinancialAdminScope: () => {
      ...
      return {
        status: 'ready', clubId: mocks.platformWide ? null : new URLSearchParams(location.search).get('club') || 'club-a',
        platformWide: mocks.platformWide, clubRole: mocks.clubRole, isPlatformStaff: mocks.platformWide,
        userId: mocks.scopeUserId ?? mocks.userId, message: null, reload: vi.fn(),
      };
    },
  };
});
...
vi.mock('../src/components/common/PageSkeleton', () => ({ default: () => <div>Loading credit</div> }));
...
vi.mock('../src/components/agent/CreditRequestWidget', () => ({
  CreditRequestManagerInbox: ({ clubId }: { clubId: string }) => <div data-testid="manager-credit-inbox">{clubId}</div>,
}));
vi.mock('../src/core/MasterBus', () => {
  ...
  return { masterBus: {
    subscribeDebounced: vi.fn(() => vi.fn()), getOrCreateChannel: vi.fn(() => channel),
    removeRegisteredChannel: vi.fn(), emit: vi.fn(),
  } };
});
vi.mock('../src/lib/supabase', () => ({
  supabase: {
    from: vi.fn((table: string) => {
      const query = { table, userId: mocks.userId, filters: new Map<string, unknown>(),
        limit: undefined as number | undefined, columns: undefined as string | undefined };
      mocks.queries.push(query);
      const builder: Record<string, any> = {};
      builder.select = (columns: string) => { query.columns = columns; return builder; };
      builder.eq = (key: string, value: unknown) => { query.filters.set(key, value); return builder; };
      builder.in = builder.eq;
      builder.order = () => builder;
      builder.limit = (limit: number) => { query.limit = limit; return builder; };
      builder.then = (resolve: (result: Reply) => unknown, reject: (error: unknown) => unknown) => {
        ...
        if (table === 'agents') {
          result = mocks.agentGates.get(key) || Promise.resolve({ data: (mocks.rows.get(key) || []).slice(0, query.limit), error: null });
        } else if (table === 'profiles') {
          result = mocks.profileGates.shift() || Promise.resolve({
            data: (query.filters.get('id') as string[] || []).map((id) => ({ id, username: `Name ${id}` })), error: null,
          });
        } else if (table === 'credit_assignments') {
          const ids = query.filters.get('agent_id') as string[];
          result = mocks.auditGates.get(`${query.userId}:${ids[0]}`) || Promise.resolve({
            data: mocks.auditRows.filter((row) => ids.includes(row.agent_id as string)).slice(0, query.limit), error: null,
          });
        } else {
...
function row(id = 'agent-a', club = 'club-a', overrides: Record<string, unknown> = {}) {
  return { id, user_id: `person-${id}`, club_id: club, status: 'active',
    credit_limit: '100.00', credit_used: '25.31', agent_wallet_balance: '500.00', ...overrides };
}
function OtherClub() {
...
function View({ revision = 0 }: { revision?: number }) {
  void revision;
  return <MemoryRouter initialEntries={['/credit-admin?club=club-a']}>
    <OtherClub /><CreditAdminPanel />
  </MemoryRouter>;
}
...
describe('Credit admin scoped view', () => {
  it('mounts the selected club review inbox even with no agent rows and follows club changes', async () => {
    ...
    await waitFor(() => expect(screen.getByTestId('manager-credit-inbox')).toHaveTextContent('club-b'));
  });

  it('does not mount a manager review inbox for an unselected platform-wide scope', async () => {
    mocks.platformWide = true;
    render(<View />);
    await waitFor(() => expect(screen.getByRole('button', { name: 'Export Shown Rows' })).toBeEnabled());
  ...
  it('exports recorded debt and labels its bounded shown-row scope', async () => {
    mocks.rows.set('operator-a:club-a', Array.from({ length: 101 }, (_, i) => row(`agent-${i}`)));
    render(<View />);
    await waitFor(() => expect(screen.getByRole('button', { name: 'Export Shown Rows' })).toBeEnabled());
    fireEvent.click(screen.getByRole('button', { name: 'Export Shown Rows' }));
    const [rows, filename] = mocks.export.mock.calls[0];
    expect(rows).toHaveLength(100);
    expect(rows[0]).toMatchObject({ clubId: 'club-a', debtOwed: '25.31', currentBalance: '500.00' });
    ...
    expect(mocks.queries.find((q) => q.table === 'agents')).toMatchObject({
      limit: 100,
      columns: 'id, user_id, club_id, agent_wallet_balance::text, credit_limit::text, credit_used::text, status',
    });
  });
  ...
  it('shows a failed data read as an error, not an empty or zero-debt view', async () => {
    mocks.agentGates.set('operator-a:club-a', Promise.resolve({ data: null, error: { code: '42501', message: 'Access refused' } }));
    render(<View />);
  ...
  it('drops a previous account response even when the club stays the same', async () => {
    ...
    await act(async () => { gate.resolve({ data: [row()], error: null }); await gate.promise; });
  ...
  it('drops an old audit failure after switching accounts', async () => {
    ...
    mocks.rows.set('operator-b:club-a', [row('agent-b')]);
    const view = render(<View />);
    await waitFor(() => expect(mocks.queries.some((q) => q.table === 'credit_assignments')).toBe(true));
    mocks.userId = 'operator-b';
    view.rerender(<View revision={1} />);
    await waitFor(() => expect(screen.getByRole('button', { name: 'Export Shown Rows' })).toBeEnabled());
    await act(async () => {
  ...
  it('reads canonical recent credit changes only for shown agents with exact decimal transport', async () => {
    mocks.auditRows = [
      { id: 'change-a', agent_id: 'agent-a', old_limit: '123.45', new_limit: '234.56', created_at: '2026-09-15T04:00:00Z' },
      { id: 'other-club-change', agent_id: 'other-club-agent', old_limit: '345.67', new_limit: '456.78', created_at: '2026-09-15T04:01:00Z' },
    ];
    render(<View />);
    await waitFor(() => expect(screen.getByText('Recent Credit Changes For Agents In View')).toBeInTheDocument());
    ...
    expect(query).toMatchObject({ limit: 5, columns: 'id, agent_id, old_limit::text, new_limit::text, created_at' });
  ...
  it('refuses an audit response containing an agent outside the shown scope', async () => {
    mocks.auditGates.set('operator-a:agent-a', Promise.resolve({ data: [
      { id: 'wrong-change', agent_id: 'other-club-agent', old_limit: '345.67', new_limit: '456.78', created_at: '2026-09-15T04:00:00Z' },
    ], error: null }));
    render(<View />);
    await waitFor(() => expect(screen.getByText('Recent credit changes are unavailable.')).toBeInTheDocument());
    expect(screen.queryByText('456.78')).not.toBeInTheDocument();
  ...
  it('keeps a refused canonical audit read unavailable instead of claiming no visible changes', async () => {
    mocks.auditGates.set('operator-a:agent-a', Promise.resolve({ data: null, error: { code: '42501', message: 'Audit access refused' } }));
    render(<View />);
    await waitFor(() => expect(screen.getByText('Recent credit changes are unavailable.')).toBeInTheDocument());
    expect(screen.queryByText('No visible credit changes for agents in this view.')).not.toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Export Shown Rows' })).toBeEnabled();
  });

  it('keeps nullable canonical audit limits unavailable and does not query an empty agent scope', async () => {
    mocks.auditRows = [{ id: 'change-a', agent_id: 'agent-a', old_limit: null, new_limit: '234.56', created_at: null }];
    const view = render(<View />);
    ...
    view.rerender(<View revision={1} />);
    await waitFor(() => expect(screen.getByText('No Agents Found')).toBeInTheDocument());
    expect(mocks.queries.some((q) => q.table === 'credit_assignments' && q.userId === 'operator-b')).toBe(false);
  });
  ...
  it('retires old save UI ownership on a same-component authority change without unlocking a newer save', async () => {
    ...
    await act(async () => { oldSave.resolve(true); await oldSave.promise; });
    expect(screen.getByRole('button', { name: '...' })).toBeDisabled();
    expect(mocks.toast.success).not.toHaveBeenCalled();
    await act(async () => { newSave.resolve(true); await newSave.promise; });
