...
const mocks = vi.hoisted(() => ({
  userId: 'user-a', hydrating: false,
  mine: vi.fn(), routed: vi.fn(), pending: vi.fn(), submit: vi.fn(), approve: vi.fn(), deny: vi.fn(),
  toast: { success: vi.fn(), error: vi.fn() }, emit: vi.fn(),
  ownerId: 'user-a', memberRole: 'owner', memberStatus: 'active',
...
vi.mock('../src/services/CreditRequestService', () => ({ creditRequestService: {
  getMyRequests: mocks.mine, getRequestsForApprover: mocks.routed,
  getPendingForClub: mocks.pending,
  submitRequest: mocks.submit, approveRequest: mocks.approve, denyRequest: mocks.deny,
} }));
...
vi.mock('../src/lib/supabase', () => ({ supabase: {
  from: vi.fn((table: string) => {
    ...
    query.maybeSingle = () => Promise.resolve({ error: null, data: table === 'clubs'
...
vi.mock('../src/core/MasterBus', () => ({ masterBus: {
  ...
  subscribeDebounced: vi.fn((_name, callback) => {
    ...
    return () => { mocks.refresh = undefined; };
  }),
} }));
import CreditRequestWidget, { CreditRequestManagerInbox } from '../src/components/agent/CreditRequestWidget';

const props = { userId: 'user-a', clubId: 'club-a', approverUserId: 'parent-user',
  canRequest: true, canReview: false, currentCreditLimit: 100, currentCreditUsed: 25 };
function request(overrides: Partial<CreditRequest> = {}): CreditRequest {
  return { id: 'request-a', requesterId: 'user-a', requesterName: 'Alice',
    approverId: 'parent-user', clubId: 'club-a', requestedAmount: 200.25,
    reason: 'Credit limit request', status: 'pending', createdAt: '2026-09-14T09:00:00Z', ...overrides };
}
beforeEach(() => {
  vi.clearAllMocks();
  mocks.userId = 'user-a'; mocks.hydrating = false; mocks.refresh = undefined;
  mocks.ownerId = 'user-a'; mocks.memberRole = 'owner'; mocks.memberStatus = 'active';
  mocks.mine.mockResolvedValue([]); mocks.routed.mockResolvedValue([]);
...
async function openForm() {
  ...
  fireEvent.change(screen.getByRole('spinbutton', { name: 'New credit limit' }), { target: { value: '200.25' } });
}
...
describe('Credit requests belong to an account and club', () => {
  it('gives a club owner a review inbox without requiring an agent record', async () => {
    mocks.memberRole = 'member'; mocks.memberStatus = 'active';
    mocks.pending.mockResolvedValue([request({ requesterId: 'child-user', approverId: 'previous-owner' })]);
  ...
  it('does not grant credit review to a suspended administrator', async () => {
    mocks.ownerId = 'another-owner'; mocks.memberRole = 'admin'; mocks.memberStatus = 'suspended';
    render(<CreditRequestManagerInbox clubId="club-a" />);
    await waitFor(() => expect(screen.queryByText('Checking credit request access...')).not.toBeInTheDocument());
  ...
  it('submits account IDs with the selected club and blocks a repeated pending click', async () => {
    const gate = deferred<CreditRequest>(); mocks.submit.mockReturnValue(gate.promise);
    ...
    expect(mocks.submit).toHaveBeenCalledWith('user-a', {
      approverId: 'parent-user', clubId: 'club-a', requestedAmount: 200.25, reason: 'Credit limit request',
    });
    await act(async () => { gate.resolve(request()); await gate.promise; });
  ...
  it('refuses a response from another club instead of displaying its amount or requester', async () => {
    mocks.pending.mockResolvedValue([request({ clubId: 'club-b', approverId: 'user-a', requesterName: 'Private other club' })]);
    render(<CreditRequestWidget {...props} canReview />);
    expect(await screen.findByRole('alert')).toHaveTextContent('Credit requests could not be loaded');
  ...
  it('drops an old read after a same-account club switch', async () => {
    ...
    mocks.mine.mockImplementation((_user, club) => club === 'club-a' ? gate.promise : Promise.resolve([]));
  ...
  it('drops old success after a club switch without clearing the new form', async () => {
    const gate = deferred<CreditRequest>(); mocks.submit.mockReturnValue(gate.promise);
    const view = render(<CreditRequestWidget {...props} />);
    await openForm(); fireEvent.click(screen.getByRole('button', { name: 'Submit Request' }));
    view.rerender(<CreditRequestWidget {...props} clubId="club-b" />);
    await openForm();
  ...
  it('hides pending decisions for a routed parent without club-manager authority', async () => {
    ...
    render(<CreditRequestWidget {...props} />);
    expect(await screen.findByText('Credit decisions require a club owner or administrator.')).toBeInTheDocument();
    expect(screen.queryByRole('button', { name: /Approve/ })).not.toBeInTheDocument();
    expect(mocks.approve).not.toHaveBeenCalled();
  });
  ...
  it('reviews with the account ID, uses the durable receipt and emits no duplicate credit event', async () => {
    ...
    const gate = deferred<CreditRequest>(); mocks.approve.mockReturnValue(gate.promise);
    render(<CreditRequestWidget {...props} canReview />);
    ...
    await act(async () => { gate.resolve({ ...pending, status: 'approved', approvedAmount: 200.25 }); await gate.promise; });
  ...
  it('rejects fractional cents and a lossy large credit limit before submitting', async () => {
    render(<CreditRequestWidget {...props} />); await openForm();
