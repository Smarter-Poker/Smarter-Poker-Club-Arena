...
const diamondPromptPages = new WeakSet<Page>();

/**
 * A zero-chip fixture account can legitimately receive this offer after the
 * lobby loads. Use its real decline control when it blocks an action, including
 * in fresh contexts: storageState does not carry the prompt's sessionStorage.
 * Playwright waits for the dialog to hide within the triggering action's bound.
 */
export async function handleDiamondBustPrompt(page: Page): Promise<void> {
  if (diamondPromptPages.has(page)) return;
  const dialog = page.getByRole('dialog', { name: 'Diamond Spins', exact: true });
  await page.addLocatorHandler(dialog, async () => {
    await dialog.getByRole('button', { name: 'Not Now', exact: true }).click();
  });
  diamondPromptPages.add(page);
}
...
export async function dismissClubEntryMessage(page: Page): Promise<boolean> {
  ...
  // Own both promises before a click can reject. Closing the browser after a
  // failed click must not turn the pending response into an unhandled rejection.
