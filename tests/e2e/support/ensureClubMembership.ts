...
export async function ensureClubMembership(
  ...
  await page.goto(new URL(`clubs/${clubId}`, baseURL).toString(), {
  ...
  await page
    .locator('.club-home, button.invite-btn--primary, .invite-pending, .invite-card.error-state')
    .first()
    .waitFor({ state: 'visible', timeout: CLUB_ROUTE_TIMEOUT });
  ...
  await page
    .locator('.club-home, .invite-pending, .invite-card.error-state')
    .first()
    .waitFor({ state: 'visible', timeout: CLUB_ROUTE_TIMEOUT });
  ...
  throw new Error(
    `The production E2E club join failed: ${(await error.textContent())?.trim() || clubId}`
  );
}
