...
// measured 406ms solo; vitest's 5s default is a unit-test budget and times
// out under the pre-push hook's 90-file parallel run. 90s is 222x measured,
// well above the worst contention amplification observed (7.1x).
describe(
  'a pinned source file is checked on the pull request, not on main',
  { timeout: 90_000 },
  () => {
    ...
    it('accepts an intact pin and refuses a drifted one in the same tree', () => {
      ...
      try {
        ...
        writeFileSync(
          join(root, 'tests', 'fixtures', 'probe', 'kept.txt'),
          'the reviewed bytes!\n'
        );
