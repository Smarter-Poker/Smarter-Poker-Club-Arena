-- Exact read-only guard catalog capture: 2026-09-18T22:51:28Z.
-- Empty isolated declaration stores; no production records.
CREATE SEQUENCE public.ca_guard_def_history_id_seq AS bigint;
...
GRANT EXECUTE ON FUNCTION public.fn_ca_declare_guard_redefinition(text,text) TO postgres, service_role;
DO $witness$ BEGIN IF md5(pg_get_functiondef('public.fn_ca_declare_guard_redefinition(text,text)'::regprocedure)) <> '3a3746dc6e0a5b7a1db97805588c0eb8' THEN RAISE EXCEPTION 'Guard declaration dependency changed'; END IF; END $witness$;
...
DO $witness$ BEGIN IF md5(pg_get_functiondef('public.fn_ca_guard_watchlist()'::regprocedure)) <> '92ee208d0887728444bda396d0b4d442' THEN RAISE EXCEPTION 'Guard declaration dependency changed'; END IF; END $witness$;
