-- SOURCE ONLY / UNRUN. Supplemental exact captured baseline, before all components.
-- Full fixture already contains every other retained definition in these catalogs.
DO $guard$ BEGIN
 IF current_user<>'postgres' OR inet_server_addr() IS NOT NULL
 THEN RAISE EXCEPTION 'isolated correction baseline required';END IF;
 IF to_regprocedure('public.fn_ca_post_correction(text,uuid,text,uuid,numeric,text,uuid,bigint,uuid,uuid,jsonb)') IS NOT NULL
 THEN RAISE EXCEPTION 'correction fixture baseline already defines writer';END IF;
END $guard$;
...
END $function$
;
ALTER FUNCTION public.fn_ca_post_correction(text,uuid,text,uuid,numeric,text,uuid,bigint,uuid,uuid,jsonb) OWNER TO postgres;
REVOKE ALL ON FUNCTION public.fn_ca_post_correction(text,uuid,text,uuid,numeric,text,uuid,bigint,uuid,uuid,jsonb) FROM PUBLIC,anon,authenticated,service_role;
GRANT EXECUTE ON FUNCTION public.fn_ca_post_correction(text,uuid,text,uuid,numeric,text,uuid,bigint,uuid,uuid,jsonb) TO service_role;
