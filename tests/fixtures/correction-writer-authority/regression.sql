...
-- SOURCE ONLY / UNRUN. Successor copy; historical correction-document fixture remains unchanged.
-- Requires component36 and same-session predecessor legacy seed; never production.
BEGIN;
...
DO $guard$ BEGIN
 IF current_user<>'postgres' OR inet_server_addr() IS NOT NULL OR current_setting('session_replication_role')<>'origin'
  OR to_regclass('public.accounting_correction_documents') IS NULL OR to_regclass('public.ca_correction_request_intents_v1') IS NULL
...
DO $writer_boundary$ DECLARE before_state jsonb;r jsonb;BEGIN
 ...
 PERFORM pg_temp.correction_check(r=jsonb_build_object('ok',false,'reason','correction_intent_conflict')
 AND pg_temp.correction_state()=before_state,'successor refuses changed full intent without changing original linkage or documents');
...
DO $invalid_amount$ DECLARE before_state jsonb;amount numeric;r jsonb;BEGIN
 ...
 END LOOP;
 -- The successor refuses NaN before reference lookup or attempted journal insertion.
 r:=public.fn_ca_post_correction('club_treasury',pg_temp.correction_id(101),'player_wallet',pg_temp.correction_id(2),'NaN'::numeric,
  'Nonfinite correction document must never be issued',pg_temp.correction_id(316),NULL,pg_temp.correction_id(101));
 PERFORM pg_temp.correction_check(r=jsonb_build_object('ok',false,'reason','amount_must_be_positive_cents')
  AND pg_temp.correction_state()=before_state,'nonfinite input has exact early refusal and leaves every authoritative row unchanged');
END$invalid_amount$;
RESET ROLE;
