-- Cash regression retained from cash-rake-earning-evidence/period-writer-regression.sql; terminal witness now establishes tournament liability timing.
...
BEGIN
 INSERT INTO public.rake_records(id,hand_id,club_id,table_id,rake_amount,is_tournament,tournament_id,created_at,metadata) VALUES(u(n),u(n+1000),CASE WHEN game_union_n IS NULL THEN u(club_n) ELSE u(30) END,u(401),credit,false,NULL,at_time,'{}');
...
INSERT INTO public.rake_records(id,hand_id,club_id,table_id,rake_amount,is_tournament,tournament_id,created_at,metadata) VALUES(u(299),NULL,u(30),u(401),5,true,u(499),'2026-09-12Z','{}');
INSERT INTO tournament_rake_settlements VALUES(u(499),u(30),u(40),5,'2026-09-12Z');
SELECT assert_true(fn_rakeback_recompute_periods(u(10),'2026-09-07','2026-09-13')->>'reason'='tournament_terminal_recognition_missing','unattributed tournament entitlement cannot be discarded from a mixed weekly book');
DELETE FROM tournament_rake_settlements WHERE tournament_id=u(499);DELETE FROM rake_records WHERE id=u(299);
INSERT INTO public.rake_records(id,hand_id,club_id,table_id,rake_amount,is_tournament,tournament_id,created_at,metadata) VALUES(u(298),u(1298),u(30),u(401),1,false,NULL,'2026-09-12Z','{}');
