...
INSERT INTO public.agents(id,club_id,user_id,role,commission_rate,player_rakeback_rate,status) VALUES('d7000000-0000-4000-8000-000000000001','d2000000-0000-4000-8000-000000000002','d1000000-0000-4000-8000-000000000001','agent',0.25,0,'active');
UPDATE public.club_members SET agent_id='d1000000-0000-4000-8000-000000000001' WHERE club_id='d2000000-0000-4000-8000-000000000002';
INSERT INTO public.accounting_agreement_history(entity_type,entity_key,club_id,subject_user_id,event_type,observed_at,after_terms)
 SELECT 'agents',a.id::text,a.club_id,a.user_id,'baseline',transaction_timestamp(),to_jsonb(a) FROM public.agents a WHERE a.id='d7000000-0000-4000-8000-000000000001';
INSERT INTO public.accounting_agreement_history(entity_type,entity_key,club_id,subject_user_id,event_type,observed_at,after_terms)
...
CREATE FUNCTION pg_temp.assert_target_earning_closed() RETURNS void LANGUAGE plpgsql AS $$BEGIN
 ...
 PERFORM pg_temp.satellite_full_assert(EXISTS(SELECT 1 FROM public.accounting_tournament_fee_recognitions r JOIN public.chip_ledger l ON l.id=r.bank_journal_id
  WHERE r.tournament_id='d3000000-0000-4000-8000-000000000002' AND r.status='recognized' AND r.net_rake=20
  ...
  AND (SELECT count(*)=2 AND sum(rake_credit)=20 FROM public.accounting_tournament_recognized_sources
   WHERE tournament_id='d3000000-0000-4000-8000-000000000002' AND disposition='earned'),
  'positive payable sources bind both players to the exact real fee retirement receipt');
 PERFORM pg_temp.satellite_full_assert((SELECT count(*)=2 AND sum(amount)=5 AND bool_and(amount=2.50 AND commission_rate=0.25)
  FROM public.agent_commissions WHERE source_type='tournament_fee_accrual')
  AND (SELECT count(*)=1 FROM public.accounting_period_recompute_requests WHERE club_id='d2000000-0000-4000-8000-000000000002' AND status='pending'),
  'same shared commission writer posts both2.50 entitlements and queues one exact club week');
END$$;
...
BEGIN
 ...
 PERFORM pg_temp.satellite_full_assert(refused AND before_state=pg_temp.satellite_full_financial_state(),
  'late target failure atomically rolls back180 prize,20 retirement,5 commission, sources, stats and request');
END $target_rollback$;
...
BEGIN
 ...
 PERFORM pg_temp.satellite_full_assert(receipt->>'receipt_version'='2' AND receipt#>>'{rake,accounting,status}'='recognized'
  AND receipt#>>'{rake,accounting,payable}'='true','actual terminal receipt independently certifies recognized fee accounting');
 ...
 PERFORM pg_temp.satellite_full_assert(receipt=replay AND before_state=pg_temp.satellite_full_financial_state(),
  'recognized terminal retry creates no duplicate prize,retirement,commission,stats or weekly request');
