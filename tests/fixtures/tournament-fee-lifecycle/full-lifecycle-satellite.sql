...
INSERT INTO public.accounting_agreement_history(entity_type,entity_key,club_id,subject_user_id,event_type,observed_at,after_terms)
 SELECT 'club_members',m.club_id::text||':'||m.user_id::text,m.club_id,m.user_id,'baseline',transaction_timestamp(),to_jsonb(m)
 FROM public.club_members m WHERE m.club_id IN('d2000000-0000-4000-8000-000000000001','d2000000-0000-4000-8000-000000000002');
...
-- Fees from the real direct entry and real satellite delivery are refunded by
-- the current cancellation authority. No refund receipt or reversal is seeded.
SET CONSTRAINTS ALL IMMEDIATE;
DO $fee_sources$
BEGIN
 PERFORM pg_temp.satellite_full_assert((SELECT count(*)=2 AND sum(rake_credit)=20 FROM public.accounting_tournament_fee_sources
  WHERE tournament_id='d3000000-0000-4000-8000-000000000002'),
  'actual wallet and satellite entry producers capture both exact observed ten-chip fees');
 PERFORM pg_temp.satellite_full_assert(NOT EXISTS(SELECT 1 FROM public.accounting_tournament_fee_recognitions
  WHERE tournament_id='d3000000-0000-4000-8000-000000000002'),
  'open target fee charges create no payable weekly recognition');
END $fee_sources$;
SET CONSTRAINTS ALL DEFERRED;
CREATE FUNCTION pg_temp.cancel_target_final_fault() RETURNS trigger LANGUAGE plpgsql AS $$BEGIN
 IF NEW.tournament_id='d3000000-0000-4000-8000-000000000002' THEN
  IF NEW.total_refunded<>200 OR NEW.fees_reversed<>20
   OR NOT EXISTS(SELECT 1 FROM public.tournament_escrow WHERE tournament_id=NEW.tournament_id
    AND prize_balance=0 AND fee_balance=0 AND bounty_balance=0 AND closed_at IS NOT NULL) THEN
   RAISE EXCEPTION 'FAIL target refund final evidence incomplete';END IF;
  RAISE EXCEPTION 'native final cancellation refusal' USING ERRCODE='ZX009';
 END IF;RETURN NEW;
END$$;
CREATE TRIGGER native_cancel_target_final_fault AFTER INSERT ON public.tournament_cancellation_receipts
 FOR EACH ROW EXECUTE FUNCTION pg_temp.cancel_target_final_fault();
DO $cancel_fault$
DECLARE before_state jsonb:=pg_temp.satellite_full_financial_state();refused boolean:=false;
BEGIN
 BEGIN PERFORM public.atomic_cancel_tournament('d3000000-0000-4000-8000-000000000002',NULL);
 EXCEPTION WHEN SQLSTATE 'ZX009' THEN refused:=true; END;
 PERFORM pg_temp.satellite_full_assert(refused AND before_state=pg_temp.satellite_full_financial_state(),
  'late cancellation receipt fault rolls back both real refunds, fee reversals and every public relation');
END $cancel_fault$;
DROP TRIGGER native_cancel_target_final_fault ON public.tournament_cancellation_receipts;
DO $cancel_exact$
DECLARE first_receipt jsonb;second_receipt jsonb;before_state jsonb;
BEGIN
 first_receipt:=public.atomic_cancel_tournament('d3000000-0000-4000-8000-000000000002',NULL);
 PERFORM pg_temp.satellite_full_assert(first_receipt->>'ok'='true' AND (first_receipt->>'total_refunded')::numeric=200
  AND EXISTS(SELECT 1 FROM public.accounting_tournament_fee_recognitions WHERE tournament_id='d3000000-0000-4000-8000-000000000002'
   AND status='cancelled' AND net_rake=0 AND union_wallet_transaction_id IS NULL AND bank_journal_id IS NULL)
  AND (SELECT count(*)=2 AND sum(rake_credit)=0 FROM public.accounting_tournament_recognized_sources
   WHERE tournament_id='d3000000-0000-4000-8000-000000000002' AND disposition='refunded')
  AND NOT EXISTS(SELECT 1 FROM public.agent_commissions WHERE source_type='tournament_fee_accrual'),
  'target cancellation refunds 200 chips and both 10-chip fees without any commission or bank accrual');
 PERFORM pg_temp.satellite_full_assert((SELECT chip_balance=200 FROM public.club_members
  WHERE club_id='d2000000-0000-4000-8000-000000000002' AND user_id='d1000000-0000-4000-8000-000000000001')
  AND (SELECT chip_balance=300 FROM public.club_members
  WHERE club_id='d2000000-0000-4000-8000-000000000002' AND user_id='d1000000-0000-4000-8000-000000000002'),
  'direct entrant and satellite winner recover exactly their original chip custody');
 before_state:=pg_temp.satellite_full_financial_state();
 second_receipt:=public.atomic_cancel_tournament('d3000000-0000-4000-8000-000000000002',NULL);
 PERFORM pg_temp.satellite_full_assert(before_state=pg_temp.satellite_full_financial_state()
  AND (second_receipt->>'total_refunded')::numeric=200,
  'cancellation retry preserves exact refund receipts with no second debit or credit');
END $cancel_exact$;
SET CONSTRAINTS ALL IMMEDIATE;
