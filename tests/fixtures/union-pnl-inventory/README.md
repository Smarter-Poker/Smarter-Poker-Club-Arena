...
Installation concurrency replays the measured tables-before-seats deadlock and table-before-tournament admission refusal in isolated PostgreSQL, then executes the exact migration lock-admission statements. The migration drains tournament writers before table readers, preserving the original tournament-creation and hand-settlement order. It proves original writers finish, plain reads remain available and a competing later source owner refuses only the migration and releases its locks. The test changes no production state.
