-- The two bodies the 2026-09-25 weekly rakeback cost rewrite REPLACES, exactly as
-- production had them installed when the rewrite was written and measured
-- (migration 20260925205938, section 2.2 pins the same md5(prosrc) values before
-- it will install anything).
--
-- They are installed here under fixture.* names, beside the rewrite rather than
-- instead of it, so period-coverage-regression.sql can call both on the same
-- rows in the same cluster and require IDENTICAL answers: the same verdict from
-- the weekly tournament gate, and the same receipt and the same certificates,
-- payee for payee, from the period calculator. A rewrite that is only "cheaper"
-- has not been shown to be the same function; this is where it is.
--
-- The predecessor calculator still calls public.fn_accounting_tournament_week_quality,
-- which on this cluster is the REWRITTEN gate. That is deliberate: the gate is
-- compared with its own predecessor separately, so each half of the rewrite is
-- compared in isolation and a difference names the function that caused it.
CREATE SCHEMA IF NOT EXISTS fixture;

CREATE OR REPLACE FUNCTION fixture.predecessor_week_quality(p_club_id uuid, p_from timestamp with time zone, p_to timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
...
CREATE OR REPLACE FUNCTION fixture.predecessor_calculate_cash_rakeback_periods(p_club_id uuid, p_period_start date, p_period_end date, p_user_ids uuid[] DEFAULT NULL::uuid[])
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
 ...
 WITH scoped_records AS (
  ...
  SELECT r.id,r.hand_id,r.rake_amount,count(a.id) AS attribution_count,
    ...
    count(a.id) FILTER(WHERE a.hand_id IS DISTINCT FROM r.hand_id OR a.club_id IS NULL
      ...
      OR NOT EXISTS(SELECT 1 FROM public.clubs c WHERE c.id=a.club_id AND (c.is_union IS NOT TRUE
...
-- The bytes above are the pinned predecessors, or this file is not what it says.
DO $pin$ BEGIN
 IF (SELECT md5(prosrc) FROM pg_proc WHERE oid='fixture.predecessor_week_quality(uuid,timestamptz,timestamptz)'::regprocedure)
    IS DISTINCT FROM '79e2a8eea9f4ca5b29f88ac4464336e5'
  OR (SELECT md5(prosrc) FROM pg_proc WHERE oid='fixture.predecessor_calculate_cash_rakeback_periods(uuid,date,date,uuid[])'::regprocedure)
    IS DISTINCT FROM '0f12fff51636d61be422a37c3b8685f8' THEN
  RAISE EXCEPTION 'FAIL: the captured rakeback cost predecessors are not the pinned production bodies';
 END IF;
END $pin$;
