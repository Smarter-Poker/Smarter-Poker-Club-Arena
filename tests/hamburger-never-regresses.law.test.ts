...
const TRIGGERS = [
  ...
  },
  {
    ...
    mustContain: ["useButtonImage('icon-hamburger')"],
    mustMatch: [/aria-label="Table Menu"/i],
  },
] as const;
...
/**
 * The asset names PR #2321 put in the triggers' place. Named explicitly so a
 * re-run of that migration fails here instead of shipping.
 */
const BANNED_TRIGGER_ART =
  /command-center-v1|global-header-command-center|CommandGridIcon|command-grid/;
...
describe('the hamburger is the menu, and stays the menu', () => {
  it.each(TRIGGERS)('$name still opens the menu with a hamburger', (trigger) => {
    ...
    for (const pattern of (trigger as { mustMatch?: RegExp[] }).mustMatch ?? []) {
      expect(source, `${trigger.file} no longer matches ${pattern}`).toMatch(pattern);
    }
  });

  it.each(TRIGGERS)('$name uses no gear artwork and no grid glyph', (trigger) => {
    const source = maskComments(read(trigger.file));
    expect(source, `${trigger.file} points at the banned command-center art`).not.toMatch(
      BANNED_TRIGGER_ART
    );
    expect(source, `${trigger.file} uses a grid or ellipsis glyph`).not.toMatch(NOT_A_HAMBURGER);
  });

  it('the gear artwork is referenced nowhere in the app', () => {
    // #2321 wired it into five triggers at once. One grep is the cheapest way
    // to be sure a partial revert did not leave one behind.
    ...
    const walk = (dir: string) => {
      for (const entry of readdirSync(dir, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) walk(full);
          ...
          if (BANNED_TRIGGER_ART.test(readFileSync(full, 'utf8'))) {
...
describe('the root cause: "em bars" cannot be misread as a ban on the hamburger', () => {
  /*
   * WHY THIS SUITE EXISTS.
   *
   * Dan, 2026-08-20: "forbid the use of em bars anywhere." He means EM DASHES,
   * the punctuation mark. Several files quote that sentence verbatim, and
   * titleCase.ts renders it as "inside the entire club arena, and forbid the
   * use of em bars anywhere" -- with nothing nearby saying the subject is
   * punctuation. Read literally it looks like a rule about horizontal lines.
   *
   * It has been read that way twice, and both times it took the hamburger off
   * every page:
   *
   *   #2321  gear on all five triggers, rasters deleted, cache tombstones,
   *          and a law making restoration fail CI.        (reverted by #2401)
   *   #2429  the same again with a six-tile command grid. (reverted by #2432)
   *
   * The loop needs no human: the ban was written down IN THE REPO, so the next
   * agent to read the repo re-enforced it, reverted whoever undid it, and
   * wrote the law back. Reverting icons never fixed that. Removing the
   * ambiguity is the fix, and these tests keep the fix in place -- the next
   * agent reads the file, not the incident report.
   */
  const QUOTING_FILES = [
    'scripts/ci/check-ui-text.mjs',
    'src/utils/titleCase.ts',
    'src/utils/popupStyle.ts',
    'tests/utils/popupStyle.test.tsx',
    'tests/unit/houseCopyRulesReachEveryPage.test.ts',
    '.github/workflows/ci.yml',
  ] as const;

  it.each(QUOTING_FILES)('%s says plainly that "em bars" means em dashes', (file) => {
    const text = read(file);
    expect(text, `${file} no longer quotes Dan's phrase`).toMatch(/em bars/i);
    expect(
      text,
      `${file} quotes "em bars" without saying it means EM DASHES. That exact ` +
        `ambiguity cost the hamburger menu twice (#2321, #2429). Put the gloss back.`
    ).toMatch(/em bars[\s\S]{0,900}?em dash/i);
  });

  it('CLAUDE.md carries the rule in full', () => {
    const claude = read('CLAUDE.md');
    expect(claude).toMatch(/"EM BARS" MEANS EM DASHES/);
    expect(claude).toMatch(/does not ban the hamburger/i);
  });

  it('AGENT-PLAYBOOK.md says it before anything else', () => {
    // Every agent is told to read the playbook first, so the correction has to
    // be the FIRST RULE in it, not a footnote below the verification pass.
    // Bounded by the structure (the first heading and its section) rather than
    // a byte count -- see tests/unit/noFixedSizeSourceWindows.test.ts.
    const playbook = read('AGENT-PLAYBOOK.md');
    const [firstSection] = playbook.split(/^---$/m);
    expect(firstSection, 'the playbook must open with the em-bars correction').toMatch(
      /^#\s.*EM BARS.*EM DASHES/im
    );
    expect(firstSection).toMatch(/does not ban the hamburger/i);
  });

  it('no file re-declares a ban on three-bar artwork', () => {
    // The two documents that drove the loop. If either name comes back, some
    // agent has re-litigated this and the next regression is already written.
    const offenders: string[] = [];
    const walk = (dir: string) => {
      for (const entry of readdirSync(dir, { withFileTypes: true })) {
        ...
        const full = join(dir, entry.name);
        if (entry.isDirectory()) walk(full);
        else if (/three-bar-artwork-is-banned|noThreeBarArtwork/.test(entry.name)) {
          offenders.push(full.slice(ROOT.length + 1));
        }
      }
    };
    ...
    expect(offenders, 'the three-bar ban is back; read CLAUDE.md 10.7').toEqual([]);
  });
});
...
describe('no boxes over header or footer icons', () => {
  ...
  it('the chrome focus state is a glow, never an outline or a hard ring', () => {
    for (const [file, selector] of CHROME) {
      ...
      // A ring drawn as a bordered pseudo-element is still a box. The World Hub
      // proved this: its header replaced the outline with
