...
export function cashoutV2Receipt(kind: FixtureCashoutKind = 'hold', options: {
  amount?: number; replayed?: boolean; currentStatus?: RequestState; note?: string;
} = {}) {
  const id = CASHOUT_IDS;
  const hold = kind === 'hold';
  const playerAction = hold || kind === 'cancellation';
  const requestStatus = { hold: 'pending', approval: 'approved', cancellation: 'cancelled', decline: 'rejected' }[kind];
  ...
  const cashier = {
    contract_version: 1, event_id: id.event, invoice_id: id.invoice,
    cashout_id: id.cashout, escrow_id: id.escrow,
    source_transaction_id: id.transaction, source_ledger_id: id.ledger,
    club_id: id.club, player_id: id.player, assigned_agent_id: id.agent,
    issuer_representative_id: id.agent, actor_user_id: playerAction ? id.player : id.agent,
    actor_role: playerAction ? 'player' : 'agent', event_kind: kind,
    display_state: hold ? 'held' : kind === 'approval' ? 'approved' : 'refunded',
    amount, occurred_at: hold ? createdAt : changedAt, issued_at: changedAt,
    hold_event_id: hold ? null : id.holdEvent, hold_invoice_id: hold ? null : id.holdInvoice,
    ...
    custody_movement_recorded: true, cashout_completed: kind === 'approval',
    refund_recorded: kind === 'cancellation' || kind === 'decline',
  };
  return { ...cashier, cashier, success: true, replayed: options.replayed ?? false,
    op_id: id.operation, request_status: requestStatus, accepted_note: options.note?.trim() || null,
    actor_wallet_after: kind === 'decline' ? null : '750.00',
    request: { id: id.cashout, club_id: id.club, player_id: id.player, agent_id: id.agent,
      amount, status: currentStatus, created_at: createdAt, updated_at: hold && currentStatus === 'pending' ? createdAt : changedAt,
      acknowledged_at: currentStatus === 'approved' || currentStatus === 'rejected' ? changedAt : null,
...
export function cashoutLookupEnvelope(args: Record<string, unknown>, receipt: unknown = null) {
  return { contract_version: 1, actor_user_id: args.p_expected_actor_id, op_id: args.p_op_id,
    action: args.p_action, club_id: args.p_club_id, amount: args.p_amount,
    cashout_id: args.p_cashout_id, accepted_note: args.p_note, found: receipt !== null, receipt };
}
