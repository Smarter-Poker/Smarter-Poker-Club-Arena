...
import {
  expectedFutureAcl,
  expectedFuturePrivilege,
  expectedSchemaUsage,
  reachableRoles,
  futureObjectCases,
  verifySyntheticScram,
} from '../../operations/release/fixture/role-native-access.mjs';
import {
  roleFaultCases,
...
import {
  ...
  lastCommand,
  identifier,
} from '../../operations/release/fixture/role-native-protocol.mjs';
import {
  withHeldFixtureRoleAlignmentClient,
  renderFixtureRoleAlignmentSql,
} from '../../operations/release/fixture/role-alignment-render.mjs';
const inputs = await roleNativeInputs(),
  catalog = inputs['role-alignment-aligned.json'];
...
test('failed role diagnostics preserve the first safe step across cleanup failures', () => {
  ...
  retainRoleFaultFailure(
    proof,
    'fault-injection',
    Object.assign(new Error('PRIVATE TOKEN AND SQL'), { code: '42501' })
  );
  retainRoleFaultFailure(proof, 'close-all', new TypeError('PRIVATE CLEANUP DETAIL'));
  assert.deepEqual(proof, {
    status: 'failed',
    failure_step: 'fault-injection',
    failure_type: 'Error',
    sqlstate: '42501',
  });
  assert.equal(JSON.stringify(proof).includes('PRIVATE'), false);
  assert.equal(new Set(roleFaultSteps).size, roleFaultSteps.length);
});
test('failed role diagnostics discard unrecognized names, steps and non-SQLSTATE codes', () => {
  for (const code of ['PRIVATE TOKEN', 'ERR_ASSERTION', 42501, null]) {
    const proof = {};
    retainRoleFaultFailure(proof, 'PRIVATE STEP', {
      name: 'PRIVATE NAME',
      code,
      message: 'PRIVATE SQL',
      stack: 'PRIVATE STACK',
...
test('all27 source-pinned creator/schema/type cases are unique and cover9triples', () => {
  ...
  assert.deepEqual(
    Object.fromEntries(['r', 'S', 'f'].map((t) => [t, cases.filter((c) => c.type === t).length])),
    { r: 9, S: 9, f: 9 }
  );
...
test('function PUBLIC execution remains in defaults even without explicit per-schema PUBLIC', () => {
  ...
  assert.equal(
    expectedFuturePrivilege(catalog, entry('supabase_admin', 'cron', 'f'), 'anon', 'EXECUTE'),
    true
  );
  assert.equal(
    expectedSchemaUsage(catalog, 'cron', 'anon'),
    false,
    'schema permission remains independent'
  );
});
test('owner defaults merge grants without deleting PostgreSQL TRUNCATE or MAINTAIN', () => {
  ...
  assert.equal(
    acl.some((g) => g.grantee === 'anon' && g.privilege === 'TRUNCATE'),
    false
  );
});
...
test('read-all-data grants effective reads and schema usage without granting onward', () => {
  ...
  assert.equal(
    expectedFuturePrivilege(catalog, e, 'supabase_read_only_user', 'SELECT', true),
    false
  );
...
test('schema-only catalog defaults do not invent a CREATE grant for postgres/storage', () => {
  ...
  assert.equal(
    schema.acl.some(
      (a) => a.startsWith('postgres=') && a.split('=')[1].split('/')[0].includes('C')
    ),
    false
  );
});
test('SCRAM proof rejects wrong passwords, iteration abuse and malformed verifier', () => {
  const salt = Buffer.alloc(16, 1),
    salted = pbkdf2Sync(password, salt, 4096, 32, 'sha256');
...
test('fault matrix covers both guards, middle/late and every held-client category exactly once', () => {
  ...
  for (const stage of ['entry', 'final'])
    for (const fault of [
      'extra-role',
      'flags',
      'settings',
...
test('all fault slices preserve exact source bytes and unique complete-statement boundaries', async () => {
  class Held extends EventEmitter {
    processID = 99;
    async query() {
      return {
        rows: [
  ...
  await withHeldFixtureRoleAlignmentClient(client, async (binding) => {
    const rendered = renderFixtureRoleAlignmentSql(inputs['role-alignment-installer.sql'], {
      password,
...
test('an absent client fault can render while alive and still submit SQL after intentional end', async () => {
  ...
  let submitted = false;
  await assert.rejects(
    withHeldFixtureRoleAlignmentClient(client, async (binding) => {
      const rendered = renderFixtureRoleAlignmentSql(inputs['role-alignment-installer.sql'], {
        password,
        clientBinding: binding,
      });
...
function clients(mode = 'ok') {
  ...
  class Client extends EventEmitter {
    constructor(config) {
      super();
      this.config = config;
      this.processID = 101 + records.length;
      records.push(this);
    }
    async connect() {}
    async query() {
      if (mode === 'hung-query') return new Promise(() => {});
      return {
        rows: [
          {
            pid: this.processID,
            session_role: this.config.user,
            current_role: this.config.user,
            database: this.config.database,
            superuser: this.config.user === 'supabase_admin',
            tcp: this.config.host === '127.0.0.1',
            read_only: this.config.options?.includes('read_only=on') ? 'on' : 'off',
          },
        ],
      };
    }
    async end() {
      this.ended = true;
      if (mode !== 'missing-end') this.emit('end');
    }
  }
...
test('ordinary sessions preserve role defaults; password auth uses only fixed loopback', async () => {
  const f = clients(),
    o = new RoleNativeOwner(f.Client, { password, deadlineMs: 1000 });
  ...
  await o.closeAll();
  assert.ok(f.records.every((r) => r.ended));
});
test('one overall deadline rejects a stuck query while reserved cleanup still ends client', async () => {
  const f = clients('hung-query'),
    o = new RoleNativeOwner(f.Client, { password, deadlineMs: 500 });
...
test('end acknowledgement is required and background client errors refuse further queries', async () => {
  const f = clients('missing-end'),
    o = new RoleNativeOwner(f.Client, { password, deadlineMs: 1000 });
  const r = await o.connect('supabase_admin');
  r.client.emit('error', new Error('controlled'));
...
test('a client disposal rejection stays failed while cleanup budget remains', async () => {
  const f = clients(),
    owner = new RoleNativeOwner(f.Client, { password, deadlineMs: 1000 });
  const record = await owner.connect('supabase_admin');
  let closes = 0;
  record.client.end = async () => {
    closes++;
    throw new Error('controlled disposal rejection');
  };
