...
it('weekly invoice payment acknowledges every cent and rejects malformed amounts', () => {
  const sql = read('supabase/migrations/20260907210210_union_statement_payments_require_exact_cents.sql');
  ...
  expect(has('tests/sql/union-statement-payment-rollback-probe.sql', 'FAIL one cent short marked paid')).toBe(true);
});
