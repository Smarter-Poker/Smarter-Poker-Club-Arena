 ...
 FOR variant IN 1..4 LOOP
  ...
  ELSE
   ...
   IF game='crash' THEN
    ...
    -- Moving the round's clock is the probe's, not the game's: the crash lock
    -- (20260922173914) seals every other column of a live round, so the clock
    -- rewrite declares itself as maintenance exactly as the other crash probes do.
    PERFORM set_config('app.ledger_maintenance','probe clock: 1.3 seconds of flight',true);
    ...
    PERFORM set_config('app.ledger_maintenance','probe clock: ten seconds of flight',true);
