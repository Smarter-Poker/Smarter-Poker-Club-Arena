...
-- Protected isolated fixture must supply both hosts and two distinct operators
-- authorized on the relevant hosts. An authorization refusal is not replay proof.
DO $$
DECLARE
  ...
  v_report text := '';
  v_other_club uuid; v_other_host uuid; v_other_kind text;
  v_other_operator uuid := '2d1cd6c3-5700-4af9-a271-d4863fdab20d';
BEGIN
  FOREACH v_club IN ARRAY ARRAY[
    '2a1132b9-5ba2-42e6-9f01-30a7fcffebe3'::uuid,
    'a0000000-0000-0000-0000-000000000001'::uuid
  ] LOOP
    SELECT h.host_id,h.host_kind INTO STRICT v_host,v_kind FROM public.fn_wheel_host(v_club) h;
    v_owner := public.fn_diamond_game_owner(v_host,v_kind);
    IF v_kind='union' THEN
      SELECT id INTO STRICT v_source FROM public.union_wallets WHERE union_id=v_host;
    ELSE v_source:=v_host; END IF;
    v_other_club := CASE WHEN v_kind='club' THEN '2a1132b9-5ba2-42e6-9f01-30a7fcffebe3'::uuid
      ELSE 'a0000000-0000-0000-0000-000000000001'::uuid END;
    SELECT h.host_id,h.host_kind INTO STRICT v_other_host,v_other_kind FROM public.fn_wheel_host(v_other_club) h;
    IF v_host IS NULL OR v_owner IS NULL OR v_source IS NULL OR v_other_host IS NULL
       OR v_host IS NOT DISTINCT FROM v_other_host
       OR v_owner IS NOT DISTINCT FROM v_other_operator THEN
      RAISE EXCEPTION 'PROBE FIXTURE FAIL: distinct funded hosts and operators are required';
    END IF;
    IF public.fn_wheel_can_operate(v_other_host,v_other_kind,v_owner) IS NOT TRUE
       OR public.fn_wheel_can_operate(v_host,v_kind,v_other_operator) IS NOT TRUE THEN
      RAISE EXCEPTION 'PROBE FIXTURE FAIL: changed host/operator requests must pass permission checks before replay identity is tested';
    END IF;
    ...
    IF v_leg.amount IS DISTINCT FROM 1 OR v_leg.from_entity_id IS DISTINCT FROM v_source OR v_leg.to_entity_id IS DISTINCT FROM v_host THEN
    ...
    END IF;
    v_result := public.fn_diamond_game_fund_promo(v_other_club,1,v_key); -- candidate
    IF v_result->>'ok' IS DISTINCT FROM 'false' OR v_result->>'error' IS DISTINCT FROM 'That Request Key Was Already Used For Different Funding' THEN
      RAISE EXCEPTION 'PROBE FAIL: % different host claimed success: %',v_kind,v_result;
    END IF;
    PERFORM set_config('request.jwt.claim.sub',v_other_operator::text,true);
    PERFORM set_config('request.jwt.claims',json_build_object('sub',v_other_operator,'role','authenticated')::text,true);
    v_result := public.fn_diamond_game_fund_promo(v_club,1,v_key); -- candidate
    IF v_result->>'ok' IS DISTINCT FROM 'false' OR v_result->>'error' IS DISTINCT FROM 'That Request Key Was Already Used For Different Funding' THEN
