...
const read = (file: string): string => readFileSync(resolve(migrations, file), 'utf8');
...
describe('pending stage-one cutovers retain newer installed authorities', () => {
  ...
  it('keeps both the seat-first Spin exception and exact cancellation unwind', () => {
    ...
    expect(contract).toContain('IF v_count = 0');
  ...
  it('keeps bounded in-transaction rake attribution retries inside terminal ordering', () => {
    const rake = between(
      terminal,
      'CREATE OR REPLACE FUNCTION public.fn_settle_tournament_rake(',
      'CREATE OR REPLACE FUNCTION public.fn_award_satellite_seat('
    );
    const terminalRoot = rake.indexOf("hashtextextended('ca:tournament-terminal-settlement:v1',0)");
    const tournamentLock = rake.indexOf('SELECT t.id, t.status, t.club_id');

    expect(terminalRoot).toBeGreaterThan(-1);
    expect(tournamentLock).toBeGreaterThan(terminalRoot);
  ...
  it('nests the M7 money-path authority inside the M13 seat authority', () => {
    ...
    expect(satelliteMoneyPath).toContain(
      'RENAME TO fn_settle_satellite_tournament_pre_money_path_gate'
    );
    expect(moneyWrapper).toContain("'app.money_path','fn_settle_satellite_tournament',true");
    expect(moneyWrapper).toContain('public.fn_settle_satellite_tournament_pre_money_path_gate(');
