...
vi.mock('../../src/core/MasterBus', () => ({
  masterBus: {
    ...
    subscribe: vi.fn((name, handler) => {
      if (name === 'AUTH_STATE_CHANGED') account.auth = handler;
      return vi.fn();
    }),
  },
}));
...
import {
  ...
  runCashoutOperation as dispatchCashoutOperation,
  type CashoutStart,
  type CashoutKind,
} from '../../src/services/CashoutOperation';
...
const intent = {
  ...
  clubId: ID.club,
  targetId: ID.player,
  kind: 'cashout_request' as const,
  amount: 250,
  isCurrent: () => true,
};
const legacyKey =
  'smarter-poker:agent-wallet-operation:v1:' +
  createHash('sha256')
    .update(
      JSON.stringify([ID.player, ID.club, ID.player, 'cashout_request', 'club_chips', '250.00', ''])
    )
    .digest('hex');
...
function receipt(
  ...
  kind: 'hold' | 'cancellation' = 'hold'
) {
  return {
    data: {
      ...cashoutV2Receipt(kind, { amount: Number(args.p_amount), ...options }),
      op_id: args.p_op_id,
    },
    error: null,
  };
...
async function start(value = intent) {
  return captureCashoutStart(await prepareCashoutOperation(value));
}
beforeEach(() => {
  signIn(null);
  signIn(ID.player);
...
describe('prepared cashout generation and exact service receipts', () => {
  ...
  it('keeps a captured G1 behind a completed G2 without falling forward or minting G3', async () => {
    ...
    lookup.mockImplementationOnce(async (_name, args) => ({
      data: cashoutLookupEnvelope(args, {
        ...cashoutV2Receipt('hold', { replayed: true, currentStatus: 'approved' }),
        op_id: first,
      }),
      error: null,
    }));
  ...
  it('refuses an unprepared gesture while an alias is unresolved', async () => {
    ...
    expect(() =>
      captureCashoutStart({ kind: 'cashout_request', clubId: ID.club } as never)
    ).toThrow(/Prepare/);
  ...
  it('a late exact G1 acknowledgment preserves an admitted G2 live session pointer and unacknowledged history', async () => {
    ...
    response.resolve({
      data: cashoutLookupEnvelope(lookup.mock.calls[1][1], {
        ...cashoutV2Receipt('hold', { replayed: true }),
        op_id: first,
      }),
      error: null,
    });
    ...
    expect(
      history().generations[0].starts.find((item: any) => item.startId === oldPointer.startId)
        .acknowledged
    ).toBe(true);
  ...
  it('terminal cancellation retains its operation after acknowledgment', async () => {
    ...
    lookup.mockImplementationOnce(async (_name, args) => ({
      data: cashoutLookupEnvelope(args, {
        ...cashoutV2Receipt('cancellation', { replayed: true }),
        op_id: original,
      }),
      error: null,
    }));
  ...
  it('refuses staff self-actions and unsupported cancellation notes before storage or RPC', async () => {
    await expect(
      prepareCashoutOperation({ ...intent, targetId: ID.cashout, kind: 'cashout_approve' })
    ).rejects.toThrow(/Original Player/);
    await expect(
      prepareCashoutOperation({
        ...intent,
        targetId: ID.cashout,
...
describe('legacy migration and storage interruption', () => {
  ...
  it('refuses conflicting saved legacy UUIDs without discarding either one', async () => {
    ...
    sessionStorage.setItem(legacyKey, ID.other);
  ...
  it.each(['initializing', 'marker', 'active', 'start', 'pointer'])(
    'interruption after %s write retains the original UUID or refuses before dispatch',
    async (phase) => {
      localStorage.setItem(legacyKey, ID.operation);
      const session = window.sessionStorage;
      const sessionDescriptor = Object.getOwnPropertyDescriptor(window, 'sessionStorage');
      const stores = [localStorage, session];
      const originals = new Map(stores.map((store) => [store, store.setItem]));
      let faulted = false;
      const interrupt = function (this: Storage, key: string, value: string) {
        originals.get(this)!.call(this, key, value);
        const data = key === historyKey ? JSON.parse(value) : null;
        const matches =
          phase === 'marker'
            ? key === legacyKey && value.startsWith('cashout-generations:v2:')
            : phase === 'pointer'
              ? this === session && key === historyKey
              : this === localStorage &&
                key === historyKey &&
                (phase === 'initializing'
                  ? data.state === 'initializing'
                  : phase === 'active'
                    ? data.state === 'active' && !data.generations[0].starts.length
      ...
      // Intercept the window property consumed by the coordinator. All methods
      // still use the original storage backing; this avoids host Storage proxy
      // and bound-method behavior swallowing a setItem replacement.
      const faultingSession: Storage = {
        get length() {
          return session.length;
        },
        clear: session.clear.bind(session),
        key: session.key.bind(session),
        getItem: session.getItem.bind(session),
        removeItem: session.removeItem.bind(session),
        setItem: (key, value) => interrupt.call(session, key, value),
      };
      const fault = vi.spyOn(localStorage, 'setItem').mockImplementation(interrupt);
      Object.defineProperty(window, 'sessionStorage', {
        configurable: true,
        value: faultingSession,
      });
      try {
        expect(window.sessionStorage).toBe(faultingSession);
        await expect((async () => runCashoutOperation(await start()))()).rejects.toThrow(
        ...
        expect(rpc).not.toHaveBeenCalled();
        if (phase === 'pointer') {
          expect(JSON.parse(session.getItem(historyKey)!)).toMatchObject({
            version: 2,
            generation: 1,
  ...
  it('an acknowledgment write failure preserves the verified result and the same explicit replay identity', async () => {
    ...
    let faulted = false;
    const fault = vi.spyOn(localStorage, 'setItem').mockImplementation(function (key, value) {
      original.call(this, key, value);
      if (
        !faulted &&
        this === localStorage &&
        key === historyKey &&
        JSON.parse(value).generations[0].acknowledged
      ) {
        faulted = true;
        throw new Error('Acknowledgment Readback Interrupted');
      }
    });
  ...
  it.each(['marker', 'history'])(
    'missing active %s refuses instead of creating another operation',
    async (missing) => {
...
describe('exact original operation recovery before any new payment', () => {
  it('confirmation observes the original hold again and recovers another tab outcome without a payer', async () => {
    ...
    lookup.mockImplementationOnce(async (_name, args) => ({
      data: cashoutLookupEnvelope(args, {
        ...cashoutV2Receipt('hold', { replayed: true, currentStatus: 'approved' }),
        op_id: original.p_op_id,
      }),
      error: null,
    }));
    expect(await confirmCashoutOperation(captured)).toMatchObject({
      found: true,
    ...
    expect(history().generations[0].starts).toHaveLength(1);
  });
  ...
  it('confirmation synchronously revokes cached absence and shares an unknown result without dispatch or automatic lookup retry', async () => {
    ...
    const refusals = [confirming, repeated, recovery].map((value) =>
      expect(value).rejects.toThrow(/Could Not Be Verified/)
    );
  ...
  it('exact confirmation absence allows only the original operation and cannot re-observe after execution starts', async () => {
    ...
    const response = deferred<any>();
    rpc.mockReturnValueOnce(response.promise);
    const running = dispatchCashoutOperation(captured);
    await expect(confirmCashoutOperation(captured)).rejects.toThrow(
    ...
    response.resolve(receipt(rpc.mock.calls[0][1]));
    await running;
    expect(lookup).toHaveBeenCalledTimes(2);
    expect(rpc).toHaveBeenCalledOnce();
  });
  ...
  it('confirmation cannot acknowledge or dispatch across an account ABA', async () => {
    ...
    response.resolve({
      data: cashoutLookupEnvelope(lookup.mock.calls[1][1], {
        ...cashoutV2Receipt('hold', { replayed: true }),
        op_id: lookup.mock.calls[1][1].p_op_id,
      }),
      error: null,
    });
    await refused;
  ...
  it('recovers the prior canonical hold and current terminal DTO without a second payer', async () => {
    ...
    expect(recovered).toMatchObject({
      found: true,
      result: { id: ID.cashout, status: 'approved' },
    });
    expect(await dispatchCashoutOperation(captured)).toMatchObject({ status: 'approved' });
    expect(rpc).toHaveBeenCalledOnce();
    expect(history().generations[0].acknowledged).toBe(true);
  ...
  it.each([
    ...
    'refuses %s recovery and never falls through to a financial RPC or retries that start',
    async (failure) => {
      lookup.mockImplementationOnce(async (_name, args) => {
        ...
        if (failure === 'old-receipt') {
          data.found = true;
          data.receipt = { success: true, replayed: true };
        }
        return { data, error: null };
      });
      const captured = await start();
      await expect(recoverCashoutOperation(captured)).rejects.toThrow(
        /Could Not Be Verified|Receipt Was Not Confirmed/
      );
      await expect(dispatchCashoutOperation(captured)).rejects.toThrow(/Check The Exact/);
      await expect(recoverCashoutOperation(captured)).rejects.toThrow();
      expect(lookup).toHaveBeenCalledOnce();
  ...
  it('refuses account A to B to A during the lookup without any new payment', async () => {
    const gate = deferred<any>();
    lookup.mockReturnValueOnce(gate.promise);
    const captured = await start();
    const checking = recoverCashoutOperation(captured);
...
describe('receipt-only checks for an exact in-view terminal intent', () => {
  it('recaptures the same generation after its row retires but cannot call a payer even after absence', async () => {
    ...
    const terminal = {
      ...intent,
      targetId: ID.cashout,
      kind: 'cashout_cancel' as const,
      isCurrent: () => rowCurrent && viewCurrent,
      receiptViewCurrent: () => viewCurrent,
    };
  ...
  it('account ABA cannot recapture an original terminal start under a new live view', async () => {
    const original = captureCashoutStart(
      await prepareCashoutOperation({
        ...intent,
        targetId: ID.cashout,
        kind: 'cashout_cancel',
        receiptViewCurrent: () => true,
      })
    );
    signIn(ID.other);
    signIn(ID.player);
  ...
  it('a found receipt survives acknowledgment storage failure and retains its original operation', async () => {
    ...
    expect(await recoverCashoutOperation(captured)).toMatchObject({
      found: true,
      result: { id: ID.cashout },
    });
    expect(sessionStorage.getItem(historyKey)).not.toBeNull();
    expect(history().generations[0].acknowledged).toBe(false);
    const operation = lookup.mock.calls[0][1].p_op_id;
