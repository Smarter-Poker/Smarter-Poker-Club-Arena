...
const identity = vi.hoisted(() => ({
  userId: '',
  auth: new Set<(event: { payload: { userId: string; isAuthenticated: boolean } }) => void>(),
}));
...
vi.mock('../../src/utils/strictClubIdResolver', () => ({
  resolveClubUUIDStrict: vi.fn(async (id: string) => id),
}));
...
function withOperation(args: any, response: any) {
  return {
    ...response,
    data: response.data ? { ...response.data, op_id: args.p_op_id } : response.data,
  };
}
beforeEach(() => {
  vi.clearAllMocks();
  ...
  for (const handler of identity.auth)
    handler({ payload: { userId: ID.player, isAuthenticated: true } });
  localStorage.clear();
  ...
  rpc.mockReset().mockImplementation(
    async (name, args) =>
      ({
        data: {
          ...cashoutV2Receipt(name === 'fn_cashout_release_v2' ? 'cancellation' : 'hold', {
            amount: Number(args?.p_amount),
            note: args?.p_note as string | undefined,
          }),
          op_id: args?.p_op_id,
        },
        error: null,
  ...
  async (text) => {
    ...
    expect(rpc.mock.calls[0]).toEqual([
      ...
      expect.objectContaining({
        p_amount: Number(text).toFixed(2),
        p_club_id: ID.club,
        p_expected_actor_id: ID.player,
        p_op_id: expect.any(String),
      }),
    ]);
  }
);
...
it('balance and settlement refusal still prevent submission', async () => {
  ...
  await waitFor(() =>
    expect(screen.getByRole('button', { name: 'Check Or Request Cashout' })).not.toBeDisabled()
  );
...
it('does not turn a bare success into a completed callback or a held claim', async () => {
  rpc.mockResolvedValueOnce({
    data: { success: true, cashout_id: ID.cashout },
    error: null,
  } as never);
  const input = await mount();
  fireEvent.change(input, { target: { value: '1.25' } });
...
it('renders the actual terminal request on a historical hold replay', async () => {
  rpc.mockImplementationOnce(
    async (_name, args) =>
      withOperation(args, {
        data: cashoutV2Receipt('hold', { amount: 1.25, replayed: true, currentStatus: 'approved' }),
        error: null,
      }) as never
  );
  const input = await mount();
...
it('never completes an old account request after an account change during dispatch', async () => {
  ...
  rpc.mockImplementationOnce(
    (_name, args) =>
      new Promise((yes) => {
        resolve = (response) => yes(withOperation(args, response));
      }) as never
  );
  const input = await mount();
  fireEvent.change(input, { target: { value: '1.25' } });
...
it('close and reopen removes old rows and fences an old completion from a new in-flight request', async () => {
  ...
  rpc.mockImplementationOnce(
    (_name, args) =>
      new Promise((resolve) => {
        finishOld = (response) => resolve(withOperation(args, response));
      }) as never
  );
  rpc.mockImplementationOnce(
    (_name, args) =>
      new Promise((resolve) => {
        finishNew = (response) => resolve(withOperation(args, response));
      }) as never
  );
  const existing = {
    id: ID.cashout,
  ...
  expect(screen.queryByText('250 Chips')).toBeNull();
  fireEvent.change(await screen.findByLabelText('Amount'), { target: { value: '1.25' } });
  await submit();
  await waitFor(() => expect(rpc).toHaveBeenCalledTimes(2));
  await act(async () =>
    finishOld({ data: cashoutV2Receipt('hold', { amount: 1.25 }), error: null })
  );
  expect(complete).not.toHaveBeenCalled();
  expect(screen.queryByText(/Chips Are Held For Review/)).toBeNull();
  const stillBusy = screen.getByRole('button', { name: 'Checking...' });
  expect((stillBusy as HTMLButtonElement).disabled).toBe(true);
  fireEvent.click(stillBusy);
  expect(rpc).toHaveBeenCalledTimes(2);
  await act(async () =>
    finishNew({ data: cashoutV2Receipt('hold', { amount: 1.25, replayed: true }), error: null })
  );
  await waitFor(() => expect(complete).toHaveBeenCalledOnce());
...
it('an account A to B to A change during settlement admission refuses the old action without an intermediate render', async () => {
  ...
  vi.mocked(checkSettlementLock).mockImplementationOnce(
    () =>
      new Promise((resolve) => {
  ...
  act(() => {
    identity.userId = ID.other;
    for (const handler of identity.auth)
      handler({ payload: { userId: ID.other, isAuthenticated: true } });
    identity.userId = ID.player;
    for (const handler of identity.auth)
      handler({ payload: { userId: ID.player, isAuthenticated: true } });
  });
...
it.each([0, null])(
  'recovers a lost successful hold with balance %s and a changed settlement lock',
  async (balance) => {
    ...
    vi.mocked(checkSettlementLock)
      .mockClear()
      .mockResolvedValue({ locked: true } as never);
...
it('an unknown lookup cannot reach settlement preflight or payment from the mounted modal', async () => {
  ...
  await submit();
  await screen.findByText(/Previous Cashout Outcome Could Not Be Verified/);
  expect(checkSettlementLock).not.toHaveBeenCalled();
  expect(rpc).not.toHaveBeenCalled();
  expect(complete).not.toHaveBeenCalled();
});
