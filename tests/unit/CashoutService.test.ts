  ...
  supabase: {
    ...
    from: vi.fn(() => {
      ...
      chain.limit = async (...args: unknown[]) => {
        state.queryCalls.push(['limit', ...args]);
        return state.read;
      };
...
vi.mock('../../src/core/MasterBus', () => ({
  masterBus: {
    ...
    subscribe: vi.fn((name, handler) => {
      if (name === 'AUTH_STATE_CHANGED') state.auth = handler;
      return vi.fn();
    }),
  },
}));
...
const current = () => true;
const intent = (amount = 250, isCurrent = current) => ({
  clubId: ID.club,
  playerId: ID.player,
  amount,
  isCurrent,
...
const accept = (
  ...
  options: Parameters<typeof cashoutV2Receipt>[1] = {}
...
function request(amount = 250, isCurrent = current) {
  return cashoutService.requestCashout(
    ID.player,
    ID.club,
    amount,
    undefined,
...
function act(kind: FixtureCashoutKind) {
  ...
  if (kind === 'cancellation')
    return cashoutService.cancelCashout(ID.cashout, ID.player, ID.operation, intent());
  if (kind === 'approval')
    return cashoutService.approveCashout(ID.cashout, ID.agent, undefined, ID.operation, intent());
...
describe('verified v2 cashier lifecycle', () => {
  it.each(['hold', 'approval', 'cancellation', 'decline'] as const)(
    '%s requires matching canonical event and document',
    async (kind) => {
      ...
      expect(rpc).toHaveBeenCalledExactlyOnceWith(
            ...
            : 'fn_cashout_release_v2',
        {
          p_club_id: ID.club,
          p_amount: '250.00',
          p_expected_actor_id: state.userId,
          p_op_id: ID.operation,
          p_note: null,
          ...(kind === 'hold' ? {} : { p_cashout_id: ID.cashout }),
        }
      );
      ...
      expect(masterBus.emit).toHaveBeenCalledWith(
        'BALANCE_UPDATED',
        expect.objectContaining({ userId: ID.player })
      );
  ...
  it('normalizes the immutable note and returns the actual current DTO', async () => {
    ...
    const row = await cashoutService.requestCashout(
      ID.player,
      ID.club,
      250,
      '  Taxi  ',
  ...
  it.each(['approved', 'cancelled', 'rejected', 'expired'] as const)(
    'returns %s current request on a verified historical hold replay',
    async (currentStatus) => {
  ...
  it('permits a player who is also club owner to request their own hold', async () => {
    const receipt = cashoutV2Receipt();
    receipt.actor_role = receipt.cashier.actor_role = 'owner';
...
describe('no unverified success or silent legacy fallback', () => {
  const mutations: Array<[string, (receipt: any) => void]> = [
    [
      'bare legacy success',
      (r) => {
        for (const key of Object.keys(r)) if (key !== 'success') delete r[key];
      },
    ],
    [
      'missing nested document',
      (r) => {
        delete r.cashier;
      },
    ],
    [
      ...
      (r) => {
        r.invoice_id = r.cashier.invoice_id = null;
      },
    ],
    [
      'missing source ledger identity',
      (r) => {
        delete r.source_ledger_id;
        delete r.cashier.source_ledger_id;
      },
    ],
    [
      'wrong account',
      (r) => {
        r.actor_user_id = r.cashier.actor_user_id = ID.other;
      },
    ],
    [
      'wrong club',
      (r) => {
        r.club_id = r.cashier.club_id = ID.otherClub;
      },
    ],
    [
      'wrong player',
      (r) => {
        r.player_id = r.cashier.player_id = ID.other;
      },
    ],
    [
      'wrong operation',
    ...
    [
      'wrong event phase',
      (r) => {
        r.event_kind = r.cashier.event_kind = 'approval';
      },
    ],
    [
      'false custody flag',
      (r) => {
        r.custody_movement_recorded = r.cashier.custody_movement_recorded = false;
      },
    ],
    [
      'false refund flag',
      (r) => {
        r.refund_recorded = r.cashier.refund_recorded = true;
      },
    ],
    [
      'different document phase',
      (r) => {
        r.cashier.display_state = 'approved';
      },
    ],
    [
      'wrong physical wallet',
      (r) => {
        r.ledger_from_entity_id = r.cashier.ledger_from_entity_id = ID.agent;
      },
    ],
    [
      'hold has terminal link',
      (r) => {
        r.hold_event_id = r.cashier.hold_event_id = ID.holdEvent;
      },
    ],
    [
      'amount as number',
      (r) => {
        r.amount = r.cashier.amount = 250;
      },
    ],
    [
      'fractional cent',
      (r) => {
        r.amount = r.cashier.amount = '250.001';
      },
    ],
    [
      'exponential amount',
      (r) => {
        r.amount = r.cashier.amount = '2.5e2';
      },
    ],
    [
      'wrong amount',
      (r) => {
        r.amount = r.cashier.amount = '250.01';
      },
    ],
    [
      'unbounded decimal parse',
      (r) => {
        r.amount = r.cashier.amount = '1'.repeat(129) + '.00';
      },
    ],
    [
      'wallet balance not exact',
      (r) => {
        r.actor_wallet_after = '750.001';
      },
    ],
    [
      'wallet balance omitted',
      (r) => {
        delete r.actor_wallet_after;
      },
    ],
    [
      'missing updated timestamp',
      (r) => {
        delete r.request.updated_at;
      },
    ],
    [
      'missing nullable timestamp',
      (r) => {
        delete r.request.completed_at;
      },
    ],
    [
      'impossible calendar timestamp',
      (r) => {
        r.issued_at = r.cashier.issued_at = '2026-02-30T09:00:00Z';
      },
    ],
    [
      'overlong timestamp',
      (r) => {
        r.issued_at = r.cashier.issued_at = '2026-09-15T09:00:00.' + '0'.repeat(129) + 'Z';
      },
    ],
    [
      'negative wallet balance',
      (r) => {
        r.actor_wallet_after = '-1.00';
      },
    ],
    [
      'unsafe wallet cents',
      (r) => {
        r.actor_wallet_after = '9007199254740992.00';
      },
    ],
    [
      'invalid issue timestamp',
      (r) => {
        r.issued_at = r.cashier.issued_at = 'yesterday';
      },
    ],
    [
      'invented hold earning time',
      (r) => {
        r.occurred_at = r.cashier.occurred_at = r.issued_at;
      },
    ],
    [
      'different current request',
      (r) => {
        r.request.id = ID.other;
      },
    ],
    [
      'different request assigned agent',
      (r) => {
        r.request.agent_id = ID.other;
      },
    ],
    [
      'different request amount',
      (r) => {
        r.request.amount = '249.99';
      },
    ],
    [
      'fresh hold claims terminal state',
      (r) => {
        r.request.status = 'approved';
      },
    ],
    [
      'status alone completed',
      (r) => {
        r.request.status = 'completed';
      },
    ],
    [
      'contradictory pending update time',
      (r) => {
        r.request.updated_at = r.issued_at;
      },
    ],
    [
      'pending has acknowledgment',
      (r) => {
        r.request.acknowledged_at = r.issued_at;
      },
    ],
    [
      'mismatched operation note',
      (r) => {
        r.accepted_note = 'other operation';
      },
    ],
    [
      'missing accepted note',
      (r) => {
        delete r.accepted_note;
      },
    ],
    [
      'mismatched accepted note',
      (r) => {
        r.request.player_note = 'another operation';
      },
    ],
    [
  ...
  it.each(['approval', 'cancellation', 'decline'] as const)(
    'refuses %s for another request or missing original hold',
    async (kind) => {
      ...
      const noHold = cashoutV2Receipt(kind);
      noHold.hold_event_id = noHold.cashier.hold_event_id = null;
  ...
  it.each(['approval', 'decline'] as const)('refuses a player role on %s', async (kind) => {
    signIn(ID.agent);
    const receipt = cashoutV2Receipt(kind);
    receipt.actor_role = receipt.cashier.actor_role = 'player';
    rpc.mockResolvedValueOnce({ data: receipt, error: null } as never);
    await expect(act(kind)).rejects.toBeInstanceOf(CashoutOutcomeUnknownError);
    expect(masterBus.emit).not.toHaveBeenCalled();
  });
  it('never dispatches self approval', async () => {
    await expect(
      cashoutService.approveCashout(ID.cashout, ID.player, undefined, ID.operation, intent())
    ).rejects.toThrow(/Own Cashout/);
    expect(rpc).not.toHaveBeenCalled();
  });
  it('refuses an uppercase self-decline intent before club resolution or dispatch', async () => {
    ...
    signIn(player);
    await expect(
      cashoutService.rejectCashout(ID.cashout, player, undefined, ID.operation, {
        ...intent(),
        playerId: player.toUpperCase(),
      })
    ).rejects.toThrow(/Player Cancellation/);
  ...
  it('canonicalizes validated player, club, request and operation UUIDs before approval and receipt matching', async () => {
    ...
    await expect(
      cashoutService.approveCashout(
        ...
        { ...intent(), playerId: player.toUpperCase(), clubId: club.toUpperCase() }
      )
    ).resolves.toBe(true);
    expect(rpc).toHaveBeenCalledExactlyOnceWith('fn_cashout_approve_v2', {
      p_club_id: club,
      p_cashout_id: cashout,
      p_expected_actor_id: ID.agent,
      p_amount: '250.00',
      p_op_id: operation,
      p_note: null,
    });
    expect(masterBus.emit).toHaveBeenCalledWith('CASHOUT_APPROVED', {
      cashoutId: cashout,
      clubId: club,
    });
    expect(masterBus.emit).toHaveBeenCalledWith('BALANCE_UPDATED', {
  ...
  it('accepts an exact zero actor wallet balance', async () => {
    const receipt = cashoutV2Receipt();
    receipt.actor_wallet_after = '0.00';
    rpc.mockResolvedValueOnce({ data: receipt, error: null } as never);
    await expect(request()).resolves.toMatchObject({ status: 'pending' });
  });
  it.each(['approval', 'cancellation', 'decline'] as const)(
    'refuses contradictory terminal state or destination for %s',
    async (kind) => {
      signIn(kind === 'cancellation' ? ID.player : ID.agent);
      const badState = cashoutV2Receipt(kind);
      badState.request.status = 'pending';
      rpc.mockResolvedValueOnce({ data: badState, error: null } as never);
      await expect(act(kind)).rejects.toBeInstanceOf(CashoutOutcomeUnknownError);
      const badDestination = cashoutV2Receipt(kind);
      badDestination.ledger_to_entity_id = badDestination.cashier.ledger_to_entity_id =
        ID.other as typeof ID.player;
  ...
  it.each(['approval', 'cancellation', 'decline'] as const)(
    'requires exact recorded terminal times for %s',
    async (kind) => {
      signIn(kind === 'cancellation' ? ID.player : ID.agent);
      const receipt = cashoutV2Receipt(kind);
      receipt.request.updated_at = receipt.request.created_at;
      rpc.mockResolvedValueOnce({ data: receipt, error: null } as never);
      await expect(act(kind)).rejects.toBeInstanceOf(CashoutOutcomeUnknownError);
      expect(masterBus.emit).not.toHaveBeenCalled();
    }
  ...
  it('a rejected hold replay requires its actual terminal phase timestamps', async () => {
    const receipt = cashoutV2Receipt('hold', { replayed: true, currentStatus: 'rejected' });
    receipt.request.cancelled_at = null;
    rpc.mockResolvedValueOnce({ data: receipt, error: null } as never);
    await expect(request()).rejects.toBeInstanceOf(CashoutOutcomeUnknownError);
  });
  it('decline never attributes the refunded player balance to the acting agent', async () => {
    signIn(ID.agent);
    const receipt = cashoutV2Receipt('decline');
    receipt.actor_wallet_after = '750.00';
    rpc.mockResolvedValueOnce({ data: receipt, error: null } as never);
    await expect(act('decline')).rejects.toBeInstanceOf(CashoutOutcomeUnknownError);
  });
  it.each([null, undefined, []])(
    'keeps malformed transport envelope %s unknown',
    async (response) => {
  ...
  it('preserves an explicit server refusal without events', async () => {
    rpc.mockResolvedValueOnce({
      data: { success: false, error: 'Source Hold Unverified' },
      error: null,
    } as never);
  ...
  it.each(['returned', 'thrown'] as const)(
    'keeps %s transport failure unknown and never calls an older RPC',
    async (failure) => {
      if (failure === 'returned')
...
describe('captured account and view authority', () => {
  it('requires retained operation, actor and caller view guard before lookup or RPC', async () => {
    ...
    await expect(
      cashoutService.approveCashout(ID.cashout, ID.agent, undefined, ID.operation)
    ).rejects.toThrow(/Retained Operation/);
    await expect(
      cashoutService.requestCashout(ID.player, ID.club, 250, undefined, 'not-a-uuid', current)
    ).rejects.toThrow();
    signIn(null);
  ...
  it('refuses an account switch while club resolution is pending', async () => {
    ...
    vi.mocked(resolveClubUUID).mockReturnValueOnce(pending.promise);
    const result = request();
    signIn(ID.other);
    pending.resolve(ID.club);
    await expect(result).rejects.toBeInstanceOf(CashoutOutcomeUnknownError);
    expect(rpc).not.toHaveBeenCalled();
  });
  it.each(['account', 'club', 'unmount', 'aba'] as const)(
    'does not publish success after %s changes during an RPC',
    async (change) => {
  ...
  it('shares a permanently invalidated account guard with non-React callers', () => {
    const isCurrent = captureCashoutAccountGuard(ID.player);
    ...
    signIn(null);
    expect(() => captureCashoutAccountGuard(ID.player)).toThrow(/Sign In/);
  });
});
...
describe('exact cent wire amounts', () => {
  ...
  ])('refuses %s before dispatch', async (amount) => {
    await expect(request(amount)).rejects.toThrow();
    expect(rpc).not.toHaveBeenCalled();
  });
});
...
describe('request read scope and unavailable data', () => {
  it('canonicalizes an uppercase resolved club in both scoped queue readers', async () => {
    ...
    await expect(
      cashoutService.getPlayerCashouts(ID.player, club.toUpperCase(), 'pending')
    ).resolves.toEqual([
    ...
    await expect(
      cashoutService.getAgentPendingCashouts(ID.agent, club.toUpperCase())
    ).resolves.toEqual([
      expect.objectContaining({ clubId: club, playerId: ID.player, status: 'pending' }),
    ]);
    expect(rpc).toHaveBeenCalledExactlyOnceWith('fn_cashout_queue', {
  ...
  it('distinguishes unavailable player history from an empty result', async () => {
    ...
    await expect(cashoutService.getPlayerCashouts(ID.player, ID.club)).rejects.toThrow(
      'permission denied'
    );
    state.read = { data: [], error: null };
  ...
  it('filters pending before the bounded history limit and rejects inconsistent returned rows', async () => {
    ...
    await expect(
      cashoutService.getPlayerCashouts(ID.player, ID.club, 'pending')
    ).resolves.toHaveLength(1);
    const status = state.queryCalls.findIndex(
      (call) => call[0] === 'eq' && call[1] === 'status' && call[2] === 'pending'
    );
    expect(status).toBeGreaterThan(-1);
    expect(status).toBeLessThan(state.queryCalls.findIndex((call) => call[0] === 'limit'));
    state.read = {
      data: [cashoutV2Receipt('hold', { replayed: true, currentStatus: 'approved' }).request],
      error: null,
    };
    await expect(cashoutService.getPlayerCashouts(ID.player, ID.club, 'pending')).rejects.toThrow(
      /Scope/
    );
  });
  it.each(['club', 'player', 'amount', 'duplicate', 'malformed'] as const)(
    'rejects %s history inconsistency',
    async (fault) => {
      ...
      state.read = {
        data: fault === 'duplicate' ? [row, row] : fault === 'malformed' ? {} : [row],
        error: null,
      };
      await expect(cashoutService.getPlayerCashouts(ID.player, ID.club)).rejects.toThrow(
  ...
  it.each(['amount', 'club', 'status', 'duplicate', 'malformed'] as const)(
    'rejects %s agent queue inconsistency',
    async (fault) => {
      signIn(ID.agent);
      const row: any = { ...cashoutV2Receipt().request };
      ...
      rpc.mockResolvedValueOnce({
        data: fault === 'duplicate' ? [row, row] : fault === 'malformed' ? null : [row],
        error: null,
      } as never);
  ...
  it('does not invent update time for a legacy queue row', async () => {
    ...
    expect(request.createdAt).toBe(row.created_at);
  });
  it('refuses switched account during agent read instead of returning its old rows', async () => {
    ...
    const result = cashoutService.getAgentPendingCashouts(ID.agent, ID.club);
    await started.promise;
    signIn(ID.other);
