...
vi.mock('../../src/lib/supabase', () => ({
  supabase: {
    ...
    from: vi.fn((table: string) => {
      ...
      let cap = Infinity;
      const chain: any = {};
      for (const method of ['select', 'order', 'gte', 'lte', 'or'])
      ...
      chain.is = (key: string, value: unknown) => {
        ...
        state.calls.push(['is', key, value]);
        return chain;
      };
      chain.eq = (key: string, value: unknown) => {
        filters.push([key, value]);
        state.calls.push(['eq', key, value]);
        return chain;
      };
      chain.limit = (limit: number) => {
        cap = limit;
        state.calls.push(['limit', limit]);
        return chain;
      };
        ...
        const reply = state.reply
          ...
          : Promise.resolve({
              data: state.rows
                .filter(
                  (row) =>
                    state.ignoreFilters || filters.every(([key, value]) => row[key] === value)
                )
                .slice(0, cap),
              error: null,
            });
...
describe('one canonical club weekly read boundary', () => {
  it('filters correction and individual invoices before the bounded weekly read and preserves factory amounts', async () => {
    state.rows = [
      weeklyStatementRow({ invoice_type: 'accounting_correction', net_amount: '999.00' }),
      weeklyStatementRow({ invoice_type: 'club_to_agent', net_amount: '500.00' }),
      weeklyStatementRow(),
      weeklyStatementRow({ club_id: ID.otherClub }),
    ];
    const result = await readClubWeeklyStatements({
      clubId: ID.club.toUpperCase(),
      userId: ID.actor,
    });
    expect(result).toMatchObject({
      limit: 50,
      rows: [
        { clubId: ID.club, rakeFunding: '100.25', paidByClub: '60.20', retainedByClub: '40.05' },
      ],
    });
    const typeFilter = state.calls.findIndex(
      (call) =>
        call[0] === 'eq' && call[1] === 'invoice_type' && call[2] === 'club_weekly_accounting'
    );
    expect(typeFilter).toBeGreaterThan(-1);
    expect(typeFilter).toBeLessThan(state.calls.findIndex((call) => call[0] === 'limit'));
    expect(state.calls.filter((call) => call[0] === 'order')).toEqual([
      ['order', 'created_at', { ascending: false }],
      ['order', 'id', { ascending: false }],
    ]);
    expect(state.calls.find((call) => call[0] === 'select')?.[1]).toContain('gross_amount::text');
    expect(state.calls.find((call) => call[0] === 'select')?.[1]).toContain(
      'total_rake_funding:breakdown->>total_rake_funding'
    );
  });
  it.each([
    { club_id: ID.otherClub },
    { invoice_type: 'accounting_correction' },
    { summary_club_id: ID.otherClub },
    { summary_period_id: ID.invoice },
    { ready_to_issue: 'false' },
    { status: 'paid' },
    { message_sent: false },
    { from_entity_id: ID.actor },
    ...
    state.rows = [weeklyStatementRow(fault)];
  ...
  it('keeps a negative retained amount exact without labelling a payment', async () => {
    state.rows = [
      weeklyStatementRow({
        deductions: '101.20',
        total_paid_by_club: '101.2000',
        net_amount: '-0.95',
        retained_by_club: '-0.9500',
      }),
    ];
  ...
  it('preserves the captured numeric(12,2) maximum exactly and refuses amounts outside it', async () => {
    state.rows = [
      weeklyStatementRow({
        gross_amount: '9999999999.99',
        total_rake_funding: '9999999999.9900',
        deductions: '0.00',
        total_paid_by_club: '0',
        net_amount: '9999999999.99',
        retained_by_club: '9999999999.99',
      }),
    ];
    const { rows } = await readClubWeeklyStatements({ clubId: ID.club });
  ...
  it('preserves validated date-only export windows without changing their midnight filter meaning', async () => {
    ...
    const result = await FinancialExportService.fetchClubSettlements({
    ...
    const rowQueryStart = state.calls.findIndex(
      (call) => call[0] === 'select' && String(call[1]).includes('gross_amount::text')
    );
    ...
    expect(rowQuery.findIndex((call) => call[0] === 'lte')).toBeLessThan(
      rowQuery.findIndex((call) => call[0] === 'limit')
    );
  });
  it.each([
    ...
    { periodStart: '2026-09-15', periodEnd: '2026-09-14' },
    { periodStart: '' },
  ])('refuses invalid date windows before a query: %j', async (window) => {
    await expect(readClubWeeklyStatements({ clubId: ID.club, ...window })).rejects.toThrow(
      /Date Window/
    );
  ...
  it('refuses a returned row outside the requested window instead of exporting it', async () => {
    state.rows = [weeklyStatementRow()];
    await expect(
      readClubWeeklyStatements({ clubId: ID.club, periodEnd: '2026-09-14' })
    ).rejects.toThrow(/Date Scope/);
  });
  it('preserves microsecond precision when checking the requested window', async () => {
    state.rows = [weeklyStatementRow()];
    await expect(
      readClubWeeklyStatements({ clubId: ID.club, periodEnd: '2026-09-14T08:00:00.000000+00:00' })
    ).rejects.toThrow(/Date Scope/);
    await expect(
      readClubWeeklyStatements({
        clubId: ID.club,
        periodStart: '2026-09-14T08:00:00.000002+00:00',
        periodEnd: '2026-09-14T08:00:00.000001+00:00',
      })
  ...
  it('refuses duplicate weekly periods and invalid bounds', async () => {
    ...
    for (const limit of [0, -1, 1001, NaN, 1.5])
      await expect(readClubWeeklyStatements({ clubId: ID.club, limit })).rejects.toThrow(/Limit/);
  });
  it('holds account ABA through alias resolution before a query can be made', async () => {
    const gate = deferred<string>();
    vi.mocked(resolveClubUUID).mockReturnValueOnce(gate.promise);
    const result = readClubWeeklyStatements({ clubId: ID.club });
  ...
  it('discards a response after account or view retirement', async () => {
    ...
    const result = readClubWeeklyStatements({ clubId: ID.club, isCurrent: () => current });
    await started.promise;
    current = false;
    gate.resolve({ data: [weeklyStatementRow()], error: null });
    await expect(result).rejects.toThrow(/Account Or Club Changed/);
  });
  it('exports only bounded weekly funding, club payments and retained exact cents', async () => {
    ...
    expect(csv).toContain('"Retained By Club (Chips)"');
    expect(csv).toContain('"100.25","60.20","40.05"');
    expect(csv).toContain('club_weekly_summaries');
    expect(csv).not.toContain('accounting_correction');
  });
  it('never downloads an export after the account changes during its read', async () => {
    ...
    const result = FinancialExportService.exportCSV({ type: 'settlement_club', clubId: ID.club });
    await started.promise;
    signIn(ID.otherActor);
    signIn(ID.actor);
    gate.resolve({ data: [weeklyStatementRow()], error: null });
  ...
  it('narrows weekly continuation by exact actor/club/microsecond tuple without changing the 50-row default', async () => {
    const boundary = {
      createdAt: '2026-09-14T08:00:00.000002+00:00',
      id: ID.invoice,
      clubId: ID.club,
      actorId: ID.actor,
    };
    ...
    await expect(readClubWeeklyStatements({ clubId: ID.club, cursor: boundary })).rejects.toThrow(
      /Page Order/
    );
  });
  it('refuses cursor scope changes and filter syntax before the invoice query', async () => {
    for (const fault of [
      { actorId: ID.otherActor },
      { clubId: ID.otherClub },
      { id: 'x),id.neq.null' },
      { createdAt: '2026-09-14T08:00:00Z,or(x)' },
    ]) {
      state.calls = [];
      await expect(
        readClubWeeklyStatements({
