...
import {
  ...
  creditEnvelope,
  CREDIT_ID as ID,
} from '../helpers/creditReductionReceipt';
import {
  parseCreditSnapshot,
  parseCreditEnvelope,
  ...
  creditIntentText,
  receiptIntent,
} from '../../src/lib/CreditReductionContract';
...
describe('exact nonpayable credit reduction receipt', () => {
  it('keeps exact money, bigint revisions including zero and microsecond timestamps', () => {
    const snapshot = creditSnapshot({
      credit_limit: '9999999999999.99',
      control_revision: '9223372036854775806',
    });
    ...
    expect(parseCreditEnvelope(envelope, expected, 'lookup').receipt?.applied_reduction).toBe(
      '100.00'
    );
    expect(envelope.receipt?.requested_reduction).toBe('250.00');
    expect(parseCreditEnvelope(envelope, expected, 'lookup').receipt?.recorded_at).toBe(
      '2026-09-15T14:01:00.654321Z'
    );
  });
  it.each([
    '90071992547409.90',
    ...
    1,
    null,
  ])('refuses malformed or out-of-schema transport %s', (amount) => {
    expect(() =>
      parseCreditSnapshot(
        { ...creditSnapshot(), credit_limit: amount },
        ID.actor,
        ID.club,
        ID.target
      )
  ...
  it.each([
    ...
    '2026-09-15T14:00:00.123Z',
    '2026-09-15T24:00:00.123456Z',
    'infinity',
  ])('refuses noncanonical timestamp %s', (captured_at) => {
    expect(() =>
      parseCreditSnapshot(creditSnapshot({ captured_at }), ID.actor, ID.club, ID.target)
    ).toThrow();
  });
  it.each([
    { payment_proven: true },
    { chip_movement_claimed: true },
    { amount_due_claimed: true },
    { invoice_id: null },
    { document_id: ID.invoice },
    { after_limit: '1.00' },
    { applied_reduction: '250.00' },
    { after_revision: '0' },
    { credit_used: '1.00' },
    { assignment_reason: 'Changed' },
    { requested_reduction: '0.00' },
    { reason: undefined },
  ])('refuses contradictory recorded proof %j', (delta) => {
    const value = creditEnvelope(args, 'recorded');
    Object.assign(value.receipt!, delta);
  ...
  it('keeps NULL, empty and whitespace reasons distinct while checking the assignment default', () => {
    const nullIntent = receiptIntent(creditReceipt()),
      emptyIntent = receiptIntent(creditReceipt({ reason: '' }));
  ...
  it('requires exact absent and retired envelopes rather than truthiness or status alone', () => {
    ...
    for (const value of [
      ...
      {},
      true,
      { ...creditEnvelope(args), receipt: {} },
      { ...creditEnvelope(args), replayed: true },
      { ...creditEnvelope(args), operation_id: ID.receipt },
      { ...creditEnvelope(args), extra: 1 },
    ]) {
      expect(() => parseCreditEnvelope(value, expected, 'lookup')).toThrow();
    }
    ...
    expect(() =>
      parseCreditEnvelope(creditEnvelope(args, 'recorded', {}, false), expected, 'retire')
    ).toThrow();
    expect(parseCreditEnvelope(creditEnvelope(args, 'retired'), expected, 'retire').state).toBe(
      'retired'
    );
  });
  it('normalizes plain UI decimals once without rounding or treating empty as zero', () => {
    expect(creditReductionAmount('25')).toBe('25.00');
    expect(creditReductionAmount('25.1')).toBe('25.10');
    for (const value of ['', '0', '25.001', '1e2', ' 25', '1000000000.01'])
      expect(() => creditReductionAmount(value)).toThrow();
  });
});
