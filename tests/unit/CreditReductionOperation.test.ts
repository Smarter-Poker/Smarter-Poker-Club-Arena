...
} from '../../src/services/CreditReductionOperation';
...
function index() {
  return JSON.parse(allStorage().find(([key]) => key.startsWith(indexPrefix))![1]);
}
async function start(value = input) {
  return captureCreditReduction(await prepareCreditReduction(value));
}
async function pendingStart() {
  const rows = await listPendingCreditReductions(ID.actor, ID.club, () => true);
  expect(rows).toHaveLength(1);
  return capturePendingCreditReduction(rows[0]);
}
beforeEach(() => {
  signIn(null);
  signIn(ID.actor);
  ...
  transport.mockReset().mockImplementation((name, args) => {
    const handlers: Record<string, typeof snapshot> = {
      fn_agent_credit_reduction_snapshot_v1: snapshot,
      fn_agent_credit_reduction_receipt_v1: lookup,
      fn_reduce_agent_credit_v1: apply,
      fn_retire_agent_credit_reduction_v1: retire,
    };
    return (
      handlers[name] ??
      (() => {
...
describe('credit reduction original-operation recovery', () => {
  it('applies the exact prepared text intent and reports actual reduction with one invalidation and no value overwrite', async () => {
    ...
    expect(result.receipt?.applied_reduction).toBe('100.00');
    expect(apply).toHaveBeenCalledExactlyOnceWith('fn_reduce_agent_credit_v1', {
      p_expected_actor_id: ID.actor,
      p_operation_id: captured.operationId,
      p_club_id: ID.club,
      p_agent_id: ID.agent,
      p_target_user_id: ID.target,
      p_requested_reduction: '250.00',
      p_expected_credit_limit: '100.00',
      p_expected_credit_used: '0.00',
      p_expected_is_prepaid: false,
      p_expected_revision: '0',
      p_reason: null,
    });
    expect(masterBus.emit).toHaveBeenCalledExactlyOnceWith('CREDIT_UPDATED', {
      clubId: ID.club,
      userId: ID.target,
    });
  ...
  it('keeps an unknown stable lane across changed amount, reason, revision and agent-row replacement', async () => {
    ...
    await expect(runCreditReduction(original)).rejects.toThrow(/Lost/);
    ...
    snapshot.mockResolvedValueOnce({
      data: creditSnapshot({ control_revision: '1' }),
      error: null,
    });
    ...
    expect(index().records[0].reference.operationId).toBe(original.operationId);
  ...
  it('allows changed drafts before admission but refuses a competing accepted lane without replacing its operation', async () => {
    const a = await start(),
      b = await start({ ...input, amount: '1.00' });
    expect(
      localStorage.getItem(
        allStorage().find(([key]) => key.startsWith(indexPrefix))?.[0] ?? 'absent'
      )
    ).toBeNull();
  ...
  it('persists opaque references only, with no amounts, notes, target or actor plaintext', async () => {
    ...
    const captured = await start({ ...input, reason: privateNote });
    await recoverCreditReduction(captured);
    const bytes = JSON.stringify(allStorage());
    for (const value of [privateNote, ID.actor, ID.club, ID.agent, ID.target, '250.00', '100.00'])
  ...
  it('reloads an original receipt without querying a now-deleted target and never dispatches from a receipt-only handle', async () => {
    apply.mockRejectedValueOnce(new Error('lost'));
    const original = await start();
    await expect(runCreditReduction(original)).rejects.toThrow();
    sessionStorage.clear();
    ...
    expect(check.operationId).toBe(original.operationId);
    expect(snapshot).toHaveBeenCalledTimes(1);
    expect(apply).toHaveBeenCalledTimes(1);
  });
  it('does not clear or dispatch a receipt-only absent operation, even on repeated explicit checks', async () => {
    await recoverCreditReduction(await start());
    sessionStorage.clear();
    const check = await pendingStart();
    expect((await recoverCreditReduction(check)).state).toBe('absent');
  ...
  it('retains malformed, failed and unknown lookups instead of turning them into absent or a new operation', async () => {
    const captured = await start();
    lookup.mockResolvedValueOnce({ data: { found: false }, error: null });
  ...
  it('validates the full retained digest on reload, including exact NULL versus empty reason', async () => {
    ...
    sessionStorage.clear();
    lookup.mockImplementationOnce(async (_name, args) => ({
      data: creditEnvelope(args, 'recorded', { reason: '' }),
      error: null,
    }));
  ...
  it('only canonical retirement releases an absent lane, while an applied retirement response preserves the real receipt', async () => {
    const original = await start();
    await recoverCreditReduction(original);
    const result = await retireCreditReduction(await pendingStart());
    expect(result.state).toBe('retired');
    expect(retire.mock.calls[0][1].p_operation_id).toBe(original.operationId);
    expect(index().records[0].state).toBe('settled');
    const newer = await start();
    expect(newer.operationId).not.toBe(original.operationId);
    await recoverCreditReduction(newer);
    retire.mockImplementationOnce(async (_name, args) => ({
      data: creditEnvelope(args, 'recorded'),
      error: null,
    }));
  ...
  it('does not resume dispatch after an explicit retirement starts, including its uncertain failure', async () => {
    ...
    retire.mockReturnValueOnce(delayed.promise);
    ...
    delayed.reject(new Error('lost'));
    await expect(cancelling).rejects.toThrow(/Lost/);
    await expect(runCreditReduction(original)).rejects.toThrow(/Only Be Checked Or Cancelled/);
    expect(apply).not.toHaveBeenCalled();
    expect(index().records[0].state).toBe('active');
  });
  it('refuses acknowledged-generation plus absent contradiction without another apply', async () => {
    ...
    await expect(runCreditReduction(captureCreditReduction(preparation))).rejects.toThrow(
      /Unconfirmed/
    );
    expect(apply).toHaveBeenCalledTimes(1);
  });
  it('keeps a known result when local acknowledgment fails, with the lane retained for receipt recovery', async () => {
    ...
    expect(index().records[0].state).toBe('active');
    await expect(prepareCreditReduction({ ...input, amount: '1.00' })).rejects.toThrow(
      /Pending Credit Change/
    );
  });
  it('stages discoverable identity before admission and can retire it after a partial storage failure', async () => {
    const captured = await start();
    const originalSet = localStorage.setItem;
    const spy = vi.spyOn(localStorage, 'setItem').mockImplementation(function (
      this: Storage,
      key,
      value
    ) {
      if (
        key.startsWith('smarter-poker:credit-reduction-generations:') &&
        value.includes('"startId"')
      )
        throw new Error('quota');
      return originalSet.call(this, key, value);
    });
    await expect(recoverCreditReduction(captured)).rejects.toThrow('quota');
    expect(index().records[0].state).toBe('initializing');
  ...
  it('binds before alias resolution and retires actor ABA without applying or consuming stale receipts', async () => {
    ...
    vi.mocked(resolveClubUUIDStrict).mockReturnValueOnce(alias.promise);
    const preparation = prepareCreditReduction({ ...input, clubId: 'alias' });
    signIn(ID.target);
    signIn(ID.actor);
    alias.resolve(ID.club);
    await expect(preparation).rejects.toThrow(/Changed/);
    expect(snapshot).not.toHaveBeenCalled();
    const captured = await start();
    const delayed = deferred<any>();
    lookup.mockReturnValueOnce(delayed.promise);
    const check = recoverCreditReduction(captured);
  ...
  it('pins form and club ABA through a sticky caller generation before a pending lookup returns', async () => {
    let revision = 0;
    const captured = await start({ ...input, isCurrent: () => revision === 0 });
    const delayed = deferred<any>();
    lookup.mockReturnValueOnce(delayed.promise);
    const running = runCreditReduction(captured);
    await vi.waitFor(() => expect(lookup).toHaveBeenCalledTimes(1));
    revision += 2;
  ...
  it('coalesces overlapping same-operation applies while a second tab keeps the original identity', async () => {
    const prepared = await prepareCreditReduction(input),
      first = captureCreditReduction(prepared);
    const delayed = deferred<any>();
    apply.mockReturnValueOnce(delayed.promise);
    const running = runCreditReduction(first);
    await vi.waitFor(() => expect(apply).toHaveBeenCalledTimes(1));
    sessionStorage.clear();
    const second = await start();
    expect(second.operationId).toBe(first.operationId);
    const overlap = runCreditReduction(second);
    await vi.waitFor(() => expect(lookup).toHaveBeenCalledTimes(2));
    expect(apply).toHaveBeenCalledTimes(1);
    delayed.resolve({
      data: creditEnvelope(apply.mock.calls[0][1], 'recorded', {}, false),
      error: null,
    });
    expect((await running).state).toBe('recorded');
    expect((await overlap).state).toBe('recorded');
    expect(index().records).toHaveLength(1);
    expect(
      Array.from({ length: sessionStorage.length }, (_, i) => sessionStorage.key(i)).filter((key) =>
        key?.startsWith('smarter-poker:credit-reduction-generations:')
      )
    ).toHaveLength(0);
  });
