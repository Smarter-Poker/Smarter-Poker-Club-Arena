...
vi.mock('../../src/core/IdentityDNA', () => ({
  getIdentityDNAStatus: () => ({ loaded: true, authenticated: true, userId: calls.actor }),
}));
vi.mock('../../src/lib/supabase', () => ({ supabase: { rpc: calls.rpc, from: calls.from } }));
vi.mock('../../src/core/MasterBus', () => ({
  masterBus: {
    emit: calls.emit,
    subscribe: (_event: string, listener: (event: any) => void) => {
...
function switchAccount(actor: string) {
  calls.actor = actor;
  calls.listeners.forEach((listener) =>
    listener({ payload: { isAuthenticated: true, userId: actor } })
  );
}
...
const request = {
  ...
  approver_id: 'owner',
  club_id: 'club',
  requested_amount: '125.25',
  approved_amount: '125.25',
  reason: 'More capacity',
  status: 'approved',
  decision_authority_version: 1,
  reviewed_by: 'owner',
  reviewed_at: '2026-09-15T04:00:00Z',
...
describe('one credit request review authority', () => {
  ...
  it.each([0, -1, NaN, Infinity, 0.001, Number('90071992547409.90'), Number('90071992547409.91')])(
    'never substitutes a falsey/invalid limit %s',
    async (amount) => {
      await expect(creditRequestService.approveRequest('request', 'owner', amount)).rejects.toThrow(
  ...
  it('preserves an explicit amount and notes on retry', async () => {
    calls.rpc.mockResolvedValue({
      data: {
        success: true,
        replayed: true,
        request: { ...request, approved_amount: '20.00', reviewer_notes: 'Reviewed' },
      },
      error: null,
    });
  ...
  it.each([
    ...
    { success: true, request: { ...request, approved_amount: '125.25' + '0'.repeat(129) } },
    {
      ...
      request: {
        ...request,
        approved_amount: '90071992547409.90',
        requested_amount: '90071992547409.90',
      },
    },
    {
      success: true,
      request: {
        ...request,
        approved_amount: '90071992547409.91',
        requested_amount: '90071992547409.91',
      },
    },
    ...
    await expect(creditRequestService.approveRequest('request', 'owner')).rejects.toThrow(
      'Not Confirmed'
    );
  ...
  it.each([['other-owner'], ['other-owner', 'owner']])(
    'rejects a completed receipt after account transitions %j',
    async (...actors) => {
      let resolve!: (value: any) => void;
      calls.rpc.mockReturnValue(
        new Promise((done) => {
          resolve = done;
        })
      );
  ...
  it('does not consume names or emit when the account changes during name lookup', async () => {
    let resolve!: (value: any) => void;
    const names = new Promise((done) => {
      resolve = done;
    });
    let started!: () => void;
    const startedNames = new Promise<void>((done) => {
      started = done;
    });
    calls.from.mockReturnValue({
      select: () => ({
        in: () => {
          started();
          return names;
  ...
  it('denial and cancellation use the same server authority', async () => {
    calls.rpc.mockResolvedValueOnce({
      data: {
        success: true,
        request: {
          ...request,
    ...
    expect(calls.rpc).toHaveBeenNthCalledWith(1, 'fn_review_credit_request', {
      p_request_id: 'request',
      p_expected_actor_id: 'owner',
      p_decision: 'denied',
      p_notes: 'Declined',
    });
    expect(calls.rpc).toHaveBeenNthCalledWith(2, 'fn_review_credit_request', {
      p_request_id: 'request',
      p_expected_actor_id: 'requester',
...
describe('credit request creation and club reads', () => {
  ...
  it.each([
    { ...intent, clubId: undefined },
    { ...intent, approverId: '' },
    { ...intent, approverId: 'owner' },
    { ...intent, requestedAmount: 0 },
    { ...intent, requestedAmount: 0.001 },
  ])('refuses an incomplete creation intent before inserting', async (incomplete) => {
    const { insert } = creation();
    await expect(creditRequestService.submitRequest('owner', incomplete)).rejects.toThrow(
      'Positive Credit Limit'
    );
  ...
  it('refuses an account-mismatched creation intent before inserting', async () => {
    const { insert } = creation();
    await expect(creditRequestService.submitRequest('requester', intent)).rejects.toThrow(
      'Sign In'
    );
    expect(insert).not.toHaveBeenCalled();
  });
  it.each([
    null,
    { ...pendingRow, club_id: 'other' },
    { ...pendingRow, requester_id: 'other' },
    { ...pendingRow, requested_amount: 125.25 },
    { ...pendingRow, requested_amount: '125.24' },
    { ...pendingRow, status: 'approved' },
    { ...pendingRow, decision_authority_version: 1 },
  ])('does not accept a missing or mismatched pending receipt', async (data) => {
    creation(Promise.resolve({ data, error: null }));
    await expect(creditRequestService.submitRequest('owner', intent)).rejects.toThrow(
      'Not Confirmed'
    );
    expect(calls.from).toHaveBeenCalledExactlyOnceWith('credit_requests');
  });
  it.each([['other-owner'], ['other-owner', 'owner']])(
    'rejects creation completion after account transitions %j',
    async (...actors) => {
      let resolve!: (value: any) => void;
      creation(
        new Promise((done) => {
  ...
  it('rejects creation after an account switch during name resolution', async () => {
    ...
    calls.from
      .mockReturnValueOnce({
        insert: () => ({
          select: () => ({ maybeSingle: async () => ({ data: pendingRow, error: null }) }),
        }),
      })
      .mockReturnValueOnce({
        select: () => ({
  ...
  it.each(['getMyRequests', 'getRequestsForApprover'] as const)(
    'scopes %s to the requested club before order and limit',
    async (method) => {
      const operations = reading([pendingRow]);
      const rows = await creditRequestService[method]('owner', 'club');
      const clubFilter = operations.findIndex(
        (operation) => operation[0] === 'eq' && operation[1] === 'club_id'
      );
      expect(operations[clubFilter]).toEqual(['eq', 'club_id', 'club']);
      expect(clubFilter).toBeLessThan(
        operations.findIndex((operation) => operation[0] === 'order')
      );
      expect(clubFilter).toBeLessThan(
        operations.findIndex((operation) => operation[0] === 'limit')
      );
      expect(operations[0][1]).toContain('requested_amount::text');
      expect(rows[0].requestedAmount).toBe(125.25);
    }
  );
  it.each([125.25, '90071992547409.90', '90071992547409.91', '0.001', null])(
    'rejects an inexact reader amount %s',
    async (requestedAmount) => {
      reading([{ ...pendingRow, requested_amount: requestedAmount }]);
      await expect(creditRequestService.getMyRequests('owner', 'club')).rejects.toThrow(
        'Not Confirmed'
      );
    }
  );
  it('lets the current manager read pending requests addressed to an earlier manager', async () => {
    ...
    expect(
      operations.some((operation) => operation[0] === 'eq' && operation[1] === 'approver_id')
    ).toBe(false);
  });
  it.each([
    { ...pendingRow, club_id: 'other' },
    ...
    reading([row]);
    await expect(creditRequestService.getPendingForClub('club', 'owner')).rejects.toThrow(
      'Not Confirmed'
    );
    expect(calls.from).toHaveBeenCalledExactlyOnceWith('credit_requests');
  });
  it.each([['other'], ['other', 'owner']])(
    'rejects a manager inbox result after account transitions %j',
    async (...actors) => {
      let resolve!: (value: any) => void;
