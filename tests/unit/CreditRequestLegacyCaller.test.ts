...
vi.mock('../../src/core/IdentityDNA', () => ({
  getIdentityDNAStatus: () => ({ loaded: true, authenticated: true, userId: m.actor }),
}));
vi.mock('../../src/core/MasterBus', () => ({
  masterBus: {
    emit: vi.fn(),
    subscribe: (_event: string, callback: (event: any) => void) => {
...
vi.mock('../../src/services/CreditRequestService', () => ({
  creditRequestService: { submitRequest: m.submit },
}));
  ...
  m.from.mockImplementation((table: string) => {
    ...
    return {
      select: (columns: string) => {
        m.queries.push(['select', columns]);
        return {
          eq: (...args: unknown[]) => {
            m.queries.push(['eq', ...args]);
            return {
              limit: (count: number) => {
                m.queries.push(['limit', count]);
                return response;
              },
            };
          },
...
describe('legacy credit creation shares the canonical service', () => {
  it('resolves the exact club owner and delegates the user identity once', async () => {
    ...
    expect(m.submit).toHaveBeenCalledExactlyOnceWith('requester', {
      clubId: 'club',
      approverId: 'owner',
      requestedAmount: 125.25,
      reason: 'More capacity',
  ...
  it.each([
    [],
    [{ id: 'club', owner_id: null }],
    [{ id: 'club', owner_id: 'requester' }],
    [{ id: 'other', owner_id: 'owner' }],
    [
      { id: 'club', owner_id: 'owner' },
      { id: 'club', owner_id: 'other' },
    ],
  ])('refuses missing, self, wrong-club or ambiguous owners', async (...rows) => {
    owners(Promise.resolve({ data: rows, error: null }));
    await expect(
      CreditService.requestCreditIncrease('agent', 125.25, 'More capacity')
    ).rejects.toThrow('Different Current Club Owner');
  ...
  it.each([['other'], ['other', 'requester']])(
    'refuses an account change during account resolution %j',
    async (...actors) => {
      let resolve!: (value: CreditAccount) => void;
      vi.mocked(CreditService.getCreditAccount).mockReturnValue(
        new Promise((done) => {
          resolve = done;
        })
      );
  ...
  it('refuses an account change during owner resolution before creating', async () => {
    ...
    let started!: () => void;
    const startedOwner = new Promise<void>((done) => {
      started = done;
    });
    owners(
      new Promise((done) => {
  ...
  it('does not return a creation receipt after an account change', async () => {
    let resolve!: (value: any) => void;
    let started!: () => void;
    const startedCreation = new Promise<void>((done) => {
      started = done;
    });
    m.submit.mockImplementation(() => {
      started();
