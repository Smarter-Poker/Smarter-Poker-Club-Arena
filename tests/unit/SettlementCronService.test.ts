...
describe('retired browser settlement authority', () => {
  it.each([undefined, { autoExecutePayouts: true, requireCanaryCheck: false, checkIntervalMs: 1 }])(
    'refuses startup and never schedules or issues a financial call', async config => {
  ...
  it('refuses direct close/canary entry without announcing completion or writing alerts', async () => {
    ...
    await expect(SettlementCronService.runCanaryCheck()).rejects.toThrow('Weekly Accounting Is Automatic');
  ...
  it('provides no local-clock schedule or server success certificate', () => {
    ...
    expect(SettlementCronService.getStatus()).toEqual({ state: 'browser_scheduler_retired',
      isRunning: false, lastCheckAt: null, nextCheckMs: null, nextSnapshotAt: null, nextPayoutAt: null });
  });
});
