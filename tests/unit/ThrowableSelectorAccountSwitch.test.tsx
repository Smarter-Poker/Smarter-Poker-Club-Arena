...
describe('Throwable Selector Account Isolation', () => {
  ...
  it('does not deliver an in-flight throw to the callbacks for a replacement account', async () => {
    ...
    expect(mocks.useThrowable).toHaveBeenCalledWith('account-a', 'tomato', expect.any(String));
  ...
  it('reuses one durable request id after a lost response and clears it after delivery', async () => {
    const allowance = {
      isVip: false,
      unlimited: false,
      freeThrowsRemaining: 0,
    ...
    mocks.useThrowable
      ...
      .mockResolvedValueOnce({ success: true, idempotent: true });
    ...
    const firstRequestId = mocks.useThrowable.mock.calls[0]?.[2];
    const secondRequestId = mocks.useThrowable.mock.calls[1]?.[2];
  ...
  it('reuses the same request id after the selector unmounts and reopens', async () => {
    const allowance = {
      isVip: false,
      unlimited: false,
      freeThrowsRemaining: 0,
    ...
    expect(mocks.useThrowable.mock.calls[1]?.[2]).toBe(mocks.useThrowable.mock.calls[0]?.[2]);
    await waitFor(() => expect(onSelect).toHaveBeenCalledTimes(1));
