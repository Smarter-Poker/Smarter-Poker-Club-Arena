...
describe('union statement source', () => {
  it('requests the complete authoritative board without reading rolling club rake', async () => {
    ...
    expect(mocks.rpc).toHaveBeenCalledWith('ca_union_statement_board', {
      p_union_id: 'union-a',
      p_period_end: null,
      p_history: 1,
    });
  ...
  it('resolves a period id with union scope before requesting its dates', async () => {
    const query = {
      ...
      maybeSingle: vi
        .fn()
        .mockResolvedValue({ data: { start_at: '2026-08-31', end_at: '2026-09-07' }, error: null }),
    };
  ...
  it('refuses partial or mismatched date ranges and surfaces RPC failures', async () => {
    ...
    await expect(
      UnionService.getSettlementReport('union-a', '2026-08-30', '2026-09-07')
    ).rejects.toThrow();
