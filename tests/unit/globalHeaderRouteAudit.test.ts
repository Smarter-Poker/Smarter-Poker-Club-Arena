...
describe('the complete route manifest inherits one global header', () => {
  it('discovers every current route, including dynamic and legacy redirect paths', () => {
    ...
    expect(allPaths).toHaveLength(132); // +2 management consoles, +1 financial decision harness, +1 The Mint
  ...
  it('puts every shell route under AppLayout', () => {
    expect(shellPaths).toHaveLength(123); // +2: club and union table-management consoles, +1 The Mint
  ...
  it('leaves only explicit public or immersive-table exceptions outside the header', () => {
    ...
    expect(applicable.size).toBe(124); // +2: club and union table-management consoles, +1 The Mint
