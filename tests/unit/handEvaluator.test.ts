...
import { bestFive, scoreFive, compareScore, CATEGORY, cardKey } from '../../src/utils/handEvaluator';
...
describe('scoreFive - categories', () => {
  it('ranks every category in the right order', () => {
    ...
    for (let i = 1; i < rows.length; i++) {
      expect(compareScore(scoreFive(hand(rows[i - 1][0])), scoreFive(hand(rows[i][0])))).toBeGreaterThan(0);
    }
  });
...
describe('bestFive - Omaha uses exactly two from hand and three from board', () => {
  it('does NOT let four hole cards make a flush', () => {
    ...
    const fromHole = best.cards.filter((x) => keys([x]) !== '' && hand('Ah Kh Qh Jh').some((y) => cardKey(y) === cardKey(x)));
  ...
  it('handles a five-card PLO5 holding', () => {
    ...
    const fromHole = best.cards.filter((x) => hand('8s 7s 6s 5s 4s').some((y) => cardKey(y) === cardKey(x)));
    expect(fromHole).toHaveLength(2);
  });
