...
const ops: Op[] = [];
let pendingCashoutRows: Array<{ id: string }> = [];
...
const cancelCashout = vi.fn(async (_id: string, _userId: string) => true);

vi.mock('../../src/services/CashoutService', () => ({
  cashoutService: {
    cancelCashout: (id: string, userId: string) => cancelCashout(id, userId),
  },
}));
...
describe('leaveClub closes a pending cashout through the refunding path', () => {
  it('never writes to cashout_requests directly', async () => {
    pendingCashoutRows = [{ id: 'cashout-a' }];
  ...
  it('cancels each pending cashout through fn_cashout_release', async () => {
    pendingCashoutRows = [{ id: 'cashout-a' }, { id: 'cashout-b' }];
    await leaveClub('club-1');

    expect(cancelCashout).toHaveBeenCalledTimes(2);
    expect(cancelCashout).toHaveBeenCalledWith('cashout-a', 'player-1');
    expect(cancelCashout).toHaveBeenCalledWith('cashout-b', 'player-1');
  });
  ...
  it('stops the departure when the escrow cannot be returned', async () => {
    ...
    const leftAnyway = ops.some((o) => o.table === 'club_members' && o.verb === 'delete');
    expect(leftAnyway, 'the membership was removed despite stranded escrow').toBe(false);
  });
