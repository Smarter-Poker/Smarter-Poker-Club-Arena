...
describe('three-horizontal-line artwork can never return', () => {
  it('documents the copy and artwork bans as separate binding rules', () => {
    const playbook = readFileSync(join(ROOT, 'AGENT-PLAYBOOK.md'), 'utf8');
    const instructions = readFileSync(join(ROOT, 'CLAUDE.md'), 'utf8');

    for (const source of [playbook, instructions]) {
      expect(source).toContain('em dashes');
      expect(source).toContain('U+2014');
      expect(source).toContain('No M bars or three-horizontal-line menu artwork');
      expect(source).toContain('tests/unit/noThreeBarArtwork.law.test.ts');
    }
    expect(existsSync(join(ROOT, 'tests/hamburger-never-regresses.law.test.ts'))).toBe(false);
  });
