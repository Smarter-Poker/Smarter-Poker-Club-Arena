...
describe('XP stays gone', () => {
  ...
  it('no XP-shaped field name survives', () => {
    ...
    const banned = /\b(reputation_xp|total_xp|xp_total|xp_earned|xp_reward|social_xp|bonus_xp_\w+)\b/;
  ...
  it('nothing calls an XP rpc', () => {
    const banned = /rpc\(\s*['"](fn_award_social_xp|get_user_total_xp|award_xp|add_xp|grant_xp)['"]/;
    const offenders: string[] = [];
    for (const f of files) {
      if (banned.test(stripComments(readFileSync(f, 'utf8')))) {
        offenders.push(relative(process.cwd(), f));
