...
describe('retained frontend keeps the existing publication and all browser coverage', () => {
  ...
  it('preserves all production steps, report groups, cleanup and static concurrency', () => {
    // Exact accepted browser commands, configuration, and honesty check remain
    // unchanged; the additive journal callbacks and cleanup job are separate.
    const browserSteps = e2e.jobs['production-e2e'].steps.filter((step: { run?: string }) =>
      /playwright|assert-e2e-actually-ran/.test(step.run ?? '')
    );
    expect(hash(JSON.stringify(browserSteps))).toBe(
      'e7ca91c528285a41cf957b90d0b10ada03fc7a92e383c38412fa4fc8919cedcd'
    );
    const recovery = e2e.jobs['cleanup-recovery'];
    expect(recovery.concurrency).toEqual(e2e.jobs['production-e2e'].concurrency);
    expect(JSON.stringify(recovery.steps)).not.toMatch(
      /playwright|production-e2e-account.*create|consumeFixture/
    );
    expect(JSON.stringify(recovery.steps)).toContain('cleanupCertificateFixtures');
