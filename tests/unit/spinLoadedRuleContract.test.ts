...
// SOURCE ONLY / UNRUN until the existing required local CI suite executes it.
...
function loaded() {
  ...
  return { status: 'success', data: { groups: [{ name: groups[0].name,
    file: '/etc/prometheus/spin-rules.yml', rules: [{ type: 'alerting', name: rule.alert,
      query: rule.expr, duration: 1200, keepFiringFor: 0, labels: rule.labels,
      annotations: rule.annotations, health: 'ok' }] }] } };
}
...
describe('the loaded Spin rule must carry the actual reporting correction', () => {
  it('binds the descriptor to the maintained Prometheus loader configuration', () => {
    ...
    expect(config.rule_files.filter((file: string) => basename(file) === 'spin-rules.yml'))
      .toEqual(['/etc/prometheus/spin-rules.yml']);
  });
  ...
  it('rejects the real historical same-name old annotations even with a healthy rule', () => {
    ...
    alert(response).annotations = {
      ...
      description: 'v_spin_unfilled_waits counts Spins open past the point they\n'
  ...
  it.each(['query', 'duration', 'keepFiringFor', 'labels', 'annotations'])(
    'cannot acknowledge a changed or missing %s', (field) => {
      const response = loaded();
      (alert(response) as any)[field] = field === 'duration' ? 1199 : field === 'keepFiringFor' ? 1 : {};
  ...
  it.each([null, {}, { status: 'error' }, { status: 'success', data: {} },
    { status: 'success', data: { groups: [] } },
    { status: 'success', data: { groups: [null] } }])('never accepts unavailable evidence %j', (response) => {
  ...
  it('refuses stale selected source, missing/duplicate blocks and a changed source group', () => {
    expect(() => compareSpinReportingContract(source.replace('partially filled field', 'obsolete claim'), loaded()))
      .toThrow('source changed');
    expect(() => compareSpinReportingContract(source.replace('alert: SpinUnfilledBacklog', 'alert: Removed'), loaded()))
      .toThrow('missing');
    expect(() => compareSpinReportingContract(source + '\n      - alert: SpinUnfilledBacklog\n', loaded()))
      .toThrow('ambiguous');
    expect(() => compareSpinReportingContract(source.replace('name: spin-experience', 'name: renamed'), loaded()))
      .toThrow('group changed');
  });

  it('does not pin unrelated rules or their annotations', () => {
    expect(compareSpinReportingContract(source.replace('name: spin-fairness', 'name: other-fairness'), loaded()))
      .toEqual([]);
  });
});
...
describe('the deployed command actually enforces the comparison', () => {
  it.each(['current', 'old', 'unavailable', 'canary-missing'])(
    'returns the authoritative command outcome for %s evidence', (kind) => {
      ...
      try {
        ...
        for (const file of config.rule_files) {
          const document = parse(readFileSync(join(root, 'infra/monitoring', basename(file)), 'utf8'));
          for (const group of document.groups ?? []) {
            groups.push({ name: group.name, file, rules: (group.rules ?? []).map((rule: any) => {
              ...
              return rule.alert === 'SpinUnfilledBacklog'
                ...
                : { name: rule.alert ?? rule.record, type: rule.alert ? 'alerting' : 'recording' };
            }) });
          }
        }
        const target = groups.flatMap((group) => group.rules).filter((rule) => rule.name === 'SpinUnfilledBacklog');
        expect(target).toHaveLength(1);
        if (kind === 'old') target[0].annotations.summary = '{{ $value }} Spins are past their fill deadline and still open';
        const responses = {
          'http://fixture.invalid:9090/api/v1/rules': kind === 'unavailable'
            ? { status: 'error', error: 'fixture unavailable' } : { status: 'success', data: { groups } },
          'http://fixture.invalid:9090/api/v1/label/__name__/values': { status: 'success', data: [...series] },
          'http://fixture.invalid:9093/api/v2/alerts': kind === 'canary-missing'
            ? [] : [{ labels: { alertname: 'MonitoringCanary' } }],
        };
        ...
        const result = spawnSync(process.execPath, [join(root, 'scripts/ci/check-alert-rules-match.mjs')], {
          cwd: root, encoding: 'utf8', timeout: 20000, maxBuffer: 1024 * 1024,
          env: { PATH: directory + delimiter + dirname(process.execPath),
            PROMETHEUS_URL: 'http://fixture.invalid:9090', ALERTMANAGER_URL: 'http://fixture.invalid:9093' },
        });
        expect(result.error).toBeUndefined();
        expect(result.signal).toBeNull();
        expect(result.status, result.stdout + result.stderr).toBe(kind === 'current' ? 0 : kind === 'unavailable' ? 2 : 1);
        if (kind === 'old') expect(result.stderr).toContain('SpinUnfilledBacklog DIFFERS');
        if (kind === 'canary-missing') expect(result.stderr).toContain('THE CANARY IS NOT IN ALERTMANAGER');
      } finally {
...
describe('the existing directly triggered suites own this regression', () => {
  it.each(['infra/monitoring/spin-rules.yml', 'infra/monitoring/prometheus.yml',
    'scripts/ci/check-alert-rules-match.mjs', 'scripts/ci/rule-metric-producers.mjs'])(
  ...
  it('keeps unrelated documentation outside the application suites', () => {
    expect(classifyChangedPaths(['docs/example.md'])).toMatchObject({ src: false, server: false, tests: false });
  });
});
