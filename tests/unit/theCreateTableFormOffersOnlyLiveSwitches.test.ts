...
/** Generated from min_buy_in/max_buy_in; explicitly writing either raises 428C9. */
const GENERATED_READ_ONLY = ['min_buy_in_bb', 'max_buy_in_bb'];
...
describe('5b — the tournament block is not written onto a cash row', () => {
  ...
  it.each(GENERATED_READ_ONLY)('never writes generated column %s', (column) => {
    expect(code).not.toMatch(new RegExp(`^\\s*${column}:`, 'm'));
  });
});
