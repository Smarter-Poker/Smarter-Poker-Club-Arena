...
describe('tournamentEventBridge', () => {
  ...
  /**
   * TABLE_BREAK_WARNING has been declared in MasterBus's event union and payload
   * map since the TableBreakEngine port and NOTHING HAS EVER EMITTED IT - that
   * engine is constructed per table engine and no method on it is ever called.
   * TournamentManager.checkTableBalance announces the break itself now, 30
   * seconds before it moves anybody, and this relay is what makes the
   * declaration real.
   */
  it('relays the table break warning, with who is being moved', () => {
    const id = tid();
    relayTournamentEvent(id, {
      ...
      payload: {
        ...
        secondsRemaining: 30,
        playerCount: 3,
        playerIds: ['u-1', 'u-2', 'u-3'],
      },
    });
    expect(names()).toEqual(['TABLE_BREAK_WARNING']);
    expect(emitted.calls[0][1]).toMatchObject({
      tournamentId: id,
      tableId: 'tbl-1',
      secondsRemaining: 30,
  ...
  it('does not swallow a second table breaking in the same window', () => {
    const id = tid();
    const warn = (tableId: string) => ({
      type: 'table_break_warning',
      payload: { tableId, secondsRemaining: 30, playerCount: 2, playerIds: ['u-1'] },
    });
    relayTournamentEvent(id, warn('tbl-a'));
    relayTournamentEvent(id, warn('tbl-a')); // same table, same warning: one event
    relayTournamentEvent(id, warn('tbl-b')); // a different table: genuinely new
    expect(names()).toEqual(['TABLE_BREAK_WARNING', 'TABLE_BREAK_WARNING']);
    expect(emitted.calls.map(([, p]) => p.tableId)).toEqual(['tbl-a', 'tbl-b']);
  });
