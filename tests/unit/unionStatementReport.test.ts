...
describe('recorded union statement report', () => {
  it('uses historical snapshot amounts and never equates direction with payment', () => {
    const report = mapUnionStatementReport(
      'union-a',
      board([row({ amount: -100, union_fee_kept: 250, rakeback_due: 750 })]),
      now
    );
    expect(report.totalRakeCollected).toBe(1000);
    expect(report.netUnionRevenue).toBe(250);
    expect(report.clubBreakdowns[0]).toMatchObject({
      status: 'pending',
      wireDirection: 'COLLECT_FROM_UNION',
      outstanding: 100,
    });
  ...
  it('retains the last unpaid cent and derives overdue from the actual due date', () => {
    const report = mapUnionStatementReport(
      'union-a',
      board([row({ status: 'paid', paid_total: 99.99, due_at: '2026-09-01T00:00:00Z' })]),
      now
    );
    expect(report.clubBreakdowns[0]).toMatchObject({ status: 'overdue', outstanding: 0.01 });
    expect(
      mapUnionStatementReport('union-a', board([row({ status: 'overdue', due_at: null })]), now)
        .clubBreakdowns[0].status
    ).toBe('pending');
    expect(
      mapUnionStatementReport('union-a', board([row({ status: 'paid', paid_total: 100 })]), now)
        .clubBreakdowns[0].status
    ).toBe('paid');
  });
  ...
  it('aggregates more than 2000 records without truncation or floating-cent drift', () => {
    ...
    expect(mapUnionStatementReport('union-a', board(rows), now)).toMatchObject({
      totalClubs: 2501,
      totalRakeCollected: 750.3,
      netUnionRevenue: 250.1,
    });
  });
  it('rejects missing or malformed accounting data instead of returning zero', () => {
    for (const value of [null, '', 'NaN', Infinity, '0.001', '90071992547409.92']) {
      expect(() =>
        mapUnionStatementReport('union-a', board([row({ rake_generated: value })]), now)
      ).toThrow();
    }
    ...
    expect(() =>
      mapUnionStatementReport('union-a', { ...board(), period_start: null }, now)
    ).toThrow();
    expect(() => mapUnionStatementReport('union-a', null, now)).toThrow();
    expect(() =>
      mapUnionStatementReport('union-a', board([row({ snapshot_complete: false })]), now)
    ).toThrow();
  });
});
