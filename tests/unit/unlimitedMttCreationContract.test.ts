...
describe('unlimited MTT creation and saved schedules', () => {
  ...
  it.each([null, 0, 2, 100])('validates the full MTT ladder with obsolete cap %s', (maxPlayers) => {
    expect(() => tournamentService.buildRpcConfig({
      ...draft, maxPlayers, blindStructure: [],
  ...
  it('converts a valid saved two-seat satellite to the uncapped registration contract', () => {
    const payload = tournamentService.buildRpcConfig({
      ...draft, type: 'sng', maxPlayers: 2,
      satelliteTarget: { tournamentId: 'target', seatsAwarded: 1 },
    });
    expect(payload).toMatchObject({ type: 'satellite', maxPlayers: null, satelliteTargetId: 'target' });
  });

  it.each(['bounty', 'progressive_bounty', 'mystery_bounty'] as const)(
    'refuses a %s feeder instead of dropping its payout authority', (type) => {
      expect(() => tournamentService.buildRpcConfig({
        ...draft, type, satelliteTarget: { tournamentId: 'target', seatsAwarded: 1 },
      })).toThrow('Satellites cannot combine ticket prizes with bounty or Spin payouts');
    }
  );

  it.each(['mtt', 'satellite'] as const)(
    'refuses hidden payout configuration on a plain %s feeder', (type) => {
      const config = {
        ...draft, type, satelliteTarget: { tournamentId: 'target', seatsAwarded: 1 },
      };
      expect(() => tournamentService.buildRpcConfig({
        ...config, bountyConfig: { bountyType: 'fixed', baseBounty: 10 },
      })).toThrow('Satellites cannot combine ticket prizes with bounty or Spin payouts');
      expect(() => tournamentService.buildRpcConfig({
        ...config, spinConfig: { possibleMultipliers: [{ multiplier: 2, probability: 100 }] },
      })).toThrow('Satellites cannot combine ticket prizes with bounty or Spin payouts');
    }
  );
  ...
  it('removes both spellings from stale schedule drafts without mutating the draft', async () => {
    const rpc = vi.spyOn(supabase, 'rpc').mockResolvedValue({
      data: { ok: true, schedule_id: 'schedule' }, error: null,
    } as never);
    const config = Object.freeze({ type: 'satellite', maxPlayers: 2, max_players: 2, minPlayers: 3 });
    await tournamentScheduleService.upsert({
      clubId: 'club', name: 'Satellite', daysOfWeek: [1], startTimesUtc: ['18:00'], config,
    });
    expect(rpc).toHaveBeenCalledWith('fn_upsert_tournament_schedule', expect.objectContaining({
  ...
  it('normalizes an old SNG satellite schedule in the serializer spelling', async () => {
    ...
    expect(rpc).toHaveBeenCalledWith('fn_upsert_tournament_schedule', expect.objectContaining({
      p_schedule: expect.objectContaining({
        config: expect.objectContaining({ satelliteTargetId: 'target', maxPlayers: null, max_players: null }),
      }),
    }));
  ...
  it('does not place an MTT in a waitlist for a nonexistent entry ceiling', async () => {
    vi.spyOn(tournamentService, 'getTournament').mockResolvedValue({
      id: 'mtt', tournament_type: 'MTT', variant: 'freezeout', max_players: 2,
    } as never);
