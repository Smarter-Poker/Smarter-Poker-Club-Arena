...
vi.mock('../../src/services/UnionService', () => ({
  UnionService: {
    getUnions: vi.fn(async () => []),
    getUnion: vi.fn(),
    getUnionClubs: vi.fn(async () => []),
  },
}));
    ...
    from: (table: string) => {
      ...
      for (const method of ['select', 'eq', 'is', 'order', 'limit'])
        q[method] = (...args: unknown[]) => {
          m.calls.push([method, ...args]);
          return q;
        };
      q.then = (yes: any, no: any) => Promise.resolve({ data: m.rows, error: null }).then(yes, no);
      return q;
    },
  },
}));
...
function auth(id: string | null) {
  m.id = id;
  // Model the real MasterBus internal AUTH handler before mocked subscribers.
  // Its actual registration is covered in verify-bus-listeners.test.ts.
  useUnionStore.getState().reset();
  for (const fn of [...m.handlers]) fn({ payload: { isAuthenticated: !!id, userId: id } });
}
beforeEach(() => {
  vi.clearAllMocks();
  m.rows = [recordedPeriod()];
  m.calls = [];
  auth(null);
  auth(ID.actor);
});
describe('union accounting store', () => {
  it('retains tab preference and refuses old browser payer/getter calls', async () => {
    ...
    await expect(useUnionStore.getState().loadCurrentPeriod()).rejects.toMatchObject({
      code: 'automatic_weekly_accounting_only',
    });
    await expect(useUnionStore.getState().closePeriod(ID.period)).rejects.toMatchObject({
      code: 'automatic_weekly_accounting_only',
    });
    await expect(useUnionStore.getState().executeMondayPayouts(ID.period)).rejects.toMatchObject({
      code: 'automatic_weekly_accounting_only',
    });
    await expect(useUnionStore.getState().loadPeriodHistory()).rejects.toThrow();
  ...
  it('reads exact union status and history and never fabricates a current global period', async () => {
    ...
    await useUnionStore.getState().loadAccounting(ID.union, {
      ...
      periodEnd: '2026-09-14T07:00:00Z',
    });
    expect(useUnionStore.getState()).toMatchObject({
      accountingScopeId: ID.union,
      accountingObservation: { posted: true },
      currentPeriod: null,
      accountingUnavailable: false,
      periodHistory: [{ id: ID.period }],
    });
    expect(m.calls).toContainEqual(['eq', 'union_id', ID.union]);
    expect(m.calls).toContainEqual(['is', 'club_id', null]);
  ...
  it('keeps unavailable status distinct from an empty history', async () => {
    m.rpc.mockResolvedValue({ data: null, error: { message: 'refused' } });
    m.rows = [];
    await useUnionStore.getState().loadAccounting(ID.union);
    expect(useUnionStore.getState()).toMatchObject({
  ...
  it('does not allow a late A result to overwrite B then A refresh', async () => {
    ...
    m.rpc
      ...
      .mockResolvedValue({ data: null, error: { message: 'refused' } });
  ...
  it('retires account ABA during lazy dependency loading before any accounting request', async () => {
    m.rpc.mockResolvedValue({ data: observationRow(), error: null });
    const pending = useUnionStore.getState().loadAccounting(ID.union);
    // Even a cached dynamic import yields. Retire the captured account before
    // that continuation can validate it or issue either accounting read.
    auth(ID.otherActor);
    auth(ID.actor);
    await pending;
    expect(m.rpc).not.toHaveBeenCalled();
  ...
  it('keeps union membership reads from an older route out of the new route', async () => {
    ...
    vi.mocked(UnionService.getUnion)
      .mockReturnValueOnce(
        new Promise((yes) => {
          resolve = yes;
        })
      )
      .mockResolvedValueOnce({ id: ID.otherUnion } as any);
    const old = useUnionStore.getState().loadUnion(ID.union);
    await vi.waitFor(() => expect(UnionService.getUnion).toHaveBeenCalledTimes(1));
    await useUnionStore.getState().loadUnion(ID.otherUnion);
    resolve({ id: ID.union });
    await old;
    expect(useUnionStore.getState().activeUnion?.id).toBe(ID.otherUnion);
  });
  it('keeps the newest same-union club roster when concurrent refreshes finish out of order', async () => {
    ...
    let resolve!: (x: any) => void;
    vi.mocked(UnionService.getUnionClubs)
      .mockReturnValueOnce(
        new Promise((yes) => {
          resolve = yes;
        })
      )
      .mockResolvedValueOnce([{ clubId: ID.club }] as any);
    const old = useUnionStore.getState().loadUnionClubs(ID.union);
    await vi.waitFor(() => expect(UnionService.getUnionClubs).toHaveBeenCalledTimes(1));
    await useUnionStore.getState().loadUnionClubs(ID.union);
    resolve([{ clubId: 'old' }]);
    await old;
    expect(useUnionStore.getState().activeUnionClubs).toEqual([{ clubId: ID.club }]);
  });
  ...
  it('invalidates the captured accounting predicate synchronously when another union starts loading', async () => {
    ...
    let resolve!: (x: any) => void;
    vi.mocked(UnionService.getUnion).mockReturnValueOnce(
      new Promise((yes) => {
        resolve = yes;
      })
    );
  ...
  it('does not strand a concurrent same-union observer during a roster refresh', async () => {
    ...
    await useUnionStore.getState().loadUnion(ID.union);
    expect(captured()).toBe(true);
    resolve({ data: observationRow(), error: null });
    await pending;
